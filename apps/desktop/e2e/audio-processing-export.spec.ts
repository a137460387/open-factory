...
test('enables de-reverb restoration and includes aecho in the export plan', async ({ page }) => {
  await page.goto('/');
  await waitForE2eActions(page);
  await page.getByTestId('import-media-button').click();
  await addMediaCardToTimeline(page, 0);
  ...
  await expect(page.getByTestId('audio-advanced-restoration-section')).toBeVisible();
  await page.getByTestId('audio-restoration-dereverb-toggle').check();
  await expect(page.getByTestId('audio-restoration-waveform-preview')).toBeVisible();
  ...
  const plan = await page.evaluate(() => window.__E2E_ACTIONS__!.getLastExportPlan!() as { filterComplex: string });
  expect(plan.filterComplex).toContain('aecho=0.8:0.9:60:0.4');
});
