...
#[tauri::command]
pub async fn process_audio_noise_reduction(
    app: AppHandle,
...
fn process_blocking(
    ...
    if req.clip_id.trim().is_empty() {
        return Err("Clip ID is missing.".into());
    }
    if !(0.0..=1.0).contains(&req.strength) {
        return Err("Strength must be 0.0-1.0.".into());
    }
    ...
    Ok(NoiseReductionResult {
        output_path: norm_path(&output),
        original_path: norm_path(&source),
        duration_ms: started.elapsed().as_millis(),
        noise_reduction_db: estimate_db(&pcm, &processed),
    })
}
    ...
    for chunk in pcm.chunks(frame_size) {
        if chunk.len() == frame_size {
            ...
            for i in 0..frame_size {
                out.push(frame[i] * (1.0 - strength) + denoised[i] * strength);
            }
        } else {
...
fn estimate_db(orig: &[f32], proc: &[f32]) -> f32 {
    if orig.is_empty() || proc.is_empty() {
        return 0.0;
    }
...
fn is_canceled(id: &str) -> bool {
    canceled().lock().map(|c| c.contains(id)).unwrap_or(false)
}
fn clear_canceled(id: &str) {
...
mod tests {
    use super::*;
    #[test]
    fn validates_empty_media_path() {
        assert_eq!(
            validate_args("", "t", 0.5).unwrap_err(),
            "Media path is missing."
        );
    }
    #[test]
    fn validates_empty_clip_id() {
        assert_eq!(
            validate_args("/a.wav", "", 0.5).unwrap_err(),
            "Clip ID is missing."
        );
    }
    #[test]
    fn validates_strength() {
        assert!(validate_args("/a.wav", "t", -0.1).is_err());
        assert!(validate_args("/a.wav", "t", 1.5).is_err());
    }
    #[test]
    fn safe_names() {
        assert_eq!(safe_name("clip-123"), "clip-123");
        assert_eq!(safe_name("a b"), "a_b");
    }
    #[test]
    fn norms_paths() {
        assert_eq!(norm_path(Path::new(r"C:\a\b.wav")), "C:/a/b.wav");
    }
    #[test]
    fn estimates_db() {
        assert!((estimate_db(&vec![0.5; 1000], &vec![0.25; 1000]) - 6.02).abs() < 0.1);
    }
