...
vi.mock('../../macros/clip-macros', () => ({ appendMacroHistoryEntry: vi.fn(), buildMacroCommands: vi.fn(() => []), findMacroTargetClip: vi.fn(), writeClipMacros: vi.fn() }));
vi.mock('@open-factory/editor-core/utils', () => ({ logger: { warn: vi.fn() } }));
vi.mock('@open-factory/editor-core', () => ({ createOperationRecording: vi.fn(() => ({})), serializeOperationRecording: vi.fn(() => ''), parseOperationRecording: vi.fn(() => ({})), buildOperationReplaySchedule: vi.fn(() => []), getOperationProjectAtStep: vi.fn(), generateOperationRecordingSlidesHtml: vi.fn(() => ''), recordOperationCommand: vi.fn(), createId: vi.fn(() => 'id'), LoadProjectCommand: vi.fn() }));
