    ...
    (config: { videoAssetIds: string[]; audioAssetId: string; beatTimes: number[]; sensitivity: BeatSensitivity }) => {
      try {
        ...
        const videoAssets = config.videoAssetIds.flatMap((id) => {
          const asset = currentProject.media.find(
            (m: MediaAsset) => m.id === id && (m.type === 'video' || m.type === 'image'),
          );
          return asset ? [asset] : [];
        });
        const audioAsset = currentProject.media.find(
          (m: MediaAsset) => m.id === config.audioAssetId && m.type === 'audio',
        );
