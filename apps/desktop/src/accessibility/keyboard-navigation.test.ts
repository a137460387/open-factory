...
import { getCoreKeyboardFocusOrder, isShortcutCheatsheetKey, resolveSliderKeyboardValue } from './keyboard-navigation';
...
describe('keyboard accessibility helpers', () => {
  ...
  it('recognizes the shortcut cheatsheet key without hijacking modified shortcuts', () => {
    expect(isShortcutCheatsheetKey({ key: '?', shiftKey: true, ctrlKey: false, metaKey: false, altKey: false })).toBe(
      true,
    );
    expect(isShortcutCheatsheetKey({ key: '/', shiftKey: true, ctrlKey: false, metaKey: false, altKey: false })).toBe(
      true,
    );
    expect(isShortcutCheatsheetKey({ key: '?', shiftKey: true, ctrlKey: true, metaKey: false, altKey: false })).toBe(
      false,
    );
  });
  ...
  it('applies fine and shift-modified slider keyboard steps', () => {
    ...
    expect(resolveSliderKeyboardValue({ key: 'ArrowUp', value: 0.5, min: 0, max: 1, step: 0.01, shiftKey: true })).toBe(
      0.6,
    );
