...
interface ProxyBatchVerifyDialogProps {
  ...
  onVerifyAsset?(
    assetId: string,
  ): Promise<{ exists: boolean; readable: boolean; proxyMtimeMs?: number; proxySize?: number; sourceMtimeMs?: number }>;
...
export function ProxyBatchVerifyDialog({ media, onClose, onVerifyAsset, onRepairAsset }: ProxyBatchVerifyDialogProps) {
  ...
  return (
    <div
      ...
      data-testid="proxy-batch-verify-dialog"
    >
      <section className="flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-lg border border-line bg-white shadow-soft">
        <header className="flex items-center justify-between border-b border-line px-4 py-3">
          ...
          <button
            ...
            type="button"
            onClick={onClose}
            data-testid="proxy-verify-close-button"
          >
            <X size={16} />
          </button>
        </header>
        <div className="flex items-center gap-3 border-b border-line px-4 py-2">
          <button
            className="inline-flex items-center gap-1 rounded-md bg-brand px-3 py-1.5 text-xs font-medium text-white hover:bg-[#176858]"
            type="button"
            onClick={handleVerify}
            disabled={verifying || assetsWithProxy.length === 0}
            data-testid="proxy-verify-start-button"
          >
          ...
          <label className="ml-auto text-xs font-medium text-slate-600">
            ...
            <select
              ...
              value={schedule}
              onChange={(e) => setSchedule(e.target.value as ProxyVerifySchedule)}
              data-testid="proxy-verify-schedule-select"
            >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          {report && (
            <>
              ...
              {issueCount > 0 && (
                <button
                  className="mb-3 inline-flex items-center gap-1 rounded-md bg-brand px-3 py-1.5 text-xs font-medium text-white hover:bg-[#176858] disabled:opacity-50"
                  type="button"
                  onClick={handleRepairAll}
                  disabled={repairing}
                  data-testid="proxy-verify-repair-all-button"
                >
              ...
              <div className="space-y-1" data-testid="proxy-verify-results">
                {report.results.map((r) => (
                  <div
                    key={r.assetId}
                    className="flex items-center justify-between rounded border border-line bg-panel px-2 py-1.5 text-xs"
                  >
            ...
            <div className="mt-4 border-t border-line pt-3">
              ...
              {history.map((h, i) => (
                <p key={i} className="text-xs text-slate-500">
                  {t.historyEntry(h.successCount, h.failCount, h.durationMs + 'ms')}
                </p>
              ))}
            </div>
          )}
