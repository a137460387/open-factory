...
describe('local AI model settings', () => {
  ...
  it('resolves installed, missing, and invalid statuses', async () => {
    ...
    await expect(resolveLocalModelStatus('whisper', { path: 'C:/Models/base.bin' }, dependencies)).resolves.toMatchObject({
      status: 'installed',
      size: 4096
    });
    await expect(resolveLocalModelStatus('whisper', { path: 'C:/Missing/model.bin' }, dependencies)).resolves.toMatchObject({
      status: 'missing',
      reason: 'missing'
    });
    await expect(resolveLocalModelStatus('demucs', { path: 'C:/Models/demucs.pt' }, dependencies)).resolves.toMatchObject({
      status: 'invalid',
      reason: 'size'
    });
  });
