...
export function SequenceCompareDialog({ project, onClose, onDragClipToSequence }: SequenceCompareDialogProps) {
  ...
  return (
    <div
      ...
      data-testid="sequence-compare-dialog"
    >
      <section className="flex max-h-[90vh] w-full max-w-6xl flex-col overflow-hidden rounded-lg border border-line bg-white shadow-soft">
        <header className="flex items-center justify-between border-b border-line px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              type="button"
              onClick={handleSave}
              data-testid="sequence-compare-save-layout"
            >
              {t.splitRatio}
            </button>
            <button
              ...
              type="button"
              onClick={onClose}
              data-testid="sequence-compare-close-button"
            >
              <X size={16} />
            </button>
          </div>
        ...
        <div className="flex items-center gap-3 border-b border-line px-4 py-2">
          <label className="text-xs font-medium text-slate-600">
            ...
            <select
              ...
              data-testid="sequence-compare-left-select"
            >
              {allSequences.map((s) => (
                <option key={s.id} value={s.id}>
                  {s.id === project.id ? t.mainSequence : t.nestedSequence(s.name || s.id)}
                </option>
              ))}
            </select>
          </label>
          ...
          <label className="text-xs font-medium text-slate-600">
            ...
            <select
              ...
              data-testid="sequence-compare-right-select"
            >
              {allSequences.map((s) => (
                <option key={s.id} value={s.id}>
                  {s.id === project.id ? t.mainSequence : t.nestedSequence(s.name || s.id)}
          ...
          <label className="ml-auto flex items-center gap-1 text-xs text-slate-600">
            <input
              ...
              checked={syncMarkersEnabled}
              onChange={(e) => setSyncMarkersEnabled(e.target.checked)}
              data-testid="sequence-compare-sync-markers-toggle"
            />
        ...
        {syncPairs.length > 0 && (
          <div
            className="border-b border-line bg-amber-50 px-4 py-1.5 text-xs text-amber-800"
            data-testid="sequence-compare-sync-pairs"
          >
            {syncPairs.map((p) => (
              <span key={p.leftMarkerId} className="mr-3 inline-block rounded bg-amber-100 px-1.5 py-0.5">
                {t.syncMarkerPair(p.label)}
              </span>
            ))}
          </div>
        )}
        ...
        <div className="flex min-h-0 flex-1">
          <div
            className="min-h-0 flex-1 overflow-y-auto border-r border-line p-3"
            style={{ flex: splitRatio }}
            data-testid="sequence-compare-left-panel"
            onDragOver={(e) => e.preventDefault()}
            onDrop={(e) => handleDrop(e, leftId)}
          >
          ...
          <div
            ...
            data-testid="sequence-compare-right-panel"
            onDragOver={(e) => e.preventDefault()}
            onDrop={(e) => handleDrop(e, rightId)}
          >
  ...
  return (
    <div className="space-y-1" data-testid="sequence-compare-clip-list">
      {clips.map((clip) => (
        <div
          key={clip.id}
          className="cursor-grab rounded border border-line bg-panel px-2 py-1.5 text-xs text-ink"
          draggable
          onDragStart={(e) => onDragStart(e, clip.id)}
          data-testid={`sequence-compare-clip-${clip.id}`}
        >
