...
export function ProjectHealthDialog({
  ...
  return (
    <div
      ...
      data-testid="project-health-dialog"
    >
      <section
        className="flex max-h-[86vh] w-full max-w-4xl flex-col overflow-hidden rounded-lg bg-white shadow-soft"
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-line px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <button
              className="inline-flex items-center gap-2 rounded-md border border-line px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-panel disabled:cursor-not-allowed disabled:opacity-50"
              type="button"
              data-testid="project-health-auto-repair-button"
              onClick={onAutoRepair}
              disabled={scanning || issueCount === 0}
            >
              <RefreshCw size={14} />
              {t.actions.autoRepair}
            </button>
            <button
              className="inline-flex items-center gap-2 rounded-md border border-line px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-panel"
              type="button"
              data-testid="project-health-rescan-button"
              onClick={onRescan}
              disabled={scanning}
            >
              <RefreshCw size={14} />
            ...
            <button
              ...
              data-testid="project-health-close-button"
              onClick={onClose}
            >
              <X size={16} />
            </button>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          ...
          {report && issueCount === 0 ? (
            <div
              className="flex min-h-[220px] flex-col items-center justify-center rounded-md border border-line bg-panel p-6 text-center text-sm font-medium text-slate-600"
          ...
          {report && issueCount > 0 ? (
            <div className="space-y-4">
              <HealthSection
                title={t.sections.missingMedia}
                count={report.missingMedia.length}
                testId="project-health-section-missing-media"
              >
                {report.missingMedia.map((issue) => (
                  <HealthItem
                    key={issue.id}
                    testId="project-health-missing-item"
                    title={issue.name}
                    subtitle={issue.path}
                  >
                    <ReferenceList references={issue.references} />
                    <ActionButton
                      testId="project-health-fix-missing-button"
              ...
              <HealthSection
                title={t.sections.orphanMedia}
                count={report.orphanMedia.length}
                testId="project-health-section-orphan-media"
              >
                {report.orphanMedia.map((issue) => (
                  <HealthItem
                    key={issue.id}
                    testId="project-health-orphan-item"
                    title={issue.name}
                    subtitle={issue.path}
                  >
                    <ActionButton
                      testId="project-health-fix-orphan-button"
                      onClick={() => onRemoveOrphan(issue)}
                      icon={<Trash2 size={14} />}
                    >
              ...
              <HealthSection
                title={t.sections.duplicateMedia}
                count={report.duplicateMedia.length}
                testId="project-health-section-duplicate-media"
              >
                {report.duplicateMedia.map((issue) => (
                  <HealthItem
                    key={issue.id}
                    testId="project-health-duplicate-item"
                    title={t.detail.duplicateGroup(issue.assets.length, issue.size)}
                    subtitle={issue.assets.map((asset) => asset.path).join(' / ')}
                  >
                    ...
                    <ActionButton
                      testId="project-health-fix-duplicate-button"
                      onClick={() => onMergeDuplicate(issue)}
                      icon={<Merge size={14} />}
                    >
              ...
              <HealthSection
                title={t.sections.proxyMissing}
                count={report.proxyMissing.length}
                testId="project-health-section-proxy-missing"
              >
                {report.proxyMissing.map((issue) => (
                  <HealthItem
                    key={issue.id}
                    testId="project-health-proxy-item"
                    title={issue.name}
                    subtitle={`${issue.path} · ${t.detail.proxyResolution(issue.width, issue.height)}`}
                  >
                    <ActionButton
                      testId="project-health-fix-proxy-button"
                      onClick={() => onQueueProxy(issue)}
                      icon={<Gauge size={14} />}
                    >
              ...
              <HealthSection
                title={t.sections.missingFonts}
                count={report.missingFonts.length}
                testId="project-health-section-missing-fonts"
              >
                {report.missingFonts.map((issue) => (
                  <HealthItem
                    key={issue.id}
                    testId="project-health-font-item"
                    title={t.detail.missingFont(issue.fontFamily)}
                    subtitle={formatReference(issue.clip)}
                  />
                ))}
              </HealthSection>
            </div>
      ...
      {report.entries.length > 0 ? (
        <div className="mt-2 grid gap-1 text-xs text-slate-600">
          {report.entries.slice(0, 6).map((entry, index) => (
            <div
              key={`${entry.type}-${entry.assetId ?? index}-${entry.status}`}
              data-testid="project-health-repair-entry"
  ...
  return (
    <section className="space-y-2" data-testid={testId} data-count={count}>
      <div className="flex items-center justify-between">
        ...
        <span className="rounded bg-panel px-2 py-0.5 text-[11px] font-semibold text-slate-600">
          {zhCN.projectHealth.sectionCount(count)}
        </span>
      </div>
...
function ActionButton({
  ...
  return (
    <button
      className="inline-flex items-center gap-2 rounded-md border border-line bg-panel px-2 py-1.5 text-xs font-medium text-slate-700 hover:bg-white"
      type="button"
      data-testid={testId}
      onClick={onClick}
    >
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {references.map((reference) => (
        <span
          key={`${reference.sequenceId}-${reference.clipId}`}
          className="rounded bg-panel px-2 py-0.5 text-[11px] font-medium text-slate-600"
        >
