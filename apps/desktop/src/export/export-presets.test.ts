...
describe('export presets', () => {
  ...
  it('rejects deleting built-in presets', async () => {
    ...
    await expect(deleteCustomExportPreset(BUILTIN_EXPORT_PRESETS[0].id, storage)).rejects.toThrow(
      zhCN.exportPresets.cannotDeleteBuiltin,
    );
  });
      ...
      [
        {
          ...
          settings: {
            ...
            height: 720,
            format: 'mp4',
            subtitleLanguages: ['ZH', 'en', 'en'],
            subtitleBurnInLanguage: 'EN',
          },
        },
      ],
      {
  ...
  it('handles preset package name conflicts with overwrite, rename, and skip modes', () => {
    ...
    expect(
      renamed.presets.some((preset) => preset.name === `Team Review ${zhCN.exportPresets.importedCopySuffix}`),
    ).toBe(true);
  ...
  it('rejects unsupported preset package versions', () => {
    expect(() => parseExportPresetPackage(JSON.stringify({ version: 99, presets: [] }))).toThrow(
      zhCN.exportPresets.packageUnsupportedVersion,
    );
    expect(() => parseExportPresetPackage(JSON.stringify({ version: 1, presets: [] }))).toThrow(
      zhCN.exportPresets.packageInvalid,
    );
  });
  ...
  it('imports preset packages into custom storage', async () => {
    ...
    expect(JSON.parse(files.get(presetPath) ?? '{}').presets[0]).toEqual(
  ...
  it('persists PNG sequence custom preset settings', async () => {
    ...
    const presets = await saveCustomExportPreset(
      'PNG Frames',
      { format: 'png-sequence', outputMode: 'video', fps: 12, videoCodec: 'png' },
      storage,
    );
  ...
  it('keeps animated image custom preset formats', async () => {
    ...
    const presets = await saveCustomExportPreset(
      'Small GIF',
      { format: 'gif', outputMode: 'video', fps: 15, width: 640, height: 360, videoCodec: 'gif' },
      storage,
    );
  ...
  it('persists and sanitizes watermark preset settings', async () => {
    ...
    expect(JSON.parse(files.get(presetPath) ?? '{}').presets[0].settings.watermark).toEqual(
      expect.objectContaining({ type: 'text', text: 'Draft' }),
    );
