...
describe('preset market', () => {
  ...
  it('filters cards by platform, quality, and format tags', () => {
    ...
    expect(filterPresetMarketCards(cards, { platform: 'youtube', quality: '4k', format: 'mp4' }).map((card) => card.id)).toEqual(['yt']);
  ...
  it('falls back to local cache when network loading fails', async () => {
    const storage = createMemoryStorage();
    const cached = JSON.stringify({ schemaVersion: 1, presets: [makeMarketCard('cached', ['YouTube', '1080p', 'MP4'])] });
  ...
  it('detects duplicate custom presets before overwrite confirmation', () => {
    const [card] = parsePresetMarketJson(JSON.stringify({ schemaVersion: 1, presets: [makeMarketCard('yt', ['YouTube', '1080p', 'MP4'])] }));
  ...
  it('installs duplicate market presets with overwrite mode after confirmation', async () => {
    ...
    expect(storage.files.get('C:/Users/E2E/AppData/Roaming/open-factory/market-cache/installed/yt.ofpreset.json')).toContain('Market yt');
  });
});
