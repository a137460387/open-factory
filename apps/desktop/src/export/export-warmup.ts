...
export async function runExportWarmup(project: Project, dependencies: ExportWarmupDependencies, options: ExportWarmupOptions = {}): Promise<ExportWarmupResult> {
...
export function buildExportWarmupCacheKey(project: Project): string {
  ...
  const fontStamp = project.timeline.tracks
    ...
    .sort()
    .join('|');
  return [project.id, project.updatedAt, project.media.length, project.timeline.tracks.length, project.sequences?.length ?? 0, mediaStamp, fontStamp].join('::');
}
