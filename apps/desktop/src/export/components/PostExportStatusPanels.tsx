...
export function PostExportScriptResultPanel({ result }: { result: ExportPostExportScriptResult }) {
  ...
  return (
    ...
    >
      ...
      {result.error ? (
        <div className="mt-1 whitespace-pre-wrap text-amber-800" data-testid="export-post-script-error">
          {result.error}
        </div>
      ) : null}
      {result.stdout ? (
        <pre
          className="mt-2 max-h-20 overflow-auto rounded bg-white/70 p-2 whitespace-pre-wrap"
          data-testid="export-post-script-stdout"
        >
          {result.stdout}
        </pre>
      ) : null}
      {result.stderr ? (
        <pre
          className="mt-2 max-h-20 overflow-auto rounded bg-white/70 p-2 whitespace-pre-wrap"
          data-testid="export-post-script-stderr"
        >
          {result.stderr}
        </pre>
      ) : null}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          <span className="rounded-full border border-current px-2 py-0.5 font-semibold">
            {report.healed ? t.healed : t.failed}
          </span>
...
export function PostExportQualityAssurancePanel({ result }: { result: PostExportQualityAssuranceResult }) {
  ...
  return (
    <div
      className={`mt-2 rounded-md border p-2 text-[11px] ${postExportQualityStatusClass(result.status)}`}
      data-testid="post-export-quality-result"
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div
          ...
          data-testid="post-export-quality-status"
        >
...
function PostExportQualityCheckRow({ check }: { check: PostExportQualityCheckResult }) {
  ...
  return (
    <div
      className="rounded-md bg-white/70 p-2"
      data-testid={`post-export-quality-check-${check.id}`}
      data-status={check.status}
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="font-semibold text-slate-700">{t.checks[check.id]}</div>
        <span
          className={`rounded-full border px-1.5 py-0.5 text-[10px] font-semibold ${postExportQualityStatusClass(check.status)}`}
        >
      ...
      {check.expected !== undefined || check.actual !== undefined ? (
        <div className="mt-1 grid gap-1 text-[10px] text-slate-500 sm:grid-cols-2">
          {check.expected !== undefined ? (
            <div>
              {t.expected}: {formatPostExportQualityValue(check, check.expected)}
            </div>
          ) : null}
          {check.actual !== undefined ? (
            <div>
              {t.actual}: {formatPostExportQualityValue(check, check.actual)}
            </div>
          ) : null}
        </div>
      ) : null}
      {check.ranges?.length ? (
        <div className="mt-1 text-[10px] text-slate-500">{t.ranges(check.ranges.length)}</div>
      ) : null}
    </div>
  );
}
