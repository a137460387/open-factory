...
export function HardwareEncoderSettingsPanel({
  encoders,
  settings,
  setDraftSettings,
  disabled,
    ...
    <div className="grid gap-2 rounded-md border border-line bg-slate-50 p-3" data-testid="hw-encoder-settings">
      <label className="space-y-1 text-xs font-medium text-slate-600">
        ...
        <select
          ...
          value={selectedId}
          disabled={disabled}
          onChange={(e) => updateHardwareEncoderId(setDraftSettings, e.target.value)}
          data-testid="hw-encoder-select"
        >
          {encoders.map((enc) => (
      ...
      {presets.length > 0 && (
        <label className="space-y-1 text-xs font-medium text-slate-600">
          ...
          <select
            className="w-full rounded-md border border-line px-2 py-1.5 disabled:bg-slate-100"
            value={settings?.preset ?? presets[0]?.value ?? ''}
            disabled={disabled}
            onChange={(e) => updateHardwareEncoderPreset(setDraftSettings, e.target.value)}
            data-testid="hw-encoder-preset-select"
          >
            {presets.map((p) => (
      ...
      <label className="space-y-1 text-xs font-medium text-slate-600">
        ...
        <select
          ...
          value={rc}
          disabled={disabled}
          onChange={(e) => updateHardwareRateControlMode(setDraftSettings, e.target.value)}
          data-testid="hw-rate-control-select"
        >
          {RC_OPTIONS.map((o) => (
      ...
      {rc === 'cqp' && (
        <label className="space-y-1 text-xs font-medium text-slate-600">
          ...
          <input
            ...
            min={1}
            max={51}
            value={settings?.cq ?? selected?.defaultCq ?? 23}
            disabled={disabled}
            onChange={(e) => updateHardwareCq(setDraftSettings, e.target.value)}
            data-testid="hw-cq-slider"
          />
        </label>
      )}
      {(rc === 'vbr' || rc === 'cbr') && (
        <label className="space-y-1 text-xs font-medium text-slate-600">
          ...
          <input
            ...
            disabled={disabled}
            onChange={(e) => updateHardwareVideoBitrate(setDraftSettings, e.target.value)}
            data-testid="hw-bitrate-input"
          />
        </label>
      )}
      {rc === 'vbr' && (
        <label className="space-y-1 text-xs font-medium text-slate-600">
          ...
          <input
            ...
            disabled={disabled}
            onChange={(e) => updateHardwareMaxBitrate(setDraftSettings, e.target.value)}
            data-testid="hw-max-bitrate-input"
          />
        </label>
      )}
      <label className="space-y-1 text-xs font-medium text-slate-600">
        ...
        <input
          ...
          disabled={disabled}
          onChange={(e) => updateHardwareGopSize(setDraftSettings, e.target.value)}
          data-testid="hw-gop-size-input"
        />
      </label>
      {selected?.supportsBFrames && (
        <label className="space-y-1 text-xs font-medium text-slate-600">
          ...
          <input
            ...
            disabled={disabled}
            onChange={(e) => updateHardwareBFrames(setDraftSettings, e.target.value)}
            data-testid="hw-bframes-input"
          />
        </label>
      )}
    </div>
