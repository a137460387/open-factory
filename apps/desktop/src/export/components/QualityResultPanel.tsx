  ...
  return (
    <div className="border-t border-line p-3 text-xs" data-testid="quality-result-panel">
      <div className="mb-2 flex items-center justify-between gap-2">
        ...
        {running ? (
          <button
            className="rounded-md border border-rose-300 bg-rose-50 px-2 py-1 font-medium text-rose-800 hover:bg-rose-100"
            type="button"
            data-testid="quality-cancel-button"
            onClick={onCancel}
          >
            {t.cancel}
          </button>
      ...
      {error ? (
        <div className="mb-2 rounded-md border border-rose-200 bg-rose-50 p-2 text-rose-700">{error}</div>
      ) : null}
      {result ? (
  ...
  return (
    <div className="rounded-md bg-panel p-2" data-testid={`quality-result-${metric}`}>
      <div className="flex items-center justify-between gap-2">
        ...
        {level ? (
          <span className={`rounded-full border px-1.5 py-0.5 text-[10px] font-semibold ${qualityLevelClass(level)}`}>
            {t.levels[level]}
          </span>
        ) : null}
      </div>
      <div className="mt-1 text-base font-semibold tabular-nums text-slate-900">
        {formatQualityMetricValue(value, suffix)}
      </div>
