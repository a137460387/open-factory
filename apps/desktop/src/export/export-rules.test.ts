...
describe('export condition rules', () => {
  ...
  it('runs only enabled rules matching the current event trigger', async () => {
    ...
    await expect(
      runExportRuleEvent(
        rules,
        {
          ...
          task: { name: 'out.mp4', outputPath: 'D:/Renders/out.mp4' },
        },
        { copyFile, notify, playTone },
      ),
    ).resolves.toEqual([
      { ruleId: 'copy-success', action: 'copy-to-directory', targetPath: 'C:/Exports/20260613/out.mp4' },
    ]);
    ...
    await runExportRuleEvent(
      rules,
      { type: 'export-failure', task: { name: 'bad.mp4', outputPath: 'D:/Renders/bad.mp4', error: 'ffmpeg failed' } },
      { copyFile, notify, playTone },
    );
