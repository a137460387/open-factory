...
export const useEditorUIStore = create<EditorUIState>((set, get) => ({
  ...
  // Dialog / panel open setters
  setBatchTranscodeOpen(updater) { set((s) => ({ batchTranscodeOpen: applyUpdater(s.batchTranscodeOpen, updater) })); },
  setBatchWatermarkOpen(updater) { set((s) => ({ batchWatermarkOpen: applyUpdater(s.batchWatermarkOpen, updater) })); },
  setBatchProjectProcessingOpen(updater) { set((s) => ({ batchProjectProcessingOpen: applyUpdater(s.batchProjectProcessingOpen, updater) })); },
  setLutEditorOpen(updater) { set((s) => ({ lutEditorOpen: applyUpdater(s.lutEditorOpen, updater) })); },
  setColorNodeEditorOpen(updater) { set((s) => ({ colorNodeEditorOpen: applyUpdater(s.colorNodeEditorOpen, updater) })); },
  setColorAnalysisOpen(updater) { set((s) => ({ colorAnalysisOpen: applyUpdater(s.colorAnalysisOpen, updater) })); },
  setProfessionalNleExportOpen(updater) { set((s) => ({ professionalNleExportOpen: applyUpdater(s.professionalNleExportOpen, updater) })); },
  setMediaPrecheckOpen(updater) { set((s) => ({ mediaPrecheckOpen: applyUpdater(s.mediaPrecheckOpen, updater) })); },
  setVideoStitchWizardOpen(updater) { set((s) => ({ videoStitchWizardOpen: applyUpdater(s.videoStitchWizardOpen, updater) })); },
  setSyncCompareOpen(updater) { set((s) => ({ syncCompareOpen: applyUpdater(s.syncCompareOpen, updater) })); },
  setSceneReorderOpen(updater) { set((s) => ({ sceneReorderOpen: applyUpdater(s.sceneReorderOpen, updater) })); },
  setStyleTransferOpen(updater) { set((s) => ({ styleTransferOpen: applyUpdater(s.styleTransferOpen, updater) })); },
  setCollaborationNotesOpen(updater) { set((s) => ({ collaborationNotesOpen: applyUpdater(s.collaborationNotesOpen, updater) })); },
  setOperationRecordingOpen(updater) { set((s) => ({ operationRecordingOpen: applyUpdater(s.operationRecordingOpen, updater) })); },
  setComplexityScoreOpen(updater) { set((s) => ({ complexityScoreOpen: applyUpdater(s.complexityScoreOpen, updater) })); },
  setSmartRecommendationsOpen(updater) { set((s) => ({ smartRecommendationsOpen: applyUpdater(s.smartRecommendationsOpen, updater) })); },
  setContentAnalysisOpen(updater) { set((s) => ({ contentAnalysisOpen: applyUpdater(s.contentAnalysisOpen, updater) })); },
  setProfilerOpen(updater) { set((s) => ({ profilerOpen: applyUpdater(s.profilerOpen, updater) })); },
  setRhythmAnalysisOpen(updater) { set((s) => ({ rhythmAnalysisOpen: applyUpdater(s.rhythmAnalysisOpen, updater) })); },
  setTimelineSearchOpen(updater) { set((s) => ({ timelineSearchOpen: applyUpdater(s.timelineSearchOpen, updater) })); },
  setSnapshotNameOpen(updater) { set((s) => ({ snapshotNameOpen: applyUpdater(s.snapshotNameOpen, updater) })); },
  setSnapshotHistoryOpen(updater) { set((s) => ({ snapshotHistoryOpen: applyUpdater(s.snapshotHistoryOpen, updater) })); },
  setSnapshotCompareOpen(updater) { set((s) => ({ snapshotCompareOpen: applyUpdater(s.snapshotCompareOpen, updater) })); },
  setTimelineCompareOpen(updater) { set((s) => ({ timelineCompareOpen: applyUpdater(s.timelineCompareOpen, updater) })); },
  setReleaseWorkflowOpen(updater) { set((s) => ({ releaseWorkflowOpen: applyUpdater(s.releaseWorkflowOpen, updater) })); },
  setProjectEncryptionSaveOpen(updater) { set((s) => ({ projectEncryptionSaveOpen: applyUpdater(s.projectEncryptionSaveOpen, updater) })); },
  setProjectTemplateOpen(updater) { set((s) => ({ projectTemplateOpen: applyUpdater(s.projectTemplateOpen, updater) })); },
  setSettingsOpen(updater) { set((s) => ({ settingsOpen: applyUpdater(s.settingsOpen, updater) })); },
  setBeatSyncOpen(updater) { set((s) => ({ beatSyncOpen: applyUpdater(s.beatSyncOpen, updater) })); },
  setSmartRoughCutOpen(updater) { set((s) => ({ smartRoughCutOpen: applyUpdater(s.smartRoughCutOpen, updater) })); },
  setAiRoughCutOpen(updater) { set((s) => ({ aiRoughCutOpen: applyUpdater(s.aiRoughCutOpen, updater) })); },
  setDirectorModeOpen(updater) { set((s) => ({ directorModeOpen: applyUpdater(s.directorModeOpen, updater) })); },
  setMusicMatchOpen(updater) { set((s) => ({ musicMatchOpen: applyUpdater(s.musicMatchOpen, updater) })); },
  setHighlightReelOpen(updater) { set((s) => ({ highlightReelOpen: applyUpdater(s.highlightReelOpen, updater) })); },
  setContextualTranslationOpen(updater) { set((s) => ({ contextualTranslationOpen: applyUpdater(s.contextualTranslationOpen, updater) })); },
  setAiChatEditorOpen(updater) { set((s) => ({ aiChatEditorOpen: applyUpdater(s.aiChatEditorOpen, updater) })); },
  setVideoSummaryOpen(updater) { set((s) => ({ videoSummaryOpen: applyUpdater(s.videoSummaryOpen, updater) })); },
  setNarrationOpen(updater) { set((s) => ({ narrationOpen: applyUpdater(s.narrationOpen, updater) })); },
  setHistoryPanelOpen(updater) { set((s) => ({ historyPanelOpen: applyUpdater(s.historyPanelOpen, updater) })); },
  setProjectDocumentationOpen(updater) { set((s) => ({ projectDocumentationOpen: applyUpdater(s.projectDocumentationOpen, updater) })); },
  setStoryboardOpen(updater) { set((s) => ({ storyboardOpen: applyUpdater(s.storyboardOpen, updater) })); },
  setMacroHistoryOpen(updater) { set((s) => ({ macroHistoryOpen: applyUpdater(s.macroHistoryOpen, updater) })); },
  setProjectHealthOpen(updater) { set((s) => ({ projectHealthOpen: applyUpdater(s.projectHealthOpen, updater) })); },
  setMediaHealthDashboardOpen(updater) { set((s) => ({ mediaHealthDashboardOpen: applyUpdater(s.mediaHealthDashboardOpen, updater) })); },
  setDuplicateMediaOpen(updater) { set((s) => ({ duplicateMediaOpen: applyUpdater(s.duplicateMediaOpen, updater) })); },
  setMediaOrganizerOpen(updater) { set((s) => ({ mediaOrganizerOpen: applyUpdater(s.mediaOrganizerOpen, updater) })); },
  setShortcutCheatsheetOpen(updater) { set((s) => ({ shortcutCheatsheetOpen: applyUpdater(s.shortcutCheatsheetOpen, updater) })); },
  setPasteKeyframeDialogOpen(updater) { set((s) => ({ pasteKeyframeDialogOpen: applyUpdater(s.pasteKeyframeDialogOpen, updater) })); },
  setPreviewWindowOpen(updater) { set((s) => ({ previewWindowOpen: applyUpdater(s.previewWindowOpen, updater) })); },
  setAutoAudioSyncOpen(updater) { set((s) => ({ autoAudioSyncOpen: applyUpdater(s.autoAudioSyncOpen, updater) })); },
  setErrorKnowledgeOpen(updater) { set((s) => ({ errorKnowledgeOpen: applyUpdater(s.errorKnowledgeOpen, updater) })); },
  setSequenceCompareOpen(updater) { set((s) => ({ sequenceCompareOpen: applyUpdater(s.sequenceCompareOpen, updater) })); },
  setSubtitleSyncOpen(updater) { set((s) => ({ subtitleSyncOpen: applyUpdater(s.subtitleSyncOpen, updater) })); },
