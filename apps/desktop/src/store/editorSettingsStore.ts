...
import { DEFAULT_TIMELINE_GRID_SETTINGS, type TimelineGridSettings, type PiPLayoutPosition, type BeatSensitivity } from '@open-factory/editor-core';
import {
...
import { DEFAULT_TUTORIAL_SIGNALS, type TutorialProgressSettings, type TutorialSignals } from '../tutorial/tutorialState';
import { DEFAULT_PREVIEW_PERFORMANCE_SETTINGS, type PreviewPerformanceSettings } from '../lib/preview/preview-performance';
...
export const useEditorSettingsStore = create<EditorSettingsState>((set) => ({
  ...
  setTimelineGridSettings(updater) { set((s) => ({ timelineGridSettings: applyUpdater(s.timelineGridSettings, updater) })); },
  setTimelineInteractionSettings(updater) { set((s) => ({ timelineInteractionSettings: applyUpdater(s.timelineInteractionSettings, updater) })); },
  setSafeFrameGuides(updater) { set((s) => ({ safeFrameGuides: applyUpdater(s.safeFrameGuides, updater) })); },
  setThumbnailTrackVisible(updater) { set((s) => ({ thumbnailTrackVisible: applyUpdater(s.thumbnailTrackVisible, updater) })); },
  setTimelineMinimapVisible(updater) { set((s) => ({ timelineMinimapVisible: applyUpdater(s.timelineMinimapVisible, updater) })); },
  setTimelineHeatmap(updater) { set((s) => ({ timelineHeatmap: applyUpdater(s.timelineHeatmap, updater) })); },
  setPreviewPerformance(updater) { set((s) => ({ previewPerformance: applyUpdater(s.previewPerformance, updater) })); },
  setPreviewWindowResolutionScale(updater) { set((s) => ({ previewWindowResolutionScale: applyUpdater(s.previewWindowResolutionScale, updater) })); },
  setShortcutBindings(updater) { set((s) => ({ shortcutBindings: applyUpdater(s.shortcutBindings, updater) })); },
  setMacros(updater) { set((s) => ({ macros: applyUpdater(s.macros, updater) })); },
  setMacroHistory(updater) { set((s) => ({ macroHistory: applyUpdater(s.macroHistory, updater) })); },
  setSharedLibraryResources(updater) { set((s) => ({ sharedLibraryResources: applyUpdater(s.sharedLibraryResources, updater) })); },
  setCollaborationIdentity(updater) { set((s) => ({ collaborationIdentity: applyUpdater(s.collaborationIdentity, updater) })); },
  setAutosaveIntervalSeconds(updater) { set((s) => ({ autosaveIntervalSeconds: applyUpdater(s.autosaveIntervalSeconds, updater) })); },
  setTutorialProgress(updater) { set((s) => ({ tutorialProgress: applyUpdater(s.tutorialProgress, updater) })); },
  setTutorialCelebrationVisible(updater) { set((s) => ({ tutorialCelebrationVisible: applyUpdater(s.tutorialCelebrationVisible, updater) })); },
  setTutorialSignals(updater) { set((s) => ({ tutorialSignals: applyUpdater(s.tutorialSignals, updater) })); },
  setPiPLayoutPosition(updater) { set((s) => ({ pipLayoutPosition: applyUpdater(s.pipLayoutPosition, updater) })); },
  setCustomSplitLayouts(updater) { set((s) => ({ customSplitLayouts: applyUpdater(s.customSplitLayouts, updater) })); },
  setLastBackupAt(updater) { set((s) => ({ lastBackupAt: applyUpdater(s.lastBackupAt, updater) })); },
  setBeatSensitivity(updater) { set((s) => ({ beatSensitivity: applyUpdater(s.beatSensitivity, updater) })); },
  setBeatSyncSpeedEnabled(updater) { set((s) => ({ beatSyncSpeedEnabled: applyUpdater(s.beatSyncSpeedEnabled, updater) })); },
  setBeatSyncManualBpm(updater) { set((s) => ({ beatSyncManualBpm: applyUpdater(s.beatSyncManualBpm, updater) })); },
  setSceneDetectionRequestId(updater) { set((s) => ({ sceneDetectionRequestId: applyUpdater(s.sceneDetectionRequestId, updater) })); },
}));
