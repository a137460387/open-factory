...
const invokeCases: Array<{ name: string; run: () => Promise<unknown>; cmd: string; args: Record<string, unknown> }> = [
  ...
  { name: 'scanMediaIntegrity', run: () => scanMediaIntegrity('/a.mp4'), cmd: 'scan_media_integrity', args: { path: '/a.mp4' } },
  { name: 'analyzeAudioSpectrum', run: () => analyzeAudioSpectrum('/a.wav'), cmd: 'analyze_audio_spectrum', args: { path: '/a.wav' } },
  { name: 'generateGapFillMedia', run: () => generateGapFillMedia({ clipId: 'c1' } as never), cmd: 'generate_gap_fill_media', args: { request: { clipId: 'c1' } } },
  { name: 'extractCoverFrames', run: () => extractCoverFrames({ clipId: 'c1' } as never), cmd: 'extract_cover_frames', args: { request: { clipId: 'c1' } } },
  { name: 'batchExtractCoverFrames', run: () => batchExtractCoverFrames({ clipIds: [] } as never), cmd: 'batch_extract_cover_frames', args: { request: { clipIds: [] } } },
  { name: 'analyzeWaveform', run: () => analyzeWaveform('/a.wav', 8), cmd: 'analyze_waveform', args: { path: '/a.wav', samplesPerSec: 8 } },
  { name: 'detectBeats', run: () => detectBeats('/a.wav', 'high'), cmd: 'detect_beats', args: { path: '/a.wav', sensitivity: 'high' } },
  { name: 'detectSilence', run: () => detectSilence('/a.wav', -40, 200), cmd: 'detect_silence', args: { path: '/a.wav', thresholdDb: -40, minGapMs: 200 } },
  { name: 'generateProxy', run: () => generateProxy({ clipId: 'c1' } as never), cmd: 'generate_proxy', args: { plan: { clipId: 'c1' } } },
  { name: 'detectSceneChanges', run: () => detectSceneChanges({ path: '/a.mp4' } as never), cmd: 'detect_scene_changes', args: { request: { path: '/a.mp4' } } },
  { name: 'cancelSceneDetection', run: () => cancelSceneDetection('task-1'), cmd: 'cancel_scene_detection', args: { taskId: 'task-1' } },
  { name: 'detectGlitches', run: () => detectGlitches({ path: '/a.mp4' } as never), cmd: 'detect_glitches', args: { request: { path: '/a.mp4' } } },
  { name: 'runWhisper', run: () => runWhisper({ clipId: 'c1' } as never), cmd: 'run_whisper', args: { request: { clipId: 'c1' } } },
  { name: 'runDemucs', run: () => runDemucs({ clipId: 'c1' } as never), cmd: 'run_demucs', args: { request: { clipId: 'c1' } } },
  { name: 'cancelDemucs', run: () => cancelDemucs('clip-1'), cmd: 'cancel_demucs', args: { clipId: 'clip-1' } },
  { name: 'processAudioNoiseReduction', run: () => processAudioNoiseReduction({ clipId: 'c1' } as never), cmd: 'process_audio_noise_reduction', args: { request: { clipId: 'c1' } } },
  { name: 'cancelAudioNoiseReduction', run: () => cancelAudioNoiseReduction('clip-1'), cmd: 'cancel_audio_noise_reduction', args: { clipId: 'clip-1' } },
  { name: 'detectPrivacyRegions', run: () => detectPrivacyRegions({ path: '/a.mp4' } as never), cmd: 'detect_privacy_regions', args: { request: { path: '/a.mp4' } } },
  { name: 'startRecording', run: () => startRecording({ width: 1280 } as never), cmd: 'start_recording', args: { request: { width: 1280 } } },
  { name: 'stopRecording', run: () => stopRecording('task-1'), cmd: 'stop_recording', args: { taskId: 'task-1' } },
  { name: 'scanDirectory(默认深度)', run: () => scanDirectory('/media'), cmd: 'scan_directory', args: { path: '/media', depth: 3 } },
  { name: 'scanDirectory(显式深度)', run: () => scanDirectory('/media', 1), cmd: 'scan_directory', args: { path: '/media', depth: 1 } },
  ...
  { name: 'listHardwareEncoders（无 mock 分支）', run: () => listHardwareEncoders(), cmd: 'list_hardware_encoders', args: {} },
  { name: 'getAvailableMemoryBytes', run: () => getAvailableMemoryBytes(), cmd: 'get_available_memory_bytes', args: {} },
  { name: 'getSystemResourceSnapshot', run: () => getSystemResourceSnapshot(), cmd: 'get_system_resource_snapshot', args: {} },
];
