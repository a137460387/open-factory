...
import {
  ...
  resolveColorNodeGraphPreviewPasses,
  resolveWebGlSourceProcessing,
...
describe('WebGL preview compositor bypass processing', () => {
  ...
  const masks: ClipMask[] = [
    { id: 'mask-a', type: 'rect', x: 0.1, y: 0.1, w: 0.5, h: 0.5, inverted: false, feather: 0.1, enabled: true },
  ];

  it('keeps color and effect processing for normal preview draws', () => {
    const result = resolveWebGlSourceProcessing(
      { inputColorSpace: 'slog2', brightness: 0.25 },
      effects,
      chromaKey,
      masks,
      { colorPipeline: 'aces' },
      ...
      [
        {
          ...
          type: 'motion-blur',
          enabled: true,
          params: { intensity: 0.5, angle: 90, samples: 16, jitter: 0.25 },
        },
      ],
      undefined,
      undefined,
