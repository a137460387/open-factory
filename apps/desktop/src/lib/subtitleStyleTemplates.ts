...
export async function loadSubtitleStyleTemplates(storage: SubtitleStyleTemplateStorage = bridgeSubtitleStyleTemplateStorage): Promise<SubtitleStyleTemplate[]> {
...
export async function deleteCustomSubtitleStyleTemplate(id: string, storage: SubtitleStyleTemplateStorage = bridgeSubtitleStyleTemplateStorage): Promise<SubtitleStyleTemplate[]> {
...
function mergeSubtitleStyleTemplates(customTemplates: SubtitleStyleTemplate[]): SubtitleStyleTemplate[] {
  ...
  return [
    ...
    ...customTemplates
      ...
      .map((template) => ({ ...template, kind: 'custom' as const, style: normalizeSubtitleStyleTemplateStyle(template.style) }))
  ];
}

async function loadCustomSubtitleStyleTemplates(storage: SubtitleStyleTemplateStorage): Promise<SubtitleStyleTemplate[]> {
...
async function writeCustomSubtitleStyleTemplates(templates: SubtitleStyleTemplate[], storage: SubtitleStyleTemplateStorage): Promise<void> {
  const path = getSubtitleStyleTemplatesPath(await storage.getAppDataDir());
...
function createCustomSubtitleStyleTemplateId(name: string, templates: SubtitleStyleTemplate[]): string {
  ...
  const base = `custom-${name.toLowerCase().replace(/[^a-z0-9\u4e00-\u9fa5]+/g, '-').replace(/^-+|-+$/g, '') || 'subtitle-style'}`;
  if (!used.has(base)) {
    return base;
  }
  let index = 2;
