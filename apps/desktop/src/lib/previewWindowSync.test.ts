...
describe('preview window playback sync', () => {
  it('normalizes incoming playhead payloads', () => {
    expect(normalizePreviewWindowPlaybackState({ source: 'preview-window', playheadTime: 1.23456, isPlaying: true, updatedAt: 10 })).toEqual({
  ...
  it('applies remote state only when playhead or playback changes meaningfully', () => {
    ...
    expect(shouldApplyPreviewWindowPlaybackState({ playheadTime: 1, isPlaying: false }, tinyMove, 'main', 1 / 24)).toBe(false);
    expect(shouldApplyPreviewWindowPlaybackState({ playheadTime: 1, isPlaying: false }, frameMove, 'main', 1 / 24)).toBe(true);
    expect(shouldApplyPreviewWindowPlaybackState({ playheadTime: 1, isPlaying: false }, playToggle, 'main', 1 / 24)).toBe(true);
  });
});
