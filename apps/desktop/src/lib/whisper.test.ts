...
import { DEFAULT_CLIP_SPEED, DEFAULT_COLOR_CORRECTION, DEFAULT_TRANSFORM, createDefaultTimeline, type Clip } from '@open-factory/editor-core';

describe('whisper helpers', () => {
  it('returns clear errors when whisper paths are not configured or do not exist', async () => {
    await expect(assertWhisperSettingsReady({ executablePath: '', modelPath: '' })).rejects.toThrow('Whisper 路径未配置。');
    await expect(
  ...
  it('enables subtitle generation only for configured audio/video clips with existing media', () => {
    const asset = { id: 'asset-1', type: 'video' as const, name: 'clip.mp4', path: 'C:/Media/clip.mp4', duration: 2, width: 1280, height: 720 };
  ...
  it('builds a subtitle track from Whisper SRT output', async () => {
    const asset = { id: 'asset-1', type: 'video' as const, name: 'clip.mp4', path: 'C:/Media/clip.mp4', duration: 4, width: 1280, height: 720, hasAudio: true };
      ...
      { executablePath: 'C:/Tools/whisper.exe', modelPath: 'C:/Models/base.bin' },
      {
        ...
        run: async (request) => {
          ...
          return {
            ...
            contents: ['1', '00:00:00,000 --> 00:00:01,000', 'Hello', '', '2', '00:00:01,500 --> 00:00:02,500', 'Factory', ''].join('\n'),
            durationMs: 20
          };
        }
      }
  ...
  it('aligns Whisper SRT cues to the selected clip timeline window', async () => {
    const asset = { id: 'asset-1', type: 'video' as const, name: 'clip.mp4', path: 'C:/Media/clip.mp4', duration: 8, width: 1280, height: 720, hasAudio: true };
    const track = await buildWhisperSubtitleTrackForClip(
...
function makeClip(type: 'audio' | 'video', patch: Partial<Extract<Clip, { type: 'audio' | 'video' }>> = {}): Extract<Clip, { type: 'audio' | 'video' }> {
  return {
    id: `clip-${type}`,
    type,
    name: type,
