...
describe('subtitle style template storage', () => {
  it('stores custom templates under AppData', () => {
    expect(getSubtitleStyleTemplatesPath('C:/Users/E2E/AppData/Roaming/open-factory/')).toBe('C:/Users/E2E/AppData/Roaming/open-factory/subtitle-styles.json');
  });

  it('saves loads and deletes custom templates', async () => {
    const files = new Map<string, string>();
    const storage = makeStorage(files);
    const saved = await saveCustomSubtitleStyleTemplate('Review White', { ...DEFAULT_SUBTITLE_STYLE, color: '#ffffff', outlineWidth: 2 }, storage);
    const custom = saved.find((template) => template.kind === 'custom');

    expect(custom).toMatchObject({ id: 'custom-review-white', name: 'Review White', style: { color: '#ffffff', outlineWidth: 2 } });
    ...
    expect(await loadSubtitleStyleTemplates(storage)).not.toEqual(expect.arrayContaining([expect.objectContaining({ id: 'custom-review-white' })]));
  });

  it('sanitizes stored custom templates and ignores malformed files', () => {
    const parsed = parseStoredSubtitleStyleTemplates(JSON.stringify({
      schemaVersion: 1,
      templates: [
    ...
    expect(parsed).toEqual([expect.objectContaining({ id: 'custom-a', style: expect.objectContaining({ fontSize: 200, color: DEFAULT_SUBTITLE_STYLE.color }) })]);
