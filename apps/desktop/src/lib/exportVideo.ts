...
export async function chooseExportPath(project: Project, format = 'mp4'): Promise<string | undefined> {
  ...
  return saveFileDialog(`${project.name || 'open-factory-export'}.${extension}`, [
    { name: zhCN.exportDialog.exportFilterName(extension), extensions: [extension] },
  ]);
}
...
export async function startCurrentFrameExport(
  project: Project,
  outputPath: string,
  time: number,
  callbacks: ExportCallbacks = { onProgress: () => undefined },
): Promise<void> {
