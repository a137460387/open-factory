...
export function StartupUpdateChecker() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" data-testid="update-dialog">
      <div className="flex max-h-[80vh] w-full max-w-xl flex-col overflow-hidden rounded-lg bg-white shadow-soft">
        <div className="flex items-center justify-between border-b border-line px-4 py-3">
          <div>
            ...
            <div className="text-xs text-slate-500">{notice.source === 'tauri-updater' ? t.sourceNative : t.sourceEndpoint}</div>
          </div>
          <button className="inline-flex h-8 w-8 items-center justify-center rounded-md text-slate-500 hover:bg-panel" type="button" title={zhCN.common.close} aria-label={zhCN.common.close} onClick={() => setDialogOpen(false)}>
            <X size={16} />
          </button>
        </div>
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          <h3 className="text-sm font-semibold text-ink">{t.releaseNotes}</h3>
          <div className="mt-2 whitespace-pre-wrap rounded-md border border-line bg-panel p-3 text-sm leading-6 text-slate-700" data-testid="update-release-notes">
            {notice.releaseNotes || t.noReleaseNotes}
          </div>
        </div>
        <div className="flex items-center justify-end gap-2 border-t border-line px-4 py-3">
          <button className="rounded-md border border-line bg-white px-3 py-1.5 text-sm font-medium text-slate-700 hover:bg-panel" type="button" onClick={() => setDialogOpen(false)}>
            {zhCN.common.close}
          </button>
          {notice.install ? (
            <button
