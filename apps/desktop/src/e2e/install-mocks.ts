...
let exportRunCalls: Array<{
  taskId?: string;
  fullArgs: string[];
  duration: number;
  outputPath?: string;
  settings?: FfmpegExportPlan['settings'];
}> = [];
...
files.set(
  ...
  [
    ...
    '0.25,1.25,CSV subtitle A',
    '00:00:01.500,00:00:02.500,CSV subtitle B',
    '3,4,CSV subtitle C',
  ].join('\n'),
);
...
const mocks: TauriMocks = {
  ...
  getFileStat: (path) => ({
    ...
    size:
      path === silencePatternAudio
        ? createSilencePatternWav().byteLength
        : path === tinyAudio || path === relinkedAudio
          ? createToneWav().byteLength
          : path === fourKHevcVideo
            ? 500 * 1024 * 1024
            : path.endsWith('.wav')
              ? 2048
  ...
  scanDirectory: (path) => {
    if (path.includes('/snapshots/')) {
      return Array.from(files.keys()).filter(
        (candidate) => candidate.startsWith(`${path}/`) && exists.get(candidate) !== false,
      );
    }
    if (Array.from(files.keys()).some((candidate) => candidate.startsWith(`${path.replace(/[\\/]+$/, '')}/`))) {
      const root = path.replace(/[\\/]+$/, '');
      return Array.from(files.keys()).filter(
        (candidate) => candidate.startsWith(`${root}/`) && exists.get(candidate) !== false,
      );
    }
    if (path === pluginDir) {
      return [pluginPath, permissionDeniedPluginPath, brokenPluginPath].filter(
        (candidate) => exists.get(candidate) !== false,
      );
    }
    if (path === appDataDir) {
      return [
        lutLibraryPath,
        `${appDataDir}/luts/readme.txt`,
        pluginPath,
        permissionDeniedPluginPath,
        brokenPluginPath,
      ].filter((candidate) => exists.get(candidate) !== false);
    }
  ...
  runExport: async (plan, taskId) => {
    ...
    if (outputPath.includes('%')) {
      ...
      files.set(
        outputPath,
        outputPath.endsWith('.png') || outputPath.endsWith('.jpg') || outputPath.endsWith('.jpeg')
          ? 'mock image frame'
          : 'mock mp4',
      );
      exists.set(outputPath, true);
    ...
    files.set(
      ...
      [
        `ffmpeg mock log`,
        `task=${exportCancelKey(taskId)}`,
        `args=${plan.fullArgs.join(' ')}`,
        `stderr=mock export completed`,
      ].join('\n'),
    );
    ...
    return {
      ...
      report: {
        ...(plan.passes?.some((pass) => pass.kind === 'loudness-analysis')
          ? { loudness: { integratedLoudness: -14.1 } }
          : {}),
  ...
  createSharePackage: async (request) => {
    ...
    emit('share-package-progress', {
      stage: 'readme',
      progress: total > 0 ? 1 / total : 1,
      progressPct: total > 0 ? 100 / total : 100,
      current: 1,
      total,
      outputPath: request.outputPath,
    });
  ...
  createSharedLibraryArchive: async (request) => {
    const payload = JSON.stringify({
      manifest: request.manifestContents,
      files: request.files.map((entry) => entry.archivePath),
    });
  ...
  importSharedLibraryArchive: async (request) => {
    ...
    const payload = raw.startsWith('mock shared library archive\n')
      ...
      : { manifest: '{"schemaVersion":1,"resources":[]}', files: [] };
    persistFiles();
    return {
      destinationDir: request.destinationDir,
      fileCount: 1 + (payload.files?.length ?? 0),
      manifestContents: String(payload.manifest ?? '{"schemaVersion":1,"resources":[]}'),
    };
  },
        ...
        ? [
            {
              ...
              status: postExportQualityStatus === 'fail' ? ('fail' as const) : ('pass' as const),
              message: postExportQualityStatus === 'fail' ? '导出时长误差超过 1 帧' : '导出时长在 1 帧误差内',
              expected: request.expectedDuration,
              actual: request.expectedDuration,
            },
          ]
        : []),
      ...(request.blackFrames
        ? [
            {
              id: 'blackFrames' as const,
              status: postExportQualityStatus === 'warning' ? ('warning' as const) : ('pass' as const),
              message: postExportQualityStatus === 'warning' ? '检测到意外黑帧 1 段' : '未检测到意外黑帧',
              actual: postExportQualityStatus === 'warning' ? 1 : 0,
            },
          ]
        : []),
      ...(request.silence
        ? [{ id: 'silence' as const, status: 'pass' as const, message: '未检测到意外静音', actual: 0 }]
        : []),
      ...(request.fileSize
        ? [
            {
              id: 'fileSize' as const,
              status: 'pass' as const,
              message: '导出文件大小在预期范围内',
              actual: files.get(request.outputPath)?.length ?? 4096,
            },
          ]
        : []),
      ...(request.resolution
        ? [
            {
              ...
              status: 'pass' as const,
              message: '输出分辨率与预设一致',
              expected: `${request.expectedWidth}x${request.expectedHeight}`,
              actual: `${request.expectedWidth}x${request.expectedHeight}`,
            },
          ]
        : []),
    ],
  ...
  probeMedia: (path) => {
    ...
    return {
      ...
      videoCodec:
        path === fourKHevcVideo ? 'hevc' : path.endsWith('.mp4') || path.endsWith('.mov') ? 'h264' : undefined,
      ...
      colorSpace:
        path === fourKHevcVideo ? 'bt2020nc' : path.endsWith('.mp4') || path.endsWith('.mov') ? 'bt709' : undefined,
      colorTransfer:
        path === fourKHevcVideo
          ? 'smpte2084'
          : isDisplayP3
  ...
  detectSceneChanges: async (request) => {
    ...
    emit('scene-detect-progress', {
      progress: 0.35,
      ptsTime: request.duration ? request.duration * 0.35 : undefined,
      analyzedFrames: totalFrames ? Math.round(totalFrames * 0.35) : undefined,
      totalFrames,
    });
    await wait(10);
    ...
    if (systemContent.includes('截帧图片')) {
      return {
        content: JSON.stringify({
          ...
          scenes: [
            { time: 0, description: '开场场景' },
            { time: 3, description: '结尾场景' },
          ],
          emotionArc: '从平静到激动',
    ...
    if (systemContent.includes('video encoding advisor')) {
      return {
        content: JSON.stringify([
          ...
          {
            parameter: 'loudnessNormalization',
            currentValue: 'off',
            suggestedValue: 'ebu',
            reason: '建议启用EBU响度标准化',
            priority: 'medium',
          ...
          {
            parameter: 'subtitleFormat',
            currentValue: 'none',
            suggestedValue: 'srt',
            reason: '项目含字幕轨，建议导出SRT',
            priority: 'low',
          },
        ]),
        inputTokens: 100,
          ...
          issues: [
            {
              type: '曝光',
              severity: 'medium',
              description: '画面整体偏暗，亮部细节不足',
              suggestedFix: '建议调整亮度+0.3',
            },
    ...
    if (systemContent.includes('降噪推荐助手')) {
      return {
        content: JSON.stringify({
          recommendedFilters: [
            {
              filter: 'afftdn' as const,
              params: { nr: 20, nf: -25 },
              reason: '检测到嗡声(hum)干扰，建议使用自适应降噪',
            },
    ...
    if (systemContent.includes('B-roll推荐助手')) {
      return {
        content: JSON.stringify({
          suggestions: [
            {
              segmentId: 'subtitle-clip-long',
              mediaId: 'media-broll-nature',
              insertTime: 1,
              reason: '自然风景与文本匹配',
              confidence: 0.88,
            },
...
window.fetch = (input, init) => {
  ...
  if (url.includes('api-free.deepl.com/v2/translate')) {
    ...
    return Promise.resolve(
      new Response(JSON.stringify({ translations: texts.map((text) => ({ text: `${text} 翻译` })) }), { status: 200 }),
    );
  }
  if (url.includes('translation.googleapis.com/language/translate/v2')) {
    ...
    return Promise.resolve(
      new Response(
        JSON.stringify({ data: { translations: texts.map((text) => ({ translatedText: `${text} 翻译` })) } }),
        { status: 200 },
      ),
    );
  }
  ...
  if (url === silencePatternAudio) {
    return Promise.resolve(
      new Response(silencePatternWav.buffer.slice(0) as ArrayBuffer, { headers: { 'Content-Type': 'audio/wav' } }),
    );
  }
  if (url === tinyAudio || url === relinkedAudio) {
    return Promise.resolve(
      new Response(tinyAudioWav.buffer.slice(0) as ArrayBuffer, { headers: { 'Content-Type': 'audio/wav' } }),
    );
  }
...
window.__E2E_ACTIONS__ = {
  setupLargeTimelineFixture: (clipCountInput?: unknown) => {
    const clipCount =
      typeof clipCountInput === 'number' && Number.isFinite(clipCountInput) && clipCountInput > 0
        ? Math.floor(clipCountInput)
        : 500;
    ...
    for (let index = 0; index < clipCount; index += 1) {
      ...
      track.clips.push(
        makeLargeTimelineVideoClip(
          `clip-large-${String(index).padStart(4, '0')}`,
          track.id,
          mediaId,
          localIndex * 1.25,
        ),
  ...
  createLargeProject: (options?: unknown) => {
    ...
    const clipCountOpt =
      typeof opts.clipCount === 'number' && Number.isFinite(opts.clipCount) && opts.clipCount > 0
        ? Math.floor(opts.clipCount)
        : 0;
    const trackCountOpt =
      typeof opts.trackCount === 'number' && Number.isFinite(opts.trackCount) && opts.trackCount > 0
        ? Math.floor(opts.trackCount)
        : 0;
    const clipsPerTrackOpt =
      typeof opts.clipsPerTrack === 'number' && Number.isFinite(opts.clipsPerTrack) && opts.clipsPerTrack > 0
        ? Math.floor(opts.clipsPerTrack)
        : 0;
    ...
    for (let index = 0; index < totalClips; index += 1) {
      ...
      const track = tracks[trackIndex];
      track.clips.push(
        makeLargeTimelineVideoClip(`clip-lp-${String(index).padStart(4, '0')}`, track.id, mediaId, localIndex * 1.25),
      );
    }
    const timeline = { transitions: [], markers: [], tracks };
    useEditorStore.getState().setProject({
    ...
    const timeline = {
      ...
      tracks: [
        createTrack({
          ...
          name: 'Video 1',
          clips: [
            { ...makeEditingVideoClip('clip-optimization-4k', 0, 8, 0, 0), mediaId: asset.id, name: '4K Optimization' },
          ],
        }),
        createTrack({ id: 'track-audio', type: 'audio', name: 'Audio 1', clips: [] }),
        createTrack({ id: 'track-text', type: 'text', name: 'Text 1', clips: [] }),
  ...
  setupSubtitleProofreadingFixture: () => {
    ...
    const shortSubtitle = {
      ...makeMockSubtitleClip('sub-proof-short', 'track-subtitle-proof', '太短', 0),
      duration: 0.4,
    };
    ...
    const timeline = {
      ...
      tracks: [
        ...
        createTrack({
          ...
          name: 'Boom Mic',
          clips: [
            makeAutoSyncAudioClip('clip-auto-secondary', 'Boom Mic', 'media-auto-secondary', 'track-auto-secondary', 1),
          ],
        }),
        createTrack({ id: 'track-text', type: 'text', name: 'Text 1', clips: [] }),
      ],
    ...
    const mediaAssets: MediaAsset[] = [
      {
        ...
        aiAnalysis: {
          ...
          mood: '专业',
          objects: ['产品', '桌子'],
          analysisTime: '2025-01-01T00:00:00Z',
          providerId: 'mock',
        },
      },
      {
    ...
    const timeline = {
      ...
      tracks: [
        createTrack({
          ...
          type: 'video',
          name: 'Camera A',
          clips: [makeMulticamVideoClip('clip-camera-a', 'media-camera-a', 'track-camera-a', 'Camera A')],
        }),
        createTrack({
          id: 'track-camera-b',
          type: 'video',
          name: 'Camera B',
          clips: [makeMulticamVideoClip('clip-camera-b', 'media-camera-b', 'track-camera-b', 'Camera B')],
        }),
        createTrack({ id: 'track-audio', type: 'audio', name: 'Audio 1', clips: [] }),
        createTrack({ id: 'track-text', type: 'text', name: 'Text 1', clips: [] }),
      ],
  ...
  enqueueMockMediaJob: (input: unknown) => {
    ...
    return useMediaJobStore.getState().enqueueMonitorJob({
      ...
      type:
        job.type === 'gif-preview' ||
        job.type === 'vfr-conversion' ||
        job.type === 'frame-rate-conversion' ||
        job.type === 'stabilization-analysis' ||
        job.type === 'waveform'
          ? job.type
          : 'proxy',
      status:
        job.status === 'running' || job.status === 'success' || job.status === 'error' || job.status === 'canceled'
          ? job.status
          : 'pending',
  ...
  getReleaseFiles: () =>
    Array.from(files.keys()).filter(
      (path) => path.includes('/releases/') && path.endsWith('.json') && exists.get(path) !== false,
    ),
  getBackupFiles: (path: unknown) => {
    ...
    return Array.from(files.keys()).filter(
      (candidate) =>
        candidate.startsWith(`${root}/`) && candidate.endsWith('.cutproj.json') && exists.get(candidate) !== false,
    );
  },
  ...
  addKeyframe: (clipId: unknown, property: unknown, time: unknown, value: unknown) => {
    ...
    commandManager.execute(
      new AddKeyframeCommand(timelineAccessor, clipId, property, { time, value, easing: 'linear' }),
    );
  },
  ...
  setTutorialProgress: (settings: unknown) => {
    ...
    writeTutorialProgressSettings(
      {
        tutorialStep:
          typeof input.tutorialStep === 'number' && Number.isFinite(input.tutorialStep)
            ? Math.round(input.tutorialStep)
            : 0,
  ...
  clearE2eFiles: () => {
    ...
    for (const path of Array.from(files.keys()).filter(
      (item) => item.includes('/snapshots/') || item.includes('/releases/'),
    )) {
      files.delete(path);
      exists.set(path, false);
      mtimes.delete(path);
    ...
    for (const path of Array.from(files.keys()).filter(
      (item) => item.includes('/effect-presets/') && item.endsWith('.ofeffect.json'),
    )) {
      files.delete(path);
      exists.set(path, false);
      mtimes.delete(path);
    ...
    for (const path of Array.from(files.keys()).filter(
      (item) => item.includes('/Backups/') || item.startsWith('C:/Backups/'),
    )) {
      files.delete(path);
      exists.set(path, false);
      mtimes.delete(path);
    ...
    for (const path of Array.from(files.keys()).filter(
      (item) => item.includes('/shared-library/') || item.includes('/timeline-templates/'),
    )) {
      files.delete(path);
      exists.set(path, false);
      mtimes.delete(path);
  ...
  setupAISubtitlePolishFixture: async () => {
    ...
    const timeline = {
      ...
      tracks: [
        ...
        createTrack({
          id: 'track-ai-subtitle',
          type: 'subtitle',
          name: 'AI Subtitles',
          language: 'zh',
          clips: [sub1, sub2, sub3],
        }),
      ],
    };
    useEditorStore.getState().setProject({
  ...
  setupAIChatEditorFixture: async () => {
    ...
    const timeline = {
      transitions: [],
      markers: [],
      tracks: [createTrack({ id: 'track-chat-video', type: 'video', name: 'Video 1', clips: [videoClip] })],
    };
    useEditorStore.getState().setProject({
      ...project,
      media: [
  ...
  setupAIVideoSummaryFixture: async () => {
    ...
    const timeline = {
      transitions: [],
      markers: [],
      tracks: [createTrack({ id: 'track-summary-video', type: 'video', name: 'Video 1', clips: [videoClip] })],
    };
    useEditorStore.getState().setProject({
      ...project,
      media: [asset],
  ...
  setupAINarrationScriptFixture: async () => {
    ...
    const timeline = {
      ...
      tracks: [createTrack({ id: 'track-narration-video', type: 'video', name: 'Video 1', clips: [videoClip] })],
    };
    useEditorStore.getState().setProject({
      ...project,
      media: [
  ...
  setupAISemanticSearchFixture: async () => {
    ...
    const assetC: MediaAsset = {
      id: 'media-ai-search-unanalyzed',
      type: 'video',
      name: 'unanalyzed-clip.mp4',
      path: tinyVideo,
      duration: 10,
      width: 640,
      height: 360,
  ...
  setupAISceneMatchFixture: async () => {
    ...
    const assetC: MediaAsset = {
      id: 'media-scene-match-c',
      type: 'video',
      name: 'nature-forest.mp4',
      path: tinyVideo,
      duration: 15,
      width: 1280,
      height: 720,
    ...
    const assetD: MediaAsset = {
      ...
      aiAnalysis: {
        ...
        mood: 'moody',
        objects: ['buildings', 'lights'],
        analysisTime: new Date().toISOString(),
        providerId: 'mimo',
      },
    };
    const assetE: MediaAsset = {
      id: 'media-scene-match-e',
      type: 'video',
      name: 'unanalyzed-1.mp4',
      path: tinyVideo,
      duration: 10,
      width: 640,
      height: 360,
    ...
    const assetF: MediaAsset = {
      id: 'media-scene-match-f',
      type: 'video',
      name: 'unanalyzed-2.mp4',
      path: tinyVideo,
      duration: 8,
      width: 640,
      height: 360,
    ...
    const clipA = {
      id: 'scene-match-clip-a',
      type: 'video' as const,
      name: 'outdoor-sunny.mp4',
      mediaId: 'media-scene-match-a',
      trackId: 'track-scene-match-video',
      start: 0,
      duration: 30,
      trimStart: 0,
      trimEnd: 0,
    ...
    const timeline = {
      transitions: [],
      markers: [],
      tracks: [createTrack({ id: 'track-scene-match-video', type: 'video', name: 'Video 1', clips: [clipA] })],
    };
    useEditorStore.getState().setProject({
      ...project,
  ...
  setupAISubtitleStyleFixture: async () => {
    ...
    const assetVideo: MediaAsset = {
      id: 'media-sub-style-video',
      type: 'video',
      name: 'style-video.mp4',
      path: tinyVideo,
      duration: 30,
      width: 1920,
      height: 1080,
  ...
  setupAIQualityAssessmentFixture: async () => {
    ...
    const timeline = {
      transitions: [],
      markers: [],
      tracks: [createTrack({ id: 'track-qa-video', type: 'video', name: 'Video 1', clips: [] })],
    };
    useEditorStore.getState().setProject({
      ...project,
      media: [asset],
    ...
    const timeline = {
      ...
      tracks: [
        createTrack({
          ...
          type: 'video',
          name: 'Video 1',
          clips: [makeStoryboardClip('clip-mm-video', 'video', 'product-video.mp4', 'media-mm-video', 0, 10)],
        }),
        createTrack({ id: 'track-audio', type: 'audio', name: 'Audio 1', clips: [] }),
      ],
    };
  ...
  setupHighlightReelFixture: async () => {
    ...
    const mediaAssets: MediaAsset[] = [
      {
        id: 'media-highlight-a',
        type: 'video',
        name: 'highlight-a.mp4',
        path: tinyVideo,
        duration: 4,
        width: 1920,
        height: 1080,
      ...
      {
        id: 'media-highlight-b',
        type: 'video',
        name: 'highlight-b.mp4',
        path: tinyVideo,
        duration: 3,
        width: 1920,
        height: 1080,
      ...
      {
        id: 'media-highlight-c',
        type: 'video',
        name: 'highlight-c.mp4',
        path: tinyVideo,
        duration: 5,
        width: 1920,
        height: 1080,
  ...
  setupBeatSnapFixture: async () => {
    ...
    const beatSnapSuggestions = [{ clipId: 'clip-bs-3', edge: 'in' as const, suggestedTime: 7.12, originalTime: 7.0 }];
    useEditorStore.getState().setProject({
      ...project,
      media: [asset],
      timeline,
  ...
  setupMediaOrganizeFixture: async () => {
    ...
    const timeline = {
      transitions: [],
      markers: [],
      tracks: [createTrack({ id: 'track-mo', type: 'video', name: 'Video 1', clips: [] })],
    };
    const existingCollections = [
      {
        id: 'col-manual-1',
        name: '手动分组',
        mediaIds: ['media-organize-0'],
    ...
    useEditorStore.getState().setProject({
      ...
      media: mediaAssets,
      timeline,
      mediaCollections: existingCollections,
      sequences: [{ id: PRIMARY_SEQUENCE_ID, name: DEFAULT_PRIMARY_SEQUENCE_NAME, timeline }],
      activeSequenceId: PRIMARY_SEQUENCE_ID,
    });
    useEditorStore.getState().setSelectedClipIds([]);
  ...
  setupAIDenoiseFixture: async () => {
    ...
    const clip = {
      ...
      type: 'audio' as const,
      name: 'test-audio.wav',
      mediaId: 'media-denoise-audio',
      trackId: 'track-denoise-audio',
      start: 0,
      duration: 10,
      trimStart: 0,
      trimEnd: 0,
        ...
        recommendedFilters: [
          {
            ...
            params: { nr: 20, nf: -25 },
            reason: '检测到嗡声(hum)干扰，建议使用自适应降噪',
          } as DenoiseFilterRecommendation,
          {
            filter: 'highpass',
            params: { f: 80 },
            reason: '检测到嗡声低频干扰，建议高通滤波',
          } as DenoiseFilterRecommendation,
        ],
        appliedFilters: [],
    ...
    const timeline = {
      transitions: [],
      markers: [],
      tracks: [createTrack({ id: 'track-denoise-audio', type: 'audio', name: 'Audio 1', clips: [clip] })],
    };
    useEditorStore.getState().setProject({
      ...project,
      media: [asset],
  ...
  setupAIDenoiseFixtureNoProvider: async () => {
    ...
    const asset: MediaAsset = {
      id: 'media-denoise-audio-2',
      type: 'audio',
      name: 'test-audio-2.wav',
      path: tinyVideo,
      duration: 10,
      width: 0,
      height: 0,
    ...
    const clip = {
      ...
      type: 'audio' as const,
      name: 'test-audio-2.wav',
      mediaId: 'media-denoise-audio-2',
      trackId: 'track-denoise-audio-2',
      start: 0,
      duration: 10,
      trimStart: 0,
      trimEnd: 0,
  ...
  setupAIDenoiseLocalFixture: async () => {
    ...
    const asset: MediaAsset = {
      id: 'media-local-denoise-audio',
      type: 'audio',
      name: 'noisy-audio.wav',
      path: tinyVideo,
      duration: 10,
      width: 0,
      height: 0,
    ...
    const clip = {
      id: 'local-denoise-clip-1',
      type: 'audio' as const,
      name: 'noisy-audio.wav',
      mediaId: 'media-local-denoise-audio',
      trackId: 'track-local-denoise-audio',
      start: 0,
      duration: 10,
      trimStart: 0,
      trimEnd: 0,
    ...
    const timeline = {
      transitions: [],
      markers: [],
      tracks: [createTrack({ id: 'track-local-denoise-audio', type: 'audio', name: 'Audio 1', clips: [clip] })],
    };
    useEditorStore.getState().setProject({ ...project, media: [asset], timeline });
  ...
  setupAIBrollFixture: async () => {
    ...
    const mainClip = {
      ...
      type: 'video' as const,
      name: 'main-video.mp4',
      mediaId: 'media-broll-video',
      trackId: 'track-broll-video',
      start: 0,
      duration: 30,
      trimStart: 0,
      trimEnd: 0,
    ...
    const longSubtitleClip = {
      id: 'subtitle-clip-long',
      type: 'subtitle' as const,
      name: 'Long subtitle',
      trackId: 'track-broll-subtitle',
      start: 0,
      duration: 5,
      text: 'nature forest walking',
      trimStart: 0,
      trimEnd: 0,
      speed: DEFAULT_CLIP_SPEED,
    ...
    useEditorStore.getState().setProject({
      ...project,
      media: [videoAsset, natureAsset, cityAsset],
      timeline,
      sequences: [{ id: PRIMARY_SEQUENCE_ID, name: DEFAULT_PRIMARY_SEQUENCE_NAME, timeline }],
      activeSequenceId: PRIMARY_SEQUENCE_ID,
    });
  ...
  setupAIBrollFixtureNoGaps: async () => {
    ...
    const videoAsset: MediaAsset = {
      id: 'media-broll-video-2',
      type: 'video',
      name: 'main-video-2.mp4',
      path: tinyVideo,
      duration: 30,
      width: 1280,
      height: 720,
    ...
    const mainClip = {
      id: 'broll-main-clip-2',
      type: 'video' as const,
      name: 'main-video-2.mp4',
      mediaId: 'media-broll-video-2',
      trackId: 'track-broll-video-2',
      start: 0,
      duration: 30,
      trimStart: 0,
      trimEnd: 0,
    ...
    const shortClip = {
      id: 'subtitle-clip-short',
      type: 'subtitle' as const,
      name: 'Short subtitle',
      trackId: 'track-broll-subtitle-2',
      start: 0,
      duration: 2,
      text: 'hi',
      trimStart: 0,
  ...
  setupAILoudnessFixture: () => {
    ...
    const clip = {
      ...
      type: 'audio' as const,
      name: 'loud-audio.wav',
      mediaId: 'media-loudness-audio',
      trackId: 'track-loudness-audio',
      start: 0,
      duration: 10,
      trimStart: 0,
      trimEnd: 0,
  ...
  setupFlashWarningFixture: () => {
    ...
    const asset = {
      id: 'media-flash-video',
      type: 'video' as const,
      name: 'flash-video.mp4',
      path: tinyVideo,
      duration: 10,
      width: 1920,
      height: 1080,
    ...
    const clip = {
      ...
      type: 'video' as const,
      name: 'flash-video.mp4',
      mediaId: 'media-flash-video',
      trackId: 'track-flash-video',
      start: 0,
      duration: 10,
      trimStart: 0,
      trimEnd: 0,
  ...
  setupFlashWarningReduceFixture: () => {
    ...
    const asset = {
      id: 'media-flash-reduce',
      type: 'video' as const,
      name: 'flash-reduce.mp4',
      path: tinyVideo,
      duration: 10,
      width: 1920,
      height: 1080,
    ...
    const clip = {
      id: 'clip-flash-reduce',
      type: 'video' as const,
      name: 'flash-reduce.mp4',
      mediaId: 'media-flash-reduce',
      trackId: 'track-flash-reduce',
      start: 0,
      duration: 10,
      trimStart: 0,
      trimEnd: 0,
      ...
      colorCorrection: { ...DEFAULT_COLOR_CORRECTION },
      transform: { ...DEFAULT_TRANSFORM },
      flashWarnings: [{ startTime: 2, endTime: 4, flashRate: 4, severity: 'low' as const, isRedFlash: false }],
    };
    const timeline = {
      transitions: [],
      markers: [],
  ...
  setupContinuityWarningFixture: () => {
    ...
    const clipA = {
      ...
      type: 'video' as const,
      name: 'cont-a.mp4',
      mediaId: 'media-cont-a',
      trackId: 'track-cont-video',
      start: 0,
      duration: 5,
      trimStart: 0,
      trimEnd: 0,
    ...
    const timeline = {
      ...
      continuityWarnings: [
        ...
        },
        {
          clipAId: 'clip-cont-a',
          clipBId: 'clip-cont-b',
          type: 'jump_cut' as const,
          confidence: 0.85,
          reason: '构图中心距离<5%, 时长差<0.5s',
        },
      ],
    };
    useEditorStore
  ...
  setupMotionTypeFixture: () => {
    ...
    const clipPan = {
      ...
      type: 'video' as const,
      name: 'pan.mp4',
      mediaId: 'media-mt-pan',
      trackId: 'track-mt-video',
      start: 0,
      duration: 5,
      trimStart: 0,
      trimEnd: 0,
      ...
      colorCorrection: { ...DEFAULT_COLOR_CORRECTION },
      transform: { ...DEFAULT_TRANSFORM },
      motionType: { type: 'pan' as const, confidence: 0.92, analyzedAt: new Date().toISOString() },
    };
    const clipTilt = {
      id: 'clip-mt-tilt',
      type: 'video' as const,
      name: 'tilt.mp4',
      mediaId: 'media-mt-tilt',
      trackId: 'track-mt-video',
      start: 5,
      duration: 5,
      trimStart: 0,
      ...
      colorCorrection: { ...DEFAULT_COLOR_CORRECTION },
      transform: { ...DEFAULT_TRANSFORM },
      motionType: { type: 'tilt' as const, confidence: 0.88, analyzedAt: new Date().toISOString() },
    };
    const clipStatic = {
      id: 'clip-mt-static',
      type: 'video' as const,
      name: 'static.mp4',
      mediaId: 'media-mt-static',
      trackId: 'track-mt-video',
      start: 10,
      duration: 5,
      trimStart: 0,
      ...
      colorCorrection: { ...DEFAULT_COLOR_CORRECTION },
      transform: { ...DEFAULT_TRANSFORM },
      motionType: { type: 'static' as const, confidence: 0.95, analyzedAt: new Date().toISOString() },
    };
    const timeline = {
      transitions: [],
      markers: [],
      ...
      const clip = p.timeline.tracks
        ...
        .find(
          (c) =>
            ...
            c.mediaId === m.id &&
            'motionType' in c &&
            (c as { motionType?: { type: string } }).motionType?.type === mt,
        );
      return clip != null;
    });
      ...
      colorConsistencyWarnings: [
        {
          clipAId: 'clip-cc-a',
          clipBId: 'clip-cc-b',
          type: 'skin_tone' as const,
          deltaRGB: 45.2,
          reason: '肤色ΔRGB=45.2 > 30',
        },
      ],
    };
    useEditorStore
  ...
  applyColorCompensation: () => {
    const p = useEditorStore.getState().project;
    if (!p) return;
    const warnings = (p.timeline.colorConsistencyWarnings ?? []).filter(
  ...
  setupPacingAnalysisFixture: () => {
    ...
    const clips = Array.from({ length: 18 }, (_, i) => ({
      ...
      type: 'video' as const,
      name: `pacing-${i}.mp4`,
      mediaId: 'media-pacing',
      trackId: 'track-pacing',
      start: i * 5,
      duration: 5,
      trimStart: 0,
      trimEnd: 0,
  ...
  setupCharacterTimelineFixture: () => {
    ...
    const clip1 = {
      id: 'clip-char-1',
      type: 'video' as const,
      name: 'char-clip-1.mp4',
      mediaId: 'media-char',
      trackId: 'track-char-video',
      start: 0,
      duration: 5,
      trimStart: 0,
      trimEnd: 0,
    ...
    const characterTimeline = {
      characters: {
        character_1: {
          label: '戴眼镜的男性',
          appearances: [{ clipId: 'clip-char-1', startTime: 0, endTime: 4, confidence: 0.85 }],
        },
        character_2: {
          label: '红色上衣的女性',
          appearances: [{ clipId: 'clip-char-2', startTime: 5, endTime: 9, confidence: 0.78 }],
        },
      },
  ...
  setupCharacterTimelineMergeFixture: () => {
    ...
    const clip1 = {
      id: 'clip-char-merge-1',
      type: 'video' as const,
      name: 'char-merge-1.mp4',
      mediaId: 'media-char-merge',
      trackId: 'track-char-merge',
      start: 0,
      duration: 5,
      trimStart: 0,
      trimEnd: 0,
    ...
    const clip2 = {
      id: 'clip-char-merge-2',
      type: 'video' as const,
      name: 'char-merge-2.mp4',
      mediaId: 'media-char-merge',
      trackId: 'track-char-merge',
      start: 5,
      duration: 5,
    ...
    const characterTimeline = {
      characters: {
        character_1: {
          label: '戴眼镜, 蓝色上衣, 男性',
          appearances: [
            { clipId: 'clip-char-merge-1', startTime: 0, endTime: 4, confidence: 0.85 },
            { clipId: 'clip-char-merge-2', startTime: 5, endTime: 9, confidence: 0.82 },
          ],
        },
      },
      lastAnalyzedAt: new Date().toISOString(),
  ...
  setupPreflightChecklistFixture: () => {
    ...
    const clip1 = {
      id: 'clip-preflight-1',
      type: 'video' as const,
      name: 'preflight-1.mp4',
      mediaId: 'media-preflight',
      trackId: 'track-preflight',
      start: 0,
      duration: 5,
      trimStart: 0,
      trimEnd: 0,
    ...
    const timeline = {
      ...
      continuityWarnings: [
        {
          clipAId: 'clip-preflight-1',
          clipBId: 'clip-preflight-2',
          type: 'jump_cut' as const,
          confidence: 0.9,
          reason: '跳切检测',
        },
  ...
  setupPreflightAcknowledgeFixture: () => {
    ...
    const asset = {
      id: 'media-preflight-ack',
      type: 'video' as const,
      name: 'preflight-ack.mp4',
      path: tinyVideo,
      duration: 10,
      width: 1920,
      height: 1080,
    ...
    const clip1 = {
      id: 'clip-preflight-ack-1',
      type: 'video' as const,
      name: 'preflight-ack-1.mp4',
      mediaId: 'media-preflight-ack',
      trackId: 'track-preflight-ack',
      start: 0,
      duration: 5,
      trimStart: 0,
      trimEnd: 0,
    ...
    const clip2 = {
      id: 'clip-preflight-ack-2',
      type: 'video' as const,
      name: 'preflight-ack-2.mp4',
      mediaId: 'media-preflight-ack',
      trackId: 'track-preflight-ack',
      start: 5,
      duration: 5,
    ...
    const timeline = {
      ...
      continuityWarnings: [
        {
          clipAId: 'clip-preflight-ack-1',
          clipBId: 'clip-preflight-ack-2',
          type: 'jump_cut' as const,
          confidence: 0.9,
          reason: '跳切检测',
        },
        ...
        flash: [
          {
            ...
            message: '闪烁警告: flashRate=5.0, severity=high, 红色闪烁',
            time: 1.0,
            clipId: 'clip-preflight-ack-1',
          },
        ],
        continuity: [
          {
            id: continuityIssueId,
            category: 'continuity' as const,
            severity: 'warning' as const,
            message: '连续性警告: jump_cut - 跳切检测',
            clipId: 'clip-preflight-ack-1',
          },
        ],
      },
  ...
  setupEmotionToneMultiFixture: () => {
    ...
    const clipCalm = {
      ...
      type: 'video' as const,
      name: 'emo-calm.mp4',
      mediaId: 'media-emo-m1',
      trackId: 'track-emo-multi',
      start: 0,
      duration: 3,
      trimStart: 0,
      trimEnd: 0,
    ...
    const clipEnergetic = {
      id: 'clip-emo-energetic',
      type: 'video' as const,
      name: 'emo-energetic.mp4',
      mediaId: 'media-emo-m2',
      trackId: 'track-emo-multi',
      start: 3,
      duration: 3,
      trimStart: 0,
      ...
      transform: { ...DEFAULT_TRANSFORM },
      emotionAnalysis: {
        emotionTone: 'energetic' as const,
        intensity: 0.9,
        reason: '快节奏舞蹈',
        analyzedAt: new Date().toISOString(),
      },
    ...
    const timeline = {
      ...
      tracks: [
        createTrack({
          ...
          type: 'video',
          name: 'Video 1',
          clips: [clipCalm, clipEnergetic, clipTense],
        }),
      ],
    };
    useEditorStore
  ...
  setupDubbingAdaptationCompressFixture: () => {
    ...
    useEditorStore.getState().setProject({
      ...project,
      ttsSegments: [{ id: 'tts-seg-1', subtitleClipId: 'clip-sub-1', originalDuration: 10, dubbedDuration: 13 }],
    });
    commandManager.clear();
  },
...
function makeEditingVideoClip(
  id: string,
  start: number,
  duration: number,
  trimStart: number,
  trimEnd: number,
): Extract<Clip, { type: 'video' }> {
  return {
    id,
    type: 'video',
    name: `${id}.mp4`,
...
function makeLargeTimelineVideoClip(
  id: string,
  trackId: string,
  mediaId: string,
  start: number,
...
function makeMockSubtitleClip(
  id: string,
  trackId: string,
  text: string,
  start: number,
): Extract<Clip, { type: 'subtitle' }> {
  return {
    id,
    type: 'subtitle',
    name: text,
...
function makeAutoSyncAudioClip(
  id: string,
  name: string,
  mediaId: string,
  trackId: string,
...
function makeMulticamVideoClip(
  id: string,
  mediaId: string,
  trackId: string,
  name: string,
): Extract<import('@open-factory/editor-core').Clip, { type: 'video' }> {
  return {
    id,
    type: 'video',
    name,
...
function buildMockGifArgs(
  request: GifPreviewRequest | GifExportRequest,
  outputPath: string,
  scaleWidth: number,
  loopCount: number,
): string[] {
        ...
        clips: [
          {
            ...
            multicam: {
              ...
              switches: [{ id: 'sw-init', time: 0, angleId: 'angle-a' }],
              aiCutSuggestions: [
