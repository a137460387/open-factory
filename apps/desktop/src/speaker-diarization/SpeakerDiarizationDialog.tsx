...
export default function SpeakerDiarizationDialog({ segments, tracks, sourceName, onApply, onClose }: SpeakerDiarizationDialogProps) {
  const t = zhCN.speakerDiarization;
  const lowCount = segments.filter((segment) => segment.confidenceLabel === 'low').length;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 p-4" data-testid="speaker-diarization-dialog">
      <section className="flex max-h-[82vh] w-full max-w-3xl flex-col rounded-md border border-line bg-white shadow-soft">
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4 text-sm">
          {lowCount > 0 ? (
            <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800" data-testid="speaker-diarization-low-confidence">
          ...
          <div className="grid gap-3 md:grid-cols-[180px_1fr]">
            <div className="rounded-md border border-line bg-panel p-3">
              ...
              <div className="mt-2 space-y-2">
                {tracks.map((track) => (
                  <div key={track.id} className="rounded border border-line bg-white px-2 py-1.5 text-xs text-slate-700" data-testid="speaker-diarization-track">
        ...
        <div className="flex justify-end gap-2 border-t border-line px-4 py-3">
          <button className="rounded-md border border-line px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-panel" type="button" onClick={onClose}>
            {t.close}
          </button>
          <button className="rounded-md bg-brand px-3 py-2 text-xs font-semibold text-white hover:bg-[#176858]" type="button" data-testid="speaker-diarization-apply" onClick={onApply}>
            {t.apply}
          </button>
        </div>
      </section>
