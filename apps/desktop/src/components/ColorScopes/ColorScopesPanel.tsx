...
export function ColorScopesPanel({ frame, active }: { frame?: PreviewFrameReadback; active: boolean }) {
  ...
  useEffect(() => {
    ...
    workerRef.current ??= new Worker(new URL('../../workers/color-scopes.worker.ts', import.meta.url), { type: 'module' });
...
function VectorscopeCanvas({ scopes }: { scopes?: ColorScopes }) {
  const ref = useCanvas((context, width, height) => {
    ...
    context.lineTo(centerX + Math.cos((123 * Math.PI) / 180) * radius, centerY - Math.sin((123 * Math.PI) / 180) * radius);
    context.stroke();
    if (!scopes) {
      return;
    }
...
function drawHistogramChannel(context: CanvasRenderingContext2D, buckets: number[], max: number, width: number, height: number, color: string): void {
