...
import { Section, RangeField, RangeNumberField, ToggleField, AnimatedField } from './InspectorFields';
      ...
      {clip.type === 'video' || clip.type === 'audio' ? (
        <Section title={zhCN.inspector.sections.aiLocalDenoise}>
          ...
          <AiLocalDenoiseControls
            ...
            aiLocalDenoise={clip.aiLocalDenoise}
            aiLocalDenoiseProcessing={aiLocalDenoiseProcessing}
            setAiLocalDenoiseProcessing={setAiLocalDenoiseProcessing}
            aiLocalDenoiseProgress={aiLocalDenoiseProgress}
            setAiLocalDenoiseProgress={setAiLocalDenoiseProgress}
            aiLocalDenoiseStage={aiLocalDenoiseStage}
            setAiLocalDenoiseStage={setAiLocalDenoiseStage}
            aiLocalDenoiseResult={aiLocalDenoiseResult}
            setAiLocalDenoiseResult={setAiLocalDenoiseResult}
            commit={commit}
          />
        </Section>
      ) : null}
            ...
            <div className="space-y-3 border-t border-line p-2">
              ...
              <label className="block text-xs font-medium text-[var(--color-text-secondary)]">
                ...
                <select
                  ...
                  onChange={(event) =>
                    commit({ spatialAudio: { ...spatialAudio, distance: event.target.value as SpatialAudioDistance } })
                  }
                >
...
function AiLocalDenoiseControls({
    ...
    return (
      <div className="space-y-2" data-testid="ai-local-denoise-progress">
        ...
        <button
          ...
          type="button"
          onClick={() => {
            void cancelAudioNoiseReduction(clipId);
  ...
  return (
    <button
      ...
      type="button"
      disabled={!aiLocalDenoise?.enabled || !assetPath}
      onClick={async () => {
        if (!assetPath) return;
