...
export function EffectPanel({
  ...
  runEffectCommand,
  chooseLut,
  commitChromaKeyColors: _commitChromaKeyColors,
    ...
    <>
      ...
      {clip.type === 'video' || clip.type === 'image' || clip.type === 'nested-sequence' ? (
        <Section title={zhCN.inspector.sections.chromaKey}>
          ...
          {keyingMode === 'chroma-key' ? (
            <ChromaKeyControls
              chromaKey={chromaKey}
              chromaKeyPickActive={chromaKeyPickActive}
              commit={commit}
              updateChromaKeyColor={updateChromaKeyColor}
              addChromaKeyColor={addChromaKeyColor}
              removeChromaKeyColor={removeChromaKeyColor}
              toggleChromaKeyPicker={toggleChromaKeyPicker}
            />
          ) : null}
          {keyingMode === 'luma-key' ? (
            ...
            <div className="space-y-2" data-testid="difference-matte-controls">
              ...
              <RangeNumberField
                ...
                onCommit={(differenceThreshold) =>
                  commit({ chromaKey: { ...chromaKey, enabled: true, mode: 'difference-matte', differenceThreshold } })
                }
      ...
      {clip.type === 'video' ? (
        <Section title={zhCN.inspector.sections.videoRestoration}>
          <VideoRestorationControls
            videoRestoration={videoRestoration}
            deinterlaceSuggestion={deinterlaceSuggestion}
            asset={asset}
            updateVideoRestoration={updateVideoRestoration}
          />
        </Section>
      ) : null}
      ...
      <div className="space-y-2" data-testid="chroma-key-color-list">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              className={`inline-flex h-8 w-8 items-center justify-center rounded-md border border-line text-[var(--color-text-secondary)] hover:bg-panel ${chromaKeyPickActive ? 'bg-emerald-50 ring-1 ring-emerald-300' : 'bg-[var(--color-bg-elevated)]'}`}
              type="button"
...
function PrivacyRedactionPanel({
  privacyRedactions,
  commit,
}: Pick<ClipInspectorBodyProps, 'privacyRedactions' | 'commit'>) {
  return (
...
function VideoRestorationControls({
  videoRestoration,
  deinterlaceSuggestion,
  asset,
  updateVideoRestoration,
}: Pick<ClipInspectorBodyProps, 'videoRestoration' | 'deinterlaceSuggestion' | 'asset' | 'updateVideoRestoration'>) {
  return (
    <>
          ...
          <div className="mt-3 space-y-2" data-testid="video-restoration-temporal-custom">
            <RangeNumberField
              ...
              onCommit={(lumaSpatial) =>
                updateVideoRestoration({ temporalDenoise: { ...videoRestoration.temporalDenoise, lumaSpatial } })
              }
            ...
            <RangeNumberField
              ...
              onCommit={(chromaSpatial) =>
                updateVideoRestoration({ temporalDenoise: { ...videoRestoration.temporalDenoise, chromaSpatial } })
              }
