  ...
  const filteredMedia =
    mediaSource === 'all'
      ...
      : mediaSource === 'favorites'
        ? media.filter((m) => favoriteIds.includes(m.id))
        : media;
    ...
    try {
      ...
      const clips: Clip[] = activeCards.map((card, index) => {
        ...
        const base = createBaseClip({
          ...
          trimEnd: Math.max(
            0,
            asset.duration - Math.min(card.trimStart, Math.max(0, asset.duration - clipDuration)) - clipDuration,
          ),
        });
        cursor += clipDuration;
      ...
      commandManager.execute(
        new BatchAddClipsCommand(timelineAccessor, clips, [{ id: trackId, name: 'AI导演', type: trackType }]),
      );
      ...
      if (addMusicTrack) {
        ...
        commandManager.execute(
          new AddTrackCommand(timelineAccessor, {
            id: musicTrackId,
            type: 'audio',
            name: '背景音乐（待添加）',
            clips: [],
      ...
      <div className="flex flex-col h-full" data-testid="director-mode-panel">
        <div className="flex items-center justify-between border-b border-line px-3 py-2">
          ...
          <button
            ...
            type="button"
            onClick={onClose}
            data-testid="director-mode-close"
          >
            {zhCN.common.close}
          </button>
        </div>
  ...
  return (
    <div className="flex flex-col h-full" data-testid="director-mode-panel">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
          ...
          <>
            <div>
              ...
              <select
                ...
                data-testid="director-mode-provider"
              >
            ...
            <div className="flex gap-1 text-xs">
              {[1, 2, 3].map((s) => (
                <div
                  key={s}
                  className={`flex-1 text-center py-1 rounded ${wizardStep === s ? 'bg-blue-600 text-white' : wizardStep > s ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}
                  data-testid={`director-mode-step-${s}`}
                >
            ...
            {wizardStep === 1 && (
              <div className="space-y-2">
                <textarea
                  ...
                  data-testid="director-mode-description"
                />
                <div>
                  ...
                  <div className="flex gap-1 flex-wrap">
                    {DURATION_OPTIONS.map((d) => (
                      <button
                        ...
                        data-testid={`director-mode-duration-${d}`}
                      >
                        {d}s
                      </button>
                    ))}
                    <button
                      ...
                      data-testid="director-mode-duration-custom"
                    >
                      {t.customDuration}
                    </button>
                  </div>
                  {targetDuration === -1 && (
                    <input
                      ...
                      data-testid="director-mode-custom-duration"
                    />
                  )}
                </div>
                <div>
                  ...
                  <div className="flex gap-1 flex-wrap">
                    {STYLE_OPTIONS.map((opt) => (
                      <button
                        key={opt.value}
                        className={`rounded-md px-2 py-1 text-xs font-medium ${style === opt.value ? 'bg-blue-600 text-white' : 'bg-white border border-line text-slate-700 hover:bg-panel'}`}
                        type="button"
                        onClick={() => setStyle(opt.value)}
                        data-testid={`director-mode-style-${opt.value}`}
                      >
                        {opt.label}
                      </button>
                    ))}
                ...
                {(['all', 'favorites'] as const).map((src) => (
                  <label key={src} className="flex items-center gap-2 text-xs cursor-pointer">
                    <input
                      type="radio"
                      name="mediaSource"
                      checked={mediaSource === src}
                      onChange={() => setMediaSource(src)}
                      data-testid={`director-mode-source-${src}`}
                    />
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 text-xs cursor-pointer">
                  <input
                    type="checkbox"
                    checked={addMarkers}
                    onChange={(e) => setAddMarkers(e.target.checked)}
                    data-testid="director-mode-add-markers"
                  />
                  {t.addMarkers}
                </label>
                <label className="flex items-center gap-2 text-xs cursor-pointer">
                  <input
                    type="checkbox"
                    checked={addMusicTrack}
                    onChange={(e) => setAddMusicTrack(e.target.checked)}
                    data-testid="director-mode-add-music"
                  />
                  {t.addMusicTrack}
                </label>
                <div>
                  ...
                  <div className="flex gap-1">
                    <button
                      className={`rounded-md px-2 py-1 text-xs font-medium ${outputTarget === 'main' ? 'bg-blue-600 text-white' : 'bg-white border border-line text-slate-700 hover:bg-panel'}`}
                      type="button"
                      onClick={() => setOutputTarget('main')}
                      data-testid="director-mode-output-main"
                    >
                      {t.outputMainSequence}
                    </button>
                    <button
                      className={`rounded-md px-2 py-1 text-xs font-medium ${outputTarget === 'new' ? 'bg-blue-600 text-white' : 'bg-white border border-line text-slate-700 hover:bg-panel'}`}
                      type="button"
                      onClick={() => setOutputTarget('new')}
                      data-testid="director-mode-output-new"
                    >
                      {t.outputNewSequence}
                    </button>
                  </div>
            ...
            <div className="flex gap-2">
              {wizardStep > 1 && (
                <button
                  ...
                  type="button"
                  onClick={handlePrev}
                  data-testid="director-mode-prev"
                >
                  {t.prev}
                </button>
              )}
              {wizardStep < 3 ? (
                <button
                  ...
                  type="button"
                  disabled={wizardStep === 1 && !description.trim()}
                  onClick={handleNext}
                  data-testid="director-mode-next"
                >
                  {t.next}
                </button>
              ) : (
                <button
                  ...
                  data-testid="director-mode-generate"
                >
                  {t.startGenerate}
                </button>
              )}
        ...
        {phase === 'generating' && (
          <div className="space-y-2">
            <div className="text-xs text-slate-600" data-testid="director-mode-generating">
              {t.generating}
            </div>
            ...
            <button
              ...
              data-testid="director-mode-cancel"
            >
              {t.cancel}
            </button>
          </div>
          ...
          <div className="space-y-2" data-testid="director-mode-storyboard">
            ...
            <div className="space-y-1.5 max-h-80 overflow-y-auto">
              {storyboard.map((card, index) => (
                <div
                  ...
                  className={`flex items-start gap-2 rounded-md border p-2 text-xs ${card.deleted ? 'border-red-200 bg-red-50 opacity-50' : 'border-line bg-white'}`}
                  data-testid={`director-mode-card-${index}`}
                >
                  <div className="flex flex-col gap-0.5 shrink-0">
                    <button
                      ...
                      data-testid={`director-mode-up-${index}`}
                    >
                      ▲
                    </button>
                    <button
                      ...
                      onClick={() => moveCard(index, 1)}
                      data-testid={`director-mode-down-${index}`}
                    >
                      ▼
                    </button>
                  </div>
                  ...
                  <button
                    ...
                    data-testid={`director-mode-delete-${index}`}
                  >
                    {card.deleted ? zhCN.common.reset : t.deleteClip}
                  </button>
                </div>
              ))}
            </div>
            <div className="flex gap-2">
              <button
                ...
                data-testid="director-mode-back"
              >
                {zhCN.common.back}
              </button>
              <button
                ...
                data-testid="director-mode-confirm"
              >
                {t.confirmCreate}
              </button>
            </div>
        ...
        {phase === 'done' && (
          <div className="text-center py-8" data-testid="director-mode-done">
            ...
            <button
              ...
              type="button"
              onClick={onClose}
              data-testid="director-mode-done-close"
            >
              {zhCN.common.close}
            </button>
          </div>
