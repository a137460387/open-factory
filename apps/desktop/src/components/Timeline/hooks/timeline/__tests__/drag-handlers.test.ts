...
describe('createDragHandlers — onPointerMove', () => {
  ...
  it('move 多选时保持组内偏移', () => {
    const clips = [
      makeClip({ id: 'clip-a', start: 5, duration: 5 }),
      makeClip({ id: 'clip-b', start: 20, duration: 5 }),
    ];
...
describe('createDragHandlers — onPointerUp', () => {
  ...
  it('rolling-trim 结束执行 RollingTrimCommand', () => {
    ...
    const { handlers } = setup(makeDrag({ mode: 'rolling-trim', previewRollingDelta: 0.5, rightClip }), [
      makeClip({ id: 'clip-a', start: 5, duration: 5 }),
      rightClip,
    ]);
    handlers.onPointerUp();
