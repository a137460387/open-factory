...
describe('createClipOperationsHandlers — 节拍 / 注释 / 便签', () => {
  ...
  it('quickAddTimelineNote 在 playhead 处创建便签并联动面板', () => {
    ...
    expect(params.setters.setTimelineNoteEditor).toHaveBeenCalledWith(expect.objectContaining({ start: 5, end: 6 }));
  ...
  it('onTimelineNoteRangeDraft 打开编辑器并联动面板', () => {
    ...
    expect(params.setters.setTimelineNoteEditor).toHaveBeenCalledWith(expect.objectContaining({ start: 1, end: 3 }));
    expect(params.setters.setTimelineNotePanelOpen).toHaveBeenCalledWith(true);
  });
