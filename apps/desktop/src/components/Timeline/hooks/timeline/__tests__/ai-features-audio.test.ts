...
beforeEach(() => {
  ...
  canGenerateSubtitlesForClipMock
    .mockReset()
    .mockImplementation((_clip: unknown, _asset: unknown, ready: boolean) => Boolean(ready));
...
describe('createAiFeatureHandlers — 对话检测', () => {
  function dialogueParams(
    ...
    assetFor: (id: string) => ReturnType<typeof makeAsset> | undefined,
  ...
  it('getDialogueDetectionTarget 选中无效时回退首个有效 clip', () => {
    ...
    const { handlers } = dialogueParams(clips, (id) => (id === 'clip-a' ? makeAsset({ type: 'audio' }) : undefined), [
      'clip-z',
    ]);
  ...
  it('runDialogueDetection 检测失败时 toast 警告', async () => {
    ...
    const { handlers } = dialogueParams(clips, () => makeAsset({ type: 'audio' }));
    await handlers.runDialogueDetection('medium');
    expect(showToastMock).toHaveBeenCalledWith(expect.objectContaining({ kind: 'warning', message: 'decode failed' }));
  });
});
...
describe('createAiFeatureHandlers — Whisper 字幕生成', () => {
  ...
  it('生成抛错时 toast 错误并复位对话框', async () => {
    ...
    const { params, handlers } = whisperSetup(true);
    await handlers.generateSubtitles('clip-a');
    expect(showToastMock).toHaveBeenCalledWith(expect.objectContaining({ kind: 'error', message: 'whisper crashed' }));
    expect(params.setters.setWhisperDialog).toHaveBeenLastCalledWith(undefined);
  });
});
  ...
  function subtitleSetup(options: { withSource?: boolean; subtitleClips?: unknown[] } = {}) {
    ...
    const tracks = [
      makeTrack({
        id: 'track-sub',
        type: 'subtitle',
        clips: (options.subtitleClips as ReturnType<typeof makeClip>[] | undefined) ?? [subtitleClip],
      }),
    ];
