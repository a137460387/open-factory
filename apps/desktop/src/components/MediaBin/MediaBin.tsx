...
export function MediaBin({
  ...
  const state = useMediaBinState({
    media, mediaFolders, mediaMetadata, mediaContentAnalysis,
    favoriteIds, recentMediaIds, pinnedIds,
    subclips, mediaCollections,
        ...
        >
          <div className="flex items-center justify-between border-b border-line px-3 py-2">
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              <button className="inline-flex items-center gap-2 rounded-md border border-line bg-panel px-2 py-2 text-sm font-medium text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-secondary)]" onClick={onScanDuplicates} data-testid="scan-duplicate-media-button">
                <Merge size={15} />
                {t.scanDuplicates}
              </button>
              <button className="inline-flex items-center gap-2 rounded-md border border-line bg-panel px-2 py-2 text-sm font-medium text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-secondary)]" onClick={onBatchGenerateCovers} data-testid="batch-generate-covers-button">
                <Import size={15} />
              ...
              <button className="inline-flex items-center gap-2 rounded-md border border-line bg-panel px-2 py-2 text-sm font-medium text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-secondary)]" onClick={onAddAdjustmentLayer} data-testid="new-adjustment-layer-button">
                <SlidersHorizontal size={15} />
                {t.newAdjustmentLayer}
              </button>
              <button className="inline-flex items-center gap-2 rounded-md border border-line bg-panel px-2 py-2 text-sm font-medium text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-secondary)]" onClick={() => onCreateFolder(null)} data-testid="media-folder-create-button">
                <FolderPlus size={15} />
                {t.newFolder}
              </button>
              <button className="inline-flex items-center gap-2 rounded-md bg-brand px-3 py-2 text-sm font-medium text-white hover:bg-[#176858]" onClick={onImport} data-testid="import-media-button">
                <Import size={16} />
                {t.import}
              </button>
            </div>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-3">
            ...
            />
            {state.filter !== 'titles' && state.filter !== 'shared' && state.filter !== 'effects' && state.jobs.length > 0 ? (
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <div className="truncate text-[var(--color-text-muted)]">
                      {state.runningJob ? `${t.jobType[state.runningJob.type]} · ${state.runningJob.assetName}` : state.runnerActive ? t.preparingQueue : zhCN.common.idle}{' '}
                      · {t.pendingCount(state.pendingCount)}
                      {state.failedCount > 0 ? ` · ${t.failedCount(state.failedCount)}` : ''}
                    </div>
                  </div>
                  <button className="rounded-md border border-line bg-[var(--color-bg-elevated)] px-2 py-1 text-[11px] font-medium hover:bg-[var(--color-bg-secondary)]" onClick={state.clearFinishedJobs}>
                    {zhCN.common.clear}
                  </button>
                </div>
              </div>
            ...
            {state.filter === 'shared' ? (
              ...
              <div className="flex min-h-0 flex-1 flex-col">
                <MediaLibraryListView
                  ...
                  onSort={(sortKey) => state.updateMediaLibraryView({
                    sortKey,
                    sortDirection: state.mediaLibraryView.sortKey === sortKey && state.mediaLibraryView.sortDirection === 'asc' ? 'desc' : 'asc',
                  })}
          ...
          {state.mediaInfo ? <MediaInfoDialog state={state.mediaInfo} onClose={() => state.setMediaInfo(undefined)} /> : null}
          {state.sourcePaths ? (
          ...
          {state.batchMetadataAssets.length > 0 ? (
            <BatchMetadataDialog
              ...
              onSubmit={(metadata) => {
                onBatchUpdateMetadata(state.batchMetadataAssets.map((asset) => asset.id), metadata);
            ...
            <BatchRenameDialog
              ...
              onConfirm={(preview, renameFiles) => {
                void Promise.resolve(
                  onBatchRenameMedia(state.batchRenameAssets.map((asset) => asset.id), preview, renameFiles),
          ...
          {state.subclipDialogAssetId ? (
            <SubclipDialog
              ...
              editingSubclip={state.editingSubclipId ? subclips.find((s) => s.id === state.editingSubclipId) : undefined}
...
function SharedLibraryGrid({ resources }: { resources: SharedLibraryResource[] }) {
  if (resources.length === 0) {
    return (
      <div className="rounded-md border border-line bg-panel p-3 text-sm text-[var(--color-text-secondary)]" data-testid="shared-library-empty">
...
function EffectPresetGrid({ presets, loading, error, selectedClipId, onApply, onRefresh }: {
  presets: EffectPreset[]; loading: boolean; error?: string; selectedClipId?: string;
  onApply(preset: EffectPreset): void; onRefresh(): void;
}) {
      ...
      <div className="grid gap-2" data-testid="effect-preset-list">
        {presets.map((preset) => (
          <div key={preset.id} className="rounded-md border border-line bg-[var(--color-bg-elevated)] p-3 shadow-sm" data-testid="effect-preset-card" data-preset-id={preset.id}>
            ...
            <button className="mt-3 w-full rounded-md bg-brand px-3 py-1.5 text-xs font-semibold text-white hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-50" type="button" disabled={!selectedClipId} data-testid="effect-preset-apply-button" onClick={() => onApply(preset)}>
              {t.apply}
            </button>
          </div>
        ))}
...
function MediaFolderTree(props: {
  folders: MediaFolder[]; media: MediaAsset[]; mediaMetadata: Record<string, MediaMetadata>;
  mediaContentAnalysis: Record<string, ClipContentAnalysis>; gridSize: any; projectFrameRate: number;
  onCreateFolder(parentId?: string | null): void; onRenameFolder(folderId: string, name: string): void;
  onDeleteFolder(folderId: string): void; onSetFolderCollapsed(folderId: string, collapsed: boolean): void;
  onMoveMediaToFolder(assetIds: string[], folderId?: string | null): void;
  ...
  onSetLabel(assetId: string, labelColor?: MediaLabelColor): void; onSetRating(assetId: string, rating: number): void;
  onSetFlag(assetId: string, flag?: MediaFlag): void; onBatchTranscode(paths: string[]): void;
  onExportGif(asset: MediaAsset): void; onAnalyzeSpectrum(asset: MediaAsset): void;
  onShowInfo(asset: MediaAsset): void; onFindSources(asset: MediaAsset): void;
  selectedMediaIds: Set<string>; onToggleSelected(assetId: string): void;
  onOpenBatchMetadata(assetId: string): void; onOpenBatchRename(assetId: string): void;
...
function MediaFolderNode({ folder, depth, folders, media, mediaMetadata, mediaContentAnalysis, gridSize, projectFrameRate, onCreateFolder, onRenameFolder, onDeleteFolder, onSetFolderCollapsed, onMoveMediaToFolder, onAddToTimeline, onAddVersion, onCompareVersions, onRelink, onGenerateProxy, onConvertToCfr, onSetLabel, onSetRating, onSetFlag, onBatchTranscode, onExportGif, onAnalyzeSpectrum, onShowInfo, onFindSources, selectedMediaIds, onToggleSelected, onOpenBatchMetadata, onOpenBatchRename, mediaHighlights }: {
  folder: MediaFolder; depth: number; folders: MediaFolder[]; media: MediaAsset[];
  mediaMetadata: Record<string, MediaMetadata>; mediaContentAnalysis: Record<string, ClipContentAnalysis>;
  gridSize: any; projectFrameRate: number;
  onCreateFolder(parentId?: string | null): void; onRenameFolder(folderId: string, name: string): void;
  onDeleteFolder(folderId: string): void; onSetFolderCollapsed(folderId: string, collapsed: boolean): void;
  onMoveMediaToFolder(assetIds: string[], folderId?: string | null): void;
  onAddToTimeline(assetId: string): void; onAddVersion(assetId: string): void; onCompareVersions(assetId: string): void;
  ...
  return (
    <div className="space-y-2" style={{ marginLeft: `${(depth - 1) * 12}px` }}>
      <div
        ...
        onDrop={(event) => { event.preventDefault(); const assetId = event.dataTransfer.getData(MEDIA_CARD_DRAG_MIME); if (assetId) onMoveMediaToFolder([assetId], folder.id); }}
        onContextMenu={(event) => { event.preventDefault(); onDeleteFolder(folder.id); }}
      >
        ...
        {editing ? (
          <input className="min-w-0 flex-1 rounded-lg border border-line px-1 py-0.5 text-xs outline-none" value={draftName} autoFocus data-testid={`media-folder-name-input-${folder.id}`} onChange={(event) => setDraftName(event.target.value)} onBlur={commitRename} onKeyDown={(event) => { if (event.key === 'Enter') commitRename(); if (event.key === 'Escape') { setDraftName(folder.name); setEditing(false); } }} />
        ) : (
          <button className="min-w-0 flex-1 truncate text-left font-semibold text-[var(--color-text-secondary)]" type="button" data-testid={`media-folder-name-${folder.id}`} onDoubleClick={() => setEditing(true)}>{folder.name}</button>
        )}
      ...
      {!folder.collapsed ? (
        <div className="space-y-2">
          {children.map((child) => (
            <MediaFolderNode key={child.id} folder={child} depth={depth + 1} folders={folders} media={media} mediaMetadata={mediaMetadata} mediaContentAnalysis={mediaContentAnalysis} gridSize={gridSize} projectFrameRate={projectFrameRate} onCreateFolder={onCreateFolder} onRenameFolder={onRenameFolder} onDeleteFolder={onDeleteFolder} onSetFolderCollapsed={onSetFolderCollapsed} onMoveMediaToFolder={onMoveMediaToFolder} onAddToTimeline={onAddToTimeline} onAddVersion={onAddVersion} onCompareVersions={onCompareVersions} onRelink={onRelink} onGenerateProxy={onGenerateProxy} onConvertToCfr={onConvertToCfr} onSetLabel={onSetLabel} onSetRating={onSetRating} onSetFlag={onSetFlag} onBatchTranscode={onBatchTranscode} onExportGif={onExportGif} onAnalyzeSpectrum={onAnalyzeSpectrum} onShowInfo={onShowInfo} onFindSources={onFindSources} selectedMediaIds={selectedMediaIds} onToggleSelected={onToggleSelected} onOpenBatchMetadata={onOpenBatchMetadata} onOpenBatchRename={onOpenBatchRename} mediaHighlights={mediaHighlights} />
          ))}
          <MediaCardGrid media={folderMedia} mediaMetadata={mediaMetadata} mediaContentAnalysis={mediaContentAnalysis} gridSize={gridSize} projectFrameRate={projectFrameRate} onAddToTimeline={onAddToTimeline} onAddVersion={onAddVersion} onCompareVersions={onCompareVersions} onRelink={onRelink} onGenerateProxy={onGenerateProxy} onConvertToCfr={onConvertToCfr} onSetLabel={onSetLabel} onSetRating={onSetRating} onSetFlag={onSetFlag} onBatchTranscode={onBatchTranscode} onExportGif={onExportGif} onAnalyzeSpectrum={onAnalyzeSpectrum} onShowInfo={onShowInfo} onFindSources={onFindSources} selectedMediaIds={selectedMediaIds} onToggleSelected={onToggleSelected} onOpenBatchMetadata={onOpenBatchMetadata} onOpenBatchRename={onOpenBatchRename} folderId={folder.id} mediaHighlights={mediaHighlights} />
        </div>
      ) : null}
    </div>
  );
...
function RootMediaDropZone({ onMoveMediaToFolder }: { onMoveMediaToFolder(assetIds: string[], folderId?: string | null): void }) {
  return (
    <div className="rounded-md border border-dashed border-line bg-[var(--color-bg-elevated)] px-2 py-1.5 text-xs font-medium text-[var(--color-text-muted)]" data-testid="media-folder-root-dropzone" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const assetId = event.dataTransfer.getData(MEDIA_CARD_DRAG_MIME); if (assetId) onMoveMediaToFolder([assetId], null); }}>
      {zhCN.mediaBin.rootFolder}
    </div>
  );
}

function MediaLibraryListView({ media, settings, selectedAssetId, onSort, onSelectAsset, onAddToTimeline, onExportGif }: {
  media: MediaAsset[]; settings: any; selectedAssetId?: string | null;
  onSort(sortKey: any): void; onSelectAsset?(assetId: string): void;
  onAddToTimeline(assetId: string): void; onExportGif(asset: MediaAsset): void;
}) {
  ...
  return (
    <div className="overflow-x-auto rounded-md border border-line bg-[var(--color-bg-elevated)]" data-testid="media-list-view">
      <table className="min-w-[1100px] w-full border-collapse text-xs">
          ...
          <tr>
            {columns.map((column) => (
              <th key={column.key} className="border-b border-line px-2 py-2 font-semibold">
                {column.sortable ? (
                  <button className="inline-flex items-center gap-1 hover:text-brand" type="button" data-testid={column.testId} onClick={() => onSort(column.key)}>
                    {column.label}
        ...
        <tbody>
          {media.map((asset) => (
            <tr key={asset.id} className={`border-b border-line last:border-b-0 cursor-pointer ${selectedAssetId === asset.id ? 'bg-[var(--color-bg-selected)]' : 'hover:bg-[var(--color-bg-hover)]'}`} data-testid={`media-list-row-${asset.id}`} onClick={() => onSelectAsset?.(asset.id)}>
              <td className="max-w-[180px] px-2 py-2"><div className="truncate font-semibold text-ink" title={asset.path}>{asset.name}</div></td>
              ...
              <td className="px-2 py-2 tabular-nums text-[var(--color-text-secondary)]" data-testid={`media-list-size-${asset.id}`}>{formatBytes(asset.size)}</td>
              <td className="px-2 py-2 tabular-nums text-[var(--color-text-secondary)]">{formatImportedAt(asset.importedAt)}</td>
              <td className="px-2 py-2">
                <div className="flex justify-end gap-1">
                  {asset.type === 'video' ? <button className="rounded border border-line px-2 py-1 font-medium text-[var(--color-text-secondary)] hover:bg-panel" type="button" data-testid={`media-list-export-gif-${asset.id}`} onClick={(e) => { e.stopPropagation(); onExportGif(asset); }}>GIF</button> : null}
                  <button className="rounded border border-line bg-panel px-2 py-1 font-medium text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-secondary)]" type="button" data-testid={`media-list-add-${asset.id}`} onClick={(e) => { e.stopPropagation(); onAddToTimeline(asset.id); }}>{zhCN.mediaBin.add}</button>
                </div>
              </td>
            </tr>
          ))}
          ...
          return (
            <div key={asset.id} className="flex-none overflow-hidden rounded-md border border-line bg-panel" style={{ width }} data-testid={`media-timeline-item-${asset.id}`}>
              ...
              <div className="space-y-1 p-2">
                <div className="truncate text-xs font-semibold text-ink" title={asset.path}>{asset.name}</div>
                <div className="truncate text-[11px] text-[var(--color-text-muted)]">{formatImportedAt(asset.importedAt)}</div>
                <div className="flex gap-1">
                  {asset.type === 'video' ? <button className="rounded border border-line bg-[var(--color-bg-elevated)] px-1.5 py-1 text-[11px] font-medium text-[var(--color-text-secondary)] hover:bg-panel" type="button" data-testid={`media-timeline-export-gif-${asset.id}`} onClick={(e) => { e.stopPropagation(); onExportGif(asset); }}>GIF</button> : null}
                  <button className="flex-1 rounded border border-line bg-[var(--color-bg-elevated)] px-1.5 py-1 text-[11px] font-medium text-[var(--color-text-secondary)] hover:bg-panel" type="button" data-testid={`media-timeline-add-${asset.id}`} onClick={(e) => { e.stopPropagation(); onAddToTimeline(asset.id); }}>{zhCN.mediaBin.add}</button>
                </div>
              </div>
            </div>
          );
...
function MediaCardGrid({ media, gridSize, mediaMetadata, mediaContentAnalysis, projectFrameRate, onAddToTimeline, onAddVersion, onCompareVersions, onRelink, onGenerateProxy, onConvertToCfr, onSetLabel, onSetRating, onSetFlag, onBatchTranscode, onExportGif, onAnalyzeSpectrum, onShowInfo, onFindSources, selectedMediaIds, onToggleSelected, onOpenBatchMetadata, onOpenBatchRename, folderId, mediaHighlights }: {
  ...
  selectedMediaIds: Set<string>; onToggleSelected(assetId: string): void;
  onOpenBatchMetadata(assetId: string): void; onOpenBatchRename(assetId: string): void;
  folderId: string; mediaHighlights?: Map<string, VisualHighlightMarker[]>;
}) {
  const containerRef = useRef<HTMLDivElement>(null);
  const columnCount = useColumnCount(containerRef as RefObject<HTMLDivElement | null>, gridSize);
  if (media.length === 0) return null;
  return (
    <div ref={containerRef} className="grid gap-2" style={GRID_COLUMN_STYLES[gridSize] ?? GRID_COLUMN_STYLES.medium} data-testid={`media-folder-grid-${folderId}`} data-grid-size={gridSize} data-media-card-grid="true">
      {media.map((asset, index) => (
        <MediaCard key={asset.id} mediaIndex={index} asset={asset} metadata={mediaMetadata[asset.id]} contentAnalysis={mediaContentAnalysis[asset.id]} projectFrameRate={projectFrameRate} onAdd={() => onAddToTimeline(asset.id)} onAddVersion={() => onAddVersion(asset.id)} onCompareVersions={() => onCompareVersions(asset.id)} onRelink={() => onRelink(asset.id)} onGenerateProxy={() => onGenerateProxy(asset.id)} onConvertToCfr={() => onConvertToCfr(asset.id)} onSetLabel={(labelColor) => onSetLabel(asset.id, labelColor)} onSetRating={(rating) => onSetRating(asset.id, rating)} onSetFlag={(flag) => onSetFlag(asset.id, flag)} onBatchTranscode={() => onBatchTranscode([asset.path])} onExportGif={() => onExportGif(asset)} onAnalyzeSpectrum={() => onAnalyzeSpectrum(asset)} onShowInfo={() => onShowInfo(asset)} onFindSources={() => onFindSources(asset)} selected={selectedMediaIds.has(asset.id)} onToggleSelected={() => onToggleSelected(asset.id)} batchSelectionCount={selectedMediaIds.has(asset.id) ? selectedMediaIds.size : 1} onOpenBatchMetadata={() => onOpenBatchMetadata(asset.id)} onOpenBatchRename={() => onOpenBatchRename(asset.id)} highlights={mediaHighlights?.get(asset.id)} />
      ))}
    </div>
  );
}
          ...
          return (
            <Fragment key={virtualRow.key}>
              {rowItems.map((asset, itemIndex) => (
                <MediaCard key={asset.id} mediaIndex={rowStart + itemIndex} asset={asset} metadata={mediaMetadata[asset.id]} contentAnalysis={mediaContentAnalysis[asset.id]} projectFrameRate={projectFrameRate} onAdd={() => onAddToTimeline(asset.id)} onAddVersion={() => onAddVersion(asset.id)} onCompareVersions={() => onCompareVersions(asset.id)} onRelink={() => onRelink(asset.id)} onGenerateProxy={() => onGenerateProxy(asset.id)} onConvertToCfr={() => onConvertToCfr(asset.id)} onSetLabel={(labelColor) => onSetLabel(asset.id, labelColor)} onSetRating={(rating) => onSetRating(asset.id, rating)} onSetFlag={(flag) => onSetFlag(asset.id, flag)} onBatchTranscode={() => onBatchTranscode([asset.path])} onExportGif={() => onExportGif(asset)} onAnalyzeSpectrum={() => onAnalyzeSpectrum(asset)} onShowInfo={() => onShowInfo(asset)} onFindSources={() => onFindSources(asset)} selected={selectedMediaIds.has(asset.id)} onToggleSelected={() => onToggleSelected(asset.id)} batchSelectionCount={selectedMediaIds.has(asset.id) ? selectedMediaIds.size : 1} onOpenBatchMetadata={() => onOpenBatchMetadata(asset.id)} onOpenBatchRename={() => onOpenBatchRename(asset.id)} highlights={mediaHighlights?.get(asset.id)} />
              ))}
            </Fragment>
          );
        })}
...
function TitleTemplateGrid({ onAddTitleTemplate }: { onAddTitleTemplate(templateId: TitleTemplateId): void }) {
  return (
    <div className="grid grid-cols-1 gap-3" data-testid="title-template-grid">
      <div className="rounded-md border border-line bg-panel px-3 py-2 text-xs font-medium text-[var(--color-text-secondary)]">{zhCN.mediaBin.titleTemplateCount(TITLE_TEMPLATE_IDS.length)}</div>
      {TITLE_TEMPLATE_IDS.map((templateId) => {
        const label = zhCN.titleTemplates[templateId];
        return (
          <div key={templateId} className="rounded-md border border-line bg-[var(--color-bg-elevated)] p-3 shadow-sm" draggable data-testid={`title-template-card-${templateId}`} onDragStart={(event) => { event.dataTransfer.effectAllowed = 'copy'; event.dataTransfer.setData(TITLE_TEMPLATE_DRAG_MIME, templateId); }}>
            <div className="flex items-center gap-2">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-line bg-panel text-brand"><FileText size={17} /></div>
              <div className="min-w-0 flex-1">
            ...
            <button className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-md border border-line bg-panel px-2 py-1.5 text-sm font-medium hover:bg-[var(--color-bg-secondary)]" type="button" data-testid={`add-title-template-${templateId}`} onClick={() => onAddTitleTemplate(templateId)}>
              <Plus size={15} />
...
function MediaSourcePathsDialog({ state, onClose }: { state: { asset: MediaAsset; paths: string[] }; onClose(): void }) {
  return (
          ...
          {state.paths.length > 0 ? (
            <ul className="space-y-2">
              {state.paths.map((path) => (
                <li key={path} className="rounded-md border border-line bg-panel px-2 py-1.5 font-mono text-[var(--color-text-secondary)]" data-testid="media-source-path">{path}</li>
              ))}
            </ul>
          ) : (
