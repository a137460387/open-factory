    ...
    >
      <div className="flex min-w-0 items-start gap-1">
        <div className="min-w-0 flex-1">
          <div className="truncate text-[11px] font-semibold text-slate-700" title={track.name}>{track.name}</div>
        ...
        <button className="flex h-6 w-6 flex-none items-center justify-center rounded border border-line bg-white text-slate-600 hover:bg-slate-50" type="button" title={expanded ? zhCN.mixer.collapseChannel : zhCN.mixer.expandChannel} data-testid={`mixer-expand-${track.id}`} onClick={onToggle}>
...
function ChannelRoutingBadge({summary, trackId}: {summary: ChannelRoutingSummary; trackId: string}) {
  ...
  return (
    <div className={`flex h-6 w-6 flex-none items-center justify-center rounded border ${active ? 'border-brand bg-brand text-white' : 'border-line bg-white text-slate-500'}`} title={summary.title} data-testid={`mixer-channel-routing-${trackId}`} data-routing-kind={summary.kind} data-routed-clip-count={summary.routedClipCount}>
      <Icon size={14} />
    </div>
  );
}
    ...
    <div className="min-h-0 overflow-y-auto border-t border-line pt-2 text-[11px] text-slate-700">
      <div className="flex items-center gap-2">
        ...
        <label className="flex items-center gap-1 font-semibold">
          <input type="checkbox" checked={eq.enabled} data-testid={`mixer-eq-enabled-${track.id}`} onChange={(event) => updateEQ({enabled: event.target.checked})} />
          {zhCN.mixer.eq}
        </label>
      </div>
      ...
      <div className="mt-3 flex items-center gap-2 border-t border-line pt-2">
        <label className="flex items-center gap-1 font-semibold">
          <input type="checkbox" checked={compressor.enabled} data-testid={`mixer-compressor-enabled-${track.id}`} onChange={(event) => updateCompressor({enabled: event.target.checked})} />
          {zhCN.mixer.compressor}
        </label>
      </div>
      <div className="mt-2 grid grid-cols-2 gap-2">
        <MiniSlider label={zhCN.mixer.threshold} value={compressor.threshold} min={-60} max={0} step={1} unit="dB" disabled={!compressor.enabled} testId={`mixer-compressor-threshold-${track.id}`} onChange={(threshold) => updateCompressor({threshold})} />
        <MiniSlider label={zhCN.mixer.ratio} value={compressor.ratio} min={1} max={20} step={0.1} unit=":1" disabled={!compressor.enabled} testId={`mixer-compressor-ratio-${track.id}`} onChange={(ratio) => updateCompressor({ratio})} />
        <MiniSlider label={zhCN.mixer.attack} value={compressor.attack} min={0.01} max={200} step={1} unit="ms" disabled={!compressor.enabled} testId={`mixer-compressor-attack-${track.id}`} onChange={(attack) => updateCompressor({attack})} />
        <MiniSlider label={zhCN.mixer.release} value={compressor.release} min={10} max={1000} step={10} unit="ms" disabled={!compressor.enabled} testId={`mixer-compressor-release-${track.id}`} onChange={(release) => updateCompressor({release})} />
        <MiniSlider label={zhCN.mixer.makeupGain} value={compressor.makeupGain} min={0} max={24} step={0.5} unit="dB" disabled={!compressor.enabled} testId={`mixer-compressor-makeup-${track.id}`} onChange={(makeupGain) => updateCompressor({makeupGain})} />
      </div>
    </div>
  );
}

function EQBandControls({trackId, band, index, disabled, onChange}: {
  trackId: string; band: TrackEQBand; index: number; disabled: boolean; onChange(patch: Partial<TrackEQBand>): void;
}) {
  ...
  return (
    <div className="grid grid-cols-[38px_1fr_64px_50px] items-center gap-2">
      ...
      <input className="min-w-0 accent-brand" type="range" min={-24} max={24} step={0.5} value={band.gain} disabled={disabled} title={zhCN.mixer.gain} data-testid={`mixer-eq-gain-${trackId}-${band.id}`} onChange={(event) => onChange({gain: Number(event.target.value)})} />
      <input className="h-6 min-w-0 rounded border border-line bg-white px-1 text-right tabular-nums" type="number" min={20} max={20000} step={10} value={band.frequency} disabled={disabled} title={zhCN.mixer.frequency} data-testid={`mixer-eq-frequency-${trackId}-${band.id}`} onChange={(event) => onChange({frequency: Number(event.target.value)})} />
      <input className="h-6 min-w-0 rounded border border-line bg-white px-1 text-right tabular-nums" type="number" min={0.1} max={4} step={0.1} value={band.q} disabled={disabled} title={zhCN.mixer.q} data-testid={`mixer-eq-q-${trackId}-${band.id}`} onChange={(event) => onChange({q: Number(event.target.value)})} />
    </div>
  );
}
...
function MiniSlider({label, value, min, max, step, unit, disabled, testId, onChange}: {
  label: string; value: number; min: number; max: number; step: number; unit: string; disabled: boolean; testId: string; onChange(value: number): void;
}) {
  return (
    <label className={`min-w-0 ${disabled ? 'text-slate-400' : ''}`}>
      ...
      <input className="w-full accent-brand" type="range" min={min} max={max} step={step} value={value} disabled={disabled} data-testid={testId} onChange={(event) => onChange(Number(event.target.value))} />
    </label>
  );
}

export function MasterStrip({level, volume, onVolumeChange}: {
  level: AudioMeterLevel; volume: number; onVolumeChange(value: number): void;
}) {
  return (
...
function VolumeFader({value, testId, onChange}: {value: number; testId: string; onChange(value: number): void}) {
  return (
    <div className="relative h-[84px] min-w-0 overflow-visible">
      <input className="absolute left-1/2 top-1/2 h-5 w-[82px] -translate-x-1/2 -translate-y-1/2 -rotate-90 accent-brand" type="range" min={0} max={2} step={0.01} value={value} aria-orientation="vertical" title={zhCN.mixer.volume} data-testid={testId} onChange={(event) => onChange(Number(event.target.value))} />
    </div>
  );
}
  ...
  return (
    <label className="flex min-w-0 items-center justify-center gap-2 text-[10px] text-slate-600" title={zhCN.mixer.pan}>
      <span className="relative h-7 w-7 flex-none">
        ...
        <span className="absolute left-1/2 top-1/2 h-[11px] w-0.5 origin-bottom -translate-x-1/2 -translate-full rounded bg-brand" style={{transform: `translate(-50%, -100%) rotate(${angle}deg)`}} />
        <span className="absolute left-1/2 top-1/2 h-1.5 w-1.5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-brand" />
        <input className="absolute inset-0 cursor-ew-resize opacity-0" type="range" min={-1} max={1} step={0.01} value={value} aria-label={zhCN.mixer.pan} data-testid={testId} onChange={(event) => onChange(Number(event.target.value))} />
      </span>
...
function MixerToggle({label, title, active, testId, onClick}: {
  label: string; title: string; active: boolean; testId: string; onClick(): void;
}) {
  return (
    <button className={`h-6 w-6 rounded border text-[11px] font-semibold ${active ? 'border-brand bg-brand text-white' : 'border-line bg-white text-slate-600 hover:bg-panel'}`} type="button" title={title} data-testid={testId} onClick={onClick}>
      {label}
    </button>
  );
}
...
export function summarizeTrackChannelRouting(track: Track, mediaById: Map<string, MediaAsset>): ChannelRoutingSummary {
  ...
  if (routedModes.length > 0) {
    ...
    if (uniqueModes.length === 1) {
      ...
      return {kind: 'routed', title: zhCN.mixer.channelRoutingRouted(zhCN.inspector.audioChannelRoutingOptions[mode], routedModes.length), routedClipCount: routedModes.length};
    }
    return {kind: 'mixed', title: zhCN.mixer.channelRoutingMixed(routedModes.length), routedClipCount: routedModes.length};
  }
  ...
  if (sourceChannels.some((channels) => channels === 1)) return {kind: 'mono', title: zhCN.mixer.channelRoutingMono, routedClipCount: 0};
  if (sourceChannels.some((channels) => channels >= 2)) return {kind: 'stereo', title: zhCN.mixer.channelRoutingStereo, routedClipCount: 0};
