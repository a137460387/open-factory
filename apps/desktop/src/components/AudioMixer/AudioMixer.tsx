...
import {DuckingPanel, makeDefaultDuckingSettings, normalizeDuckingSettings, collectTrackLoudnessSamples, type DuckingSettings, type DuckingPreview} from './DuckingPanel';
    ...
    <section className="flex min-h-0 flex-col bg-white" data-testid="audio-mixer">
      <div className="flex h-10 items-center gap-2 border-b border-line px-3">
        ...
        <div className="flex rounded border border-line bg-panel p-0.5">
          <MixerTabButton active={tab === 'mix'} testId="audio-mixer-tab-mix" onClick={() => setTab('mix')}>{zhCN.mixer.mixTab}</MixerTabButton>
          <MixerTabButton active={tab === 'channel-analysis'} testId="audio-mixer-tab-channel-analysis" onClick={() => setTab('channel-analysis')}>{zhCN.mixer.channelAnalysisTab}</MixerTabButton>
          <MixerTabButton active={tab === 'spectrum'} testId="audio-mixer-tab-spectrum" onClick={() => setTab('spectrum')}>频谱</MixerTabButton>
        </div>
        {tab === 'mix' ? (
          <button className="h-7 rounded border border-line bg-white px-2 text-xs font-semibold text-slate-700 hover:bg-panel disabled:cursor-not-allowed disabled:opacity-50" type="button" disabled={tracks.length < 2} title={tracks.length < 2 ? t('mixer.duckingNoTracks') : t('mixer.duckingTitle')} data-testid="audio-ducking-button" onClick={() => {setDuckingOpen((current) => !current); setDuckingPreview(undefined); setDuckingError(undefined);}}>
            {t('mixer.duckingButton')}
          </button>
        ) : null}
        {tab === 'mix' ? (
          <button className="h-7 rounded border border-line bg-white px-2 text-xs font-semibold text-slate-700 hover:bg-panel" type="button" data-testid="audio-noise-reduction-button" onClick={() => {setNoiseReductionTrackId(selectedTrackId); setNoiseReductionOpen(true);}}>
            降噪
          </button>
        ) : null}
      </div>
      {tab === 'mix' && duckingOpen ? (
        <DuckingPanel tracks={tracks} settings={duckingSettings} preview={duckingPreview} error={duckingError} analyzing={duckingAnalyzing} onChange={(patch) => {setDuckingSettings((current) => normalizeDuckingSettings({...current, ...patch}, tracks)); setDuckingPreview(undefined); setDuckingError(undefined);}} onAnalyze={() => void analyzeDucking()} onApply={applyDucking} onCancel={() => {setDuckingOpen(false); setDuckingPreview(undefined); setDuckingError(undefined);}} />
      ) : null}
      {tab === 'channel-analysis' ? (
        <ChannelAnalysisPanel tracks={tracks} selectedTrackId={activeAnalysisTrack?.id ?? ''} frame={activeAnalysisFrame} onTrackChange={setAnalysisTrackId} />
      ) : tab === 'spectrum' ? (
        <div className="flex min-h-0 flex-1 flex-col gap-2 p-3">
          <Suspense fallback={<div className="flex items-center justify-center py-8 text-xs text-slate-400">加载频谱分析器...</div>}>
            <LazySpectrumAnalyzer frequencyData={trackFrequencyBands[analysisTrackId] ?? []} sampleRate={48000} showControls height={220} />
          </Suspense>
        </div>
      ) : (
        <div className="mixer-scrollbar flex min-h-0 flex-1 gap-2 overflow-x-auto px-3 py-2">
          {tracks.map((track) => (
            <ChannelStrip key={track.id} track={track} level={trackLevels[track.id] ?? getSilentMeterLevel()} channelRoutingSummary={summarizeTrackChannelRouting(track, mediaById)} expanded={Boolean(expandedTrackIds[track.id])} onToggle={() => toggleTrack(track.id)} onUpdate={(patch) => updateTrack(track.id, patch)} />
          ))}
      ...
      {noiseReductionOpen ? (
        <Suspense fallback={null}>
          <LazyNoiseReductionDialog open={noiseReductionOpen} onClose={() => setNoiseReductionOpen(false)} trackId={noiseReductionTrackId} />
        </Suspense>
      ) : null}
    </section>
  );
...
function MixerTabButton({active, testId, children, onClick}: {
  active: boolean; testId: string; children: string; onClick(): void;
}) {
  return (
    <button className={`h-6 rounded px-2 text-xs font-semibold ${active ? 'bg-white text-ink shadow-sm' : 'text-slate-600 hover:bg-white/70'}`} type="button" data-testid={testId} onClick={onClick}>
      {children}
    </button>
  );
}

function ChannelAnalysisPanel({tracks, selectedTrackId, frame, onTrackChange}: {
  tracks: Track[]; selectedTrackId: string; frame?: ChannelAnalysisFrame; onTrackChange(trackId: string): void;
}) {
  ...
  async function handleExport(): Promise<void> {
    ...
    try {
      ...
      const path = await saveFileDialog(`channel-analysis-${selectedTrackId}.json`, [{name: 'JSON', extensions: ['json']}]);      if (path) await writeFile(path, json);
    } catch (error) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-2 p-3">
      <div className="flex items-center gap-2">
        <select className="h-8 rounded border border-line bg-white px-2 text-xs" value={selectedTrackId} data-testid="channel-analysis-track-select" onChange={(event) => onTrackChange(event.target.value)}>
          {tracks.map((track) => <option key={track.id} value={track.id}>{track.name}</option>)}
        </select>
        <button className={`h-8 rounded border px-3 text-xs font-semibold ${recording ? 'border-red-500 bg-red-500 text-white' : 'border-line bg-white text-slate-700 hover:bg-panel'}`} type="button" data-testid="channel-analysis-record-button" onClick={() => setRecording(!recording)}>
          {recording ? '停止录制' : '录制快照'}
        </button>
        {history.length > 0 ? (
          <>
            <input className="flex-1" type="range" min={0} max={history.length - 1} value={playbackIndex} data-testid="channel-analysis-playback-slider" onChange={(event) => setPlaybackIndex(Number(event.target.value))} />
            <span className="text-xs text-slate-500">{playbackIndex + 1}/{history.length}</span>
          </>
        ) : null}
        <button className="h-8 rounded border border-line bg-white px-2 text-xs text-slate-700 hover:bg-panel disabled:opacity-50" type="button" disabled={!displayedSnapshot} data-testid="channel-analysis-export-button" onClick={() => void handleExport()}>
          导出JSON
        </button>
      </div>
      ...
      {displayedSnapshot ? (
        <div className="grid grid-cols-4 gap-2 text-center text-[11px]">
          <div><div className="font-medium text-slate-500">相关性</div><div className="tabular-nums">{formatCorrelation(displayedSnapshot.correlation)}</div></div>
          <div><div className="font-medium text-slate-500">峰值</div><div className="tabular-nums">{displayedSnapshot.peaks[0]?.hz.toFixed(0) ?? '—'} Hz</div></div>
          <div><div className="font-medium text-slate-500">峰值幅度</div><div className="tabular-nums">{displayedSnapshot.peaks[0]?.magnitude.toFixed(1) ?? '—'}</div></div>
          <div><div className="font-medium text-slate-500">频率点</div><div className="tabular-nums">{displayedSnapshot.frequency.length}</div></div>
        </div>
      ) : null}
    </div>
  );
...
function FrequencyResponseChart({snapshot}: {snapshot?: ChannelAnalysisSnapshot}) {
  ...
  return (
    <div className="flex min-h-0 flex-col">
      ...
      <svg className="min-h-0 flex-1 rounded border border-line bg-white" viewBox="0 0 320 120" role="img" data-testid="channel-analysis-frequency-chart">
        ...
        <text x="4" y="12" fontSize="8" fill="#94a3b8">+24</text>
        <text x="4" y="64" fontSize="8" fill="#94a3b8">0</text>
        <text x="4" y="116" fontSize="8" fill="#94a3b8">-24</text>
        <text x="20" y="116" fontSize="8" fill="#94a3b8">20Hz</text>
        <text x="150" y="116" fontSize="8" fill="#94a3b8">1kHz</text>
        <text x="290" y="116" fontSize="8" fill="#94a3b8">20kHz</text>
...
function PhaseScope({points}: {points: PhasePoint[]}) {
  ...
  return (
    <div className="flex min-h-0 flex-col">
      ...
      <svg className="min-h-0 flex-1 rounded border border-line bg-white" viewBox="0 0 120 120" role="img" data-testid="channel-analysis-phase-scope">
