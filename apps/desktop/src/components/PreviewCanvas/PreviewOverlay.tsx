        ...
        {!reviewMode && canvasEditMode && selectedEditableClip && selectedPathMask ? (
          ...
          >
            <PathMaskControls
              ...
              onBeginDrag={props.onBeginPathMaskDrag}
            />
          </div>
    ...
    >
      ...
      >
        {clip.multicam.angles.map((angle, index) => (
          <button
            key={angle.id}
            type="button"
            className={`group relative min-h-0 overflow-hidden rounded-md border bg-black text-left ${activeAngleId === angle.id ? 'border-amber-300 ring-2 ring-amber-300/60' : 'border-white/15 hover:border-white/40'}`}
          ...
          >
            <canvas
              ref={(node) => {
                if (node) canvasRefs.current.set(angle.id, node);
                else canvasRefs.current.delete(angle.id);
              }}
              width={480}
              height={270}
      ...
      >
        <div className="flex items-center justify-between gap-2 border-b border-white/10 px-3 py-2">
          ...
          <button
            type="button"
            className={`inline-flex h-7 shrink-0 items-center rounded border px-2 text-[11px] font-semibold ${liveMode ? 'border-amber-300 bg-amber-300 text-black' : 'border-white/20 bg-white/10 text-white hover:bg-white/15'}`}
...
function MulticamHistoryRow({
  ...
  return (
    <div
      className={`grid grid-cols-[54px_minmax(0,1fr)_54px_auto] items-center gap-2 rounded border px-2 py-1.5 text-[11px] ${entry.tooFrequent ? 'border-amber-300/45 bg-amber-300/15' : 'border-white/10 bg-white/5'}`}
