...
import type { FrameInspectorSample, PreviewCanvasProps } from './types';
...
export function PreviewCanvas({
  ...
  // 帧检查器采样状态：bd315fd6 拆分时该 state 丢失（被替换为 undefined/空函数占位），按原实现恢复
  const [frameInspectorSample, setFrameInspectorSample] = useState<FrameInspectorSample | undefined>(undefined);
  ...
  const selectedMulticamSequence = useMemo(
    () => (selectedMulticamClip ? project.sequences.find((s) => s.id === selectedMulticamClip.sequenceId) : undefined),
  ...
  const selectedEditableClip = useMemo(
    () => editableCanvasClips.find((c) => c.clip.id === selectedClipId),
  ...
  const chromaKeyPickTarget = useMemo(
    () => editableCanvasClips.find((c) => c.clip.id === chromaKeyPickClipId),
    [chromaKeyPickClipId, editableCanvasClips],
  );
  const selectedPathMask = useMemo(
    () => selectedEditableClip?.clip.masks?.find((m) => m.type === 'path'),
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      try {
        ...
        if (session?.clipId === selectedMulticamClip.id) {
          session.command.record(playheadTime, angle.id);
        } else {
          const cmd = new RecordAngleCutCommand(projectAccessor, selectedMulticamClip.id, [
            { sceneTime: playheadTime, angleId: angle.id },
          ]);
          commandManager.execute(cmd);
          liveCutSessionRef.current = multicamLiveMode ? { clipId: selectedMulticamClip.id, command: cmd } : null;
        }
      } catch (error) {
        showToast({
          kind: 'warning',
  ...
  useEffect(() => {
    if (typeof colorScopesVisible === 'boolean') setScopesOpen(colorScopesVisible);
  }, [colorScopesVisible]);
  useEffect(() => {
  ...
  // Ctrl/Cmd+G 聚焦帧精确搜索：d00bf554 引入、bd315fd6 拆分时丢失，按原实现恢复
  useEffect(() => {
  ...
  const selectSnapshotCompare = async (path: string) => {
    ...
    try {
      setSnapshotCompareProject(await readProjectSnapshot(entry, projectPath));
      setCompareMode('left-right');
    } catch (error) {
      setSnapshotComparePath('');
      setSnapshotCompareProject(undefined);
  ...
  const refreshSnapshotEntries = async () => {
    ...
    try {
      ...
      showToast({ kind: 'warning', title: zhCN.projectSnapshots.loadFailed, message: '' });
    } finally {
      setSnapshotCompareLoading(false);
    }
  };
  ...
  const recordMulticamAngleCut = (angleId: string) => {
    ...
    try {
      const session = liveCutSessionRef.current;
      if (multicamLiveMode && session?.clipId === selectedMulticamClip.id) {
        session.command.record(playheadTime, angleId);
      } else {
        const cmd = new RecordAngleCutCommand(projectAccessor, selectedMulticamClip.id, [
  ...
  const adaptiveIndicatorTitle =
    previewPerformance.adaptiveEnabled === false
      ...
      : t.adaptiveQualityTooltip(
          renderer.adaptivePreviewState.averageFps,
          t.qualityLabels[renderer.effectivePreviewPerformance.qualityMode],
        );
  ...
  return (
    ...
    >
      <PreviewControls
        title={t.title}
        previewCanvasSizeLabel={renderer.previewCanvasSizeLabel}
        reviewMode={reviewMode}
        compareEnabled={compareEnabled}
        compareMode={compareMode}
        canvasEditMode={canvasEditMode}
        frameInspectMode={frameInspectMode}
        scopesOpen={scopesOpen}
        isPlaying={isPlaying}
        snapshotComparePath={snapshotComparePath}
        snapshotCompareLoading={snapshotCompareLoading}
        snapshotEntries={snapshotEntries}
        reviewTool={reviewTool}
        reviewText={reviewText}
        onSnapshotCompareSelect={(p) => void selectSnapshotCompare(p)}
        onSnapshotPointerDown={() => void refreshSnapshotEntries()}
        onCompareModeChange={setCompareMode}
        onToggleCompare={toggleCompareMode}
        onToggleCanvasEditMode={() =>
          setCanvasEditMode((v) => {
            const n = !v;
            if (n) {
              setFrameInspectMode(false);
              interaction.clearFrameInspector();
            }
            return n;
          })
        }
        onToggleFrameInspectMode={() =>
          setFrameInspectMode((v) => {
            const n = !v;
            if (n) {
              setCanvasEditMode(false);
              setChromaKeyPickClipId(undefined);
            }
            if (!n) interaction.clearFrameInspector();
            return n;
          })
        }
        onToggleScopes={() =>
          setScopesOpen((v) => {
            const n = !v;
            onColorScopesVisibleChange?.(n);
            return n;
          })
        }
        onTogglePlayback={() => setIsPlaying(!isPlaying)}
        onReviewToolChange={setReviewTool}
        onReviewTextChange={setReviewText}
        onExportReviewReport={onExportReviewReport}
      />
      <div
        className={
          scopesOpen
      ...
      >
        <div className={scopesOpen ? 'flex min-h-0 items-center justify-center p-4' : 'contents'}>
          <div
            ref={compareFrameRef}
            className={`relative aspect-video w-full max-w-[960px] overflow-hidden rounded-md shadow-soft ${previewZoom > 1 && !canvasEditMode && !frameInspectMode && !chromaKeyPickTarget && !compareEnabled ? 'cursor-grab active:cursor-grabbing' : ''}`}
            style={{ backgroundColor: theme.colors.canvasBackground }}
            onWheel={interaction.updatePreviewZoomFromWheel}
            onPointerDown={interaction.beginPreviewPan}
            onPointerMove={interaction.updatePreviewPan}
            onPointerUp={interaction.endPreviewPan}
            onPointerCancel={interaction.endPreviewPan}
          >
            <PreviewEffects
              adaptiveIndicatorStatus={adaptiveIndicatorStatus}
              adaptiveIndicatorTitle={adaptiveIndicatorTitle}
              lowQualityPreview={renderer.lowQualityPreview}
              audioOnlyPreview={renderer.audioOnlyPreview}
              showGpuMetricsPanel={showGpuMetricsPanel}
              gpuPreviewMetrics={renderer.gpuPreviewMetrics}
              gpuTextureMemoryLabel={renderer.gpuTextureMemoryLabel}
              effectivePreviewPerformance={renderer.effectivePreviewPerformance}
              adaptivePreviewState={renderer.adaptivePreviewState}
            ...
            <PreviewOverlay
              ...
              canvasEditMode={canvasEditMode}
              frameInspectMode={frameInspectMode}
              selectedEditableClip={selectedEditableClip}
              selectedPathMask={selectedPathMask}
              selectedMulticamClip={selectedMulticamClip}
              selectedMulticamSequence={selectedMulticamSequence}
              selectedPanoramaClip={selectedPanoramaClip}
              chromaKeyPickTarget={chromaKeyPickTarget}
              project={project}
              playheadTime={playheadTime}
              fps={fps}
              multicamLiveMode={multicamLiveMode}
              isPlaying={isPlaying}
              frameInspectorSample={frameInspectorSample}
              selectedInspectorClip={selectedInspectorClip}
              compareEnabled={compareEnabled}
              compareShowsDifference={compareShowsDifference}
              compareMode={compareMode}
              compareSplitRatio={compareSplitRatio}
              compareDividerDragging={compareDividerDragging}
              previewRenderSize={renderer.previewRenderSize}
              previewQualityMode={renderer.effectivePreviewPerformance.qualityMode}
              canvasRef={canvasRef}
              originalCanvasRef={originalCanvasRef}
              differenceCanvasRef={differenceCanvasRef}
              previewSurfaceRef={previewSurfaceRef}
              previewSurfaceStyle={previewSurfaceStyle}
              previewZoom={previewZoom}
              onBeginReviewAnnotation={interaction.beginReviewAnnotation}
              onEndReviewAnnotation={interaction.endReviewAnnotation}
              onBeginCanvasHitDrag={interaction.beginCanvasHitDrag}
              onUpdateCanvasTransformDrag={interaction.updateCanvasTransformDrag}
              onEndCanvasTransformDrag={interaction.endCanvasTransformDrag}
              onBeginCanvasTransformDrag={interaction.beginCanvasTransformDrag}
              onBeginPanoramaPreviewDrag={interaction.beginPanoramaPreviewDrag}
              onUpdatePanoramaPreviewDrag={interaction.updatePanoramaPreviewDrag}
              onEndPanoramaPreviewDrag={interaction.endPanoramaPreviewDrag}
              onUpdatePanoramaFov={interaction.updatePanoramaFov}
              onPickChromaKeyColor={interaction.pickChromaKeyColor}
              onUpdateFrameInspector={interaction.updateFrameInspector}
              onClearFrameInspector={interaction.clearFrameInspector}
              onSampleFrameInspectorColor={interaction.sampleFrameInspectorColor}
              onApplyFrameInspectorColor={() => interaction.applyFrameInspectorColor()}
              onAddPathMaskAnchor={interaction.addPathMaskAnchor}
              onCloseSelectedPathMask={interaction.closeSelectedPathMask}
              onBeginPathMaskDrag={interaction.beginPathMaskDrag}
              onUpdatePathMaskDrag={interaction.updatePathMaskDrag}
              onEndPathMaskDrag={interaction.endPathMaskDrag}
              onUpdateCompareSplit={interaction.updateCompareSplitFromPointer}
              onCompareDividerDraggingChange={setCompareDividerDragging}
              onMulticamLiveModeChange={(enabled) => {
                liveCutSessionRef.current = null;
                setMulticamLiveMode(enabled);
              }}
              onSelectMulticamAngle={recordMulticamAngleCut}
            ...
            <button
              ...
              onClick={interaction.resetPreviewZoom}
            >
      ...
      <PreviewTimeline
        ...
        frameSearchError={frameSearchError}
        frameSearchFocused={frameSearchFocused}
        frameSearchHistory={frameSearchHistory}
        frameSearchCandidates={frameSearchCandidates}
        showFrameSearchCandidates={showFrameSearchCandidates}
        showFrameSearchHistory={showFrameSearchHistory}
        onFrameSearchQueryChange={setFrameSearchQuery}
        onFrameSearchErrorChange={setFrameSearchError}
        onFrameSearchFocusedChange={setFrameSearchFocused}
        onFrameSearchHistoryChange={setFrameSearchHistory}
        onJumpToFrame={setPlayheadTime}
        onSelectClip={setSelectedClipIds}
        onStopPlayback={() => setIsPlaying(false)}
        frameSearchInputRef={frameSearchInputRef}
      />
    </section>
  );
}
