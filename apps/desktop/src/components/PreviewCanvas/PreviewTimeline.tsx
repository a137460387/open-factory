...
export function PreviewTimeline(props: PreviewTimelineProps) {
  const {
    playheadTime, fps, project,
    frameSearchQuery, frameSearchError, frameSearchFocused,
    frameSearchHistory, frameSearchCandidates,
    showFrameSearchCandidates, showFrameSearchHistory,
    onFrameSearchQueryChange, onFrameSearchErrorChange,
    onFrameSearchFocusedChange, onFrameSearchHistoryChange,
    onJumpToFrame, onSelectClip, onStopPlayback,
    frameSearchInputRef,
  } = props;
  ...
  function jumpToFrameSearchCandidate(candidate: FrameSearchCandidate): void {
    ...
    recordFrameSearchHistory({ type: candidate.type, query: candidate.label, label: candidate.label, time: candidate.time, ...(candidate.type === 'clip' ? { selectedClipIds: [candidate.id] } : {}) });
  ...
  function executeFrameSearch(): void {
    ...
    if (!query) { onFrameSearchErrorChange(undefined); return; }
    if (isFrameJumpLikeQuery(query)) {
      ...
      if (!parsed.ok) { onFrameSearchErrorChange(frameSearchErrorMessage(parsed.error, fps)); return; }
      onStopPlayback();
      onJumpToFrame(parsed.value.seconds);
      recordFrameSearchHistory({
        type: parsed.value.kind, query,
    ...
    if (candidate) { jumpToFrameSearchCandidate(candidate); return; }
    ...
    <div className="flex items-center gap-3 border-t border-black/30 px-3 py-2 text-xs text-slate-300">
      ...
      <div className="relative min-w-0 flex-1">
        <input
          ...
          onChange={(e) => { onFrameSearchQueryChange(e.target.value); onFrameSearchErrorChange(undefined); }}
          onKeyDown={(e) => {
            if (e.key === 'Enter') { e.preventDefault(); executeFrameSearch(); }
            if (e.key === 'Escape') { onFrameSearchErrorChange(undefined); onFrameSearchFocusedChange(false); e.currentTarget.blur(); }
          }}
        />
        {frameSearchError ? <div className="absolute left-0 top-8 z-50 rounded border border-red-400/50 bg-red-950 px-2 py-1 text-[11px] text-red-100 shadow-soft" data-testid="frame-search-error">{frameSearchError}</div> : null}
        {showFrameSearchCandidates && frameSearchCandidates.length > 0 ? (
          <div className="absolute bottom-8 left-0 z-50 max-h-44 w-full overflow-auto rounded-md border border-white/15 bg-[#0b1120] py-1 shadow-soft" data-testid="frame-search-candidates">
            {frameSearchCandidates.map((c) => (
              <button key={`${c.type}-${c.id}`} type="button" className="flex w-full items-center justify-between gap-3 px-2 py-1.5 text-left text-xs text-slate-100 hover:bg-white/10" data-testid={`frame-search-candidate-${c.type}-${c.id}`} onMouseDown={(e) => e.preventDefault()} onClick={() => jumpToFrameSearchCandidate(c)}>
                <span className="min-w-0 truncate"><span className="mr-2 rounded bg-white/10 px-1.5 py-0.5 text-[10px] uppercase text-slate-300">{c.type === 'marker' ? t.frameSearchMarkerType : t.frameSearchClipType}</span>{c.label}</span>
                <span className="shrink-0 font-mono tabular-nums text-slate-400">{secondsToTimecode(c.time, fps, project.settings.timecodeFormat ?? 'ndf')}</span>
              </button>
            ))}
          </div>
        ) : null}
        {showFrameSearchHistory ? (
          <div className="absolute bottom-8 left-0 z-50 max-h-44 w-full overflow-auto rounded-md border border-white/15 bg-[#0b1120] py-1 shadow-soft" data-testid="frame-search-history">
            {frameSearchHistory.map((entry, index) => (
              <button key={`${entry.type}-${entry.query}-${entry.time}`} type="button" className="flex w-full items-center justify-between gap-3 px-2 py-1.5 text-left text-xs text-slate-100 hover:bg-white/10" data-testid={`frame-search-history-${entry.type}-${index}`} onMouseDown={(e) => e.preventDefault()} onClick={() => jumpToFrameSearchHistoryEntry(entry)}>
                <span className="min-w-0 truncate"><span className="mr-2 rounded bg-white/10 px-1.5 py-0.5 text-[10px] uppercase text-slate-300">{frameSearchHistoryTypeLabel(entry.type)}</span>{entry.label}</span>
