...
export function SubtitleSyncPanel({
  ...
  return (
    <div
      ...
      data-testid="subtitle-sync-panel"
    >
      <section className="flex max-h-[80vh] w-full max-w-xl flex-col overflow-hidden rounded-lg border border-line bg-white shadow-soft">
        <header className="flex items-center justify-between border-b border-line px-4 py-3">
          ...
          <button
            ...
            type="button"
            onClick={onClose}
            data-testid="subtitle-sync-close-button"
          >
            <X size={16} />
          </button>
        </header>
        <div className="flex items-center gap-3 border-b border-line px-4 py-2">
          <label className="text-xs font-medium text-slate-600">
            ...
            <select
              ...
              value={sensitivity}
              onChange={(e) => setSensitivity(mapSensitivityLabel(e.target.value))}
              data-testid="subtitle-sync-sensitivity-select"
            >
          ...
          <button
            className="ml-auto inline-flex items-center gap-1 rounded-md bg-brand px-3 py-1.5 text-xs font-medium text-white hover:bg-[#176858]"
            type="button"
            onClick={handleScan}
            disabled={scanning}
            data-testid="subtitle-sync-scan-button"
          >
          ...
          {!scanning && report && report.warningCount > 0 && (
            <div className="space-y-2" data-testid="subtitle-sync-warnings">
              <p className="mb-3 text-xs text-slate-600">
                {t.totalSubtitles(report.totalSubtitles)} · {t.alignedCount(report.alignedCount)} ·{' '}
                {t.warningCount(report.warningCount)}
              </p>
              {report.warnings.map((w) => (
                <div
                  key={w.subtitleClipId}
                  className="flex items-center justify-between rounded-md border border-amber-200 bg-amber-50 p-2"
                  data-testid={`subtitle-sync-warning-${w.subtitleClipId}`}
                >
                  <div className="flex items-center gap-2">
                    <AlertTriangle size={14} className="text-amber-500" />
                    <span className="text-xs text-amber-800">
                      {t.offsetMs(w.offsetMs)} · {w.severity === 'major' ? t.severityMajor : t.severityMinor}
                    </span>
                  </div>
                  {repairedIds.has(w.subtitleClipId) ? (
                    ...
                    <button
                      className="inline-flex items-center gap-1 rounded border border-line px-2 py-0.5 text-xs text-slate-700 hover:bg-panel"
                      type="button"
                      onClick={() => handleRepair(w.subtitleClipId)}
                      data-testid={`subtitle-sync-repair-${w.subtitleClipId}`}
                    >
