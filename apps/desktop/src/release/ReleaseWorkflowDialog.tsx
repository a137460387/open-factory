...
export function ReleaseWorkflowDialog({ project, projectPath, lastExportPath, onReleaseCreated, onApplyDiff, onClose }: ReleaseWorkflowDialogProps) {
  const t = zhCN.releaseWorkflow;
  const defaultVersion = useMemo(() => splitVersion(incrementSemverPatch(project.releaseVersion)), [project.releaseVersion]);
  ...
  const latestQualityAssurance = useExportQueueStore((state) => state.history.find((entry) => entry.report?.qualityAssurance)?.report?.qualityAssurance);
  const selectedPreset = useMemo(() => BUILTIN_EXPORT_PRESETS.find((preset) => preset.id === exportPresetId), [exportPresetId]);
  ...
  async function publishRelease() {
    ...
    try {
      ...
      setRecords((current) => [saved, ...current.filter((item) => item.path !== saved.path)].sort((left, right) => right.releasedAt.localeCompare(left.releasedAt)));
      setTargetRecordPath(saved.path);
      setChangelog('');
    } catch (error) {
      showToast({ kind: 'error', title: t.publishFailed, message: error instanceof Error ? error.message : t.publishFailed });
    } finally {
      setPublishing(false);
    }
  }
  ...
  async function compareReleases() {
    ...
    try {
      ...
      showToast({ kind: 'error', title: t.compareFailed, message: error instanceof Error ? error.message : t.compareFailed });
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 p-4" data-testid="release-workflow-dialog">
      <div className="flex max-h-[88vh] w-full max-w-6xl flex-col overflow-hidden rounded-md border border-line bg-white shadow-soft">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_360px] gap-px overflow-hidden bg-line">
          <div className="min-h-0 overflow-y-auto bg-white p-4">
            <section className="space-y-3">
              <div className="grid gap-3 md:grid-cols-3">
                <NumberField label={t.major} value={version.major} testId="release-version-major" onChange={(major) => setVersion((current) => ({ ...current, major }))} />
                <NumberField label={t.minor} value={version.minor} testId="release-version-minor" onChange={(minor) => setVersion((current) => ({ ...current, minor }))} />
                <NumberField label={t.patch} value={version.patch} testId="release-version-patch" onChange={(patch) => setVersion((current) => ({ ...current, patch }))} />
              </div>
            ...
            <section className="mt-5 grid gap-3 md:grid-cols-2">
              <label className="block text-xs font-medium text-slate-600">
                {t.exportPreset}
                <select className="mt-1 w-full rounded-md border border-line bg-white px-2 py-1.5 text-sm text-ink" value={exportPresetId} data-testid="release-export-preset-select" onChange={(event) => setExportPresetId(event.target.value)}>
              ...
              <label className="block text-xs font-medium text-slate-600">
                {t.exportPath}
                <input className="mt-1 w-full rounded-md border border-line px-2 py-1.5 text-sm text-ink" value={outputPath} data-testid="release-export-path-input" onChange={(event) => setOutputPath(event.target.value)} />
              </label>
              <label className="block text-xs font-medium text-slate-600">
                {t.assignee}
                <input className="mt-1 w-full rounded-md border border-line px-2 py-1.5 text-sm text-ink" value={assignee} data-testid="release-assignee-input" onChange={(event) => setAssignee(event.target.value)} />
              </label>
            </section>
            ...
            <section className="mt-5">
              ...
              <div className="grid gap-2 md:grid-cols-2">
                {(['qualityGate', 'mediaRelink', 'subtitleProof', 'exportPreset'] as const).map((id) => (
                  <label key={id} className="flex items-center gap-2 rounded-md border border-line bg-panel px-3 py-2 text-sm text-slate-700">
                    <input
                      className="h-4 w-4 accent-brand"
                      type="checkbox"
                      checked={checklistOptions[id]}
                      data-testid={`release-check-toggle-${id}`}
                      onChange={(event) => setChecklistOptions((current) => ({ ...current, [id]: event.target.checked }))}
                    />
            ...
            <section className="mt-5">
              <label className="block text-xs font-medium text-slate-600">
                {t.changelog}
                <textarea className="mt-1 min-h-28 w-full resize-y rounded-md border border-line px-3 py-2 text-sm text-ink" value={changelog} data-testid="release-changelog-input" onChange={(event) => setChangelog(event.target.value)} />
              </label>
            </section>
          </div>
          ...
          <aside className="min-h-0 overflow-y-auto bg-panel p-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-line bg-white text-slate-600 hover:bg-panel" type="button" title={t.refresh} aria-label={t.refresh} data-testid="release-refresh-button" onClick={() => void refreshRecords()}>
                <RefreshCw size={14} />
              </button>
            </div>
            <div className="space-y-2" data-testid="release-history-list">
              {records.length === 0 ? (
                <div className="text-sm text-slate-500">{loadingRecords ? zhCN.common.unavailable : t.emptyHistory}</div>
              ) : (
            ...
            <div className="mt-5 border-t border-line pt-4">
              ...
              <div className="space-y-2">
                <ReleaseSelect label={t.compareBase} value={baseRecordPath} records={records} testId="release-compare-base-select" onChange={setBaseRecordPath} />
                <ReleaseSelect label={t.compareTarget} value={targetRecordPath} records={records} testId="release-compare-target-select" onChange={setTargetRecordPath} />
                <button className="w-full rounded-md bg-brand px-3 py-2 text-sm font-semibold text-white hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-50" type="button" data-testid="release-compare-button" disabled={records.length < 2} onClick={() => void compareReleases()}>
                  {t.compare}
                </button>
              </div>
              {compareState ? (
                <div className="mt-3 space-y-2" data-testid="release-diff-panel">
                  <div className="text-xs font-semibold text-slate-600" data-testid="release-diff-summary">
                    {zhCN.projectSnapshots.diffSummary(compareState.diff.diff.summary.added, compareState.diff.diff.summary.deleted, compareState.diff.diff.summary.modified, compareState.diff.diff.summary.trackChanges)}
                  </div>
                  <div className="max-h-48 overflow-y-auto rounded-md border border-line bg-white">
                    {compareState.diff.diff.items.length === 0 ? (
                      ...
                      compareState.diff.diff.items.map((item) => <DiffRow key={item.id} item={item} checked={selectedDiffs.includes(item.id)} onToggle={() => toggleDiff(item.id)} />)
                    )}
                  </div>
                  <button className="w-full rounded-md border border-line bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-panel disabled:cursor-not-allowed disabled:opacity-50" type="button" data-testid="release-apply-diff-button" disabled={selectedDiffs.length === 0} onClick={applySelectedDiffs}>
        ...
        <div className="flex items-center justify-between border-t border-line p-4">
          ...
          <div className="flex gap-2">
            ...
            <button className="rounded-md bg-brand px-3 py-2 text-sm font-semibold text-white hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-50" type="button" data-testid="release-publish-button" disabled={publishing} onClick={() => void publishRelease()}>
...
function ChecklistRow({ item }: { item: ReleaseChecklistItemResult }) {
  const t = zhCN.releaseWorkflow;
  return (
    <div className="rounded-md border border-line bg-white p-2 text-xs" data-testid={`release-check-result-${item.id}`} data-status={item.status}>
      <div className="flex items-center justify-between gap-2">
        <span className="font-semibold text-slate-700">{t.checks[item.id]}</span>
        <span className={`rounded-full border px-2 py-0.5 font-semibold ${statusClass(item.status)}`}>{t.status[item.status]}</span>
      </div>
...
function ReleaseHistoryRow({ record }: { record: ProjectReleaseEntry }) {
  return (
    <div className="rounded-md border border-line bg-white p-3 text-xs" data-testid="release-history-row">
      ...
      {record.changelog ? <div className="mt-2 whitespace-pre-wrap rounded-md bg-panel p-2 text-slate-600" data-testid="release-history-changelog">{record.changelog}</div> : null}
    </div>
  );
}

function ReleaseSelect({ label, value, records, testId, onChange }: { label: string; value: string; records: ProjectReleaseEntry[]; testId: string; onChange(value: string): void }) {
  return (
    <label className="block text-xs font-medium text-slate-600">
      {label}
...
function DiffRow({ item, checked, onToggle }: { item: TimelineVersionDiffItem; checked: boolean; onToggle(): void }) {
  return (
    <label className="flex items-start gap-2 border-b border-line px-3 py-2 text-xs last:border-b-0" data-testid="release-diff-row">
      <input className="mt-0.5 h-4 w-4 accent-brand" type="checkbox" checked={checked} onChange={onToggle} data-testid={`release-diff-check-${item.id}`} />
...
function NumberField({ label, value, testId, onChange }: { label: string; value: number; testId: string; onChange(value: number): void }) {
  return (
    <label className="block text-xs font-medium text-slate-600">
      {label}
      <input className="mt-1 w-full rounded-md border border-line px-2 py-1.5 text-sm text-ink" type="number" min={0} value={value} data-testid={testId} onChange={(event) => onChange(Number(event.target.value))} />
    </label>
  );
}
