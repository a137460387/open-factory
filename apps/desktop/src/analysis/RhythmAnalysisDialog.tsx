...
import {
  analyzeClipRhythm,
  buildRhythmAnalysisHtml,
  formatReportDuration,
  serializeRhythmAnalysisJson,
  type Project,
  type ReportLocale,
  type RhythmAnalysisReport,
...
export function RhythmAnalysisDialog({ project, onClose }: RhythmAnalysisDialogProps) {
  ...
  async function exportJson(): Promise<void> {
    const outputPath = await saveFileDialog(`${project.name}-rhythm-analysis.json`, [
  ...
  async function exportHtml(): Promise<void> {
    const outputPath = await saveFileDialog(`${project.name}-rhythm-analysis.html`, [
  ...
  return (
    <div
      ...
      data-testid="rhythm-analysis-dialog"
    >
      <section className="flex max-h-[88vh] w-full max-w-5xl flex-col overflow-hidden rounded-lg bg-white shadow-soft">
        <header className="flex items-center justify-between border-b border-line px-4 py-3">
          ...
          <button
            ...
            data-testid="rhythm-analysis-close-button"
            onClick={onClose}
          >
            <X size={16} />
          </button>
            ...
            <section className="rounded-md border border-line bg-panel p-3">
              ...
              <div className="space-y-2" data-testid="rhythm-analysis-reference-list">
                {report.references.map((reference) => (
                  <div
                    key={reference.type}
                    className="grid grid-cols-[1fr_auto] gap-2 rounded border border-line bg-white px-2 py-1.5 text-xs"
                  >
                    <span className="font-medium text-slate-700">{t.referenceTypes[reference.type]}</span>
                    <span className="tabular-nums text-slate-500">
                      {formatReportDuration(reference.averageShotDuration, locale)}
                    </span>
                  </div>
                ))}
              </div>
          ...
          <section className="grid gap-4 lg:grid-cols-2">
            ...
            <div className="rounded-md border border-line bg-white p-3">
              ...
              {report.suggestions.length > 0 ? (
                ...
                <div
                  className="rounded border border-line bg-panel px-3 py-4 text-sm text-slate-500"
                  data-testid="rhythm-analysis-no-suggestions"
                >
                  {t.noSuggestions}
                </div>
              )}
        ...
        <footer className="flex flex-wrap justify-end gap-2 border-t border-line px-4 py-3">
          <button
            className="inline-flex items-center gap-2 rounded-md border border-line bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-panel"
            type="button"
            data-testid="rhythm-analysis-export-json-button"
          ...
          <button
            className="inline-flex items-center gap-2 rounded-md bg-brand px-3 py-2 text-sm font-semibold text-white hover:bg-[#176858]"
            type="button"
            data-testid="rhythm-analysis-export-html-button"
    ...
    <section className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5" data-testid="rhythm-analysis-stats">
      {stats.map((stat) => (
        <div
          ...
          data-testid={`rhythm-analysis-stat-${stat.key}`}
        >
...
function RhythmCurveCanvas({ report }: { report: RhythmAnalysisReport }) {
  ...
  return (
    <canvas
      ref={canvasRef}
      className="h-56 w-full rounded-md border border-line bg-panel"
      width={720}
      height={240}
      role="img"
      aria-label={t.cutCurve}
      data-testid="rhythm-analysis-curve-canvas"
    />
  );
}
...
function RhythmChangeTable({ report, locale }: { report: RhythmAnalysisReport; locale: ReportLocale }) {
  ...
  if (report.changePoints.length === 0) {
    return (
      <div className="rounded border border-line bg-panel px-3 py-4 text-sm text-slate-500">{t.noChangePoints}</div>
    );
  }
  return (
