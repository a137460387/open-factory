...
export function ContentAnalysisDialog({ targets, selectedClipIds, analyzingClipId, onAnalyze, onAnalyzePreferred, onExport, onClose }: ContentAnalysisDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" data-testid="content-analysis-dialog">
      <section className="flex max-h-[88vh] w-full max-w-5xl flex-col overflow-hidden rounded-lg bg-white shadow-soft">
        <header className="flex items-center justify-between border-b border-line px-4 py-3">
          ...
          <button className="inline-flex h-8 w-8 items-center justify-center rounded-md text-slate-500 hover:bg-panel" type="button" title={zhCN.common.close} aria-label={zhCN.common.close} data-testid="content-analysis-close-button" onClick={onClose}>
            <X size={16} />
          </button>
        </header>
        <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto p-4">
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-line bg-panel p-3">
            <div className="min-w-0 text-sm text-slate-600">{targets.length === 0 ? t.noTargets : t.targetSummary(targets.length, preferredCount)}</div>
            <button
          ...
          <div className="grid min-h-0 gap-3 lg:grid-cols-[320px_minmax(0,1fr)]">
            <div className="space-y-2" data-testid="content-analysis-target-list">
              {targets.length === 0 ? <div className="rounded-md border border-dashed border-line p-4 text-sm text-slate-500">{t.noTargets}</div> : null}
              {targets.map((target) => (
                <TargetRow
                  key={target.clip.id}
                  target={target}
...
function TargetRow({
  ...
  return (
    <div className="rounded-md border border-line bg-white p-3 shadow-sm" data-testid={`content-analysis-target-${target.clip.id}`} data-analyzed={analysis ? 'true' : 'false'}>
      <div className="flex items-start justify-between gap-2">
        ...
        <span className="rounded bg-panel px-1.5 py-0.5 text-[11px] font-semibold text-slate-600">{target.clip.type}</span>
      </div>
      {analysis ? <SceneTagList analysis={analysis} assetId={target.asset.id} /> : <div className="mt-2 text-xs text-slate-500">{zhCN.contentAnalysis.notAnalyzed}</div>}
      <div className="mt-3 flex gap-2">
        <button className="inline-flex flex-1 items-center justify-center gap-1 rounded-md border border-line bg-panel px-2 py-1.5 text-xs font-semibold hover:bg-white disabled:opacity-50" type="button" disabled={analyzing} data-testid={`content-analysis-run-${target.clip.id}`} onClick={onAnalyze}>
        ...
        <button className="inline-flex flex-1 items-center justify-center gap-1 rounded-md border border-line bg-white px-2 py-1.5 text-xs font-semibold hover:bg-panel disabled:opacity-50" type="button" disabled={!analysis} data-testid={`content-analysis-export-${target.clip.id}`} onClick={onExport}>
          <Download size={13} />
...
function AnalysisOverview({ targets }: { targets: ContentAnalysisTarget[] }) {
  ...
  if (!latest) {
    return <div className="rounded-md border border-line bg-white p-4 text-sm text-slate-500">{zhCN.contentAnalysis.emptyOverview}</div>;
  }
  return (
    <div className="space-y-3 rounded-md border border-line bg-white p-4" data-testid="content-analysis-overview">
      <div>
        <div className="text-xs font-semibold uppercase tracking-normal text-slate-500">{zhCN.contentAnalysis.sceneTypes}</div>
        <SceneTagList analysis={latest} assetId="overview" />
      </div>
      <div>
        <div className="mb-2 text-xs font-semibold uppercase tracking-normal text-slate-500">{zhCN.contentAnalysis.emotionCurve}</div>
  ...
  return (
    <div className="mt-2 flex flex-wrap gap-1" data-testid={`scene-tags-${assetId}`}>
      {analysis.sceneTypes.map((sceneType) => (
        <span key={sceneType} className="rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[11px] font-semibold text-emerald-800" data-testid={`content-analysis-scene-tag-${sceneType}-${assetId}`}>
...
function EmotionCurve({ analysis }: { analysis: ClipContentAnalysis }) {
  ...
  const path = points
    ...
    .join(' ');
  return (
    <svg className="h-28 w-full rounded-md border border-line bg-panel" viewBox="0 0 100 40" role="img" aria-label={zhCN.contentAnalysis.emotionCurve} data-testid="content-analysis-emotion-curve">
