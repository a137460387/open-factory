...
export function normalizeGifWorkflowSettings(value: Partial<GifWorkflowSettings>, sourceDuration?: number): GifWorkflowSettings {
  const maxDuration = Number.isFinite(sourceDuration) && sourceDuration !== undefined && sourceDuration > 0 ? sourceDuration : Number.POSITIVE_INFINITY;
  const startTime = clampNumber(value.startTime, 0, Math.max(0, maxDuration), DEFAULT_GIF_WORKFLOW_SETTINGS.startTime);
  const remainingDuration = Number.isFinite(maxDuration) ? Math.max(0.1, maxDuration - startTime) : Number.POSITIVE_INFINITY;
  return {
    ...
    startTime,
    duration: clampNumber(value.duration, 0.1, remainingDuration, Math.min(DEFAULT_GIF_WORKFLOW_SETTINGS.duration, remainingDuration)),
    loopCount: Math.round(clampNumber(value.loopCount, 0, 100, DEFAULT_GIF_WORKFLOW_SETTINGS.loopCount)),
    dither: value.dither === 'bayer' || value.dither === 'floyd_steinberg' ? value.dither : DEFAULT_GIF_WORKFLOW_SETTINGS.dither
  };
}
...
export function estimateGifFileSizeBytes(input: GifSizeEstimateInput): number {
  ...
  const frames = Math.max(1, Math.ceil(clampNumber(input.frameRate, 1, 30, DEFAULT_GIF_WORKFLOW_SETTINGS.frameRate) * Math.max(0.1, input.duration)));
