...
export function DuplicateMediaDialog({ groups, onConfirm, onClose }: DuplicateMediaDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 p-4" data-testid="duplicate-media-dialog">
      <section className="w-full max-w-2xl rounded-md border border-line bg-white shadow-soft">
        ...
        <div className="max-h-[70vh] space-y-3 overflow-y-auto p-4 text-sm">
          {groups.length === 0 ? <div className="rounded-md border border-line bg-panel p-3 text-sm text-slate-600">{t.empty}</div> : null}
          {groups.map((group) => (
            <div key={group.id} className="rounded-md border border-line p-3" data-testid="duplicate-media-group">
              <div className="mb-2 text-xs font-semibold text-slate-700">{t.groupSummary(group.assets.length, formatBytes(group.size))}</div>
