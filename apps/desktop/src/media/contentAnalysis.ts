...
export async function analyzeClipContentLocally(
  clip: Clip,
  asset: MediaAsset,
  analyzedAt = new Date().toISOString(),
): Promise<ClipContentAnalysis> {
...
export async function exportClipContentAnalysisJson(
  clip: Pick<Clip, 'id' | 'name' | 'contentAnalysis'>,
): Promise<string | undefined> {
  const outputPath = await saveFileDialog(`${sanitizeFileName(clip.name || clip.id)}-content-analysis.json`, [
...
function buildVisualSamples(
  ...
  const bitrates =
    analysis.bitratePoints.length > 0
      ? analysis.bitratePoints
      : [{ time: 0, bitRate: video?.bitRate ?? analysis.format.bitRate ?? 1_000_000 }];
...
function estimateBaseBrightness(
  video: MediaAnalysis['videoStreams'][number] | undefined,
  analysis: MediaAnalysis,
): number {
...
function findNearestBitratePoint(
  points: MediaAnalysis['bitratePoints'],
  time: number,
): MediaAnalysis['bitratePoints'][number] {
