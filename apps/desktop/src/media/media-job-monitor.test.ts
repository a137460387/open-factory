...
describe('media job monitor', () => {
  ...
  it('sorts queue rows by active status, proxy priority, and creation time', () => {
    ...
    expect(sortMediaJobsForMonitor(jobs).map((job) => job.id)).toEqual(['running', 'waiting-high', 'waiting-early', 'waiting-late', 'failed', 'done']);
  });
  ...
  it('starts high priority proxy jobs before older low priority proxy jobs', () => {
    ...
    store.enqueueMonitorJob({ id: 'low-a', assetId: 'asset-low-a', assetName: 'low-a.mov', type: 'proxy', priority: 'low' });
    store.enqueueMonitorJob({ id: 'low-b', assetId: 'asset-low-b', assetName: 'low-b.mov', type: 'proxy', priority: 'low' });
    store.enqueueMonitorJob({ id: 'high', assetId: 'asset-high', assetName: 'high.mov', type: 'proxy', priority: 'high' });
