...
describe('batch watermark helpers', () => {
  it('builds one export job for each selected video or image asset', () => {
    ...
    expect(jobs.map((job) => job.outputPath)).toEqual([
      'D:/Exports/Scene A_watermarked.mp4',
      'D:/Exports/Still_watermarked.mp4',
      'D:/Exports/Scene B_watermarked.mp4',
    ]);
  ...
  it('renders file name templates with base name index and extension', () => {
    expect(buildBatchWatermarkFileName(asset('a', 'clip.v1.mp4', 'video'), '{name}_{index}_wm.{ext}', 7)).toBe(
      'clip.v1_007_wm.mp4',
    );
