...
export function normalizeTutorialProgressSettings(
  settings: Partial<TutorialProgressSettings> | undefined,
): TutorialProgressSettings {
  const step =
    typeof settings?.tutorialStep === 'number' && Number.isFinite(settings.tutorialStep)
      ? Math.round(settings.tutorialStep)
      : 0;
  return {
