...
export function TutorialOverlay({ progress, onSkip, onCloseCelebration }: TutorialOverlayProps) {
  ...
  if (completed) {
    return (
      <div
        ...
        data-testid="tutorial-complete-overlay"
      >
  ...
  return (
    <div
      className="pointer-events-none fixed inset-0 z-[80] text-white"
      data-testid="tutorial-overlay"
      data-step-index={progress.tutorialStep}
      data-step-id={step.id}
    >
      <TutorialMask rect={rect} />
      {rect ? (
        <div
          className="pointer-events-none fixed rounded-md border-2 border-amber-300 shadow-[0_0_0_4px_rgba(250,204,21,0.28),0_0_28px_rgba(250,204,21,0.5)]"
...
function TutorialBubble({
  ...
  return (
    <aside
      className="pointer-events-auto fixed rounded-md border border-white/20 bg-slate-950/95 p-4 text-white shadow-soft"
      style={style}
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[11px] font-semibold uppercase tracking-wide text-amber-200">
            {t.progress(progress.tutorialStep + 1, TUTORIAL_STEPS.length)}
          </div>
          <h2 className="mt-1 text-base font-semibold">{copy.title}</h2>
        </div>
        <button
          className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-slate-300 hover:bg-white/10 hover:text-white"
          type="button"
          title={t.skip}
          aria-label={t.skip}
      ...
      <div
        className="mt-3 rounded border border-amber-200/30 bg-amber-200/10 px-2 py-1.5 text-xs text-amber-100"
        data-testid="tutorial-step-waiting"
      >
        {t.waitingForAction}
      </div>
    </aside>
