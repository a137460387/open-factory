...
import type { ProjectConfig, ExportConfig, PluginInfo } from '../src/types.js';
...
describe('Result 辅助函数', () => {
  ...
  it('err 创建失败结果', () => {
    ...
    if (!result.ok) expect(result.error.message).toBe('失败');
  });
});
...
describe('TimelineAPI', () => {
  ...
  it('removeTrack 移除存在的轨道', () => {
    ...
    if (!result.ok) throw new Error('前置失败');
  ...
  it('addClip 向轨道添加片段', () => {
    ...
    if (!trackResult.ok) throw new Error('前置失败');
  ...
  it('moveClip 移动片段并保持时长', () => {
    ...
    if (!clipResult.ok) throw new Error('前置失败');
