...
export function extractSceneClipFeatures(input) {
    ...
    for (const frame of input.frames) {
        ...
        for (const pixel of pixels) {
            const [red, green, blue] = pixel.map(clampByte);
...
export function orderSceneClipFeatures(features, strategy) {
    ...
    if (strategy === 'brightness-asc' || strategy === 'brightness-desc') {
        ...
        return stable
            .sort((left, right) => direction * (left.feature.brightness - right.feature.brightness) ||
...
export function buildSceneReorderClipIds(currentIds, selectedIds, orderedSelectedIds) {
...
export function buildSceneReorderStarts(timeline, selectedClipIds, orderedSelectedClipIds) {
...
export function sceneHistogramDistance(left, right) {
...
function orderByGreedyColorSimilarity(features) {
...
function orderByMotionRhythm(features) {
...
function orderByDurationBalance(features) {
...
function isRgbPixel(pixel) {
    return Array.isArray(pixel) && pixel.length >= 3;
}
//# sourceMappingURL=scene-reorder.js.map