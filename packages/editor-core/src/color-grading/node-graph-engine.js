...
export class NodeGraphEngine {
     ...
     */
    static executeNode(node, _previousResults) {
        switch (node.type) {
            case 'primary-wheel':
    ...
    static executePrimaryWheel(node) {
        const p = node.params;
    ...
    static executePrimarySlider(node) {
        const p = node.params;
        const prefix = `cg_${node.id.replace(/-/g, '_')}`;
        return {
            nodeId: node.id,
    ...
    static executeHSLQualifier(node) {
        const p = node.params;
        const prefix = `cg_${node.id.replace(/-/g, '_')}`;
        return {
            nodeId: node.id,
     ...
     */
    static executeTrackingMask(node) {
        const p = node.params;
        return {
            nodeId: node.id,
            uniforms: {
...
//# sourceMappingURL=node-graph-engine.js.map