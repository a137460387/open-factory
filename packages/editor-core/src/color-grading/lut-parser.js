...
/** 解析 .3dl 文件 */
export function parse3dlFile(content) {
    const lines = content
        .split('\n')
        .map((l) => l.trim())
        .filter((l) => l && !l.startsWith('#'));
...
/** 导出为 .cube 格式 */
export function exportToCube(lut, title) {
    const lines = [];
    if (title) {
...
/** 生成 LUT 预览缩略图数据 */
export function generateLUTPreview(lut, width = 256, height = 32) {
...
//# sourceMappingURL=lut-parser.js.map