...
import { isDefaultColorCorrection } from '../model';
...
export const EXPORT_COST_EFFECT_COMPLEXITY_FACTORS = {
    blur: 1.35,
    sharpen: 1.2,
    vignette: 1.15,
    'film-grain': 1.25,
...
export function calculateEstimateConfidence(sampleCount) {
...
export function buildEstimateHistoryComparison(samples) {
    return samples
        .filter((s) => Number.isFinite(s.estimatedDurationSeconds) &&
        s.estimatedDurationSeconds > 0 &&
        Number.isFinite(s.exportDurationSeconds) &&
        s.exportDurationSeconds > 0)
        .slice(0, MAX_HISTORY_COMPARISON_ENTRIES)
        .map((s, index) => {
        const estimated = s.estimatedDurationSeconds;
...
export function learnComplexityCoefficients(historySamples, currentFactors = {}) {
    const result = [];
    const allEffectTypes = Object.keys(EXPORT_COST_EFFECT_COMPLEXITY_FACTORS);
    for (const effectType of allEffectTypes) {
        const defaultFactor = EXPORT_COST_EFFECT_COMPLEXITY_FACTORS[effectType] ?? 1;
        const relevantSamples = historySamples.filter((s) => {
            if (!Number.isFinite(s.estimatedDurationSeconds) || !s.estimatedDurationSeconds)
                return false;
            if (!Number.isFinite(s.exportDurationSeconds) || s.exportDurationSeconds <= 0)
                return false;
            return true;
        });
        ...
        const avgErrorRatio = relevantSamples.reduce((sum, s) => sum + s.exportDurationSeconds / s.estimatedDurationSeconds, 0) /
...
export function applyLearnedCoefficients(learned) {
    const result = {};
...
export function createDebouncedEstimator(fn, delayMs) {
    let timer;
    let lastArgs;
    let result;
    let hasResult = false;
...
export function calculateFilterComplexityFactor(timeline, settings = {}, qualityEvaluation = false) {
...
export function calculateHistoricalExportSpeed(samples) {
...
export function calculateHistoricalEstimateErrorPercent(estimatedSeconds, actualSeconds) {
...
export function parseExportBitrate(value) {
...
export function assertExportCostEffectCoverage() {
...
function clipProcessingOffset(clip, clipWeight, breakdown) {
    let offset = 0;
...
function collectSettingsFactors(settings, qualityEvaluation) {
    const factors = [];
...
function hasMasterProcessing(input) {
...
function calculateCodecThroughputFactor(settings) {
...
function calculateLastHistoricalEstimateError(samples) {
...
function defaultVideoBitsPerSecond(width, height, fps, format) {
    if (format === 'gif' || format === 'webp' || format === 'apng') {
