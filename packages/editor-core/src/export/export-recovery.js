...
export function buildExportRecoveryDecision(plan, error, attempts) {
...
export function appendExportRecoveryLog(entries, decision, originalError, result = 'pending') {
    return [
        ...entries,
        {
...
export function finalizeExportRecoveryLog(entries, result) {
    if (entries.length === 0) {
        return [];
    }
    return entries.map((entry, index) => (index === entries.length - 1 ? { ...entry, result } : entry));
}
export function buildExportRecoveryReport(entries, healed) {
    if (entries.length === 0) {
        return undefined;
    }
    return {
...
export function hasEnoughDiskSpace(availableBytes, expectedBytes, reserveBytes = 512 * 1024 * 1024) {
...
export function fallbackExportCodecPlan(plan) {
...
export function stripDrawtextFromExportPlan(plan) {
...
function mapExportPlanArgs(plan, mapArgs, settingsPatch, mapFilterComplex = (filterComplex) => filterComplex) {
...
function replaceCodecArg(args, fallbackCodec) {
    const next = [...args];
...
function stripDrawtextFilters(value) {
    return value
        .split(',')
        .filter((part) => !part.trim().startsWith('drawtext='))
        .join(',');
}
//# sourceMappingURL=export-recovery.js.map