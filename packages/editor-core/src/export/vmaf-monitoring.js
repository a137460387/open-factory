 ...
 * 检测 VMAF 环境能力
 */
export async function detectVmafCapabilities() {
    try {
        // 检查 FFmpeg 是否支持 VMAF
 ...
 * 确定最佳监控模式
 */
export function determineMonitoringMode(capabilities, config = {}) {
 ...
 * 生成 VMAF 采样计划
 */
export function generateVmafSamplePlan(duration, config) {
 ...
 */
export function buildVmafSampleCommand(sourcePath, outputPath, timestamp, config) {
    const args = [
        'ffmpeg',
        '-ss',
        String(timestamp),
 ...
 * 解析 VMAF 结果
 */
export function parseVmafResult(jsonOutput) {
    try {
        ...
        // 取平均值
        const vmafScores = frames.map((f) => f.metrics?.vmaf ?? 0);
        const psnrScores = frames.map((f) => f.metrics?.psnr).filter(Boolean);
        const ssimScores = frames.map((f) => f.metrics?.ssim).filter(Boolean);
        return {
 ...
 * 分析 VMAF 监控结果
 */
export function analyzeVmafResults(samples) {
    if (samples.length === 0) {
        return {
            samples: [],
            averageVmaf: 0,
 ...
 * 生成 VMAF 质量报告
 */
export function generateVmafReport(result, projectName) {
    const lines = [];
 ...
 */
export function createDegradedQualityReport(plan, settings, duration) {
    // 基于编码参数估算质量
 ...
 * 基于编码参数估算质量分数
 */
function estimateQualityFromSettings(settings) {
    let score = 70; // 基础分数
    // CRF 影响
 ...
 * 从设置中提取 CRF 值
 */
function getCrfFromSettings(settings) {
...
function getQualityRating(score) {
    if (score >= VMAF_QUALITY_THRESHOLDS.excellent)
        return 'excellent';
    if (score >= VMAF_QUALITY_THRESHOLDS.good)
        return 'good';
    if (score >= VMAF_QUALITY_THRESHOLDS.fair)
        return 'fair';
    return 'poor';
}
/**
 * 获取质量评级标签
 */
function getQualityRatingLabel(rating) {
    const labels = {
        excellent: '优秀 (≥90)',
        good: '良好 (≥75)',
        fair: '一般 (≥60)',
...
//# sourceMappingURL=vmaf-monitoring.js.map