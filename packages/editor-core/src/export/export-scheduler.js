 ...
 * 分析项目复杂度
 */
export function analyzeProjectComplexity(project) {
 ...
 * 专门用于导出调度参数选择。
 */
export function calculateExportComplexityScore(metrics) {
    let score = 0;
    // 基础分数：片段数量
    ...
    // 复杂特效加分
    if (metrics.hasTemporalInterpolation)
        score += 10;
    if (metrics.hasMasks)
        score += 5;
    if (metrics.hasComplexEffects)
        score += 5;
    // 转场分数
 ...
 * 根据复杂度选择最优 preset
 */
export function selectOptimalPreset(complexityScore, qualityTarget, hardwareAcceleration) {
    // 硬件加速时使用固定 preset
 ...
 * 计算最优线程数
 */
export function calculateOptimalThreads(complexityScore, hardwareConcurrency, availableMemoryMb, resourceEstimate, maxThreads) {
 ...
 * 计算最优 CRF/CQ 值
 */
export function calculateOptimalCrf(complexityScore, qualityTarget) {
 ...
 * 估算编码速度倍数
 */
export function estimateSpeedMultiplier(preset, threads, complexityScore) {
 ...
 * 估算输出文件大小（MB）
 */
export function estimateFileSizeMb(settings, durationSeconds, crf) {
    const width = settings.width || 1920;
    const height = settings.height || 1080;
    const fps = settings.fps || 30;
    // 基础比特率估算（基于 CRF）
 ...
 * 智能导出调度主函数
 */
export function scheduleExport(plan, project, config) {
    const reasons = [];
    // 1. 分析项目复杂度
    ...
    // 4. 计算最优线程数
    const threads = calculateOptimalThreads(complexityScore, config.hardwareConcurrency, config.availableMemoryMb, resourceEstimate, config.maxThreads);
    ...
    // 6. 硬件加速决策
    const shouldUseHardwareAcceleration = Boolean(config.hardwareAccelerationEnabled && config.hardwareEncoderId && resourceEstimate.memoryClass !== 'heavy');
 ...
 * 将调度决策应用到导出计划
 */
export function applySchedulerDecision(plan, decision) {
 ...
 * 替换或添加命令行参数
 */
function replaceOrAddArg(args, key, value) {
 ...
 * 获取推荐的导出配置
 */
export function getRecommendedExportConfig(project, hardwareConcurrency = navigator.hardwareConcurrency || 4, availableMemoryMb = 4096) {
    const metrics = analyzeProjectComplexity(project);
    const complexityScore = calculateExportComplexityScore(metrics);
    // 根据复杂度推荐质量目标
    let qualityTarget;
...
//# sourceMappingURL=export-scheduler.js.map