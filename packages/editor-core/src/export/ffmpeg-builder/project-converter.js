import { DEFAULT_SUBTITLE_MODE, normalizeColorCorrection, normalizeMasterVolume, normalizeSubtitleLanguage, normalizeSubtitleLanguageList, normalizeTrackCompressor, normalizeTrackEQ, normalizeTransitionDuration, normalizeTransitionType, getProjectPrimaryTimeline, getProjectSequences, normalizeChromaKey, normalizeAudioFadeCurve, normalizeAudioFadeDuration, normalizeAudioChannelRouting, normalizeAudioDenoise, normalizeAILocalDenoise, normalizeClipBorder, normalizeClipPanoramaView, normalizeClipProjection, normalizeAudioPitchSemitones, normalizeFrameInterpolation, normalizeQualityEnhancement, normalizeSequenceFrameRate, normalizeSlowMotionMode, normalizeStabilization, normalizeTextPath, normalizeTransform, normalizeMasks, normalizeVideoRestoration, normalizeMediaColorProfile, } from '../../model';
...
import { getClipSourceVisibleDuration, getClipSpeed, getTimelinePlaybackDuration, getTrackPan, getTrackVolume, } from '../../timeline';
import { round } from '../../time';
import { normalizeMotionGraphic } from '../../motion-graphics';
import { normalizeDataSubtitleSource } from '../../data-subtitle';
import { DEFAULT_EXPORT_COLOR_MANAGEMENT, isDefaultExportColorManagement, normalizeExportColorManagement, normalizeProjectWorkingColorSpace, } from '../../color-management';
...
export function buildExportProjectFromProject(project, options) {
    ...
    const defaultColorManagement = colorPipeline === 'sdr-srgb'
    ...
    const colorManagement = options.settings?.colorManagement && !isDefaultExportColorManagement(options.settings.colorManagement)
...
export function buildExportTimeline(timeline, mediaById, options, mixerState) {
    return {
            ...
            return {
                ...
                clips: track.clips.map((clip) => {
                    ...
                    return {
                        ...
                        imageSequence: clip.type === 'image' && media?.imageSequence
                            ? {
                                frameRate: normalizeSequenceFrameRate(clip.sequenceFrameRate ?? media.imageSequence.frameRate) ??
                        ...
                        subtitleStyle: clip.type === 'subtitle'
                            ? {
                                text: clip.text,
                                fontSize: clip.style.fontSize,
                                fontColor: clip.style.color,
                        ...
                        creditsStyle: clip.type === 'credits'
                            ? {
                                text: clip.text,
                                rows: clip.rows,
                        ...
                        privacyRedactions: 'privacyRedactions' in clip && Array.isArray(clip.privacyRedactions)
                            ? clip.privacyRedactions.filter((r) => r && r.enabled !== false && Array.isArray(r.keyframes) && r.keyframes.length > 0)
                            : [],
                    };
                }),
            };
...
export function buildExportClipKeyframes(keyframes, duration, trackVolume) {
...
/** @internal */
export function nestedInputPlaceholder(sequenceId) {
