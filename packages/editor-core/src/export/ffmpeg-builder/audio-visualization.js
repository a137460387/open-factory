import { normalizeAudioSpectrumParams } from '../../effects';
...
export function collectAudioSpectrumEffects(clips) {
...
export function buildAudioSpectrumFilter(inputLabel, outputLabel, params, settings) {
...
export function buildAudioSpectrumOverlayYExpression(params) {
    return params.position === 'top' ? '0' : 'main_h-overlay_h';
}
export function buildAudioVisualizationBackgroundFilters(background, settings, duration, imageInputIndex) {
    const width = Math.max(2, Math.round(settings.width));
    ...
    if (background.type === 'gradient') {
        ...
        return [
            `color=c=${cssColorToFfmpeg(background.color)}:s=${width}x${height}:r=${fps}:d=${formatFfmpegSeconds(duration)},format=rgba,geq=r='${buildGradientChannelExpression(start.r, end.r)}':g='${buildGradientChannelExpression(start.g, end.g)}':b='${buildGradientChannelExpression(start.b, end.b)}':a='255'[base0]`,
        ];
    }
...
export function buildAudioVisualizationFilter(inputLabel, outputLabel, visualization, settings) {
    const width = Math.max(2, Math.round(settings.width));
    const height = Math.max(2, Math.round(settings.height));
...
export function buildAudioSpectrumVisualFilter(input) {
    ...
    if (input.mirror) {
        ...
        filters.push(`[${decoratedLabel}]split=2[${normalLabel}][${flipSourceLabel}]`, `[${flipSourceLabel}]vflip[${flippedLabel}]`, `[${normalLabel}][${flippedLabel}]overlay=x=0:y=0:format=auto[${input.outputLabel}]`);
    }
...
export function hasAudioVisualizationThemeDecorations(theme) {
    return Boolean(theme && ((theme.glow && theme.glowStrength > 0) || theme.particles || (theme.border && theme.borderWidth > 0)));
}
export function appendAudioVisualizationThemeDecorationFilters(filters, inputLabel, outputLabel, theme) {
    ...
    if (theme.glow && theme.glowStrength > 0) {
        ...
        filters.push(`[${currentLabel}]split=2[${baseLabel}][${glowSourceLabel}]`, `[${glowSourceLabel}]gblur=sigma=${formatFfmpegNumber(2 + theme.glowStrength * 8)},colorchannelmixer=${buildColorChannelMixerForHex(theme.glowColor)}:aa=${formatOpacity(Math.min(0.9, 0.25 + theme.glowStrength * 0.65))}[${glowLabel}]`, `[${glowLabel}][${baseLabel}]overlay=format=auto[${combinedLabel}]`);
        currentLabel = combinedLabel;
    }
    if (theme.particles) {
        const particleLabel = `${outputLabel}_particles`;
        filters.push(`[${currentLabel}]noise=alls=8:allf=t+u,colorchannelmixer=${buildColorChannelMixerForHex(theme.particleColor)}[${particleLabel}]`);
        currentLabel = particleLabel;
    }
    if (theme.border && theme.borderWidth > 0) {
        const borderLabel = `${outputLabel}_border`;
        filters.push(`[${currentLabel}]drawbox=x=0:y=0:w=iw:h=ih:color=${cssColorToFfmpeg(theme.borderColor)}@0.85:t=${Math.max(1, Math.round(theme.borderWidth))}[${borderLabel}]`);
        currentLabel = borderLabel;
    }
    return currentLabel;
}
export function buildAudioSpectrumGradientFilters(inputLabel, outputLabel, colorStart, colorEnd) {
...
export function buildColorChannelMixerForHex(color) {
...
export function buildCircularAlphaMaskFilter() {
    return "geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':a='alpha(X,Y)*if(lte((X-W/2)*(X-W/2)+(Y-H/2)*(Y-H/2),(min(W,H)/2)*(min(W,H)/2)),1,0)'";
}
export function buildAudioVisualizationOverlayPosition(style, _settings) {
...
export function buildGradientChannelExpression(start, end) {
    if (start === end) {
        return String(start);
    }
...
export function resolveExportAudioVisualizationTheme(visualization) {
...
//# sourceMappingURL=audio-visualization.js.map