...
import { EXPORT_PREVIEW_SAMPLE_KINDS, SLATE_DURATION_SECONDS, buildExportRangeOutputArgs, buildFfmpegFullArgs, buildGifExportPasses, buildLoudnessNormalizationPasses, buildSlateVideoFilters, buildTimecodeBurnInFilter, buildWatermarkFilters, normalizeExportAudioVisualization, normalizeExportSlate, normalizeExportWatermark, normalizeSettingsForExportFormat, normalizeTimecodeBurnIn, } from './settings-normalize';
import { buildAdjustmentLayerFilters, buildMediaCompositeFilter, buildPlaybackStartByClipId, buildVisualItems, hasSphericalVideoClips, } from './visual-filters';
import { buildCreditsRollFilter, buildCustomShaderSequenceArtifact, buildCustomShaderSequenceClip, buildCustomShaderSequenceInputArgs, buildImageSequenceArtifact, buildInputArgs, buildMotionGraphicSequenceArtifact, buildPathTextSequenceArtifact, buildPathTextSequenceOverlayFilter, buildSoftSubtitleCodec, buildSubtitleArtifact, buildSubtitleBurnInFilter, buildSubtitleInputArgs, buildSubtitleLanguageGroups, buildTextFilter, normalizeSubtitleFormat, pngSequenceOutputPath, resolveAudioVisualizationBackground, selectSubtitleBurnInGroup, subtitleLanguageToFfmpegMetadata, } from './text-subtitle-filters';
import { buildAudioSpectrumFilter, buildAudioSpectrumOverlayYExpression, buildAudioVisualizationBackgroundFilters, buildAudioVisualizationFilter, buildAudioVisualizationOverlayPosition, collectAudioSpectrumEffects, } from './audio-visualization';
...
import { buildBitrateArgs, buildContainerArgs, buildExportColorManagementFilters, buildExportColorMetadataArgs, buildExportContainerMetadataArgs, buildVideoEncodingArgs, formatVolume, nestedInputPlaceholder, } from './utils';
export function buildFfmpegExportPlan(project, capabilities, depth = 0, sequenceStack = [], options = {}) {
    ...
    if (requestedTimecodeBurnIn?.enabled && !drawtextAvailable) {
        warnings.push(capabilities?.drawtextWarning ??
    ...
    const inputs = [];
    const visualInputByClipId = new Map();
    const audioInputByClipId = new Map();
    const customShaderSequenceClips = new Map();
    const pathTextSequenceInputByClipId = new Map();
    const filters = [];
    const textArtifacts = [];
    ...
    let imageWatermarkInputIndex;
    let audioVisualizationBackgroundImageInputIndex;
    ...
    if (!audioOnly) {
        ...
        if (!audioVisualization) {
            const visualItems = buildVisualItems(project.timeline, orderedPlaybackClips, playbackStartByClipId, renderableTrackIndexes, visualInputByClipId, customShaderSequenceClips, settings, filters, warnings, textArtifacts, capabilities);
    ...
    if (!videoFramesOnly && subtitleClips.length > 0 && settings.exportSidecarSubtitle) {
        for (const group of selectedSubtitleGroups) {
            textArtifacts.push(buildSubtitleArtifact(group.clips, 'sidecar', subtitleFormat, {
                language: group.language,
                includeLanguageInFileName: multipleSubtitleLanguages,
            }));
        }
    }
    let loudnessAnalysisFilterComplex;
    ...
    if (!videoFramesOnly) {
        ...
        if (audioLabels.length === 0) {
            ...
            audioFilters.push(`${audioLabels.map((label) => `[${label}]`).join('')}amix=inputs=${audioLabels.length}:duration=longest:normalize=0,atrim=duration=${formatFfmpegSeconds(duration)},asetpts=PTS-STARTPTS,aresample=${settings.sampleRate},volume=${formatVolume(masterVolume)}[${mixedAudioLabel}]`);
        }
        ...
        if (loudnessPreset) {
            ...
            if (needsAudioSplit) {
                filters.push(...audioFilters, `[${masterOutputLabel}]asplit=${audioSplitLabels.length + 1}[${finalAudioLabel}]${audioSplitLabels.map((label) => `[${label}]`).join('')}`, `[${finalAudioLabel}]${buildLoudnormRenderFilter(loudnessPreset)}[aout]`);
            }
            else {
    ...
    if (!audioOnly && !videoFramesOnly && audioSpectrumEffects.length > 0) {
        audioSpectrumEffects.forEach((item, index) => {
            ...
            const nextVideo = `base${videoStep + 1}`;
            filters.push(`[${currentVideo}][${spectrumLabel}]overlay=x=0:y='${buildAudioSpectrumOverlayYExpression(item.params)}':eval=frame:enable='between(t,${formatFfmpegSeconds(item.start)},${formatFfmpegSeconds(item.start + item.duration)})'[${nextVideo}]`);
            currentVideo = nextVideo;
            videoStep += 1;
        });
    }
    ...
    if (!audioOnly) {
        ...
        filters.push(`[${currentVideo}]trim=duration=${formatFfmpegSeconds(outputDuration)},setpts=PTS-STARTPTS,fps=${settings.fps}${colorManagementFilters.length > 0 ? `,${colorManagementFilters.join(',')}` : ''},format=${outputPixelFormat}[vout]`);
    }
    ...
    const outputArgs = frameExportTime !== null
        ? [
            '-ss',
    ...
    const gifPlan = gifExport && frameExportTime === null
...
export function buildFfmpegCurrentFrameExportPlan(project, time, capabilities) {
    return buildFfmpegExportPlan(project, capabilities, 0, [], { frameExport: { time } });
}
export function buildStemExportPlans(project, capabilities, stemTracks, outputDir) {
...
/** @internal */
export function sanitizeStemPathComponent(name) {
    return name
        .replace(/[<>:"/\\|?*() ]/g, '_')
        .replace(/_+/g, '_')
        .replace(/^_|_$/g, '')
...
/** @internal */
export function buildStemOutputPath(outputDir, projectName, stemName, trackIndex, format) {
...
export function calculateExportPreviewSampleTimes(duration) {
...
export function buildFfmpegPreviewSamplePlans(project, outputPaths, capabilities) {
...
export function buildNestedSequencePlans(project, capabilities, warnings, depth, sequenceStack) {
    const sequenceIds = new Set(project.timeline.tracks.flatMap((track) => track.clips.flatMap((clip) => clip.type === 'nested-sequence' && clip.nestedSequenceId ? [clip.nestedSequenceId] : [])));
    const nestedPlans = [];
