import { buildExportColorTagArgs, buildIccMetadataArgs, buildZscaleColorConversionFilter, getFfmpegColorSpaceProfile, normalizeExportColorManagement, normalizeProjectWorkingColorSpace, } from '../../color-management';
...
export function buildBitrateArgs(flag, bitrate) {
    const value = bitrate?.trim();
    return value ? [flag, value] : [];
}
export function buildVideoEncodingArgs(settings, capabilities, warnings, skipVideoCodec) {
    if (skipVideoCodec) {
        return [];
    }
...
export function buildVideoProfileArgs(settings) {
...
export function buildContainerArgs(settings) {
    const format = settings.format.toLowerCase();
...
export function buildExportColorMetadataArgs(settings) {
...
export function buildExportContainerMetadataArgs(metadata) {
    if (!metadata) {
        return [];
    }
    const entries = [
...
export function buildExportColorManagementFilters(settings) {
    const colorManagement = normalizeExportColorManagement(settings.colorManagement);
    ...
    const filters = [...buildAcesOdtFilterChain(colorPipeline, colorManagement.outputColorSpace)];
    if (colorManagement.inputColorSpace !== colorManagement.outputColorSpace) {
        filters.push(`colorspace=ispace=${input.space}:iprimaries=${input.primaries}:itrc=${input.trc}:space=${output.space}:primaries=${output.primaries}:trc=${output.trc}`);
    }
...
export function shouldGenerateIccProfile(settings) {
    const colorManagement = normalizeExportColorManagement(settings.colorManagement);
    return (colorManagement.embedIccProfile &&
...
export function buildTimelineExpression(frames, clipStart, fallback, variable = 't') {
    if (frames.length < 2) {
        return formatFfmpegNumber(frames[0]?.value ?? fallback);
    }
...
export function formatAtempo(value) {
    const fixed = value.toFixed(3).replace(/0+$/, '').replace(/\.$/, '');
    return fixed.includes('.') ? fixed : `${fixed}.0`;
}
export function formatPitchRatio(semitones) {
...
export function formatCompressorLinear(db) {
...
export function formatOffsetExpression(value) {
    const formatted = formatSigned(value);
    return value < 0 ? formatted : `+${formatted}`;
}
export function cssColorToAssColor(value, opacity) {
...
export function buildHardwareEncoderArgs(settings, fps, capabilities, warnings) {
    const args = [];
