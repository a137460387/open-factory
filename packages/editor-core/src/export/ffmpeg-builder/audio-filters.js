...
import { buildNoiseReductionFilterString, normalizeNoiseReductionParams, } from '../../audio/noise-reduction';
...
import { formatFfmpegNumber, formatVolume, formatPan, formatCompressorLinear, formatAtempo, formatPitchRatio, } from './utils';
import { LOUDNORM_MEASURED_I_PLACEHOLDER, LOUDNORM_MEASURED_TP_PLACEHOLDER, LOUDNORM_MEASURED_LRA_PLACEHOLDER, LOUDNORM_MEASURED_THRESH_PLACEHOLDER, LOUDNORM_OFFSET_PLACEHOLDER, normalizeExportMasterProcessing, } from './settings-normalize';
/** @internal */
export function getAnimatedFrames(clip, property) {
...
/** @internal */
export function buildLocalExpression(frames, fallback, variable = 't') {
    if (frames.length < 2) {
...
/** @internal */
export function buildSegmentExpression(left, right, variable) {
...
/** @internal */
export function buildEasingExpression(progress, easing) {
    if (easing === 'ease-in') {
...
/** @internal */
export function buildBounceEasingExpression(progress) {
    const n1 = '7.5625';
    const d1 = '2.75';
 ...
 * 构建音频效果链的 FFmpeg 滤镜
 */
export function buildAudioEffectChainFilters(effects) {
    if (effects.length === 0)
        return [];
 ...
 * 构建混音器通道的完整音频滤镜链
 */
export function buildMixerChannelAudioFilters(channelVolume, channelPan, effects) {
    const filters = [];
    // 音量
    if (channelVolume !== 0) {
...
// ---------------------------------------------------------------------------
export function buildAudioFilters(clips, inputByClipId, settings, filters, capabilities, warnings) {
    const labels = [];
    for (const clip of clips.filter((item) => item.type === 'audio' || ((item.type === 'video' || item.type === 'nested-sequence') && item.hasEmbeddedAudio))) {
        ...
        filters.push(`[${inputIndex}:a:0]atrim=start=0:duration=${formatFfmpegSeconds(getExportClipSourceDuration(clip))},asetpts=PTS-STARTPTS${pitchAndReverseFilters.length > 0 ? `,${pitchAndReverseFilters.join(',')}` : ''}${speedFilters.length > 0 ? `,${speedFilters.join(',')}` : ''}${fadeFilters}${restorationFilters}${denoiseFilters}${trackProcessingFilters}${effectsChainFilters.length > 0 ? `,${effectsChainFilters.join(',')}` : ''},adelay=${delay}:all=1,${buildVolumeFilter(clip)}${buildAudioChannelRoutingFilter(clip)}${buildPanFilter(clip)}${buildSpatialAudioFilter(clip, settings)}${automationFilters},aformat=channel_layouts=stereo,aresample=${settings.sampleRate}[${label}]`);
        labels.push(label);
    }
    return labels;
}
export function buildPitchAndReverseAudioFilters(clip, sampleRate) {
    const filters = [];
...
export function getLoudnessNormalizationPreset(mode) {
    if (mode === 'youtube') {
...
export function buildLoudnormAnalysisFilter(preset) {
    return `loudnorm=${[...preset.args, 'print_format=json'].join(':')}`;
}
export function buildLoudnormRenderFilter(preset) {
    return `loudnorm=${[
        ...preset.args,
...
export function buildAudioDenoiseFilters(clip, capabilities, warnings) {
 ...
 * @internal
 */
export function buildAfftdnNoiseReductionFilter(params) {
 ...
 * @internal
 */
export function buildMixerChannelNoiseReductionFilter(effects) {
...
export function buildAudioRestorationFilters(clip) {
...
export function buildPanFilter(clip) {
    if (Math.abs(clip.pan) < 0.001) {
        return '';
    }
    return `,stereopan=pan=${formatPan(clip.pan)}`;
}
export function buildAutomationFilters(clip) {
...
export function buildSpatialAudioFilter(clip, settings) {
    ...
    const parts = [];
    if (xFrames.length >= 2) {
        parts.push(`pan=stereo|c0='${buildSpatialPanGainExpression(xFrames, spatial.x, 'left')}'*c0|c1='${buildSpatialPanGainExpression(xFrames, spatial.x, 'right')}'*c1`);
    }
    else {
...
export function escapeSofalizerArg(arg) {
...
export function escapeFilterFileValue(value) {
    return normalizeFfmpegPath(value).replace(/:/g, '\\:').replace(/'/g, "\\'");
}
export function buildSpatialPanGainExpression(frames, fallbackX, channel) {
...
export function buildSpatialVolumeExpression(frames, spatial) {
    const mapped = frames.map((frame) => ({
        ...frame,
...
export function buildAudioChannelRoutingFilter(clip) {
...
export function buildTrackAudioFilters(clip) {
    const filters = [];
    if (clip.eq.enabled) {
        filters.push(...buildEqualizerFilters(clip.eq));
    }
    if (clip.compressor.enabled) {
        filters.push(`acompressor=threshold=${formatCompressorLinear(clip.compressor.threshold)}:ratio=${formatFfmpegNumber(clip.compressor.ratio)}:attack=${formatFfmpegNumber(clip.compressor.attack)}:release=${formatFfmpegNumber(clip.compressor.release)}:makeup=${formatCompressorLinear(clip.compressor.makeupGain)}`);
    }
    return filters.length > 0 ? `,${filters.join(',')}` : '';
}
...
export function buildAudioFadeFilters(clip) {
    const filters = [];
...
export function formatAudioFadeCurve(curve) {
    if (curve === 'ease-in') {
        return ':curve=qsin';
    }
    if (curve === 'ease-out') {
...
export function buildVolumeFilter(clip) {
...
export function buildAtempoFilters(speed) {
...
//# sourceMappingURL=audio-filters.js.map