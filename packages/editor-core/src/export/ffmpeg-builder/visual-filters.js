...
import { isDefaultColorCurves, isNeutralThreeWayColor, normalizeThreeWayColor, serializeColorCurvesToCube, PrimaryWheels, PrimarySliders, toFfmpegSelectiveColor } from '../../color-grading';
...
import { getFfmpegBlendMode, normalizeClipBlendMode } from '../../blend-modes';
...
export function buildVisualItems(timeline, orderedPlaybackClips, playbackStartByClipId, renderableTrackIndexes, inputByClipId, customShaderSequenceClips, settings, filters, warnings, textArtifacts, capabilities) {
    ...
    for (const clip of orderedPlaybackClips.filter((item) => item.type === 'video' ||
        item.type === 'image' ||
        item.type === 'text' ||
        item.type === 'credits' ||
...
export function buildPlaybackStartByClipId(timeline) {
...
export function findExportTransitionPair(timeline, transition) {
    for (const track of timeline.tracks) {
        const clips = [...track.clips].sort((left, right) => left.start - right.start || left.id.localeCompare(right.id));
...
export function buildTransitionClipFilter(inputIndex, clip, label, settings, textArtifacts, warnings, capabilities) {
    const sourceDuration = getExportClipSourceDuration(clip);
    const trim = clip.type === 'video' || clip.type === 'nested-sequence'
...
export function isTransitionVisualClip(clip) {
    return clip.type === 'video' || clip.type === 'image' || clip.type === 'nested-sequence';
}
export function areExportClipsAdjacent(fromClip, toClip) {
    return Math.abs(fromClip.start + fromClip.duration - toClip.start) <= 0.001;
}
export function clampExportTransitionDuration(transition, fromClip, toClip) {
...
export function buildSmartTransitionFilters(transition, label, duration, offset, settings) {
...
export function mapTransitionType(type) {
    switch (type) {
        case 'fade-black':
        case 'flash-black':
            return 'fadeblack';
...
export function buildShapeWipeGeqExpression(type) {
    if (type === 'shape-star') {
...
export function buildTransitionPreviewArgs(type, options = {}) {
    ...
    const offset = '0';
    const baseFilter = type === 'shape-heart' || type === 'shape-star'
...
export function visualKindOrder(item) {
    if (item.kind === 'media') {
        return 0;
    }
    return item.kind === 'adjustment' ? 1 : 2;
}
export function buildMediaCompositeFilter(currentVideo, nextVideo, item, settings, duration) {
...
export function buildAdjustmentLayerFilters(inputLabel, outputLabel, clip, textArtifacts, settings) {
...
export function buildVisualClipFilter(inputIndex, clip, label, settings, textArtifacts, warnings, capabilities) {
    const sourceDuration = getExportClipSourceDuration(clip);
    const trim = clip.type === 'video' || clip.type === 'nested-sequence'
        ? `trim=start=0:duration=${formatFfmpegSeconds(sourceDuration)}`
        : `trim=duration=${formatFfmpegSeconds(sourceDuration)}`;
    ...
    if (redactionExprs.length > 0)
        filters.push(...redactionExprs);
    return filters.join(',');
}
export function buildColorNodeGraphVisualFilter(inputIndex, clip, label, settings, textArtifacts, warnings, capabilities, trim) {
...
export function buildColorGradingGraphVisualFilter(inputIndex, clip, label, settings, textArtifacts, warnings, capabilities, trim) {
...
export function buildVisualPreColorFilters(clip, settings, warnings, capabilities) {
    const filters = [];
    ...
    if (settings.scaleMode === 'fit' && !isReframeEnabled(settings.targetAspectRatio)) {
        filters.push(`scale=${settings.width}:${settings.height}:force_original_aspect_ratio=decrease`, `pad=${settings.width}:${settings.height}:(ow-iw)/2:(oh-ih)/2:color=black`);
    }
...
export function buildVisualPostColorFilters(clip, settings, textArtifacts, label, includeColorCorrection = true) {
    const filters = [];
...
export function buildVisualPostKeyFilters(clip, settings, textArtifacts, warnings, capabilities, label) {
    const filters = [];
    if (isKenBurnsAnimatedScaleClip(clip)) {
        filters.push(buildSetptsFilter(clip, false, warnings), buildKenBurnsZoompanFilter(clip, settings), 'setsar=1', buildSetptsFilter(clip, true, warnings));
    }
...
export function buildPanoramaProjectionFilters(clip) {
...
export function hasSphericalVideoClips(clips) {
    return clips.some((clip) => (clip.type === 'video' || clip.type === 'nested-sequence') && clip.projection !== 'flat');
}
export function buildDifferenceMatteClipFilter(inputIndex, clip, label, settings, textArtifacts, warnings, capabilities, trim, key) {
    ...
    return [
        ...
        `[${referenceSourceLabel}]trim=start=${referenceTime}:duration=${formatFfmpegSeconds(frameDuration)},setpts=PTS-STARTPTS,loop=loop=-1:size=1:start=0,${buildVisualPostKeyFilters(clip, settings, textArtifacts, warnings, capabilities, referenceLabel).join(',')}`,
...
export function buildPrivacyBlurClipFilter(inputIndex, clip, label, settings, textArtifacts, warnings, capabilities, trim) {
...
export function buildPrivacyBlurMaskGraph(inputLabel, outputLabel, mask, index) {
...
export function buildPrivacyBlurEffectFilter(mask) {
...
export function buildMaskTimelineExpression(mask, property) {
...
export function buildChromaKeyFilters(clip) {
    const key = normalizeChromaKey(clip.chromaKey);
    if (!key.enabled) {
        return [];
    }
    ...
    const filters = key.colors.map((color) => `chromakey=color=0x${formatChromaKeyColor(color)}:similarity=${formatFfmpegNumber(key.similarity)}:blend=${formatFfmpegNumber(key.blend)}`);
...
export function isDifferenceMatteEnabled(key) {
    return key.enabled && key.mode === 'difference-matte';
}
export function formatChromaKeyColor(color) {
    return color
        .map((channel) => Math.round(channel).toString(16).padStart(2, '0'))
        .join('')
        .toUpperCase();
}
export function buildStabilizationFilters(clip) {
    if (!isStabilizationExportable(clip.stabilization)) {
        return [];
    }
    const trfPath = clip.stabilization.trfPath ?? '';
    return [
        `vidstabtransform=smoothing=${formatFfmpegNumber(clip.stabilization.smoothing)}:zoom=${formatFfmpegNumber(clip.stabilization.zoom)}:input=${escapeDrawtextValue(trfPath)}`,
    ];
}
export function buildSlowMotionFilters(clip, settings, capabilities, warnings) {
...
export function buildFrameInterpolationFilters(clip, capabilities, warnings) {
...
export function buildFrameInterpolationFilterArg(fps, mode, sceneProtected) {
...
export function buildVideoRestorationFilters(clip) {
    if (clip.type !== 'video' && clip.type !== 'nested-sequence') {
        return [];
    }
    ...
    if (restoration.temporalDenoise.preset !== 'off') {
        filters.push(`hqdn3d=luma_spatial=${formatFfmpegNumber(restoration.temporalDenoise.lumaSpatial)}:chroma_spatial=${formatFfmpegNumber(restoration.temporalDenoise.chromaSpatial)}:luma_tmp=${formatFfmpegNumber(restoration.temporalDenoise.lumaTmp)}`);
    }
    if (restoration.spatialDenoise.enabled) {
        filters.push(`nlmeans=s=${formatFfmpegNumber(restoration.spatialDenoise.strength)}:p=${Math.round(restoration.spatialDenoise.patchSize)}:r=${Math.round(restoration.spatialDenoise.researchSize)}`);
    }
    return filters;
}
export function buildQualityEnhancementFilters(clip) {
    if (clip.type !== 'video' && clip.type !== 'nested-sequence') {
        return [];
    }
...
export function getMinimumClipSpeed(clip) {
...
export function buildMaskFilters(clip) {
...
export function buildClipBorderFilters(clip) {
...
export function isSimpleRectMask(mask) {
    return mask.type === 'rect' && !mask.inverted && mask.feather <= 0.001;
}
export function buildSimpleRectMaskFilter(mask) {
...
export function buildGeqMaskFilter(masks) {
    const expression = masks.map((mask) => {
        const inside = mask.type === 'path'
...
export function buildRectMaskExpression(mask) {
...
export function buildEllipseMaskExpression(mask) {
...
export function buildPathMaskExpression(mask) {
...
export function getPathVertex(vertices, index) {
    return {
        x: vertices[index * 2] ?? 0,
        y: vertices[index * 2 + 1] ?? 0,
    };
}
export function buildPathTriangleExpression(a, b, c) {
...
export function buildPathEdgeExpression(from, to, comparator) {
...
export function buildSetptsFilter(clip, includeStartOffset, warnings) {
...
export function buildStaticSetptsFilter(clip, includeStartOffset, speed) {
...
export function buildSpeedRampSetptsExpression(clip, includeStartOffset) {
...
export function buildSpeedRampSegments(clip) {
...
export function buildKenBurnsZoompanFilter(clip, settings) {
    const scaleX = getAnimatedFrames(clip, 'scaleX');
    const scaleY = getAnimatedFrames(clip, 'scaleY');
...
export function buildOpacityFilters(clip, label) {
...
export function buildOverlayXExpression(clip) {
...
export function buildOverlayYExpression(clip) {
...
export function buildColorCorrectionFilters(clip, textArtifacts) {
    ...
    for (const layer of lutLayers) {
        ...
        if (Math.abs(layer.intensity - 1) < 0.001) {
            ...
            filters.push(`split[lut${idx}a][lut${idx}b]`, `[lut${idx}b]lut3d=file=${escapeDrawtextValue(layer.path)}[lut${idx}c]`, `[lut${idx}a][lut${idx}c]blend=all_expr='A*(1-${intensity})+B*${intensity}'`);
        }
    }
    const hasBasicCorrection = colorCorrection.brightness !== DEFAULT_COLOR_CORRECTION.brightness ||
    ...
    if (hasBasicCorrection) {
        filters.push(`eq=brightness=${formatFfmpegNumber(colorCorrection.brightness)}:contrast=${formatFfmpegNumber(colorCorrection.contrast)}:saturation=${formatFfmpegNumber(colorCorrection.saturation)}`);
    }
...
export function buildThreeWayColorFilter(value) {
    ...
    return `colorbalance=${params.map(([name, value]) => `${name}=${formatFfmpegNumber(value)}`).join(':')}`;
}
export function colorBalanceValue(value, channel) {
    return Math.min(1, Math.max(-1, value[channel] + value.intensity - 1));
}
export function buildEffectFilters(effects, fps = 30) {
 ...
 * 构建调色节点图的 FFmpeg 滤镜链
 */
export function buildColorGradingFilters(graph) {
    ...
    for (const node of wheelNodes) {
        const filter = PrimaryWheels.toFfmpegFilter(node.params);
        if (filter)
            filters.push(filter);
    }
    // 一级滑块
    for (const node of sliderNodes) {
        const filter = PrimarySliders.toFfmpegFilter(node.params);
        if (filter)
            filters.push(filter);
    }
    // 曲线
    ...
    for (const node of hslNodes) {
        const hslFilter = toFfmpegSelectiveColor(node.params);
        if (hslFilter)
            filters.push(hslFilter);
    }
    // 窗口遮罩（geq 滤镜实现区域遮罩）
    for (const node of windowMaskNodes) {
        const maskFilter = buildWindowMaskFfmpegFilter(node.params);
        if (maskFilter)
 ...
 */
export function buildWindowMaskFfmpegFilter(params) {
...
export function buildSourceColorSpaceConversionFilters(clip, settings) {
...
export function getExportClipSourceDuration(clip) {
...
//# sourceMappingURL=visual-filters.js.map