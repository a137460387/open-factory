import { normalizeSubtitleLanguage, normalizeTextPath } from '../../model';
...
import { serializeSubtitleCueInputsToAss, serializeSubtitleCueInputsToSrt, serializeSubtitleCueInputsToVtt } from '../../subtitles/srt';
...
// Input builders
// ---------------------------------------------------------------------------
export function buildInputArgs(clip) {
...
export function buildCustomShaderSequenceInputArgs(settings) {
    return ['-f', 'image2', '-framerate', String(settings.fps), '-start_number', '1'];
}
export function buildCustomShaderSequenceClip(clip) {
    return {
        ...clip,
        trimStart: 0,
        trimEnd: 0,
...
export function buildPathTextSequenceArtifact(clip, settings) {
...
export function buildMotionGraphicSequenceArtifact(clip, settings) {
...
export function buildCustomShaderSequenceArtifact(clip, settings) {
...
export function buildImageSequenceArtifact(clip) {
    const safeId = safeLabel(clip.id);
...
// ---------------------------------------------------------------------------
export function pngSequenceOutputPath(outputPath) {
...
export function formatSequenceFrameDuration(value) {
...
// ---------------------------------------------------------------------------
export function buildTextFilter(inputLabel, outputLabel, clip, settings) {
    const safeId = safeLabel(clip.id);
    ...
    return {
        ...
        filter: [
            ...
            `[${textSourceLabel}]drawtext=textfile=${placeholder}${fontPath}${openType}:fontsize=${fontSize}:fontcolor=${fontColor}:x='${x}':y='${y}':alpha=1:box=1:boxcolor=${backgroundColor}@${backgroundOpacity}:boxborderw=${Math.max(0, Math.round((style?.fontSize ?? 48) * 0.25))}:enable='between(t,${formatFfmpegSeconds(clip.start)},${formatFfmpegSeconds(clip.start + clip.duration)})'[${textDrawLabel}]`,
...
export function buildAdvancedTextFilter(inputLabel, outputLabel, clip, settings, style, textSourceLabel, textLayerLabel) {
    const safeId = safeLabel(clip.id);
    ...
    segments.forEach((segment, index) => {
        ...
        filters.push(`[${previousLabel}]drawtext=textfile=${placeholder}${fontPath}${openType}:fontsize=${fontSize}:fontcolor=${cssColorToFfmpeg(segment.style.color)}:x='${x}':y='${y}':alpha=1:box=1:boxcolor=${backgroundColor}@${backgroundOpacity}:boxborderw=${Math.max(0, Math.round(segment.style.fontSize * 0.25))}:enable='between(t,${formatFfmpegSeconds(clip.start)},${formatFfmpegSeconds(clip.start + clip.duration)})'[${nextLabel}]`);
        previousLabel = nextLabel;
    });
    const opacityFilters = buildOpacityFilters(clip, textLayerLabel);
    filters.push(`[${previousLabel}]${opacityFilters.join(',')}`);
    filters.push(`[${inputLabel}][${textLayerLabel}]overlay=x=0:y=0:eval=frame:enable='between(t,${formatFfmpegSeconds(clip.start)},${formatFfmpegSeconds(clip.start + clip.duration)})'[${outputLabel}]`);
    return { filter: filters.join(';'), artifacts };
}
export function shouldUseAdvancedTextFilters(style) {
    const richText = style.richText ?? undefined;
    const hasRichStructure = richText
        ? richText.paragraphs.length > 1 ||
            richText.paragraphs.some((paragraph) => paragraph.runs.length > 1 ||
                paragraph.runs.some((run) => run.bold !== undefined ||
    ...
    const hasCustomLayout = layout.fitMode !== defaultLayout.fitMode ||
...
export function buildOpenTypeDrawtextOptions(style) {
...
export function exportTextStyleToTextStyle(style) {
    return {
...
// Audio visualization
// ---------------------------------------------------------------------------
export function resolveAudioVisualizationBackground(visualization) {
...
// Credits roll
// ---------------------------------------------------------------------------
export function buildCreditsRollFilter(inputLabel, outputLabel, clip, settings) {
    const safeId = safeLabel(clip.id);
    ...
    return {
        ...
        filter: [
            ...
            `[${textSourceLabel}]drawtext=textfile=${placeholder}${fontPath}:fontsize=${fontSize}:fontcolor=${fontColor}:x='${x}':y='${y}':line_spacing=${lineSpacing}:alpha=1:enable='between(t,${formatFfmpegSeconds(clip.start)},${formatFfmpegSeconds(clip.start + clip.duration)})'[${textDrawLabel}]`,
            `[${textDrawLabel}]${opacityFilters.join(',')}`,
            `[${inputLabel}][${textLayerLabel}]overlay=x=0:y=0:eval=frame:enable='between(t,${formatFfmpegSeconds(clip.start)},${formatFfmpegSeconds(clip.start + clip.duration)})'[${outputLabel}]`,
        ].join(';'),
    };
...
// ---------------------------------------------------------------------------
export function buildPathTextSequenceOverlayFilter(inputLabel, outputLabel, inputIndex, clip) {
    const safeId = safeLabel(clip.id);
...
// ---------------------------------------------------------------------------
export function buildTextFontSizeExpression(clip, baseFontSize) {
...
export function buildDrawtextPositionExpression(clip, axis, staticValue) {
...
// ---------------------------------------------------------------------------
export function buildSubtitleBurnInFilter(inputLabel, outputLabel, clips, format, options = {}) {
...
export function buildSubtitleArtifact(clips, pathMode, format, options = {}) {
...
export function buildSubtitleLanguageGroups(timeline, clips, selectedLanguages) {
    if (clips.length === 0) {
        return [];
    }
...
export function selectSubtitleBurnInGroup(groups, language) {
    if (groups.length === 0) {
        return undefined;
    }
    if (!language) {
...
export function subtitleLanguageToFfmpegMetadata(language) {
    const normalized = normalizeSubtitleLanguage(language);
    const map = {
        ar: 'ara',
        de: 'deu',
...
export function buildSubtitleCueInputs(clips) {
...
export function buildSubtitleCueInput(clip, start, duration, text, id) {
    return {
        id,
        start,
        duration,
...
export function projectFrameRateFromClip(clip) {
    return clip.sequenceFrameRate ?? 30;
}
export function serializeSubtitleCueInputs(cues, format) {
    if (format === 'vtt') {
...
export function buildSubtitleInputArgs(format) {
    if (format === 'vtt') {
        return ['-f', 'webvtt'];
    }
    if (format === 'ass') {
...
export function buildSoftSubtitleCodec(format, settings) {
    if (format === 'ass' || format === 'ssa') {
        return 'ass';
    }
...
export function formatOpacity(value) {
...
export function cssColorToAssColor(hex, opacity) {
...
//# sourceMappingURL=text-subtitle-filters.js.map