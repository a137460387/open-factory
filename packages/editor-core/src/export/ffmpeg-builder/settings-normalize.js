...
export const EXPORT_PREVIEW_SAMPLE_KINDS = ['start', 'middle', 'end'];
export const DEFAULT_EXPORT_MASTER_EQ_BANDS = [
...
export const DEFAULT_EXPORT_MASTER_PROCESSING = {
    eq: {
        enabled: false,
...
// ---------------------------------------------------------------------------
export function normalizeLoudnessNormalization(mode) {
    return mode === 'youtube' || mode === 'ebu-r128' ? mode : 'off';
}
export function normalizeVideoProfile(profile) {
    return profile === 'baseline' || profile === 'main' || profile === 'high' ? profile : undefined;
}
export function normalizeExportAudioVisualization(input) {
    const defaultVisualization = DEFAULT_EXPORT_SETTINGS.audioVisualization;
    const style = input?.style === 'spectrum-bars' || input?.style === 'circular-spectrum' || input?.style === 'waveform-line'
        ? input.style
...
export function normalizeAudioVisualizationBackground(input, fallback) {
    ...
    if (input?.type === 'solid') {
        return {
            type: 'solid',
            color: normalizeHexColor(input.color, fallback.type === 'solid' || fallback.type === 'gradient' ? fallback.color : '#050816'),
        };
    }
    return fallback;
}
...
export function toHexChannel(value) {
    return Math.round(Math.min(255, Math.max(0, value)))
        .toString(16)
        .padStart(2, '0');
}
export function buildMasterAudioFilters(masterProcessing) {
...
export function buildEqualizerFilters(eq) {
    const filters = [];
...
export function formatFfmpegNumber(value) {
...
// ---------------------------------------------------------------------------
export function normalizeExportReframeSettings(settings) {
...
export function normalizeExportSpatialAudioAssets(input) {
    ...
    const roomImpulseResponses = input.roomImpulseResponses && typeof input.roomImpulseResponses === 'object'
        ? Object.fromEntries(Object.entries(input.roomImpulseResponses)
            .filter((entry) => ['small-room', 'hall', 'outdoor'].includes(entry[0]) &&
...
export function mergeExportMetadata(base, override) {
    if (!override) {
        return base;
    }
    return {
        ...(base ?? {}),
        ...Object.fromEntries(Object.entries(override).filter((entry) => typeof entry[1] === 'string' && entry[1].trim().length > 0)),
    };
}
export function normalizeExportMasterProcessing(input) {
...
export function hasExportMasterProcessing(input) {
    return buildMasterAudioFilters(normalizeExportMasterProcessing(input)).length > 0;
}
export function normalizeExportMasterEq(input) {
...
export function normalizeExportMasterEqBand(input, fallback) {
...
export function normalizeSettingsForExportFormat(settings) {
...
export function constrainDimensions(width, height, maxDimension) {
...
export function normalizeExportWatermark(watermark) {
...
export function normalizeWatermarkPosition(position) {
...
export function normalizeTimecodeBurnIn(timecode) {
    ...
    return {
        ...
        backgroundColor: typeof timecode.backgroundColor === 'string' && timecode.backgroundColor.trim()
            ? timecode.backgroundColor.trim()
            : '#000000',
        includeFrameNumber: timecode.includeFrameNumber === true,
    };
}
export function normalizeExportSlate(slate) {
    return slate?.enabled === true ? { enabled: true } : null;
}
// ---------------------------------------------------------------------------
// Filter builder functions
// ---------------------------------------------------------------------------
export function buildTimecodeBurnInFilter(inputLabel, outputLabel, timecode) {
    ...
    return `[${inputLabel}]drawtext=text='${textExpression}':fontsize=${timecode.fontSize}:fontcolor=${cssColorToFfmpeg(timecode.color)}:box=1:boxcolor=${cssColorToFfmpeg(timecode.backgroundColor)}@0.72:boxborderw=8:x='${position.x}':y='${position.y}'[${outputLabel}]`;
}
export function buildSlateVideoFilters(outputLabel, settings, project, timelineDuration, slateDuration) {
    ...
    return [
        `color=c=black:s=${settings.width}x${settings.height}:r=${settings.fps}:d=${formatFfmpegSeconds(slateDuration)},format=rgba,drawbox=x=0:y=0:w=iw:h=ih:color=black@1:t=fill,${drawTextFilters.join(',')}[${outputLabel}]`,
    ];
}
export function buildWatermarkFilters(inputLabel, outputLabel, watermark, settings, imageInputIndex) {
    if (watermark.type === 'image') {
    ...
    return [
        `[${inputLabel}]drawtext=text='${escapeDrawtextValue(watermark.text)}'${font}:fontsize=${watermark.fontSize}:fontcolor=${cssColorToFfmpeg(watermark.color)}:x='${position.x}':y='${position.y}'[${outputLabel}]`,
    ];
}
export function buildWatermarkExpression(position, widthVar, heightVar, itemWidthVar, itemHeightVar) {
...
export function calculateWatermarkOverlayPosition(position, canvasWidth, canvasHeight, watermarkWidth, watermarkHeight) {
...
// ---------------------------------------------------------------------------
export function buildGifExportPasses(inputs, baseFilterComplex, settings, duration, textArtifacts, outputRange) {
    textArtifacts.push({
        clipId: 'gif-palette',
        text: '',
    ...
    const paletteInput = { index: inputs.length, path: GIF_PALETTE_PLACEHOLDER, args: [] };
...
export function buildExportRangeOutputArgs(range) {
    return range ? ['-ss', formatFfmpegSeconds(range.start)] : [];
}
export function buildLoudnessNormalizationPasses(inputs, analysisFilterComplex, renderFullArgs, duration) {
...
export function buildFfmpegFullArgs(inputs, filterComplex, maps, outputArgs) {
    return [
        '-y',
        '-progress',
        'pipe:2',
...
//# sourceMappingURL=settings-normalize.js.map