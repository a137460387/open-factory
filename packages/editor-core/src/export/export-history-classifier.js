export const EXPORT_CATEGORY_RULES = [
    {
        tag: 'social-media',
        label: '社媒发布',
...
export function classifyExportEntry(entry, presetId, presetName, projectSnapshotId) {
...
function inferCategory(entry, presetId, presetName) {
...
export function classifyExportHistory(entries, presetMap) {
...
export function filterExportHistory(entries, filter) {
    return entries.filter((entry) => {
        ...
        if (filter.dateFrom) {
            if (entry.finishedAt < filter.dateFrom)
                return false;
        }
        if (filter.dateTo) {
            if (entry.finishedAt > filter.dateTo)
                return false;
        }
        if (filter.statusOnly) {
            if (entry.status !== filter.statusOnly)
                return false;
        }
        if (filter.searchText) {
            ...
            if (!name.includes(search) && !output.includes(search))
                return false;
        }
        return true;
    });
...
export function calculateExportCategoryStats(entries) {
...
export function findExportAssociation(entryId, presetMap) {
    return presetMap.get(entryId);
}
/** Manual category override: reclassify an entry. */
export function overrideEntryCategory(entry, newCategory) {
...
//# sourceMappingURL=export-history-classifier.js.map