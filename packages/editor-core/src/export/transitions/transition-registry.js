...
/** 根据类型查找转场定义 */
export function getTransitionDefinition(type) {
    return TRANSITION_REGISTRY.find((t) => t.type === type);
}
/** 获取转场的默认持续时间 */
export function getTransitionDefaultDuration(type) {
    return getTransitionDefinition(type)?.defaultDuration ?? 0.5;
}
/** 判断转场是否为自定义滤镜（非标准 xfade） */
export function isCustomTransition(type) {
    const def = getTransitionDefinition(type);
    return def != null && def.customBuilder != null;
}
/** 搜索转场（按名称/描述匹配） */
export function searchTransitions(query) {
    const q = query.toLowerCase().trim();
    if (!q)
        return TRANSITION_REGISTRY;
    return TRANSITION_REGISTRY.filter((t) => t.label.toLowerCase().includes(q) || t.description.toLowerCase().includes(q) || t.type.toLowerCase().includes(q));
}
