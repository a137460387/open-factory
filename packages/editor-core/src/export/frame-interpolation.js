...
export function clampFrameInterpolationProtectionFrames(value, fallback = DEFAULT_FRAME_INTERPOLATION_PROTECTION_FRAMES) {
...
export function buildSceneBoundaryProtectionRanges(sceneTimes, fps, duration, protectionFrames) {
...
export function isFrameProtectedBySceneBoundary(frameNumber, ranges) {
...
export function selectAdaptiveFrameInterpolationMode(motionScore) {
...
export function resolveFrameInterpolationMode(mode, motionScore) {
    return mode === 'adaptive' ? selectAdaptiveFrameInterpolationMode(motionScore) : mode;
}
export function averageClipMotionScore(clip) {
...
export function buildFrameInterpolationCacheKey(mediaPath, settings) {
...
export function frameInterpolationCacheDir(appDataDir) {
    return `${appDataDir.replace(/[\\/]+$/, '')}/interp-cache`;
}
export function frameInterpolationCachePath(appDataDir, mediaPath, settings) {
    return `${frameInterpolationCacheDir(appDataDir)}/${buildFrameInterpolationCacheKey(mediaPath, settings)}`;
}
export function mapSsimToFrameInterpolationQualityGrade(ssim) {
...
export function collectMissingInterpolationFrames(totalFrameCount, existingFrameNumbers) {
...
//# sourceMappingURL=frame-interpolation.js.map