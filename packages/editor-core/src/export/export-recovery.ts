...
export function classifyExportError(error: string): ExportRecoveryErrorKind {
  ...
  if (/unknown encoder|encoder .* not found|codec .* not supported|unsupported codec|invalid encoder|experimental codec/.test(normalized)) {
    return 'unsupported-codec';
  }
  if (/fontconfig|font file|cannot find font|failed to load font|drawtext.*font/.test(normalized)) {
    return 'missing-font';
  }
  if (/segmentation fault|access violation|exit code\s*:? ?(1|255)|process failed|ffmpeg failed|aborted|crash/.test(normalized)) {
    return 'ffmpeg-crash';
  }
  return 'unknown';
}

export function buildExportRecoveryDecision(plan: FfmpegExportPlan, error: string, attempts: number): ExportRecoveryDecision {
  if (attempts >= MAX_EXPORT_RECOVERY_ATTEMPTS) {
    return { errorKind: classifyExportError(error), action: 'none', canRetry: false, message: 'Export recovery limit reached.' };
  }
  const errorKind = classifyExportError(error);
  if (errorKind === 'ffmpeg-crash') {
    if (attempts > 0) {
      return { errorKind, action: 'none', canRetry: false, message: 'FFmpeg crash recovery has already been attempted.' };
    }
    return { errorKind, action: 'retry-same', canRetry: true, message: 'Retrying once with the same FFmpeg arguments.', plan };
  }
  if (errorKind === 'unsupported-codec') {
    return { errorKind, action: 'fallback-codec', canRetry: true, message: 'Falling back to a software H.264/H.265 encoder.', plan: fallbackExportCodecPlan(plan) };
  }
  if (errorKind === 'out-of-memory') {
    return { errorKind, action: 'reduce-concurrency', canRetry: true, message: 'Reducing export concurrency to 1 and retrying.', plan };
  }
  if (errorKind === 'missing-font') {
    return { errorKind, action: 'skip-drawtext', canRetry: true, message: 'Skipping drawtext filters and retrying.', plan: stripDrawtextFromExportPlan(plan) };
  }
  if (errorKind === 'disk-space') {
    return { errorKind, action: 'prompt-disk-cleanup', canRetry: false, message: 'Disk space is insufficient; user cleanup is required.' };
  }
  return { errorKind, action: 'none', canRetry: false, message: 'No automatic export recovery is available for this error.' };
}
...
export function finalizeExportRecoveryLog(entries: ExportRecoveryLogEntry[], result: ExportRecoveryLogEntry['result']): ExportRecoveryLogEntry[] {
  if (entries.length === 0) {
    return [];
  }
...
export function buildExportRecoveryReport(entries: ExportRecoveryLogEntry[], healed: boolean): ExportRecoveryReport | undefined {
  if (entries.length === 0) {
    return undefined;
  }
  return {
...
export function hasEnoughDiskSpace(availableBytes: number, expectedBytes: number, reserveBytes = 512 * 1024 * 1024): boolean {
...
export function fallbackExportCodecPlan(plan: FfmpegExportPlan): FfmpegExportPlan {
  ...
  return mapExportPlanArgs(plan, (args) => replaceCodecArg(args, fallbackCodec), { videoCodec: fallbackCodec, hardwareEncoding: false });
}

export function stripDrawtextFromExportPlan(plan: FfmpegExportPlan): FfmpegExportPlan {
  const recovered = mapExportPlanArgs(plan, (args) => args.map((arg) => stripDrawtextFilters(arg)), undefined, (filterComplex) => stripDrawtextFilters(filterComplex));
  return {
    ...recovered,
...
function mapExportPlanArgs(
  ...
  return {
    ...
    nestedPlans: plan.nestedPlans.map((nested) => ({ ...nested, plan: mapExportPlanArgs(nested.plan, mapArgs, settingsPatch, mapFilterComplex) }))
  };
}
