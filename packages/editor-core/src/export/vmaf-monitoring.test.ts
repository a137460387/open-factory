...
describe('determineMonitoringMode', () => {
  it('returns disabled when vmaf not available', () => {
    expect(determineMonitoringMode({ vmafAvailable: false, realtimeSupported: false, availableModels: [] })).toBe(
      'disabled',
    );
  });
  ...
  it('returns post-export by default', () => {
    expect(
      determineMonitoringMode({ vmafAvailable: true, realtimeSupported: false, availableModels: ['vmaf_v0.6.1'] }),
    ).toBe('post-export');
  });
  ...
  it('returns realtime when supported', () => {
    expect(
      determineMonitoringMode(
        { vmafAvailable: true, realtimeSupported: true, availableModels: ['vmaf_v0.6.1'] },
        { mode: 'realtime' },
      ),
    ).toBe('realtime');
  });
  ...
  it('falls back to post-export when realtime not supported', () => {
    expect(
      determineMonitoringMode(
        { vmafAvailable: true, realtimeSupported: false, availableModels: ['vmaf_v0.6.1'] },
        { mode: 'realtime' },
      ),
    ).toBe('post-export');
  });
