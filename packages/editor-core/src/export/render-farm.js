...
export function suggestRenderFarmInstances(cpuCores) {
    if (!Number.isFinite(cpuCores) || !cpuCores || cpuCores <= 0) {
        return 1;
    }
    return clampRenderFarmInstances(Math.floor(cpuCores / 4));
}
export function clampRenderFarmInstances(value) {
    if (!Number.isFinite(value)) {
        return 1;
    }
    return Math.min(4, Math.max(1, Math.round(value)));
}
export function calculateRenderFarmSegments(duration, options = {}) {
...
export function isRenderFarmPlanEligible(plan) {
    return plan.duration > RENDER_FARM_SPLIT_THRESHOLD_SECONDS && !plan.passes?.length && plan.nestedPlans.length === 0;
}
export function createRenderFarmSegmentStatuses(segments, tempSegmentsDir, taskId, outputPath) {
...
export function buildRenderFarmSegmentPath(tempSegmentsDir, taskId, index, outputPath) {
...
export function buildRenderFarmSegmentPlan(plan, segment) {
...
export function buildRenderFarmConcatList(segments) {
    return `${segments.map((segment) => `file '${escapeConcatPath(segment.outputPath)}'`).join('\n')}\n`;
}
export function buildRenderFarmConcatPlan(segments, outputPath, concatListPath, sourcePlan) {
...
export function calculateRenderFarmProgress(segments) {
...
export async function runRenderFarmWithFallback(context) {
...
async function runSegmentsInPool(context, segments) {
...
function updateSegment(context, segments, segmentId, patch) {
...
function replaceFinalOutputArg(args, outputPath) {
    if (args.length === 0) {
...
//# sourceMappingURL=render-farm.js.map