...
// ─── Constants ──────────────────────────────────────────
export const COMPLETION_ACTION_LABELS = {
    notify: '系统通知',
    shutdown: '自动关机',
    sleep: '休眠',
    hibernate: '睡眠',
...
export const DEFAULT_COMPLETION_ACTIONS = [
...
export function startBatchRender(batch, now) {
    return {
        ...batch,
        status: 'running',
...
export function completeBatchRender(batch, now) {
    return {
        ...batch,
        status: 'completed',
...
export function cancelBatchRender(batch, now) {
    return {
        ...batch,
        status: 'canceled',
        finishedAt: now ?? new Date().toISOString(),
...
export function calculateBatchProgress(batch, taskStatuses) {
    let completed = 0;
    let failed = 0;
    let totalProgress = 0;
    let currentTaskId;
...
export function addCompletionAction(actions, type) {
    const id = `action-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;
    return [...actions, { id, type, enabled: true }];
}
export function removeCompletionAction(actions, actionId) {
    return actions.filter((a) => a.id !== actionId);
}
export function toggleCompletionAction(actions, actionId) {
    return actions.map((a) => (a.id === actionId ? { ...a, enabled: !a.enabled } : a));
}
export function updateCompletionAction(actions, actionId, patch) {
    return actions.map((a) => (a.id === actionId ? { ...a, ...patch } : a));
}
export function getEnabledActions(actions) {
    return actions.filter((a) => a.enabled);
}
export function buildCompletionNotificationMessage(template, context) {
    return template
...
//# sourceMappingURL=batch-render-actions.js.map