...
export const BUILT_IN_ERROR_ENTRIES = [
    {
...
export function matchErrorKnowledge(stderr, entries, feedbackMap) {
...
export function getTopMatches(stderr, entries, feedbackMap, limit = 3) {
    return matchErrorKnowledge(stderr, entries, feedbackMap).slice(0, limit);
}
export function buildFeedbackMap(records) {
...
export function createDefaultKnowledgeStore() {
    return {
        version: 1,
...
export function addFeedback(store, entryId, helpful) {
    const record = { entryId, helpful, timestamp: Date.now() };
    return {
        ...store,
        feedback: [...store.feedback, record],
    };
}
export function mergeKnowledgeUpdate(local, remoteEntries, source) {
...
export function normalizeEntry(entry) {
    return {
        ...
        baseWeight: typeof entry.baseWeight === 'number' && Number.isFinite(entry.baseWeight)
            ? Math.min(2, Math.max(0, entry.baseWeight))
            : 0.5,
    };
}
export function filterEntriesByMinCount(entries, minCount) {
    return entries.length >= minCount;
}
function calculateFeedbackBonus(entryId, feedbackMap) {
    return feedbackMap.get(entryId) ?? 0;
}
function calculateDecayFactor(latestTimestamp, cutoff) {
...
//# sourceMappingURL=error-knowledge.js.map