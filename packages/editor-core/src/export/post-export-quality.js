export const DEFAULT_POST_EXPORT_QUALITY_ASSURANCE_SETTINGS = {
    enabled: false,
    duration: false,
    blackFrames: false,
    silence: false,
...
export function normalizePostExportQualityAssuranceSettings(settings) {
    return {
...
export function hasEnabledPostExportQualityChecks(settings) {
    return (settings.enabled &&
        (settings.duration || settings.blackFrames || settings.silence || settings.fileSize || settings.resolution));
}
export function buildPostExportBlackDetectArgs(outputPath, minDurationSeconds = DEFAULT_POST_EXPORT_QUALITY_ASSURANCE_SETTINGS.blackFrameDurationSeconds) {
    return [
        '-hide_banner',
        '-i',
        outputPath,
...
export function buildPostExportSilenceDetectArgs(outputPath, thresholdDb = DEFAULT_POST_EXPORT_QUALITY_ASSURANCE_SETTINGS.silenceThresholdDb, minDurationSeconds = DEFAULT_POST_EXPORT_QUALITY_ASSURANCE_SETTINGS.silenceDurationSeconds) {
    return [
        '-hide_banner',
        '-i',
        outputPath,
...
export function parseBlackDetectOutput(text) {
    return text.split(/\r?\n/).flatMap((line) => {
...
export function parseSilenceDetectOutput(text) {
    const ranges = [];
    let currentStart;
...
export function buildPostExportQualityAssuranceResult(settings, measurements, completedAt = new Date().toISOString()) {
    const checks = [];
...
export function summarizePostExportQualityStatus(checks) {
...
export function shouldRetryPostExportQuality(result, settings, retryAttempt) {
    return settings.autoRetry === true && retryAttempt < 1 && result.status === 'fail';
}
function checkDuration(measurements) {
...
function checkRanges(id, ranges, minDuration, warningMessage, passMessage) {
...
function checkFileSize(settings, actual) {
...
function parseNamedNumber(line, name) {
...
//# sourceMappingURL=post-export-quality.js.map