import { getProjectPrimaryTimeline, getProjectSequences, isDefaultColorCorrection, normalizeColorCorrection, normalizeTransitionDuration, normalizeTransitionType, } from '../model';
...
export function exportTimeline(project, format) {
    return format === 'fcp-xml' ? exportFinalCutXml(project) : exportCmx3600Edl(project);
}
export function exportCmx3600Edl(project) {
    ...
    events.forEach((event, index) => {
        ...
        lines.push(`${edit}  AX       V     C        ${secondsToTimecode(event.sourceStart, fps)} ${secondsToTimecode(event.sourceEnd, fps)} ${secondsToTimecode(event.recordStart, fps)} ${secondsToTimecode(event.recordEnd, fps)}`);
...
export function exportFinalCutXml(project, options = {}) {
    const fps = normalizeFps(project.settings.fps);
...
export function exportProfessionalNle(project, format, options = {}) {
    if (format === 'aaf') {
...
export function exportAaf(project, options = {}) {
    const fps = normalizeFps(project.settings.fps);
    ...
    events.forEach((event, index) => {
        lines.push(`MobSlot ${index + 1}`, `SourceClip ${sanitizeAafText(event.name)}`, `MobSlotTimecode ${secondsToTimecode(event.recordStart, fps)} -> ${secondsToTimecode(event.recordEnd, fps)}`, `SourceMob ${sanitizeAafText(event.sourceName)}`, event.sourcePath ? `SourcePath ${sanitizeAafText(event.sourcePath)}` : 'SourcePath <generated>', `MasterMob ${sanitizeAafText(project.name)}`, '');
    });
    return `${lines.join('\n').trimEnd()}\n`;
}
export function exportOmf(project, options = {}) {
    const fps = normalizeFps(project.settings.fps);
    const events = flattenTimelineForExport(project, options).filter((event) => event.trackType === 'video' || event.trackType === 'audio');
    const lines = [
        'OMFI 2.0',
    ...
    events.forEach((event, index) => {
        lines.push(`MobSlot ${index + 1}`, `SourceClip ${sanitizeAafText(event.name)}`, `MobSlotTimecode ${secondsToTimecode(event.recordStart, fps)} -> ${secondsToTimecode(event.recordEnd, fps)}`, event.sourcePath ? `SourcePath ${sanitizeAafText(event.sourcePath)}` : 'SourcePath <generated>', '');
    });
    return `${lines.join('\n').trimEnd()}\n`;
}
export function flattenTimelineForExport(project, options = {}) {
    ...
    return collectTimelineEvents(primary, sequences, mediaById, new Set([sequences.find((sequence) => sequence.timeline === primary)?.id ?? 'sequence-main']), options.mediaPathMap)
...
function collectTimelineEvents(timeline, sequences, mediaById, visited, mediaPathMap) {
    const events = [];
...
function mapNestedEvents(clip, nestedEvents) {
...
function clipToEvent(clip, trackType, media, mediaPathMap) {
    const sourceStart = clip.type === 'image' || clip.type === 'motion-graphic' ? 0 : clip.trimStart;
    const sourceEnd = clip.type === 'image' || clip.type === 'motion-graphic'
        ? clip.duration
...
function clipItemXml(event, index, fps, indent) {
...
function transitionItemXml(transition, index, event, fps, indent) {
    const pad = ' '.repeat(indent);
...
function buildColorCorrectionXml(colorCorrection, indent) {
...
function sanitizeEdlText(value) {
    return value.replace(/\r?\n/g, ' ').trim();
}
function sanitizeAafText(value) {
...
function resolveMediaPath(path, mediaPathMap) {
...
//# sourceMappingURL=timeline-export.js.map