 ...
 */
export function calculateTrend(points, windowSize = 10) {
    if (points.length === 0) {
        return {
            metric: '',
            unit: '',
 ...
 */
export function detectBottlenecks(metrics, config = DEFAULT_MONITOR_CONFIG) {
    if (metrics.length === 0)
        return [];
    const bottlenecks = [];
 ...
 */
export function analyzeTaskPerformance(tasks) {
    if (tasks.length === 0) {
        return {
            avgDuration: 0,
            byType: {},
    ...
    for (const task of tasks) {
        ...
        if (task.duration)
            byType[task.taskType].totalDuration += task.duration;
        if (task.status === 'completed')
            byType[task.taskType].success++;
    }
    const byTypeStats = {};
 ...
 */
export function generateOptimizations(metrics, tasks, bottlenecks, config = DEFAULT_MONITOR_CONFIG) {
 ...
 */
export function generateDashboard(metricsHistory, tasks, config = DEFAULT_MONITOR_CONFIG) {
    ...
    const taskDurationTrend = calculateTrend(completedTasks.map((t) => ({ timestamp: t.startTime, value: t.duration || 0 })));
 ...
 */
export function checkThreshold(value, warning, critical) {
    if (value >= critical)
        return 'critical';
    if (value >= warning)
        return 'warning';
 ...
 */
export function calculatePerformanceScore(bottlenecks) {
    let score = 100;
