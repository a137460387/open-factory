import { normalizeCollaborationNotes } from './model';
...
function sortCollaborationNotes(notes) {
    return normalizeCollaborationNotes([...notes]);
}
export function filterCollaborationNotesByAuthor(notes, authorName) {
...
export function toggleCollaborationNoteResolved(notes, noteId, resolved, updatedAt = new Date().toISOString()) {
    return sortCollaborationNotes(notes.map((note) => {
        if (note.id !== noteId) {
            return note;
        }
        return {
...
export function buildCollaborationReport(project, options = {}) {
...
export function buildCollaborationReportHtml(project, options = {}) {
    return renderCollaborationReportHtml(buildCollaborationReport(project, options));
}
export function renderCollaborationReportHtml(report) {
...
function renderRows(rows, locale) {
    ...
    return rows
        ...
        const time = row.end !== undefined && row.end > row.start
...
//# sourceMappingURL=collaboration-notes.js.map