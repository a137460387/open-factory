...
export function calculateTimelineThumbnailTimestamps(input) {
...
export function buildTimelineThumbnailCacheKey(mediaPath, timestamp) {
    return `${normalizeCachePath(mediaPath)}|t=${round(Math.max(0, timestamp)).toFixed(3)}`;
}
export function planTimelineThumbnailCache(mediaPath, timestamps, cachedKeys) {
    const hits = [];
    const misses = [];
...
export function calculateTimelineThumbnailTrackInterval(input) {
...
export function calculateTimelineThumbnailTrackTimestamps(input) {
    const zoom = Number.isFinite(input.zoom) ? Math.max(0.001, input.zoom) : 0.001;
...
export function buildTimelineThumbnailTrackSamples(timeline, input) {
...
export function sortTimelineThumbnailSamplesByPriority(samples, playheadTime) {
...
function findVideoClipAtTime(track, time) {
    return track?.clips
        .filter((clip) => clip.type === 'video')
        .sort((left, right) => left.start - right.start || left.id.localeCompare(right.id))
        .find((clip) => time >= clip.start && time < clip.start + clip.duration);
}
function timelineTimeToClipSourceTimestamp(clip, timelineTime) {
...
//# sourceMappingURL=timeline-thumbnails.js.map