import { DEFAULT_COLOR_CORRECTION, DEFAULT_PRIMARY_SEQUENCE_NAME, DEFAULT_TRANSFORM, PRIMARY_SEQUENCE_ID, createProject, createTrack, } from './model';
export const TIMELINE_TEMPLATE_SCHEMA_VERSION = 1;
export const BUILT_IN_TIMELINE_TEMPLATES = [
    ...
    createBuiltInTemplate({
        ...
        tracks: [
            ...
            builtInTrack('track-detail', 'video', '细节', [
                builtInMediaClip('clip-product-detail', 'track-detail', 'placeholder-product-detail', 'video', '产品细节', 5, 5),
            ]),
...
export function serializeTimelineTemplate(project, options) {
    ...
    const placeholders = [];
    const placeholderByMediaId = new Map();
    const mediaById = new Map(project.media.map((asset) => [asset.id, asset]));
    const tracks = sourceTracks.map(({ track, clips }, trackIndex) => {
...
export function instantiateTimelineTemplate(template, bindings = {}) {
    ...
    const tracks = template.tracks.map((templateTrack, trackIndex) => {
...
export function instantiateTimelineTemplateProject(template, bindings = {}, options = {}) {
...
export function getMissingTimelineTemplatePlaceholders(template, bindings = {}) {
    return template.placeholders.filter((placeholder) => !normalizePlaceholderBinding(bindings[placeholder.id])?.path);
}
export function fillTimelineTemplatePlaceholders(template, bindings) {
    const filled = {};
...
export function renderTimelineTemplatePreviewSvg(template, options = {}) {
...
export function normalizeTimelineTemplateDefinition(value) {
    if (!value || typeof value !== 'object') {
        return undefined;
    }
    const candidate = value;
    if (candidate.schemaVersion !== TIMELINE_TEMPLATE_SCHEMA_VERSION ||
...
function normalizeTemplateTrack(value) {
    if (!value || typeof value !== 'object') {
        return [];
    }
    const track = value;
...
function normalizeTemplateClip(value) {
    if (!value || typeof value !== 'object') {
        return [];
    }
    const entry = value;
...
function getOrCreatePlaceholder(asset, byMediaId, placeholders) {
...
function normalizePlaceholderBinding(value) {
...
function createBuiltInTemplate(input) {
    return {
        schemaVersion: TIMELINE_TEMPLATE_SCHEMA_VERSION,
        ...input,
...
function builtInTrack(id, type, name, clips) {
    return { id, sourceTrackId: id, type, name, clips };
}
function builtInMediaClip(id, trackId, mediaPlaceholderId, type, name, start, duration) {
...
function builtInTextClip(id, trackId, text, start, duration) {
    const clip = {
...
function builtInSubtitleClip(id, trackId, start, duration) {
    const clip = {
...
function builtInBaseClip(id, trackId, name, start, duration) {
    return {
        id,
        name,
        trackId,
...
//# sourceMappingURL=timeline-templates.js.map