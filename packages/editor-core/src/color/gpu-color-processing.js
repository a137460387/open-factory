...
/** 生成处理管线缓存键 */
export function buildPipelineCacheKey(imageDataHash, colorCorrection, toneMapping, lutId, resolution) {
    const parts = [imageDataHash, resolution];
    if (colorCorrection)
        parts.push(computeParamsHash(colorCorrection));
    if (toneMapping)
        parts.push(computeParamsHash(toneMapping));
    if (lutId)
        parts.push(lutId);
    return parts.join('::');
}
// ==================== 默认工厂函数 ====================
/** 创建默认色彩校正参数 */
export function createDefaultColorCorrectionParams() {
    return {
        lift: { r: 0, g: 0, b: 0 },
        liftMaster: 0,
        gamma: { r: 0, g: 0, b: 0 },
...
/** 创建默认色调映射参数 */
export function createDefaultToneMappingParams() {
    return {
        method: 'aces-hill',
        exposure: 0,
        whitePoint: 1.0,
...
// ==================== 验证函数 ====================
/** 验证色彩校正参数 */
export function validateGPUColorCorrectionParams(params) {
    const clamp = (v, min, max) => clampValue(v, min, max);
    const clampCh = (ch) => ({
        r: clamp(ch.r, -1, 1),
        g: clamp(ch.g, -1, 1),
        b: clamp(ch.b, -1, 1),
...
/** 验证色调映射参数 */
export function validateGPUToneMappingParams(params) {
    return {
...
/** 验证管线配置 */
export function validateGPUPipelineConfig(config) {
    return {
...
export function fromPrimaryWheelAndSliders(wheels, sliders) {
    return {
...
// ==================== GLSL 着色器代码 ====================
/** 生成完整的 GPU 色彩处理片段着色器 */
export function generateColorProcessingFragmentShader() {
...
export function generateWebGPUComputeShader() {
...
export function cpuApplyLiftGammaGain(r, g, b, params) {
...
/** CPU 回退：应用色温/色调 */
export function cpuApplyTemperatureTint(r, g, b, temperature, tint) {
    const tf = temperature / 100;
    const tt = tint / 100;
    return [clampValue(r + tf * 0.1, 0, 1), clampValue(g + tt * 0.05, 0, 1), clampValue(b - tf * 0.1, 0, 1)];
}
/** CPU 回退：应用对比度 */
export function cpuApplyContrast(r, g, b, contrast, pivot) {
...
/** CPU 回退：应用饱和度 */
export function cpuApplySaturation(r, g, b, saturation) {
...
export function cpuToneMapAcesHill(r, g, b) {
...
export function cpuToneMapReinhard(r, g, b) {
    return [r / (1 + r), g / (1 + g), b / (1 + b)];
}
/** CPU 回退：色调映射 - Filmic */
export function cpuToneMapFilmic(r, g, b) {
    const film = (x) => {
...
/** CPU 回退：色调映射 */
export function cpuApplyToneMapping(r, g, b, method, exposure) {
    ...
    switch (method) {
        ...
        case 'agx': {
            ...
            const minEv = -12.47;
            const maxEv = 6.5;
            const norm = (v) => clampValue((Math.log2(Math.max(v, 0.0001)) - minEv) / (maxEv - minEv) + agxOffset, 0, 1);
            er = norm(er);
            eg = norm(eg);
            eb = norm(eb);
            break;
...
/** CPU 回退：3D LUT 三线性插值 */
export function cpuApply3DLUT(r, g, b, lutData, intensity) {
...
/** CPU 回退：完整色彩处理管线 */
export function cpuProcessPixel(r, g, b, colorCorrection, toneMapping, lutData, lutIntensity) {
    let cr = r;
    let cg = g;
    let cb = b;
    // 色彩校正
...
/** 处理整帧图像数据 (CPU 回退) */
export function cpuProcessFrame(input, width, height, colorCorrection, toneMapping, lutData, lutIntensity) {
...
export class GPUColorProcessor {
    ...
    /** 处理图像帧 */
    async processFrame(input, width, height, colorCorrection, toneMapping, lutData, lutIntensity = 1.0) {
        const start = performance.now();
        // 检查缓存
        if (this.config.enableCache) {
            const ccHash = colorCorrection
                ? computeParamsHash(colorCorrection)
                : 'none';
            const tmHash = toneMapping ? computeParamsHash(toneMapping) : 'none';
    ...
    addToCache(key, data, width, height) {
        ...
        while (this.cache.size > 0 && (this.cacheUsedBytes + entryBytes > this.config.maxCacheBytes || this.cache.size >= this.config.maxCacheSize)) {
            ...
            if (evicted)
                this.cacheUsedBytes -= evicted.bytes;
...
export class PreviewFrameCache {
    ...
    maxEntries;
    ttlMs;
    constructor(maxEntries = 32, ttlMs = CACHE_TTL_MS) {
...
export class GPUPerformanceMonitor {
    ...
    gpuTimes = [];
    maxSamples;
    constructor(maxSamples = MAX_PERFORMANCE_SAMPLES) {
        this.maxSamples = maxSamples;
    }
    /** 记录一帧 */
    recordFrame(frameTimeMs, gpuTimeMs) {
...
//# sourceMappingURL=gpu-color-processing.js.map