import { createId, getTransformScaleX, getTransformScaleY, normalizeTextPath, } from './model';
...
export function normalizeClipKeyframes(keyframes, duration) {
    if (!keyframes) {
        return undefined;
    }
    const output = {};
    for (const property of Object.keys(KEYFRAME_PROPERTY_LIMITS)) {
...
export function normalizeKeyframes(keyframes, duration, fallback, property) {
...
export function getClipKeyframeValue(clip, property, localTime) {
    return interpolateKeyframes(clip.keyframes?.[property], localTime, getClipStaticKeyframeValue(clip, property));
}
export function getClipStaticKeyframeValue(clip, property) {
    if (property === 'opacity') {
...
export function resolveAnimatedTransform(clip, localTime) {
...
export function resolveAnimatedVolume(clip, localTime) {
    return getClipKeyframeValue(clip, 'volume', localTime);
}
export function applyClipKeyframes(clip, localTime) {
    const transform = resolveAnimatedTransform(clip, localTime);
    const panorama = clip.panorama && (clip.keyframes?.yaw || clip.keyframes?.pitch || clip.keyframes?.roll)
        ? {
            ...clip.panorama,
...
export function createKeyframe(property, input, clipDuration) {
    const fallback = getKeyframeFallbackValue(property);
    ...
    ], clipDuration, fallback, property)[0];
}
function getKeyframeFallbackValue(property) {
...
export function setKeyframeForProperty(keyframes, property, keyframe, clipDuration) {
...
export function removeKeyframeForProperty(keyframes, property, keyframeId) {
    const next = cloneClipKeyframes(keyframes);
    if (!next?.[property]) {
        return next;
    }
...
export function cloneClipKeyframes(keyframes) {
    if (!keyframes) {
        return undefined;
    }
    const output = {};
...
export function cloneKeyframe(frame) {
    return {
        ...frame,
...
export function normalizeKeyframeHandle(handle) {
    if (!handle ||
...
export function normalizeKeyframeHandleMode(mode) {
    return mode === 'unified' || mode === 'independent' || mode === 'broken' ? mode : undefined;
}
export function calculateBezierHandleCoordinates(frame, previous, next, mode = frame.handleMode ?? 'independent') {
...
export function applyKeyframeHandlePatch(frame, handle, value, mode = frame.handleMode ?? 'independent') {
    const normalizedMode = normalizeKeyframeHandleMode(mode) ?? 'independent';
...
export function calculateKeyframeSpeedSamples(frames, duration, fallback, sampleCount = 32) {
...
export function alignKeyframeValues(frames, value = frames[0]?.value ?? 0) {
...
export function parseKeyframeExpression(expression, context = {}) {
 ...
 */
export function applyStepsEasing(progress, steps) {
    if (steps <= 1)
        return progress;
    const t = Math.min(1, Math.max(0, progress));
    return Math.floor(t * steps) / steps;
}
function hasBezierSegmentHandles(left, right) {
    return Boolean(left.outHandle || right.inHandle);
}
function interpolateBezierSegmentProgress(left, right, progress) {
...
function mirrorHandle(handle, targetSpan, sourceSpan) {
...
function clampHandleForDirection(handle, direction, span) {
...
function handleToPoint(frame, handle) {
    return {
        dx: handle.dx,
        dy: handle.dy,
...
function tokenizeNumericExpression(expression, context) {
    const tokens = [];
    let index = 0;
...
export function createKenBurnsKeyframes(duration, startScale = 1, endScale = 1.5) {
...
export function setKenBurnsEndScaleKeyframes(keyframes, duration, scale) {
...
function setLastScaleFrame(frames, duration, scale, property) {
    if (frames.length === 0) {
...
export function normalizeCrossPropertyValue(value, sourceProperty, targetProperty) {
...
export function normalizePastedKeyframes(groups, targetClipStart, targetClipDuration, mode, targetProperty) {
    const result = [];
