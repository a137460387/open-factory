...
/** 计算采样率（帧率/3，最低8fps） */
export function calculateSampleRate(frameRate) {
 ...
 * 检测单帧是否为大面积纯红色闪烁
 */
export function isRedFlashFrame(r, g, b) {
 ...
 */
export function detectLumaFlips(samples) {
    if (samples.length < 2)
        return [];
    const flips = [];
    let prevDelta = 0;
 ...
 * 根据翻转率确定severity
 */
export function classifySeverity(flashRate, isRedFlash) {
    if (isRedFlash)
        return 'high';
    if (flashRate >= SEVERITY_HIGH_RATE)
        return 'high';
    if (flashRate >= SEVERITY_MEDIUM_RATE)
        return 'medium';
    return 'low';
}
/**
 ...
 */
export function buildFlashReductionFilter(startTime, endTime) {
    const filters = [];
 ...
 * 合并相邻或重叠的闪烁区间
 */
export function mergeFlashIntervals(intervals, mergeGap = 0.2) {
 ...
 * 主检测函数：对帧采样数据进行闪烁检测
 */
export function detectFlashWarnings(samples) {
    if (samples.length < 3)
        return [];
...
//# sourceMappingURL=flash-warning.js.map