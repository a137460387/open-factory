...
function collectVisualSamples(media: MediaAsset[]): ContentAnalysisVisualSample[] {
  ...
  for (const m of media) {
    // Generate synthetic visual samples from media properties
    if (m.duration > 0) {
      const sampleCount = Math.max(1, Math.floor(m.duration / 5));
      for (let i = 0; i < sampleCount; i++) {
        samples.push({
          time: (i / sampleCount) * m.duration,
...
function collectAudioSamples(media: MediaAsset[]): ContentAnalysisAudioSample[] {
  ...
  for (const m of media) {
    // Generate synthetic audio samples from media properties
    if (m.duration > 0) {
      const sampleCount = Math.max(1, Math.floor(m.duration / 5));
      for (let i = 0; i < sampleCount; i++) {
        samples.push({
...
function collectTranscripts(media: MediaAsset[]): string[] {
  ...
  for (const m of media) {
    ...
    // Use type assertion to access optional aiAnalysis property
    const m2 = m as unknown as Record<string, unknown>;
    const aiAnalysis = m2.aiAnalysis as Record<string, unknown> | undefined;
    if (aiAnalysis?.transcript && typeof aiAnalysis.transcript === 'string') {
      transcripts.push(aiAnalysis.transcript);
    }
  }
  return transcripts;
}
