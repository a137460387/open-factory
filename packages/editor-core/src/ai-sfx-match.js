...
export function detectActionCandidatePoints(motionMagnitudes) {
    if (motionMagnitudes.length < 2)
        return [];
    const candidates = [];
...
export function buildSfxMatchPrompt(moments) {
    const lines = [
    ...
    for (const m of moments) {
        ...
        if (m.sceneTag)
            line += `, 场景: ${m.sceneTag}`;
        if (m.nearbySubtitle)
            line += `, 字幕: '${m.nearbySubtitle}'`;
        lines.push(line);
    }
    return lines.join('\n');
}
export function parseSfxMatchResponse(json) {
    try {
...
export function matchLocalSfxLibrary(category, library) {
    const normalized = normalizeCategory(category);
    return library.find((entry) => normalizeCategory(entry.category) === normalized) ?? null;
}
export function buildSfxSuggestions(aiSuggestions, library) {
...
//# sourceMappingURL=ai-sfx-match.js.map