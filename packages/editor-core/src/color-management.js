export const EXPORT_COLOR_SPACES = ['srgb', 'rec709', 'rec2020', 'dci-p3', 'display-p3'];
export const DEFAULT_EXPORT_COLOR_MANAGEMENT = {
    inputColorSpace: 'srgb',
    outputColorSpace: 'srgb',
    embedIccProfile: true,
};
export const DEFAULT_PROJECT_WORKING_COLOR_SPACE = 'srgb';
...
export function normalizeExportColorSpace(value, fallback = 'srgb') {
    return value === 'srgb' || value === 'rec709' || value === 'dci-p3' || value === 'display-p3' || value === 'rec2020'
        ? value
        : fallback;
}
export function normalizeProjectWorkingColorSpace(value, fallback = DEFAULT_PROJECT_WORKING_COLOR_SPACE) {
    return normalizeExportColorSpace(value, fallback);
}
export function normalizeExportColorManagement(value) {
    return {
...
export function isDefaultExportColorManagement(value) {
    const normalized = normalizeExportColorManagement(value);
    return (normalized.inputColorSpace === DEFAULT_EXPORT_COLOR_MANAGEMENT.inputColorSpace &&
...
export function getExportIccProfileBase64(colorSpace) {
...
export function getFfmpegColorSpaceProfile(colorSpace) {
...
export function buildZscaleColorConversionFilter(inputColorSpace, outputColorSpace) {
...
export function buildExportColorTagArgs(colorSpace) {
...
export function buildIccMetadataArgs(colorSpace) {
...
export function getColorSpaceDisplayName(colorSpace) {
    if (colorSpace === 'rec709') {
        return 'Rec.709';
    }
    if (colorSpace === 'rec2020') {
...
function normalizeProbeField(value) {
    return typeof value === 'string' && value.trim() ? value.trim().toLowerCase() : undefined;
}
function inferColorSpaceFromProbeFields(colorSpace, colorPrimaries, colorTransfer) {
...
//# sourceMappingURL=color-management.js.map