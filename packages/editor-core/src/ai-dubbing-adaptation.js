 ...
 */
export function computeTimingAdaptation(originalDuration, dubbedDuration, nextSegmentStart) {
 ...
 */
export function hasOutpointConflict(suggestedOutPoint, nextSegmentStart) {
 ...
 */
export function batchComputeAdaptations(segments) {
    return segments.map((seg, index) => {
        const nextSeg = segments[index + 1];
        const adaptation = computeTimingAdaptation(seg.originalDuration, seg.dubbedDuration, nextSeg?.originalDuration !== undefined
 ...
 */
export function getSegmentsNeedingAdaptation(project) {
    return (project.ttsSegments ?? []).filter((seg) => seg.timingAdaptation?.adaptationType !== 'none' && seg.timingAdaptation?.adaptationType !== undefined);
}
export async function computeTimingAdaptationSafe(originalDuration, dubbedDuration, nextSegmentStart, t = identityTranslator) {
    try {
...
//# sourceMappingURL=ai-dubbing-adaptation.js.map