 ...
 */
export function detectScenes(samples, options = {}) {
    const { histogramThreshold = 0.35, motionThreshold = 0.55, minSceneDuration = 0.5, histogramBins = 8, histogramWeight = 0.6, motionWeight = 0.4, adaptiveSensitivity = 1.0, } = options;
    ...
    for (let i = 0; i < pairScores.length; i++) {
        const threshold = computeAdaptiveThreshold(pairScores, i, windowSize, histogramThreshold, motionThreshold, histogramWeight, motionWeight, adaptiveSensitivity);
 ...
 */
function buildSyntheticHistogram(sample, bins) {
 ...
 */
function histogramDifference(a, b) {
 ...
 */
function motionChange(prev, curr) {
 ...
 */
function computeAdaptiveThreshold(scores, index, windowSize, baseHistogramThreshold, baseMotionThreshold, histogramWeight, motionWeight, sensitivity) {
 ...
 */
function buildSegments(sorted, boundaries) {
    if (sorted.length === 0) {
        return [];
    }
 ...
 */
function finalizeSegment(start, end, samples) {
...
//# sourceMappingURL=ai-scene-detector.js.map