...
export const CHAT_ACTION_WHITELIST = new Set([
    'setSpeed',
    'setVolume',
    'delete',
    'split',
 ...
 */
export function validateChatAction(action) {
    ...
    if (!CHAT_ACTION_WHITELIST.has(obj.action)) {
 ...
 */
export function buildTimelineContext(project, selectedClipId) {
    ...
    const clipSummaries = allClips.map((clip) => ({
        ...
        speed: clip.speed,
        ...('volume' in clip ? { volume: clip.volume } : {}),
    }));
    let selectedClipInfo;
    if (selectedClipId) {
        ...
        if (selected) {
            selectedClipInfo = {
                ...
                ...('volume' in selected ? { volume: selected.volume } : {}),
            };
        }
    }
 ...
 */
export function parseChatAIResponse(json) {
    const rejected = [];
    const commands = [];
 ...
 */
export function safeParseChatResponse(raw) {
    try {
 ...
 * 为执行反馈生成人类可读的命令描述（中文）。
 */
export function describeChatCommand(command) {
 ...
 */
export function findClipInTimeline(timeline, clipId) {
    for (const track of timeline.tracks) {
 ...
 */
export function getAllClipIds(timeline) {
    const ids = [];
    for (const track of timeline.tracks) {
        for (const clip of track.clips) {
            ids.push(clip.id);
...
//# sourceMappingURL=ai-chat-editor.js.map