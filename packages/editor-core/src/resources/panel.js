...
export function createInitialResourcePanelState() {
    return {
        phase: 'idle',
 ...
 */
export function resourcePanelReducer(state, action) {
    switch (action.type) {
        case 'START_SCAN':
            return {
                ...state,
        ...
        case 'PROXY_COMPLETE': {
            const proxies = state.proxies.map((p) => p.id === action.proxyId ? { ...p, status: 'ready', progress: 100 } : p);
            return { ...state, proxies };
        }
        case 'RESET':
 ...
 */
export function getResourceTypeColor(type) {
    const colors = {
        video: '#2563eb',
        audio: '#7c3aed',
        image: '#059669',
 ...
 */
export function getProxyStatusColor(status) {
    const colors = {
        pending: '#6b7280',
        generating: '#2563eb',
        ready: '#16a34a',
 ...
 */
export function getCleanupRiskLabel(risk) {
    const labels = {
        low: '低风险',
        medium: '中风险',
        high: '高风险',
 ...
 */
export function getCleanupRiskColor(risk) {
    const colors = {
        low: '#16a34a',
        medium: '#ca8a04',
        high: '#dc2626',
 ...
 */
export function getCleanupTypeLabel(type) {
    const labels = {
        'cache-expired': '过期缓存',
        'unused-file': '未使用文件',
        'duplicate-file': '重复文件',
 ...
 */
export function getResourceOverviewStats(state) {
    if (!state.stats)
        return [];
    return [
 ...
 */
export function getResourceTabs() {
    return [
