...
export function calculateTimelineMinimapViewportRect(input) {
...
export function calculateTimelineScrollLeftFromMinimapY(input) {
    const duration = sanitizeDuration(input.duration);
    const zoom = sanitizeZoom(input.zoom);
    const viewportDuration = calculateViewportDuration(input.viewportWidth, input.labelWidth, zoom, duration);
...
export function buildTimelineMinimapLayout(timeline, options) {
        ...
        ...(options.exportRanges ?? []).flatMap((range) => [
            {
                ...
                color: '#0ea5e9',
                kind: 'export-range-start',
            },
            {
                id: `${range.id}-end`,
                y: timeToMinimapY(range.end, duration, height),
                color: '#0284c7',
                kind: 'export-range-end',
            },
        ]),
...
function sampleMinimapClips(clips, maxClips) {
...
function calculateViewportDuration(viewportWidth, labelWidth, zoom, duration) {
    const contentWidth = Math.max(1, viewportWidth - labelWidth);
    return clamp(contentWidth / zoom, 0, duration);
}
function timeToMinimapY(time, duration, height) {
...
//# sourceMappingURL=timeline-minimap.js.map