...
export function computeSampleTimes(clipDuration, interval = DEFAULT_REFrame_SAMPLE_INTERVAL, sceneCuts) {
...
export function bboxToCropWindow(bbox, sourceWidth, sourceHeight, targetAspect) {
...
export function generateReframeKeyframes(aiFrames, sourceWidth, sourceHeight, targetAspect) {
    const normalizedAspect = normalizeTargetAspectRatio(targetAspect);
    if (normalizedAspect === 'source') {
        return [];
    }
    const validAspect = normalizedAspect;
    const keyframes = [];
...
export function smoothKeyframes(keyframes, windowSize = DEFAULT_SMOOTHING_WINDOW) {
...
export function interpolateReframeAtTime(keyframes, time) {
...
export function buildReframeCropFFmpegExpression(keyframes) {
    if (keyframes.length === 0) {
        return undefined;
    }
    if (keyframes.length === 1) {
...
function buildSegmentExpression(keyframes, field) {
    if (keyframes.length === 1) {
...
export function computeReframeConfidence(aiFrames) {
