 ...
 * 添加媒体到收藏夹。
 */
export function addFavorite(favorites, mediaId) {
 ...
 * 从收藏夹移除。
 */
export function removeFavorite(favorites, mediaId) {
    return favorites.filter((id) => id !== mediaId);
}
/**
 * 切换收藏状态。
 */
export function toggleFavorite(favorites, mediaId) {
    return favorites.includes(mediaId) ? removeFavorite(favorites, mediaId) : addFavorite(favorites, mediaId);
}
/**
 * 是否已收藏。
 */
export function isFavorite(favorites, mediaId) {
 ...
 */
export function trackRecentMedia(recentIds, mediaId, maxItems = FAVORITES_MAX_RECENT) {
 ...
 * 获取最近使用列表。
 */
export function getRecentMediaIds(recentIds, maxItems = FAVORITES_MAX_RECENT) {
 ...
 */
export function parseFavoritesSearchFilter(query) {
 ...
 */
export function getFavoritesStoragePath(mode, sharedPath) {
    if (mode === 'shared')
 ...
 * 初始化默认收藏夹数据。
 */
export function createDefaultFavoritesData() {
    return { favoriteIds: [], recentIds: [] };
}
/**
 * 规范化收藏夹数据（兼容旧项目缺失字段）。
 */
export function normalizeFavoritesData(input) {
    if (!input || typeof input !== 'object')
        return createDefaultFavoritesData();
    const data = input;
    return {
        favoriteIds: Array.isArray(data.favoriteIds)
            ? data.favoriteIds.filter((id) => typeof id === 'string')
            : [],
        recentIds: Array.isArray(data.recentIds)
            ? data.recentIds.filter((id) => typeof id === 'string').slice(0, FAVORITES_MAX_RECENT)
            : [],
        sharedPath: typeof data.sharedPath === 'string' ? data.sharedPath : undefined,
    };
}
//# sourceMappingURL=media-favorites.js.map