 ...
 */
export function estimateDisplacementVectors(frames, width, height, gridSize = 4, searchRadius = 4) {
 ...
 */
export function calculateShakeScore(displacementVectors, maxExpectedVariance) {
    if (displacementVectors.length === 0)
        return 0;
 ...
 */
export function classifyShakeSeverity(score) {
    if (score < 20)
        return 'low';
    if (score <= 50)
        return 'medium';
 ...
 */
export function analyseShake(frames, width, height, maxExpectedVariance) {
 ...
 */
export function buildTwoStepVidstabArgs(inputPath, trfPath, smoothing = DEFAULT_SMOOTHING, zoom = DEFAULT_ZOOM) {
...
//# sourceMappingURL=shake-analysis.js.map