...
export function sampleTextPath(points, width = 1, height = 1, segmentsPerCurve = TEXT_PATH_SEGMENTS_PER_CURVE) {
...
export function getTextPathLength(points, width = 1, height = 1) {
    return sampleTextPath(points, width, height).at(-1)?.distance ?? 0;
}
export function layoutTextAlongPath(input) {
...
export function buildPathTextFrameLayouts(input) {
...
export function resolvePathTextStartOffset(pathText, keyframes, localTime) {
    return normalizeUnit(interpolateKeyframes(keyframes?.pathStartOffset, localTime, pathText.startOffset));
}
function sampleTextPathAtDistance(samples, distance) {
...
function buildDistanceSamples(points) {
    if (points.length < 2) {
        return [];
    }
    let distance = 0;
...
function removeDuplicateSamples(points) {
