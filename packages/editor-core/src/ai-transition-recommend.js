...
export function calculateRGBHistogramChiSquareDistance(histA, histB) {
...
export function estimateMotionFromFrameDifferences(framePixels, width, height) {
...
export function estimateMotionFromLumaDiffs(lumaDiffs) {
    if (lumaDiffs.length === 0)
        return 0;
    let sum = 0;
...
export function mapToValidTransitionType(type) {
    const normalized = type.toLowerCase().trim();
    for (const valid of VALID_TRANSITION_TYPES) {
        if (valid === normalized)
            return valid;
    }
    if (normalized.includes('dissolve') || normalized.includes('cross'))
        return 'dissolve';
    if (normalized.includes('fade') && normalized.includes('black'))
        return 'fade-black';
    if (normalized.includes('flash') && normalized.includes('white'))
        return 'flash-white';
    if (normalized.includes('flash') && normalized.includes('black'))
        return 'flash-black';
    if (normalized.includes('wipe') && normalized.includes('left'))
        return 'wipe-left';
    if (normalized.includes('wipe') && normalized.includes('right'))
        return 'wipe-right';
    if (normalized.includes('wipe') && normalized.includes('up'))
        return 'wipe-up';
    if (normalized.includes('wipe') && normalized.includes('down'))
        return 'wipe-down';
    return DEFAULT_TRANSITION_TYPE;
}
export function recommendTransition(clipA, clipB) {
...
export async function recommendTransitionSafe(clipA, clipB, t = identityTranslator) {
    try {
...
//# sourceMappingURL=ai-transition-recommend.js.map