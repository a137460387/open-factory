 ...
 */
export function buildDirectorModeMediaInfo(media) {
    return media.map((m) => ({
        mediaId: m.id,
        filename: m.name,
        type: m.type,
 ...
 */
export function splitDirectorModeMediaBatches(mediaInfo, maxBatch = DIRECTOR_MODE_MAX_BATCH) {
...
export function buildDirectorModeSystemPrompt(style, addMarkers, addMusicPlaceholder) {
    const styleMap = {
        energetic: '节奏明快',
        calm: '舒缓叙事',
        documentary: '纪录片',
...
export function buildDirectorModeUserPrompt(description, targetDuration, mediaInfo) {
...
export function parseDirectorModeResponse(json) {
    const empty = { segments: [], markers: [], musicTrackPlaceholder: false };
    ...
    if (Array.isArray(input.segments)) {
        for (const item of input.segments) {
            if (!item || typeof item !== 'object')
                continue;
            const entry = item;
            if (typeof entry.mediaId !== 'string' || typeof entry.duration !== 'number')
                continue;
    ...
    if (Array.isArray(input.markers)) {
        for (const item of input.markers) {
            if (!item || typeof item !== 'object')
                continue;
            const entry = item;
            if (typeof entry.time !== 'number' || typeof entry.label !== 'string')
                continue;
 ...
 */
export function validateDirectorModeTotalDuration(segments, targetDuration) {
 ...
 */
export function buildDirectorModeStoryboardCards(plan, mediaById) {
...
//# sourceMappingURL=director-mode.js.map