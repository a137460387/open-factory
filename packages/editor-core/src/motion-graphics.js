...
export const MOTION_GRAPHIC_CHART_KINDS = ['bar', 'line', 'pie'];
const DEFAULT_DATA_VALUES = [38, 64, 46, 82, 58];
export const MOTION_GRAPHIC_TEMPLATE_DEFINITIONS = {
    scoreboard: {
        type: 'scoreboard',
        params: [
...
export function isMotionGraphicTemplateType(value) {
    return MOTION_GRAPHIC_TEMPLATE_TYPES.includes(value);
}
export function getMotionGraphicTemplateDefinition(type) {
...
export function createDefaultMotionGraphic(templateType = 'countdown') {
    return normalizeMotionGraphic({ version: 1, templateType, params: {} });
}
export function normalizeMotionGraphic(input, duration = Number.POSITIVE_INFINITY) {
...
export function getMotionGraphicNumericParamKeys(graphic) {
...
export function setMotionGraphicParam(graphic, key, value, duration = Number.POSITIVE_INFINITY) {
...
export function setMotionGraphicParamKeyframe(graphic, key, input, duration) {
    const normalized = normalizeMotionGraphic(graphic, duration);
    const definition = getMotionGraphicTemplateDefinition(normalized.templateType);
    const param = definition.params.find((item) => item.key === key);
...
export function getMotionGraphicParamValueAtTime(graphic, key, time, duration = Number.POSITIVE_INFINITY) {
    const normalized = normalizeMotionGraphic(graphic, duration);
    const definition = getMotionGraphicTemplateDefinition(normalized.templateType);
    const param = definition.params.find((item) => item.key === key);
    if (!param) {
...
export function buildMotionGraphicTemplateFile(graphic, options = {}) {
...
export function serializeMotionGraphicTemplate(graphic, options = {}) {
    return `${JSON.stringify(buildMotionGraphicTemplateFile(graphic, options), null, 2)}\n`;
}
export function parseMotionGraphicTemplate(contents) {
    let parsed;
    try {
...
function normalizeMotionGraphicParamValue(param, value) {
...
function normalizeMotionGraphicParamKeyframes(input, definition, params, duration) {
...
function normalizeMotionGraphicKeyframe(input, param, duration) {
...
//# sourceMappingURL=motion-graphics.js.map