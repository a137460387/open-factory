...
import { DEFAULT_NESTED_SEQUENCE_NAME, createId, createNestedSequenceClip, createSequence, createTrack, normalizeAudioDenoise, normalizeChromaKey, normalizeClipPanoramaView, normalizeClipProjection, normalizeCollaborationNotes, normalizeColorCorrection, normalizeFrameInterpolation, normalizeMasks, normalizeMotionTrack, normalizeQualityEnhancement, normalizeSequenceFrameRate, normalizeSlowMotionMode, normalizeStabilization, normalizeTextPath, normalizeTimelineNotes, normalizeVideoRestoration, replaceProjectActiveTimeline } from '../../model';
...
export function buildRollingTrimClips(left, right, requestedDelta, minDuration) {
...
export function buildSlideClipEdit(timeline, clipId, requestedDelta, minDuration = 1 / 30) {
...
export function packNestedSequence(project, clipIds, sequenceName) {
    ...
    const locations = uniqueIds
        ...
        .sort((left, right) => {
        return ((trackIndexById.get(left.trackId) ?? 0) - (trackIndexById.get(right.trackId) ?? 0) ||
...
export function cutMulticamClip(project, clipId, sceneTime, angleId) {
    ...
    return replaceProjectActiveTimeline(syncedProject, replaceClip(timeline, {
        ...clip,
        multicam: {
            ...clip.multicam,
            switches,
...
export function trimMulticamClip(project, clipId, switchId, frameDelta, fps) {
    const syncedProject = replaceProjectActiveTimeline(project, project.timeline);
    const timeline = syncedProject.timeline;
    const clip = findClip(timeline, clipId);
    if (clip.type !== 'nested-sequence' || !clip.multicam) {
...
export function cloneClipForNestedSequence(clip) {
    const cloned = {
        ...clip,
    ...
    if (clip.type === 'text' || clip.type === 'subtitle') {
        ...
        return { ...cloned, style: { ...clip.style } };
    }
    return cloned;
}
...
export function sortReviewAnnotations(annotations) {
    return [...annotations].sort((left, right) => left.time - right.time || left.id.localeCompare(right.id));
}
export function sortCollaborationNotes(notes) {
    return normalizeCollaborationNotes(notes);
}
export function sortTimelineNotes(notes) {
...
/** 子剪辑操作命令 */
export function updateClipColorGradingGraph(project, clipId, updater) {
    const timeline = project.timeline;
...
/** 添加调色节点 */
//# sourceMappingURL=utils-nested.js.map