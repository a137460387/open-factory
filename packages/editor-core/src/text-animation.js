...
export const TEXT_ANIMATION_PRESETS = ['fade', 'fly-up', 'slide-left', 'typewriter', 'bounce', 'scale'];
export const TEXT_ANIMATION_DIRECTIONS = ['in', 'out', 'both'];
const TEXT_ANIMATION_PROPERTIES = ['opacity', 'x', 'y', 'scaleX', 'scaleY'];
const MIN_TEXT_ANIMATION_DURATION = 0.1;
const MAX_TEXT_ANIMATION_DURATION = 2;
export function normalizeTextAnimationDuration(duration) {
...
export function normalizeTextAnimationPreset(preset) {
    return TEXT_ANIMATION_PRESETS.includes(preset) ? preset : 'fade';
}
export function normalizeTextAnimationDirection(direction) {
    return TEXT_ANIMATION_DIRECTIONS.includes(direction)
        ? direction
        : 'in';
}
export function buildTextAnimationKeyframes(input) {
...
export function mergeTextAnimationKeyframes(existing, generated, clipDuration) {
...
function appendPresetFrames(output, preset, phase, start, end, clipDuration, base, text) {
    const isIn = phase === 'in';
    const yOffset = 0.25;
    const xOffset = 0.35;
    const minScale = 0.2;
    ...
    add(output, 'scaleY', clipDuration, preset, phase, scaleFrames.map(([time, value, easing]) => [time, value === base.scaleX ? base.scaleY : value, easing]));
}
function add(output, property, clipDuration, preset, phase, frames) {
    output[property] = [
        ...(output[property] ?? []),
        ...frames.map(([time, value, easing], index) => createKeyframe(property, { id: `text-animation-${preset}-${phase}-${property}-${index}`, time, value, easing }, clipDuration)),
    ];
}
function normalizeTextAnimationTransform(transform) {
