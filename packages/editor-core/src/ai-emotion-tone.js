...
export const VALID_EMOTION_TONES = ['energetic', 'calm', 'tense', 'happy', 'sad', 'neutral'];
// --- Core functions ---
/**
 * Build an AI prompt for emotion tone analysis of a clip.
 */
export function buildEmotionTonePrompt(sceneTag) {
    const lines = [
 ...
 */
export function parseEmotionToneResponse(json) {
    try {
        const obj = JSON.parse(json);
        if (typeof obj !== 'object' || obj === null)
            return null;
        if (typeof obj.emotionTone !== 'string')
            return null;
        if (!VALID_EMOTION_TONES.includes(obj.emotionTone))
            return null;
 ...
 */
export function getClipsNeedingEmotionAnalysis(project) {
    const result = [];
 ...
 */
export async function batchAnalyzeEmotionTones(clips, analyzeFn, maxConcurrency = 3, signal) {
...
//# sourceMappingURL=ai-emotion-tone.js.map