...
export const THUMBNAIL_PLATFORM_SIZES = {
...
export function buildThumbnailSampleTimestamps(duration, count = THUMBNAIL_SAMPLE_COUNT) {
...
export function normalizeThumbnailPlatformPreset(value) {
    return value === 'bilibili' || value === 'douyin' ? value : 'youtube';
}
export function getThumbnailPlatformSize(value) {
    return THUMBNAIL_PLATFORM_SIZES[normalizeThumbnailPlatformPreset(value)];
}
export function buildThumbnailExportSettings(value, crop = true) {
...
export function rankThumbnailCandidates(candidates, limit = THUMBNAIL_TOP_CANDIDATE_COUNT) {
    return [...candidates]
...
export function scoreThumbnailFrame(sample, neighbors = {}) {
...
export function buildThumbnailOutputFileStem(name) {
    const base = name
        .trim()
        .replace(/\.[^.\\/]+$/, '')
...
export function buildThumbnailOutputFileName(name) {
    return `${buildThumbnailOutputFileStem(name)}_thumb.jpg`;
}
export function buildThumbnailOutputPath(directory, sourceName) {
...
function laplacianVarianceScore(sample) {
...
function colorRichnessScore(sample) {
...
function lowMotionScore(sample, previous, next) {
...
//# sourceMappingURL=thumbnail-scoring.js.map