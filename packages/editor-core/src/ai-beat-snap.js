...
export function findNearestBeatBinarySearch(time, beatTimes) {
...
export function isWithinSnapTolerance(time, beatTime) {
    return Math.abs(time - beatTime) * 1000 < BEAT_SNAP_TOLERANCE_MS;
}
export function calculateBeatSnapForClips(clips, beatTimes) {
    const snappedClipIds = [];
    const suggestions = [];
...
export function applyBeatSnapToClip(clip, edge, suggestedTime) {
    if (edge === 'in') {
...
export function removeSuggestion(suggestions, clipId, edge) {
    return suggestions.filter((s) => !(s.clipId === clipId && s.edge === edge));
}
//# sourceMappingURL=ai-beat-snap.js.map