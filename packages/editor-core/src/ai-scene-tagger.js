...
// ─── Public API ──────────────────────────────────────────
export function generateAutoTags(asset, contentAnalysis, options) {
...
export function generateAutoTagsBatch(assets, metadata, contentAnalyses, options) {
    return assets.map((asset) => generateAutoTags(asset, contentAnalyses[asset.id], options));
}
export function mergeAutoTagsWithExisting(existing, autoTags) {
...
// ─── Tag Generation ──────────────────────────────────────
function generateSceneTypeTags(analysis) {
    const tags = [];
    ...
    for (const [type, count] of typeCounts) {
        if (type === analysis.primarySceneType)
            continue;
...
function generateMoodTags(analysis) {
    const tags = [];
    // Average brightness
...
function generateAudioTags(analysis) {
    const tags = [];
...
function generateMetadataTags(asset) {
    const tags = [];
...
//# sourceMappingURL=ai-scene-tagger.js.map