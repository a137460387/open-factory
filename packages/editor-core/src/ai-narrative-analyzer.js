 ...
 */
export function analyzeNarrative(segments, emotionCurve) {
 ...
 */
function identifyStructure(segments, curve) {
...
function buildAct(label, segments, start, end) {
 ...
 */
function buildArc(curve, structure) {
 ...
 * falling action.
 */
function computeTension(point, curve) {
 ...
 */
function computeScore(structure, arc, curve) {
    let score = 0;
 ...
 */
function generateSuggestions(structure, arc, score, curve) {
    const suggestions = [];
    // Check act balance
...
function findTroughIndex(curve) {
    let trough = 0;
    for (let i = 1; i < curve.length; i += 1) {
...
function fallbackSegments(curve) {
    if (curve.length === 0) {
        return [{ start: 0, end: 1, sceneTypes: ['indoor'], brightness: 0.5, motion: 0.1 }];
    }
...
function fallbackCurve(segments) {
...
//# sourceMappingURL=ai-narrative-analyzer.js.map