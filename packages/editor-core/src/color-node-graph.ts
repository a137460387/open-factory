...
import {
  getLogToRec709Lut,
  isLogInputColorSpace,
  normalizeInputColorSpace,
  REC709_INPUT_COLOR_SPACE,
...
export function normalizeColorNodeCorrection(
  correction: Partial<ColorNodeCorrection> | undefined,
): ColorNodeCorrection {
  return {
    inputColorSpace: normalizeInputColorSpace(correction?.inputColorSpace),
    brightness: round(
      Math.min(1, Math.max(-1, finiteOrDefault(correction?.brightness, DEFAULT_COLOR_NODE_CORRECTION.brightness))),
    ),
    contrast: round(
      Math.min(2, Math.max(0, finiteOrDefault(correction?.contrast, DEFAULT_COLOR_NODE_CORRECTION.contrast))),
    ),
    saturation: round(
      Math.min(2, Math.max(0, finiteOrDefault(correction?.saturation, DEFAULT_COLOR_NODE_CORRECTION.saturation))),
    ),
...
export function normalizeColorNodeGraph(
  graph: Partial<ColorNodeGraph> | undefined,
  fallbackCorrection?: Partial<ColorNodeCorrection>,
): ColorNodeGraph {
...
export function buildColorNodeGraphFilterPlan(
  graph: Partial<ColorNodeGraph> | undefined,
  options: BuildColorNodeGraphFilterPlanOptions,
): ColorNodeGraphFilterPlan {
  const normalized = normalizeColorNodeGraph(graph);
...
function node(
  ...
  x: number,
  y: number,
  correction?: Partial<ColorNodeCorrection>,
): ColorNode {
  return {
    id,
    type,
...
function mergeNodeInputs(
  ...
  if (options.mediaKind === 'audio') {
    filters.push(
      `${inputLabels.map((label) => `[${label}]`).join('')}amix=inputs=${inputLabels.length}:duration=longest:normalize=0[${outputLabel}]`,
    );
    return outputLabel;
  }
  if (node.type === 'layer') {
  ...
  for (let index = 1; index < inputLabels.length; index += 1) {
    const next = index === inputLabels.length - 1 ? outputLabel : `${outputLabel}_${index}`;
    filters.push(
      `[${current}][${inputLabels[index]}]blend=all_mode=${mode}:all_opacity=${formatFfmpegNumber(node.mix ?? 1)},format=rgba[${next}]`,
    );
    current = next;
  }
  return outputLabel;
...
function buildNodeFilters(node: ColorNode, options: BuildColorNodeGraphFilterPlanOptions): string[] {
  ...
  if (
    ...
    filters.push(
      `eq=brightness=${formatFfmpegNumber(correction.brightness)}:contrast=${formatFfmpegNumber(correction.contrast)}:saturation=${formatFfmpegNumber(correction.saturation)}`,
    );
  }
