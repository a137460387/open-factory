...
/** 按分类获取预设 */
export function getEasingPresetsByCategory(category) {
    return EASING_PRESETS.filter((p) => p.category === category);
}
/** 根据 ID 查找预设 */
export function getEasingPresetById(id) {
    return EASING_PRESETS.find((p) => p.id === id);
}
/** 获取预设的手柄配置（如果有的话） */
export function getPresetHandles(presetId) {
...
/** 判断预设是否为步进类型 */
export function isStepsPreset(presetId) {
    const preset = getEasingPresetById(presetId);
    return preset?.category === 'steps';
}
/** 获取步进预设的步数 */
export function getStepsCount(presetId) {
    const preset = getEasingPresetById(presetId);
    return preset?.steps ?? null;
}
//# sourceMappingURL=easing-presets.js.map