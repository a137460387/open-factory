 ...
 */
export function buildSceneMatchContext(clip, timelineClips, media) {
 ...
 */
export function buildSceneMatchMediaPayload(media, largeLibraryThreshold = 200) {
...
export function buildSceneMatchSystemPrompt() {
...
export function buildSceneMatchUserPrompt(context, mediaItems) {
    const parts = [`当前选中片段：${context.clipName}（类型：${context.clipType}）`];
    if (context.aiAnalysis) {
        const a = context.aiAnalysis;
        const analysisParts = [];
        if (a.tags && a.tags.length > 0)
            analysisParts.push(`标签：${a.tags.join(',')}`);
        if (a.scene)
            analysisParts.push(`场景：${a.scene}`);
        if (a.mood)
            analysisParts.push(`氛围：${a.mood}`);
        if (a.objects && a.objects.length > 0)
            analysisParts.push(`物体：${a.objects.join(',')}`);
        if (analysisParts.length > 0)
            parts.push(`内容分析：${analysisParts.join('；')}`);
    }
    else {
        parts.push('内容分析：无（将基于文件名推断）');
    }
    if (context.prevScene)
        parts.push(`前一片段场景：${context.prevScene}`);
    if (context.nextScene)
        parts.push(`后一片段场景：${context.nextScene}`);
    const mediaInfo = mediaItems
        ...
        if (analysis) {
            if (analysis.tags && analysis.tags.length > 0)
                mParts.push(`tags:${analysis.tags.join(',')}`);
            if (analysis.scene)
                mParts.push(`scene:${analysis.scene}`);
            if (analysis.mood)
                mParts.push(`mood:${analysis.mood}`);
            if (analysis.objects && analysis.objects.length > 0)
                mParts.push(`objects:${analysis.objects.join(',')}`);
        }
        return mParts.join(' | ');
    })
        .join('\n');
...
export function parseSceneMatchResponse(json) {
    const empty = { similar: [], contrast: [] };
 ...
 */
export function getUnanalyzedMediaIdsForSceneMatch(allMedia, resultIds) {
    return allMedia.filter((m) => !m.aiAnalysis && !resultIds.has(m.id)).map((m) => m.id);
}
export async function parseSceneMatchResponseSafe(json, t = identityTranslator) {
    try {
...
//# sourceMappingURL=ai-scene-match.js.map