 ...
 * @returns 生成的片段和转场数组
 */
export function generateSwitchSegments(angles, switchPoints, totalDuration, options = {}) {
    if (angles.length === 0 || totalDuration <= 0) {
        return { segments: [], transitions: [] };
    }
    const transitionDuration = Math.min(options.defaultTransitionDuration ?? DEFAULT_TRANSITION_DURATION, options.maxTransitionDuration ?? MAX_TRANSITION_DURATION);
    ...
    // 生成片段
    const segments = mergedSegments.map((seg, index) => {
 ...
 * @returns 新增的切换点
 */
export function generateRealtimeSwitch(currentTime, currentAngleIndex, targetAngleIndex, angles, remainingDuration) {
    if (targetAngleIndex === currentAngleIndex)
        return undefined;
    if (targetAngleIndex < 0 || targetAngleIndex >= angles.length)
        return undefined;
    if (currentTime < 0)
        return undefined;
    return {
 ...
 * 验证切换点数组的有效性
 */
export function validateSwitchPoints(switchPoints, angleCount, totalDuration) {
    const errors = [];
    if (angleCount <= 0) {
 ...
 * 计算切换点之间的最小间隔警告
 */
export function findSwitchIntervalWarnings(switchPoints, fps = 30, minFrames = 12) {
    const warnings = [];
...
//# sourceMappingURL=switch-generator.js.map