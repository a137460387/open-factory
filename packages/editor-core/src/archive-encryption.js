...
export function stripArchiveMetadata(info) {
    return { fileCount: info.fileCount, totalSizeBytes: info.totalSizeBytes };
}
export function calculateVolumeSplits(totalSizeBytes, volumeSizeMB) {
    if (volumeSizeMB <= 0)
        return [{ volumeIndex: 1, sizeBytes: totalSizeBytes }];
...
export function buildArchiveManifest(info, hideMetadata) {
    if (hideMetadata) {
        return { fileCount: info.fileCount, totalSizeBytes: info.totalSizeBytes };
    }
    return {
...
export function formatVolumeName(baseName, volumeIndex, totalVolumes) {
    if (totalVolumes <= 1)
        return baseName;
...
//# sourceMappingURL=archive-encryption.js.map