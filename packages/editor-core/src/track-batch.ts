...
export function resolveTrackHeaderSelection(input: TrackSelectionInput): TrackSelectionResult {
  ...
  if (!ordered.includes(input.clickedTrackId)) {
    return { selectedTrackIds: input.currentSelection.filter((id) => ordered.includes(id)), anchorTrackId: input.anchorTrackId ?? input.clickedTrackId };
  }
  if (!input.shiftKey) {
    return { selectedTrackIds: [input.clickedTrackId], anchorTrackId: input.clickedTrackId };
  }
  const anchor = input.anchorTrackId && ordered.includes(input.anchorTrackId) ? input.anchorTrackId : input.currentSelection.find((id) => ordered.includes(id)) ?? input.clickedTrackId;
...
export function moveSelectedTrackIds(orderedTrackIds: string[], selectedTrackIds: string[], draggedTrackId: string, targetTrackId: string): string[] {
