 ...
 */
export function calculateJaccardSimilarity(a, b) {
 ...
 */
export function clusterCharactersInClip(frames, clipId) {
    if (frames.length === 0)
        return [];
    const clusters = [];
    let nextId = 0;
 ...
 */
export function matchCharactersAcrossClips(clipClusters) {
    const characters = {};
    let nextCharId = 1;
 ...
 */
function collectCharacterTags(charId, clipClusters, assignments) {
    const tags = [];
    for (const clip of clipClusters) {
        for (const cluster of clip.characters) {
            const key = `${clip.clipId}:${cluster.id}`;
 ...
 */
export function calculateFrameSampleTimes(clipDuration) {
    if (clipDuration <= 0)
        return [0];
    const times = [];
 ...
 */
export function buildCharacterDetectionPrompt(sampleTimes) {
    const lines = [
 ...
 */
export function parseCharacterDetectionResponse(json) {
    try {
        const parsed = JSON.parse(json);
        if (!Array.isArray(parsed.frames))
            return null;
        for (const frame of parsed.frames) {
            if (typeof frame.time !== 'number' || !Array.isArray(frame.characters))
                return null;
            for (const ch of frame.characters) {
                if (!Array.isArray(ch.descriptorTags) || !ch.box)
                    return null;
            }
        }
        return parsed;
 ...
 */
export function renameCharacter(timeline, characterId, newLabel) {
    if (!timeline.characters[characterId])
        return timeline;
    return {
        ...timeline,
        characters: {
...
//# sourceMappingURL=ai-character-timeline.js.map