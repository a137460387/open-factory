 ...
 */
export function analyzeEmotion(visualSamples, audioSamples, options = {}) {
...
function mapVisualToEmotion(sample) {
...
function mapAudioToEmotion(sample) {
...
// ─── Emotion Fusion ────────────────────────────────────────
function fuseEmotions(visualEmotions, audioEmotions, audioWeight, visualWeight) {
    ...
    for (const v of visualEmotions)
        allTimes.add(v.time);
    for (const a of audioEmotions)
        allTimes.add(a.time);
...
function findClosestPoint(points, time) {
    let closest;
...
// ─── Peak Detection ────────────────────────────────────────
function detectPeaks(curve, threshold) {
    if (curve.length < 3)
        return [];
    const peaks = [];
...
function determineOverallMood(curve) {
    if (curve.length === 0)
        return 'neutral';
    ...
    if (avgValue > 0.3 && avgArousal > 0.5)
        return 'energetic';
    if (avgValue > 0.3 && avgArousal <= 0.5)
        return 'happy';
    if (avgValue < -0.3 && avgArousal > 0.5)
...
function determineEmotionalArc(curve, peaks) {
    if (curve.length < 2)
        return 'stable';
    ...
    if (hasPositivePeak && !hasNegativePeak)
        return 'peak';
    if (hasNegativePeak && !hasPositivePeak)
        return 'valley';
    return 'stable';
}
// ─── Utility ───────────────────────────────────────────────
//# sourceMappingURL=ai-emotion-analyzer.js.map