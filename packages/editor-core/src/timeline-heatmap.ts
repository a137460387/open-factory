...
export function calculateEditDensityHeatmap(timeline: Timeline, options: TimelineHeatmapOptions = {}): TimelineHeatmapSegment[] {
  ...
  const segments = buckets.map((bucket) => {
    const value = weightedClips.reduce((total, item) => (clipIntersectsBucket(item.clip, bucket.start, bucket.end) ? total + item.weight : total), 0);
    return { ...bucket, value };
  });
  return normalizeSegments(segments);
}

export function calculateVolumeHeatmap(timeline: Timeline, options: TimelineHeatmapOptions = {}): TimelineHeatmapSegment[] {
  const buckets = createTimelineBuckets(timeline, options);
  const audioClips = timeline.tracks
    .filter((track) => track.type === 'audio' || track.type === 'video')
    .flatMap((track) =>
      track.clips.filter((clip): clip is Extract<Clip, { type: 'audio' | 'video' }> => clip.type === 'audio' || clip.type === 'video').map((clip) => ({
        clip,
...
export function calculateCutFrequencyHeatmap(timeline: Timeline, options: TimelineHeatmapOptions = {}): TimelineHeatmapSegment[] {
  const buckets = createTimelineBuckets(timeline, options);
  const cuts = timeline.tracks.flatMap((track) => track.clips.map((clip) => clip.start));
  const segments = buckets.map((bucket) => {
    const value = cuts.filter((cut) => cut >= bucket.start && cut < bucket.end).length / Math.max(0.001, bucket.end - bucket.start);
    return { ...bucket, value: round(value) };
  });
  return normalizeSegments(segments);
}

export function calculateTimelineHeatmap(type: TimelineHeatmapType, timeline: Timeline, options: TimelineHeatmapOptions = {}): TimelineHeatmapSegment[] {
  if (type === 'volume') {
...
function createTimelineBuckets(timeline: Timeline, options: TimelineHeatmapOptions): Array<Pick<TimelineHeatmapSegment, 'start' | 'end'>> {
...
function normalizeSegments(segments: Array<Omit<TimelineHeatmapSegment, 'normalized'>>, fixedMax?: number): TimelineHeatmapSegment[] {
