 ...
 * - Export operations
 */
export const DEFAULT_COMMAND_PARSER_CONFIG = {
    language: 'zh',
    minConfidence: 0.3,
...
const ZH_PATTERNS = [
    ...
    },
    {
        ...
        extractors: [(m, text) => {
                ...
                if (speedMatch)
                    return { speed: parseFloat(speedMatch[1]) };
                if (/加速|快进/.test(text))
                    return { speed: 2 };
                if (/减速|慢放/.test(text))
                    return { speed: 0.5 };
    ...
    },
    {
        ...
        extractors: [(_m, text) => {
                ...
                if (volMatch)
                    return { volume: parseInt(volMatch[1], 10) };
                if (/大|高/.test(text))
                    return { volume: 80 };
                if (/小|低/.test(text))
                    return { volume: 30 };
 ...
 */
export function parseMultipleCommands(text, config = {}) {
 ...
 */
export function buildSpeechGrammarHints(language) {
    const zhHints = [
 ...
 */
export function commandNeedsTarget(type) {
    const needsTarget = [
...
//# sourceMappingURL=natural-language-commands.js.map