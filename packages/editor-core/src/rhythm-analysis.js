...
import { formatReportDuration, normalizeReportLocale, reportHtmlLang, reportLanguageLabel, } from './project/report-i18n';
export const RHYTHM_REFERENCE_PROFILES = [
...
export function analyzeClipRhythm(project, options = {}) {
...
export function collectRhythmShots(timeline) {
...
export function calculateCutFrequencyCurve(shots, bucketSeconds = 1) {
    if (shots.length === 0) {
        return [];
    }
...
export function detectRhythmChangePoints(shots) {
    const points = [];
...
export function detectRepeatedRhythmSegments(shots, minClipCount = 10, tolerance = 0.12) {
    const segments = [];
    let startIndex = 0;
    while (startIndex < shots.length) {
        let endIndex = startIndex + 1;
        while (endIndex < shots.length &&
...
export function serializeRhythmAnalysisJson(report) {
    return JSON.stringify(report, null, 2);
}
export function buildRhythmAnalysisHtml(report, localeInput) {
...
function renderChangeRows(report, locale) {
...
//# sourceMappingURL=rhythm-analysis.js.map