...
// ── 内置规格库（6 套） ──
export const BUILTIN_BROADCAST_SPECS = [
    {
        id: 'netflix-1080p',
        name: 'Netflix 1080p',
 ...
 * 获取所有内置规格 ID 列表。
 */
export function getBuiltinSpecIds() {
    return BUILTIN_BROADCAST_SPECS.map((s) => s.id);
}
/**
 * 根据 ID 获取内置规格。
 */
export function getBuiltinSpec(id) {
    return BUILTIN_BROADCAST_SPECS.find((s) => s.id === id);
}
/**
 * 对导出参数执行完整合规检查。
 */
export function checkCompliance(spec, params) {
    const results = [];
    if (spec.videoCodec)
        results.push(checkVideoCodec(spec, params));
    if (spec.videoBitrateMinMbps !== undefined || spec.videoBitrateMaxMbps !== undefined)
        results.push(checkVideoBitrate(spec, params));
    if (spec.width || spec.height)
        results.push(checkResolution(spec, params));
    if (spec.fps !== undefined)
        results.push(checkFps(spec, params));
    if (spec.audioCodec)
        results.push(checkAudioCodec(spec, params));
    if (spec.audioBitrateMinKbps !== undefined)
        results.push(checkAudioBitrate(spec, params));
    if (spec.loudnessTargetLufs !== undefined)
        results.push(checkLoudness(spec, params));
    if (spec.truePeakMaxDbtp !== undefined)
        results.push(checkTruePeak(spec, params));
    if (spec.maxDurationSec !== undefined)
        results.push(checkDuration(spec, params));
    if (spec.aspectRatio)
        results.push(checkAspectRatio(spec, params));
    return results;
}
/**
 * 从合规检查结果中提取一键修复建议。
 */
export function buildComplianceFix(spec, results) {
    const fix = {};
    ...
    if (codecResult?.autoFix) {
        fix.codecSuggestion = codecResult.autoFix.params['suggestedCodec'];
    }
    const durationResult = results.find((r) => r.autoFix?.type === 'duration-notify');
    if (durationResult?.autoFix) {
        fix.durationOverflowSec = durationResult.autoFix.params['overflowSec'];
    }
    return fix;
}
/**
 * 计算时长超出限制的秒数（0 表示未超出）。
 */
export function calculateDurationOverflowSec(duration, maxDuration) {
    if (maxDuration <= 0)
        return 0;
    return round(Math.max(0, duration - maxDuration));
}
// ── 内部检查函数 ──
function checkVideoCodec(spec, params) {
...
function checkVideoBitrate(spec, params) {
...
function checkResolution(spec, params) {
...
function checkAudioCodec(spec, params) {
...
function checkAudioBitrate(spec, params) {
...
function checkLoudness(spec, params) {
    ...
    const level = diff <= tol * 2 ? 'warn' : 'fail';
    return {
        name: '响度',
        level,
...
function checkTruePeak(spec, params) {
...
function checkDuration(spec, params) {
...
function checkAspectRatio(spec, params) {
    const w = params.width ?? 0;
    const h = params.height ?? 0;
...
//# sourceMappingURL=broadcast-compliance.js.map