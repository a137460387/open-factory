 ...
 * 对比两个时间线快照，生成结构化diff。
 */
export function diffVersionSnapshots(from, to) {
    const items = [];
    // 轨道数量变化
    ...
    // 收集所有clip id
    const fromClips = new Map();
    const toClips = new Map();
 ...
 */
export function serializeDiffForAi(items) {
 ...
 * 解析AI返回的版本对比摘要响应。
 */
export function parseVersionDiffAiResponse(json) {
    ...
    const highlights = Array.isArray(obj.highlights)
        ? obj.highlights.filter((h) => typeof h === 'string')
        : [];
 ...
 */
export function createVersionDiffSummary(fromId, toId, items, aiResponse) {
    return {
 ...
 */
export function normalizeVersionDiffSummary(input) {
    if (!input || typeof input !== 'object')
        return undefined;
    const obj = input;
    if (typeof obj.fromSnapshotId !== 'string' || typeof obj.toSnapshotId !== 'string')
        return undefined;
    return {
        fromSnapshotId: obj.fromSnapshotId,
        toSnapshotId: obj.toSnapshotId,
        diff: Array.isArray(obj.diff) ? obj.diff : [],
        aiSummary: typeof obj.aiSummary === 'string' ? obj.aiSummary : '',
        generatedAt: typeof obj.generatedAt === 'string' ? obj.generatedAt : '',
    };
}
//# sourceMappingURL=ai-version-diff.js.map