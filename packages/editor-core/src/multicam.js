import { DEFAULT_CLIP_SPEED, DEFAULT_COLOR_CORRECTION, DEFAULT_TRANSFORM, DEFAULT_NESTED_SEQUENCE_NAME, createId, createNestedSequenceClip, createSequence, createTrack, getProjectActiveSequenceId, getProjectSequences, normalizeAudioDenoise, normalizeChromaKey, normalizeMotionTrack, normalizeMulticamSequence, normalizeSlowMotionMode, replaceProjectActiveTimeline, } from './model';
...
export function calculateAudioAlignmentOffset(reference, candidate, sampleRate, maxOffsetSeconds = 5) {
...
export function parseLtcTimecode(value, fps = 30) {
...
export function calculateTimecodeAlignmentOffsets(points, referenceClipId, fps = 30) {
...
export function calculateManualMarkerAlignmentOffsets(points, referenceClipId) {
...
export function setMulticamSwitch(multicam, time, angleId, duration) {
...
export function getActiveMulticamAngle(multicam, time) {
...
export function buildMulticamSwitchHistory(multicam, duration, fps = 30) {
    const normalized = normalizeMulticamSequence(multicam, duration);
    if (!normalized) {
        return [];
    }
...
export function serializeMulticamSwitchHistory(multicam, duration, fps = 30) {
...
export function findFrequentMulticamSwitchWarnings(multicam, duration, fps = 30, minFrames = 12) {
    const normalized = normalizeMulticamSequence(multicam, duration);
    if (!normalized) {
        return [];
    }
...
export function trimMulticamSwitch(multicam, switchId, frameDelta, fps, duration) {
    const normalized = normalizeMulticamSequence(multicam, duration);
    if (!normalized) {
        throw new Error('Invalid multicam sequence');
    }
...
export function createMulticamSequenceProject(project, clipIds, options = {}) {
    ...
    const nextTimeline = {
        ...
        tracks: activeTimeline.tracks.map((track) => {
            ...
            const nextClips = track.id === targetTrack.id
...
export function flattenMulticamProjectForExport(project) {
    const flattenedTimeline = flattenMulticamTimeline(project.timeline, project);
    const flattenedSequences = getProjectSequences(project).map((sequence) => sequence.id === getProjectActiveSequenceId(project)
...
function findVisualClipLocation(timeline, clipId) {
...
function cloneAngleClip(clip, trackId, start) {
    return {
        ...clip,
        id: createId('clip'),
        trackId,
...
function flattenMulticamTimeline(timeline, project) {
    let changed = false;
    ...
    const removedClipIds = new Set(timeline.tracks.flatMap((track) => track.clips.filter((clip) => clip.type === 'nested-sequence' && clip.multicam).map((clip) => clip.id)));
    return {
        ...timeline,
        tracks,
...
function flattenMulticamClip(clip, project) {
...
function cloneAngleSegment(source, parent, localStart, localEnd, index) {
 ...
 */
export function getActiveAngleAtTime(multicamClip, time) {
 ...
 */
export function addSwitchPoint(switchPoints, switchPoint) {
 ...
 */
export function deleteSwitchPoint(switchPoints, index) {
 ...
 * Throws if the index is out of range.
 */
export function updateSwitchPoint(switchPoints, index, updates) {
    if (index < 0 || index >= switchPoints.length) {
        throw new Error('Switch point index out of range');
    }
    const result = [...switchPoints];
...
//# sourceMappingURL=multicam.js.map