 ...
 */
export function aggregatePreflightIssues(project) {
    const issues = [];
 ...
 */
export function buildPreflightAIPrompt(issues) {
    if (issues.length === 0)
        return '项目无任何警告或问题，可以直接导出。';
    const lines = [
 ...
 */
export function parsePreflightAIResponse(json) {
    try {
        const parsed = JSON.parse(json);
        if (typeof parsed.summary !== 'string')
            return null;
        if (typeof parsed.criticalCount !== 'number' || typeof parsed.warningCount !== 'number')
            return null;
 ...
 */
export function acknowledgePreflightIssue(report, issueId) {
    if (report.acknowledgedIssueIds.includes(issueId))
        return report;
    return {
        ...report,
...
//# sourceMappingURL=ai-preflight-checklist.js.map