import { DEFAULT_CLIP_SPEED, DEFAULT_COLOR_CORRECTION, DEFAULT_TRANSFORM, DEFAULT_AUDIO_FADE_CURVE, createId, } from './model';
import { getClipSourceVisibleDuration, getClipSpeed } from './timeline';
import { round } from './time';
export function buildDialogueRoughCutClips(sourceClip, intervals) {
...
export function scoreBrollKeywordMatch(main, candidate) {
...
export function buildBrollInsertClips(mainClips, candidates, targetTrackId) {
...
export function buildRhythmAssembleClips(videoClips, beatTimes, targetTrackId) {
...
function normalizeDialogueIntervals(intervals, maxDuration) {
...
function chooseBestBrollCandidate(mainClip, candidates, offset) {
    return candidates
        .map((candidate, index) => ({
        candidate,
        index,
        ...
        ((left.index + offset) % candidates.length) - ((right.index + offset) % candidates.length))[0]?.candidate;
}
function buildBrollClipFromCandidate(mainClip, candidate, targetTrackId, index) {
...
export function createVisualClipFromAsset(asset, input) {
...
function cloneMediaClipSegment(source, patch) {
...
function cloneVisualClipSegment(source, patch) {
...
function calculateTrimEnd(source, nextTrimStart, nextDuration) {
...
function getCandidateKeywordSource(candidate) {
    if (candidate.kind === 'clip') {
        return {
...
function extractSceneTags(source) {
    return dedupe([
        source.type,
...
function extractKeywordTokens(source) {
    return dedupe([
        ...(source.keywords ?? []),
...
function scoreSetOverlap(left, right) {
...
function cloneClipValue(value) {
    return globalThis.structuredClone ? globalThis.structuredClone(value) : JSON.parse(JSON.stringify(value));
}
export function estimateBpmFromTimes(beatTimes) {
...
export function buildSmartMontageClips(config) {
    ...
    if (visualAssets.length === 0 || beats.length < 2 || !videoTrackId || !audioTrackId)
        return null;
    const orderedAssets = strategy === 'random' ? shuffleArray([...visualAssets]) : [...visualAssets];
    const visualClips = beats.slice(0, -1).flatMap((start, index) => {
        const end = beats[index + 1];
        const duration = round(end - start);
        if (duration <= 0.000001)
            return [];
...
//# sourceMappingURL=smart-rough-cut-v2.js.map