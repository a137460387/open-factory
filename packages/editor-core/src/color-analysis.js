import { buildColorMatchCurves } from './color-match';
import { clamp, round } from './time';
const DEFAULT_SCENE_DIFFERENCE_THRESHOLDS = {
    score: 0.35,
...
export function analyzeColorFrameSample(sample) {
    let pixelCount = 0;
    let brightnessSum = 0;
...
export function estimateColorTemperatureKelvin(meanRgb) {
    const linear = {
...
export function buildTimelineColorHeatmapData(results) {
    return [...results]
...
export function detectSceneColorJumps(results, thresholds = {}) {
    ...
    for (let index = 1; index < ordered.length; index += 1) {
        ...
        if (difference.score >= resolved.score ||
...
export function buildColorAlignmentUpdates(samples, referenceClipId) {
...
function calculateSceneColorDifference(left, right) {
    ...
    const score = round(clamp(temperatureDelta / 6000, 0, 1) * 0.34 +
...
function calculateHsvSaturation(r, g, b) {
    const max = Math.max(r, g, b);
    const min = Math.min(r, g, b);
    return max <= 0 ? 0 : (max - min) / max;
}
function calculateChromaMeans(r, g, b) {
    return {
        cb: 128 - 0.168736 * r - 0.331264 * g + 0.5 * b,
        cr: 128 + 0.5 * r - 0.418688 * g - 0.081312 * b,
    };
}
function classifyTintBias(cbMean, crMean) {
...
function colorTemperatureToHeatmapColor(kelvin) {
...
//# sourceMappingURL=color-analysis.js.map