...
export function calculateEditDensityHeatmap(timeline, options = {}) {
    const buckets = createTimelineBuckets(timeline, options);
    const weightedClips = timeline.tracks.flatMap((track) => track.clips.map((clip) => ({
        clip,
...
export function calculateVolumeHeatmap(timeline, options = {}) {
    const buckets = createTimelineBuckets(timeline, options);
...
export function calculateCutFrequencyHeatmap(timeline, options = {}) {
    const buckets = createTimelineBuckets(timeline, options);
...
export function calculateTimelineHeatmap(type, timeline, options = {}) {
    if (type === 'volume') {
...
function createTimelineBuckets(timeline, options) {
...
function clipIntersectsBucket(clip, start, end) {
    return clip.start < end && clip.start + clip.duration > start;
}
function normalizeSegments(segments, fixedMax) {
...
//# sourceMappingURL=timeline-heatmap.js.map