...
export const DEFAULT_BOOKMARK_GROUP = {
    id: '__default__',
    name: '未分组',
    color: '#64748b',
    collapsed: false,
...
export function normalizeBookmarkAnnotationText(text) {
...
function sortBookmarks(bookmarks, mode, groups) {
...
export function groupBookmarks(bookmarks, groups) {
...
export function calculateBookmarkNavDots(bookmarks, totalDuration, containerWidth, groups) {
...
export function captureBookmarkThumbnail(bookmark, thumbnailPath) {
    return { ...bookmark, thumbnailPath };
}
export function applyBookmarkGroupCollapseState(groups, groupId, collapsed) {
    return groups.map((group) => (group.id === groupId ? { ...group, collapsed } : group));
}
export function serializeBookmarkGroups(groups) {
    return JSON.stringify(groups, null, 2) + '\n';
}
export function parseBookmarkGroupsJson(json) {
    const parsed = JSON.parse(json);
    if (!Array.isArray(parsed)) {
        return [];
    }
    return parsed.map((item) => normalizeBookmarkGroup(item));
}
export function normalizeBookmarkGroupColor(color) {
    const match = /^#?[a-fA-F0-9]{6}$/.exec(color?.trim() ?? '');
    return match ? `#${match[0].replace('#', '')}` : BOOKMARK_GROUP_DEFAULT_COLORS[0];
}
function normalizeBookmarkGroup(raw) {
    return {
...
//# sourceMappingURL=timeline-bookmark-enhancements.js.map