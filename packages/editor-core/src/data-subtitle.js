...
const SOURCE_TYPES = ['csv', 'json', 'template'];
const DEFAULT_TEMPLATE = '{row.text}';
export function parseDataSubtitleRows(contents, sourceType) {
    return sourceType === 'json' ? parseDataSubtitleJsonRows(contents) : parseDataSubtitleCsvRows(contents);
}
export function parseDataSubtitleCsvRows(contents) {
    ...
    return (normalizeDataSubtitleRows(rows.slice(1).map((row) => {
        const values = {};
...
export function parseDataSubtitleJsonRows(contents) {
    const value = JSON.parse(contents);
    if (!Array.isArray(value)) {
        throw new Error('Data subtitle JSON must be an array');
    }
    return (normalizeDataSubtitleRows(value.map((entry, index) => {
...
export function normalizeDataSubtitleSource(input) {
    if (!input || typeof input !== 'object') {
        return undefined;
    }
    const value = input;
    const sourceType = SOURCE_TYPES.includes(value.sourceType)
        ? value.sourceType
        : 'template';
...
export function normalizeDataSubtitleRows(input) {
...
export function findDataSubtitleRowAtTime(rows, time) {
...
export function expandDataSubtitleTemplate(template, row, time, context = {}) {
...
export function resolveDataSubtitleText(source, time, context = {}) {
...
export function expandDataSubtitleClipToCueInputs(clip, context = {}) {
...
function parseDataSubtitleTime(value) {
...
function formatDataSubtitleTimecode(time, fps) {
...
//# sourceMappingURL=data-subtitle.js.map