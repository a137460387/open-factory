export const NARRATION_STYLES = ['commentary', 'advertisement', 'documentary', 'social-media'];
...
export function estimateWordCount(durationSeconds, isChinese) {
...
export function buildNarrationSystemPrompt(style, isChinese) {
...
export function buildNarrationUserPrompt(chapters) {
    const lines = [];
    ...
    for (let i = 0; i < chapters.length; i++) {
        ...
        if (ch.label)
            lines.push('  标签: ' + ch.label);
        if (ch.sceneDescription)
            lines.push('  场景描述: ' + ch.sceneDescription);
        if (ch.subtitleText)
            lines.push('  字幕片段: ' + ch.subtitleText);
        lines.push('');
    }
    return lines.join('\n');
}
export function buildChaptersFromMarkers(markers, totalDuration, sceneDescriptions, subtitleTextMap) {
...
export function parseNarrationResponse(json) {
    ...
    for (const item of json) {
        if (!item || typeof item !== 'object')
            continue;
        const record = item;
        if (typeof record.markerTime !== 'number' || typeof record.duration !== 'number')
            continue;
...
export function buildTtsRequests(segments, voiceId) {
    return segments
        .filter((s) => s.text.trim().length > 0)
        .map((s) => ({ text: s.text, markerTime: s.markerTime, voiceId }));
}
function formatNarrationTimecode(seconds) {
...
//# sourceMappingURL=ai-narration.js.map