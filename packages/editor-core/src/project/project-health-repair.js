...
export function buildProjectHealthSearchRoots(project, recentDirectories = []) {
    const roots = [];
...
export function planMissingMediaAutoRelinks(project, report, candidatePaths, roots, minScore = 0.35) {
    ...
    for (const issue of report.missingMedia) {
        ...
        const best = candidates
            ...
            left.candidate.path.localeCompare(right.candidate.path))[0];
        if (!best) {
            manualEntries.push({
                type: 'missing-media',
                status: 'manual',
...
export function applyProjectHealthAutoRepair(project, input, now = new Date().toISOString()) {
...
export function summarizeProjectHealthRepair(entries) {
    return {
...
function addRoot(roots, path, kind, priority) {
    const normalized = normalizePath(path);
...
function findCandidateRoot(path, roots) {
...
function mergeMediaReferences(project, keepAssetId, removeIds, now) {
...
function replaceTimelineMediaReferences(timeline, keepAssetId, removeIds) {
    return {
        ...timeline,
        tracks: timeline.tracks.map((track) => ({
            ...track,
            clips: track.clips.map((clip) => 'mediaId' in clip && removeIds.has(clip.mediaId) ? { ...clip, mediaId: keepAssetId } : clip),
        })),
    };
}
function filterMediaMetadata(metadata, removeIds) {
    return Object.fromEntries(Object.entries(metadata).filter(([assetId]) => !removeIds.has(assetId)));
}
//# sourceMappingURL=project-health-repair.js.map