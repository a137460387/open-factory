...
import { formatReportDuration, normalizeReportLocale, reportHtmlLang, reportLanguageLabel, } from './report-i18n';
export function collectOfflineMediaReportPaths(project) {
    const paths = new Map();
...
export function buildOfflineMediaReport(project, fileStatuses = [], options = {}) {
    ...
    const rows = project.media.flatMap((asset) => collectAssetReportPaths(asset).map((path) => {
...
export function buildMediaUsageStats(project) {
    ...
    for (const sequence of getProjectSequences(project)) {
        for (const track of sequence.timeline.tracks) {
            for (const clip of track.clips) {
                ...
                const current = stats.get(mediaId) ??
                    {
                        assetId: asset.id,
                        assetName: asset.name,
                        assetType: asset.type,
...
export function buildTimelineHeatmapData(project, bucketCount = 24) {
...
export function buildProjectArchivePreflight(project, fileStatuses = []) {
...
export function renderOfflineMediaReportHtml(report) {
...
export function buildOfflineMediaReportHtml(project, fileStatuses = [], options = {}) {
    return renderOfflineMediaReportHtml(buildOfflineMediaReport(project, fileStatuses, options));
}
function collectAssetReportPaths(asset) {
    const paths = new Map();
    addPath(paths, asset.path);
    for (const framePath of asset.imageSequence?.paths ?? []) {
        addPath(paths, framePath);
...
function estimateDefaultExportSizeBytes(project, durationSeconds) {
...
function renderUsageSegmentList(segments, locale) {
    const labels = mediaReportLabels[locale];
    if (segments.length === 0) {
        return labels.notUsed;
    }
    return `<ul class="usage-list">${segments
        .map((segment) => `<li>${escapeHtml(segment.sequenceName)} / ${escapeHtml(segment.trackName)} / ${escapeHtml(segment.clipName)}：${formatReportDuration(segment.start, locale)} - ${formatReportDuration(segment.end, locale)}（${formatReportDuration(segment.duration, locale)}）</li>`)
        .join('')}</ul>`;
}
function renderHeatmap(buckets, locale) {
    const labels = mediaReportLabels[locale];
    if (buckets.length === 0) {
...
function renderUnusedMediaList(rows, locale) {
    const labels = mediaReportLabels[locale];
    if (rows.length === 0) {
        return `<div class="meta">${labels.emptyUnused}</div>`;
    }
    return `<ul class="unused-list">${rows
        .map((row) => `<li data-media-id="${escapeHtml(row.assetId)}"><span>${escapeHtml(row.assetName)} <code>${escapeHtml(row.path)}</code></span><button type="button" onclick="this.closest('li').remove()">${labels.removeFromBin}</button></li>`)
        .join('')}</ul>`;
}
function renderDurationPieChart(stats, locale) {
    const labels = mediaReportLabels[locale];
...
//# sourceMappingURL=media-report.js.map