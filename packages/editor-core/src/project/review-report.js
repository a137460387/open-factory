...
export function buildReviewReport(project, options = {}) {
...
export function buildReviewReportHtml(project, options = {}) {
    return renderReviewReportHtml(buildReviewReport(project, options));
}
export function renderReviewReportHtml(report) {
...
function renderAnnotationRows(rows, locale) {
    const labels = reviewReportLabels[locale];
    if (rows.length === 0) {
        return `<tr><td colspan="6" class="empty">${labels.empty}</td></tr>`;
    }
    return rows
        .map((row) => `<tr data-review-annotation-id="${escapeHtml(row.id)}">
        <td>${row.index}</td>
...
function renderShotSvg(row, locale) {
    const labels = reviewReportLabels[locale];
...
function formatAnnotationType(type, locale) {
    const labels = reviewReportLabels[locale];
    if (type === 'arrow') {
        return labels.arrow;
    }
