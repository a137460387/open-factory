...
import { formatReportDuration, formatReportNumber, normalizeReportLocale, reportHtmlLang, reportLanguageLabel, } from './report-i18n';
const clipReportLabels = {
    zh: {
        title: '剪辑报告',
        generatedAt: '生成时间',
        language: '语言',
...
export function buildClipReport(project, options = {}) {
...
export function buildClipReportHtml(project, options = {}) {
    return renderClipReportHtml(buildClipReport(project, options));
}
export function renderClipReportHtml(report) {
...
function clipToReportRow(clip, track, index) {
    return {
        index,
        clipId: clip.id,
        name: clip.name,
...
function buildClipReportMediaRows(project) {
...
function buildClipReportSubtitleRows(tracks) {
    return tracks.flatMap((track) => track.clips.flatMap((clip) => clip.type === 'subtitle'
        ? [
            {
                clipId: clip.id,
                text: clip.text,
...
function buildClipReportMarkerRows(markers) {
    return markers
        .map((marker) => ({
        markerId: marker.id,
        name: marker.label,
...
function assetToMediaRow(asset) {
    return {
        mediaId: asset.id,
...
function renderMediaRows(rows, locale) {
    const labels = clipReportLabels[locale];
    if (rows.length === 0) {
        return `<tr><td colspan="5" class="empty">${labels.emptyMedia}</td></tr>`;
    }
    return rows
        .map((row) => `<tr data-media-id="${escapeHtml(row.mediaId)}">
...
function renderSubtitleRows(rows, locale) {
    const labels = clipReportLabels[locale];
    if (rows.length === 0) {
...
function renderMarkerRows(rows, locale) {
    const labels = clipReportLabels[locale];
    if (rows.length === 0) {
        return `<tr><td colspan="3" class="empty">${labels.emptyMarkers}</td></tr>`;
    }
    return rows
        .map((row) => `<tr data-marker-id="${escapeHtml(row.markerId)}">
        <td>${escapeHtml(row.name)}</td>
...
//# sourceMappingURL=clip-report.js.map