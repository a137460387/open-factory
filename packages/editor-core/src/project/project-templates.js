import { createProject, createTrack, PRIMARY_SEQUENCE_ID, DEFAULT_PRIMARY_SEQUENCE_NAME, } from '../model';
...
export function detectMediaFeatures(media) {
...
export function suggestTrackCount(mediaCount, template) {
...
export function recommendTemplate(media) {
    ...
    for (const template of PROJECT_TEMPLATES) {
        ...
        const templateAspect = template.settings.width > template.settings.height
            ? 'horizontal'
...
function buildReasonKey(summary) {
    if (summary.aspectClass === 'vertical')
        return 'verticalDetected';
    if (summary.aspectClass === 'square')
        return 'squareDetected';
    if (summary.aspectClass === 'unknown')
        return 'audioOnly';
    if (summary.avgWidth >= CINEMA_WIDE_THRESHOLD)
        return 'cinemaDetected';
    return 'horizontalDetected';
}
function buildReasonParams(summary) {
    return {
        count: summary.count,
...
export function buildRecommendationReason(recommendation, translations) {
...
export function instantiateProjectTemplate(id, options = {}) {
...
function createTemplateTrack(track) {
