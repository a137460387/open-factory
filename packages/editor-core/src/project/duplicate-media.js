...
export function detectDuplicateMediaGroups(signatures) {
    const bySignature = new Map();
    const fingerprintSignatures = [];
...
function buildFingerprintDuplicateGroups(signatures) {
    const groups = [];
    ...
    for (const signature of signatures) {
        ...
        const matches = signatures.filter((candidate) => !visited.has(candidate.assetId) &&
...
//# sourceMappingURL=duplicate-media.js.map