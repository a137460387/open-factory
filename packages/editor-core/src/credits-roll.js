...
export const DEFAULT_CREDITS_STYLE = {
    fontSize: 42,
    color: '#ffffff',
...
export function parseCreditsText(input) {
    return input
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter(Boolean)
...
export function formatCreditsRowsForTextfile(rows) {
    return rows
        .map((row) => {
        if (!row.role) {
            return row.name;
...
export function calculateCreditsContentHeight(rows, style) {
    const lineHeight = Math.max(1, style.fontSize + style.lineSpacing);
    return Math.max(lineHeight, rows.length * lineHeight);
}
export function buildCreditsRollYExpression(speed) {
    return `h-t*${formatSpeed(speed)}`;
}
export function calculateCreditsRollYRange(input) {
...
export function normalizeCreditsRows(rows, fallbackText = '') {
...
export function normalizeCreditsStyle(style) {
    return {
...
export function normalizeCreditsRollSpeed(speed) {
    return Math.round(clampNumber(Number(speed), DEFAULT_CREDITS_ROLL_SPEED, 1, 1000));
}
function parseCreditsLine(line) {
...
function normalizeCreditsCell(value) {
...
//# sourceMappingURL=credits-roll.js.map