 ...
 */
export function detectTrackGaps(track, options) {
 ...
 * 检测整个时间线的所有轨道间隙。
 */
export function computeTimelineGaps(timeline, options) {
    return timeline.tracks.flatMap((track) => detectTrackGaps(track, options));
}
/**
 * 获取间隙统计信息。
 */
export function getGapStats(gaps) {
    if (gaps.length === 0) {
        return {
            totalCount: 0,
            totalDuration: 0,
    ...
    for (const gap of gaps) {
        ...
        if (gap.duration > maxGap.duration)
            maxGap = gap;
        if (gap.duration < minGap.duration)
            minGap = gap;
 ...
 */
export function navigateGap(gaps, currentTime, direction) {
    if (gaps.length === 0)
        return undefined;
...
//# sourceMappingURL=timeline-gaps.js.map