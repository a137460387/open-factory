...
export function isPathMaskClosed(points, epsilon = PATH_CLOSE_EPSILON) {
...
export function normalizePathPoints(points) {
    ...
    return points.map((point) => normalizePathPoint(point)).filter((point) => Boolean(point));
}
export function closePathPoints(points) {
...
export function samplePathPoints(points, segmentsPerCurve = CURVE_SEGMENTS) {
    const normalized = normalizePathPoints(points);
...
export function triangulatePathMask(points) {
...
export function pathPointsToSvgPath(points, width = 1, height = 1) {
    const normalized = normalizePathPoints(points);
    ...
    for (let index = 1; index < normalized.length; index += 1) {
        ...
        if (previous.handleOut || point.handleIn) {
            ...
            commands.push(`C ${formatSvgNumber(control1.x * width)} ${formatSvgNumber(control1.y * height)} ${formatSvgNumber(control2.x * width)} ${formatSvgNumber(control2.y * height)} ${formatSvgNumber(point.x * width)} ${formatSvgNumber(point.y * height)}`);
        }
        else {
...
function sampleCubicBezier(from, control1, control2, to, t) {
    const inverse = 1 - t;
    return {
...
//# sourceMappingURL=path-mask.js.map