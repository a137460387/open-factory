...
export async function syncMulticamByAudio(angles, audioSamplesMap) {
    if (angles.length === 0)
...
export function syncMulticamByTimecode(angles, metadata) {
    if (angles.length === 0)
...
export function syncMulticamByManual(angles, markers) {
    const offsets = new Map();
    const ref = markers[0];
    if (!ref) {
        for (const a of angles)
...
export async function detectMulticamDrift(angles, audioSamplesMap) {
    if (angles.length < 2)
...
//# sourceMappingURL=multicam-sync.js.map