import { DEFAULT_CLIP_SPEED, DEFAULT_COLOR_CORRECTION, DEFAULT_TEXT_STYLE, DEFAULT_TRANSFORM, createId, } from './model';
...
export function getTitleTemplate(templateId) {
...
export function instantiateTitleTemplate(templateId, track, timeline, options = {}) {
    if (track.type !== 'text') {
...
function findAppendStart(track, timeline) {
...
function fadeInOutKeyframes(duration) {
    return [
...
function keyframe(prefix, time, value, easing) {
    return { id: createId(`kf-${prefix}`), time: round(time), value, easing };
}
function scaleTemplateKeyframes(keyframes, templateDuration, duration) {
