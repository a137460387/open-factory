...
/** 默认配置 */
export const DEFAULT_MULTI_FORMAT_CONFIG = {
 ...
 */
export function extractCropAnalysisFromProject(project, overrides) {
...
/** 在时间线中查找第一个视频片段 */
function findFirstVideoClip(timeline) {
    if (!timeline?.tracks)
        return undefined;
...
function formatKeyToString(key) {
 ...
 * 相同方向+宽高比的平台共享同一格式变体
 */
function groupPlatformsByFormat(platforms) {
 ...
 * - 方向变化大 = 0.3-0.6
 */
function assessCropQuality(sourceWidth, sourceHeight, cropResult) {
 ...
 * @returns 多格式生成结果
 */
export function generateMultiFormats(project, config = DEFAULT_MULTI_FORMAT_CONFIG) {
    ...
    targetPlatforms = targetPlatforms.filter((p) => {
        if (!config.includeSquareFormat && p.orientation === 'square')
            return false;
        return true;
    });
    // 按格式分组
    ...
    for (const [formatKey, platforms] of formatGroups) {
        // 选择代表平台（分辨率最高的）
        const representativePlatform = platforms.reduce((best, p) => p.width * p.height > best.width * best.height ? p : best);
        ...
        if (quality < config.minQualityThreshold) {
            warnings.push(`格式 ${formatKey} 裁剪质量过低 (${(quality * 100).toFixed(0)}%)，低于阈值 ${(config.minQualityThreshold * 100).toFixed(0)}%`);
            continue;
        }
        const variant = {
    ...
    const avgQuality = variants.length > 0 ? variants.reduce((sum, v) => sum + (1 - v.qualityLoss), 0) / variants.length : 0;
    return {
        sourceInfo: {
 ...
 * 为单个目标平台生成格式变体
 */
export function generateFormatVariant(project, platformId, cropOverrides) {
 ...
 */
export function generateFormatPreview(variant, containerWidth, containerHeight) {
...
// ─── 快捷工具函数 ────────────────────────────────────────────
/** 获取所有支持的方向 */
export function getSupportedOrientations() {
    return ['landscape', 'portrait', 'square'];
}
/** 按方向获取推荐格式 */
export function getRecommendedFormatsForOrientation(orientation) {
 ...
 * 最常见的多格式需求
 */
export function generateDualFormat(project, landscapePlatformId = 'youtube-1080p', portraitPlatformId = 'tiktok') {
...
//# sourceMappingURL=multi-format-generator.js.map