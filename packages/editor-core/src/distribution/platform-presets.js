...
// ─── 查找与过滤 ────────────────────────────────────────────
/** 按 ID 获取平台规格 */
export function getDistributionPlatform(id) {
 ...
 * - 平台自身推荐权重
 */
export function buildDistributionRecommendations(context) {
    const { width, height, durationSecs } = context;
    const isPortrait = height > width;
    const isLandscape = width > height;
    const isSquare = width === height;
    const recommendations = DISTRIBUTION_PLATFORMS.map((platform) => {
        let score = 0;
        const reasons = [];
        // 方向匹配
 ...
 */
export function mapToExportPlatformPreset(id) {
    const mapping = {
...
// ─── 格式化工具 ────────────────────────────────────────────
/** 格式化平台信息为简短描述 */
export function formatPlatformSummary(platform) {
    return `${platform.name} ${platform.width}×${platform.height} ${platform.fps}fps ${platform.aspectRatio}`;
}
/** 格式化最大时长 */
export function formatMaxDuration(platform) {
    if (platform.maxDurationSecs === undefined)
        return '无限制';
    if (platform.maxDurationSecs < 60)
        return `${platform.maxDurationSecs}秒`;
    if (platform.maxDurationSecs < 3600)
        return `${Math.floor(platform.maxDurationSecs / 60)}分钟`;
    return `${Math.floor(platform.maxDurationSecs / 3600)}小时`;
}
//# sourceMappingURL=platform-presets.js.map