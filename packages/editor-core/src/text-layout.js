...
export const DEFAULT_TEXT_OPEN_TYPE_FEATURES = {
    liga: false,
    smcp: false,
    tnum: false,
    swsh: false,
};
export const DEFAULT_TEXT_ARC = {
    enabled: false,
    radius: 280,
    startAngle: -30,
    clockwise: true,
...
export function normalizeRichTextDocument(value, fallbackText = '') {
...
export function normalizeTextOpenTypeFeatures(value) {
    return {
...
export function calculateTextAutoLayout(input) {
    ...
    const contentHeight = segments.reduce((height, segment) => Math.max(height, segment.yOffset + segment.style.fontSize * 1.2), 0) +
...
export function formatOpenTypeFeatureList(features) {
    const normalized = normalizeTextOpenTypeFeatures(features);
    return ['liga', 'smcp', 'tnum', 'swsh']
        .filter((key) => normalized[key])
        .map((key) => `${key}=1`)
        .join(',');
}
function normalizeRichTextRun(run) {
    ...
    return {
        ...
        fontSize: typeof run?.fontSize === 'number' && Number.isFinite(run.fontSize)
            ? round(Math.min(512, Math.max(1, run.fontSize)))
            : undefined,
    };
}
function mergeRunStyle(run, baseStyle) {
    return {
...
function estimateCharacterAdvance(char, fontSize) {
