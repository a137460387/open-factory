...
function createPhaseProgress(phase, phaseProgress, message, completedPhases) {
    let totalProgress = 0;
 ...
 */
export async function orchestrateSmartCreation(media, options = {}) {
    const { enableSpeechUnderstanding = true, narrativeTemplate, targetDuration, pacing, maxRecommendations = 10, sceneDetection, onProgress, } = options;
    ...
    const emotionCurveForNarrative = emotions.curve.map((p) => ({
        time: p.time,
        value: p.value,
    ...
    onProgress?.(createPhaseProgress('recommendation', 100, `生成 ${recommendations.clips.length} 条推荐`, completedPhases));
    ...
    if (narrativeTemplate) {
        ...
        storyline = generateNarrative({ scenes: scenes.segments, emotions, speech }, { template: narrativeTemplate, targetDuration, pacing });
        onProgress?.(createPhaseProgress('storyline', 100, `故事线包含 ${storyline.storyline.length} 个片段`, completedPhases));
 ...
 */
function estimateSaturation(m) {
    // Wide color gamut tends to be more saturated
    if (m.colorProfile?.colorPrimaries === 'bt2020')
        return 0.62;
    if (m.colorProfile?.colorPrimaries === 'smpte432')
        return 0.55;
    // HEVC/AV1 often used for HDR/wide gamut content
    if (m.videoCodec?.includes('hevc') || m.videoCodec?.includes('av1'))
        return 0.48;
    return 0.38;
}
/**
 ...
 */
function collectVisualSamples(media) {
    const samples = [];
 ...
 */
function collectAudioSamples(media) {
    const samples = [];
    for (const m of media) {
        if (!m.hasAudio)
            continue;
 ...
 */
function collectTranscripts(media) {
    const transcripts = [];
    for (const m of media) {
...
function extractClipsFromMedia(media) {
...
function generateRecommendations(clips, scenes, emotions, narrative, maxRecommendations) {
    // Build context from analysis results
    const avgEmotion = emotions.curve.length > 0 ? emotions.curve.reduce((sum, p) => sum + p.value, 0) / emotions.curve.length : 0;
...
//# sourceMappingURL=ai-smart-creation-orchestrator.js.map