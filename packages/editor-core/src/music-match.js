export function buildMusicMatchSystemPrompt() {
    return [
...
export function buildMusicMatchUserPrompt(description, mediaInfo) {
...
export function parseMusicMatchResponse(json) {
    ...
    if (!mood)
        return null;
    const validTempos = new Set(['fast', 'medium', 'slow']);
    const tempoRaw = typeof input.tempo === 'string' ? input.tempo.trim().toLowerCase() : 'medium';
    const tempo = validTempos.has(tempoRaw) ? tempoRaw : 'medium';
...
export function scoreMediaAudioSimilarity(targetMood, audioMood) {
    if (!targetMood || !audioMood)
        return 0;
    const targetWords = new Set(targetMood
        .toLowerCase()
        .split(/[\s,，、;；]+/)
        .filter(Boolean));
    const audioWords = audioMood
        ...
        .split(/[\s,，、;；]+/)
        .filter(Boolean);
    if (targetWords.size === 0 || audioWords.length === 0)
        return 0;
    let matches = 0;
...
export function calculateAudioLoopOrTrimToDuration(audioDuration, targetDuration) {
...
export function rankAudioByMoodSimilarity(targetMood, audioAssets) {
    return audioAssets
        .map((a) => ({
        mediaId: a.id,
        filename: a.name,
...
//# sourceMappingURL=music-match.js.map