...
export const CLIP_GROUP_COLORS = [
    'blue',
    'green',
    'purple',
    'amber',
...
export const CLIP_GROUP_COLOR_HEX = {
    blue: '#2563eb',
    green: '#16a34a',
    purple: '#7c3aed',
    amber: '#d97706',
...
export const DEFAULT_CLIP_GROUP_COLOR = 'blue';
export const DEFAULT_CLIP_GROUP_NAME = 'Group';
export function normalizeClipGroups(groups, availableClipIds) {
...
export function createClipGroup(input, availableClipIds) {
...
export function findClipGroupForClip(groups, clipId) {
    return (groups ?? []).find((group) => group.clipIds.includes(clipId));
}
export function findCompleteClipGroup(groups, clipIds) {
...
export function calculateClipGroupMoveStarts(clips, clipIds, draggedClipId, newDraggedStart) {
...
export function removeClipIdsFromGroups(groups, clipIds) {
    const removed = new Set(clipIds);
    return normalizeClipGroups((groups ?? []).map((group) => ({
        ...group,
        clipIds: group.clipIds.filter((clipId) => !removed.has(clipId)),
    })));
}
function normalizeClipGroup(group, availableClipIds, usedClipIds = new Set()) {
    const clipIds = Array.from(new Set((group.clipIds ?? []).filter((clipId) => typeof clipId === 'string' && clipId.trim().length > 0)))
...
function isClipGroupColor(value) {
    return typeof value === 'string' && CLIP_GROUP_COLORS.includes(value);
}
//# sourceMappingURL=clip-groups.js.map