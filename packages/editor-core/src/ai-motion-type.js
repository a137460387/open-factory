...
export function computeMotionVectorField(frames, width, height, gridSize = 4, searchRadius = 4) {
    if (frames.length < 2 || width < gridSize || height < gridSize)
        return { vectors: [] };
...
export function classifyMotionType(vectors, blockVectors, gridSize = 4) {
    const now = new Date().toISOString();
    if (vectors.length === 0)
        return { type: 'static', confidence: 1, analyzedAt: now };
    ...
    if (blockVectors && blockVectors.length > 0) {
        ...
        if (zoomResult)
            return { type: zoomResult.type, confidence: zoomResult.confidence, analyzedAt: now };
    }
    const directionStats = analyzeDirectionConsistency(vectors);
    if (directionStats.changeRatio > HANDHELD_DIRECTION_CHANGE_THRESHOLD &&
...
export function analyzeMotionType(frames, width, height, gridSize = 4, searchRadius = 4) {
...
export function buildSharedMotionData(vectors) {
    if (vectors.length === 0)
        return { shakeVectors: [], meanMagnitude: 0, variance: 0 };
    const magnitudes = vectors.map((v) => Math.sqrt(v.dx * v.dx + v.dy * v.dy));
...
export function filterMediaByMotionType(media, filterType) {
    return media.filter((item) => item.motionType?.type === filterType);
}
export function analyzeDirectionConsistency(vectors) {
    if (vectors.length === 0)
...
function detectZoom(blockVectors, gridSize) {
    if (blockVectors.length === 0)
        return null;
    const numBlocks = gridSize * gridSize;
    const avgBlocks = Array.from({ length: numBlocks }, () => ({ dx: 0, dy: 0 }));
...
function findBestBlockMatch(prev, curr, imgW, imgH, bx, by, bW, bH, radius) {
    let bestDx = 0;
    let bestDy = 0;
...
function computeBlockNCC(a, b, imgW, imgH, ox, oy, dx, dy, bW, bH) {
    let sumA = 0;
    let sumB = 0;
    let count = 0;
    for (let y = 0; y < bH; y += 1) {
        for (let x = 0; x < bW; x += 1) {
            ...
            const bx2 = ax + dx;
            const by2 = ay + dy;
            if (ax < 0 || ax >= imgW || ay < 0 || ay >= imgH)
                continue;
            if (bx2 < 0 || bx2 >= imgW || by2 < 0 || by2 >= imgH)
                continue;
            sumA += a[ay * imgW + ax];
            sumB += b[by2 * imgW + bx2];
            count += 1;
...
export async function analyzeMotionTypeSafe(frames, width, height, gridSize = 4, searchRadius = 4, t = identityTranslator) {
    try {
...
//# sourceMappingURL=ai-motion-type.js.map