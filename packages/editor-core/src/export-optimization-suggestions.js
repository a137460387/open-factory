...
export const DEFAULT_EXPORT_OPTIMIZATION_SETTINGS = {
    dismissedSuggestionIds: [],
};
const ALL_EXPORT_OPTIMIZATION_SUGGESTION_IDS = [
...
export function normalizeExportOptimizationSettings(settings) {
    if (!settings || typeof settings !== 'object') {
        return { ...DEFAULT_EXPORT_OPTIMIZATION_SETTINGS };
    }
    const input = settings;
    const allowed = new Set(ALL_EXPORT_OPTIMIZATION_SUGGESTION_IDS);
    const dismissedSuggestionIds = Array.from(new Set((Array.isArray(input.dismissedSuggestionIds) ? input.dismissedSuggestionIds : []).filter((id) => allowed.has(id))));
    return { dismissedSuggestionIds };
}
export function analyzeExportOptimizationSuggestions(project, settings, optimizationSettings = DEFAULT_EXPORT_OPTIMIZATION_SETTINGS, context = {}) {
    ...
    const highFpsMedia = usedMedia.filter((asset) => asset.type === 'video' &&
        normalizePositiveNumber(asset.frameRate) >= 55 &&
        outputFps < normalizePositiveNumber(asset.frameRate) - 0.01);
    ...
    if (typeof context.measuredIntegratedLufs === 'number' &&
        Number.isFinite(context.measuredIntegratedLufs) &&
        context.measuredIntegratedLufs <= -24 &&
        (!settings.loudnessNormalization || settings.loudnessNormalization === 'off')) {
        suggestions.push({
...
export function applyExportOptimizationSuggestion(suggestion, settings, context = {}) {
...
function collectUsedTimelineMedia(project) {
    const mediaById = new Map(project.media.map((asset) => [asset.id, asset]));
    const usedIds = new Set(project.timeline.tracks.flatMap((track) => track.clips.flatMap((clip) => ('mediaId' in clip && typeof clip.mediaId === 'string' ? [clip.mediaId] : []))));
    return Array.from(usedIds).flatMap((id) => {
        const asset = mediaById.get(id);
        return asset ? [asset] : [];
    });
}
function isFourKAsset(asset) {
    return asset.width >= 3840 || asset.height >= 2160;
}
function normalizePositiveNumber(value) {
    return typeof value === 'number' && Number.isFinite(value) && value > 0 ? value : 0;
}
//# sourceMappingURL=export-optimization-suggestions.js.map