...
export function createInitialQualityPanelState() {
    return {
        phase: 'idle',
 ...
 */
export function qualityPanelReducer(state, action) {
    switch (action.type) {
...
function filterIssues(issues, filters) {
    return issues.filter((issue) => {
        ...
        if (filters.category && issue.category !== filters.category)
            return false;
        if (filters.autoFixableOnly && !issue.autoFixable)
            return false;
        return true;
    });
}
 ...
 */
export function formatIssueTimeline(issue) {
    if (!issue.timeRange)
        return '全局';
 ...
 */
export function getQualitySummaryStats(report) {
    return [
