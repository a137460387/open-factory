 ...
 */
export function detectBlackFrame(pixels, threshold = DEFAULT_INSPECTOR_CONFIG.blackFrameThreshold) {
    if (pixels.length === 0)
        return false;
 ...
 */
export function detectColorBars(pixels, width, height) {
    ...
    for (let bar = 0; bar < 7; bar++) {
        ...
        const tolerance = 30;
        if (Math.abs(pixels[idx] - er) < tolerance &&
 ...
 */
export function calculateMotionScore(prevFrame, currFrame) {
    if (prevFrame.length === 0 || currFrame.length === 0)
        return 0;
 ...
 */
export function analyzeAudioSegment(samples, config = DEFAULT_INSPECTOR_CONFIG) {
    if (samples.length === 0) {
        return {
            timestamp: 0,
            rmsDb: -Infinity,
 ...
 */
export function analyzeFrames(frames, config = DEFAULT_INSPECTOR_CONFIG) {
    const analyses = [];
 ...
 */
export function detectQualitySceneTransitions(frameAnalyses, motionThreshold = 0.3) {
    const transitions = [];
 ...
 */
export function analyzeQualityPacing(transitions, totalDuration, windowSeconds = 30) {
    if (transitions.length === 0 || totalDuration <= 0)
        return [];
    const segments = [];
 ...
 */
export function checkPlatformCompliance(mediaInfo, platformId = DEFAULT_INSPECTOR_CONFIG.targetPlatform) {
    const spec = PLATFORM_SPECS[platformId] || PLATFORM_SPECS.custom;
    const violations = [];
    // Resolution check
 ...
 */
export function generateIssues(frameAnalyses, audioAnalyses, pacingSegments, sceneTransitions, complianceResult, config) {
    const issues = [];
 ...
 */
export async function runQualityInspection(mediaData, config = DEFAULT_INSPECTOR_CONFIG) {
    ...
    const issues = generateIssues(frameAnalyses, audioAnalyses, pacingSegments, sceneTransitions, complianceResult, config);
