 ...
 */
export function calculateCpmCurve(cuts, totalDuration, windowSeconds = DEFAULT_WINDOW_SECONDS, stepSeconds = DEFAULT_STEP_SECONDS) {
    if (totalDuration <= 0 || cuts.length === 0)
        return [];
    ...
    for (let t = 0; t <= totalDuration - windowSeconds / 2; t += stepSeconds) {
        ...
        while (head < sortedCuts.length && sortedCuts[head] < windowEnd)
            head++;
        while (tail < sortedCuts.length && sortedCuts[tail] < windowStart)
            tail++;
 ...
 */
export function calculateOverallAvgCPM(cuts, totalDuration) {
    if (totalDuration <= 0)
        return 0;
 ...
 * Fast: CPM > 180% of avg.
 */
export function classifyPacingSegments(curve, avgCPM) {
...
function mergeContiguousSegments(curve, threshold, direction) {
    const segments = [];
    let segStart = null;
 ...
 */
export function analyzePacing(clipStarts, totalDuration, windowSeconds = DEFAULT_WINDOW_SECONDS, stepSeconds = DEFAULT_STEP_SECONDS) {
...
//# sourceMappingURL=ai-pacing-analysis.js.map