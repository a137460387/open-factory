 ...
 */
export function detectAudioBeats(audioData, sampleRate = 44100) {
...
function classifyFrequencyBand(audioData, start) {
    let crossings = 0;
 ...
 */
export function analyzeVideoMotion(frames, fps = 30) {
    if (frames.length === 0)
        return [];
 ...
 */
export function findMotionPeaks(motionPoints) {
    ...
    for (let i = halfWindow; i < motionPoints.length - halfWindow; i++) {
        ...
        if (point.motionMagnitude < MOTION_HIGH_THRESHOLD)
            continue;
        let isPeak = true;
 ...
 */
export function matchRhythmToTemplate(rhythmProfile, template) {
    const totalDuration = template.metadata.estimatedDurationSec;
    if (totalDuration <= 0 || rhythmProfile.beats.length === 0)
        return template;
...
function injectRhythmKeyframes(clip, beats) {
    const rhythmKeyframes = [];
 ...
 */
export function createRhythmAlignedTemplate(template, audioData, videoFrames, options) {
...
function buildAudioRhythmProfile(beats) {
    if (beats.length < 2) {
    ...
    if (intervals.length > 2) {
        ...
        if (Math.sqrt(variance) / (avgInterval || 1) > 0.4)
            rhythmType = 'variable';
    }
...
function mergeBeatsWithMotion(audioBeats, motionPeaks, audioWeight) {
    const motionWeight = 1 - audioWeight;
    const merged = audioBeats.map((b) => ({ ...b }));
...
//# sourceMappingURL=rhythm-matcher.js.map