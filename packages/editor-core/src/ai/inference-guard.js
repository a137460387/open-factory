...
export function initInferenceGuard(provider) {
 ...
 */
export function checkFeatureStatus(options) {
    if (!globalProvider) {
        return {
            available: false,
 ...
 */
export async function withInferenceGuard(options, operation) {
    ...
    try {
        ...
        if (err instanceof InferenceFeatureDegradedError)
            throw err;
 ...
 */
export function withInferenceGuardSync(options, operation) {
    const status = checkFeatureStatus(options);
    if (!status.available) {
        if (!options.silent) {
            logger.warn(`[InferenceGuard] feature degraded: ${status.reason}`);
