 ...
 */
export function detectScenesWithCLIP(embeddings, samples, options = {}) {
    const { similarityThreshold = 0.75, minSceneDuration = 0.5, clipWeight = 0.6, histogramWeight = 0.3, motionWeight = 0.1, windowSize = 5, sensitivity = 1.0, } = options;
 ...
 */
export function refineBoundaries(boundaries, samples, audioEvents, maxSnapDistance = 0.2) {
    const refinements = [];
 ...
 */
export function computeCLIPSimilarity(a, b) {
 ...
 */
export function findSimilarScenes(segments, threshold = 0.8) {
...
function mergeByTime(embeddings, samples) {
...
function findNearestEmbedding(embeddings, time) {
...
function computePairScores(frames, clipWeight, histogramWeight, motionWeight) {
    const scores = [];
...
function computeAdaptiveThreshold(scores, index, windowSize, baseThreshold, sensitivity) {
...
function buildCLIPSegments(frames, boundaries) {
    if (frames.length === 0) {
        return [];
    }
...
function finalizeCLIPSegment(start, end, frames) {
...
function mergeCloseRefinements(refinements, minGap) {
...
//# sourceMappingURL=scene-detection.js.map