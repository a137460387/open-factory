 ...
 * 复用现有 parseSrt 解析器
 */
export function parseWhisperSrt(srtContent) {
 ...
 * 接受毫秒级时间戳
 */
export function createSegmentsFromTimestamps(timestamps) {
    return timestamps
 ...
 */
export function mergeShortSegments(segments, minDurationMs = DEFAULT_MIN_SEGMENT_DURATION_MS, mergeGapMs = DEFAULT_MERGE_GAP_MS, maxChars = DEFAULT_MAX_CHARS_PER_SEGMENT) {
    ...
    for (let i = 1; i < sorted.length; i++) {
        ...
        if ((currentDuration < minDurationMs && gapMs < mergeGapMs * 3) ||
 ...
 */
export function splitLongSegments(segments, maxDurationMs = DEFAULT_MAX_SEGMENT_DURATION_MS, maxChars = DEFAULT_MAX_CHARS_PER_SEGMENT) {
    const result = [];
 ...
 * 将所有片段的时间戳整体偏移指定毫秒数
 */
export function alignTimestamps(segments, offsetMs) {
 ...
 * 基于语言类型的平均阅读速度
 */
export function estimateReadingTimeMs(text, language = 'auto') {
    if (!text || text.trim().length === 0) {
        return 0;
    }
 ...
 * 用于插入时间线
 */
export function segmentsToSubtitleClips(segments, trackId, config = {}) {
 ...
 * 返回问题列表，空数组表示结果有效
 */
export function validateTranscriptionResult(segments, minDurationMs = DEFAULT_MIN_SEGMENT_DURATION_MS) {
    const issues = [];
 ...
 * 解析 → 合并短片段 → 拆分长片段 → 验证
 */
export function processWhisperOutput(srtContent, config = {}) {
 ...
 */
function splitSegmentText(segment, maxChars) {
...
/** 按正则模式拆分文本 */
function splitByPattern(text, pattern, maxChars) {
    const parts = [];
    let remaining = text;
