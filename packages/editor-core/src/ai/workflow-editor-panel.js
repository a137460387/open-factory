...
export function createInitialWorkflowEditorState(name = 'New Workflow') {
    return {
        phase: 'editing',
 ...
 */
export function workflowEditorReducer(state, action) {
    switch (action.type) {
        case 'LOAD_WORKFLOW':
        ...
        case 'LOAD_TEMPLATE': {
            ...
            if (!template)
                return { ...state, error: `Template not found: ${action.templateId}` };
            return {
                ...state,
        ...
        case 'ADD_NODE': {
            ...
            if (!def)
                return { ...state, error: `Unknown node type: ${action.definitionType}` };
        ...
        case 'COMPLETE_CONNECTION': {
            ...
            return {
                ...state,
                workflow: connectNodes(state.workflow, state.pendingConnection.sourceNodeId, state.pendingConnection.sourcePortId, action.targetNodeId, action.targetPortId),
...
export function getNodeAtPosition(state, canvasX, canvasY) {
    // Simple hit test: check if point is within node bounds (assuming 200x80 node size)
    const NODE_WIDTH = 200;
    const NODE_HEIGHT = 80;
    return state.workflow.nodes.find((n) => canvasX >= n.position.x &&
...
export function getLinearWorkflow(state) {
...
export function getNodeDefinitionsByCategory(definitions) {
...
export function formatExecutionStatus(context) {
    ...
    if (context.status === 'failed')
        return `Failed: ${failed} error(s), ${completed} succeeded`;
    if (context.status === 'running')
        return `Running: step ${context.currentStepIndex + 1}/${total}`;
    if (context.status === 'paused')
        return `Paused at step ${context.currentStepIndex + 1}/${total}`;
    if (context.status === 'cancelled')
        return 'Cancelled';
    return context.status;
}
//# sourceMappingURL=workflow-editor-panel.js.map