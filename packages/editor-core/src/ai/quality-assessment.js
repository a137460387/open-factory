 ...
 * @returns [r, g, b] 分量
 */
function getRGB(frame, pixelIndex) {
 ...
 * @returns 锐度值 (0-100)
 */
export function computeImageSharpness(frame, width, height) {
 ...
 */
export function estimateNoiseLevel(frame, width, height) {
    if (frame.length < width * height * 4 || width < 3 || height < 3) {
        return 0;
    }
    // 转换为亮度图
 ...
 */
export function analyzeExposure(frame) {
 ...
 */
export function computeColorBalance(frame, width, height) {
    const pixelCount = Math.floor(frame.length / 4);
    if (pixelCount === 0) {
        return { r: 0, g: 0, b: 0 };
    }
 ...
 */
export function mapScoreToEnhancedGrade(score) {
 ...
 * @returns 等级文字
 */
export function dimensionScoreToGrade(score) {
    const s = clamp(score, 0, 100);
    if (s >= 90)
        return 'excellent';
    if (s >= 75)
 ...
 * @returns 视频质量指标
 */
export function assessVideoQuality(frames, config) {
    if (frames.length === 0) {
        return {
            sharpness: 0,
            noise: 0,
 ...
 * @returns 音频质量指标
 */
export function assessAudioQuality(audioData, sampleRate, config) {
    ...
    for (let i = 0; i < audioData.length; i++) {
        if (Math.abs(audioData[i]) > 0.99) {
            currentClipRun++;
            clipCount++;
            if (currentClipRun > maxClipRun)
                maxClipRun = currentClipRun;
        }
        else {
            currentClipRun = 0;
        }
 ...
 * @returns 单帧质量评分
 */
export function assessFrameQuality(frame, width, height) {
 ...
 * @returns 质量评估结果
 */
export function computeQualityScore(metrics, audioMetrics, config) {
    ...
    for (const dim of config.dimensions) {
        ...
        for (const issueDesc of dimIssues) {
            const severity = grade === 'poor' ? 'high' : grade === 'acceptable' ? 'medium' : 'low';
            issues.push({
                type: dim,
                severity,
                dimension: dim,
        ...
        // 生成优化建议
        if (score < 75) {
            const priority = score < 40 ? 'critical' : score < 55 ? 'high' : score < 65 ? 'medium' : 'low';
...
export function generateOptimizationSuggestions(result) {
    ...
    const seen = new Set();
    const deduped = [];
    for (const sug of allSuggestions.sort((a, b) => (priorityOrder[a.priority] ?? 4) - (priorityOrder[b.priority] ?? 4))) {
 ...
 * @returns 质量对比结果
 */
export function compareQuality(baseline, comparison) {
    const improvements = [];
    const regressions = [];
    // 建立基准维度分数映射
 ...
 * @returns 质量评估配置
 */
export function applyQualityProfile(profile) {
 ...
 * @returns 默认配置
 */
export function createDefaultQualityAssessmentConfig() {
    return {
        dimensions: [
            'sharpness',
            'noise',
 ...
 * @returns 是否合法
 */
export function validateQualityAssessmentConfig(config) {
    // 维度不能为空
    ...
    // 采样数必须为正整数且合理
    if (config.sampleCount < 1 || config.sampleCount > 100)
        return false;
    // 阈值必须单调递减
    const t = config.qualityThresholds;
    if (t.excellent <= t.good || t.good <= t.acceptable || t.acceptable <= t.poor)
        return false;
    // 阈值必须在 0-100 范围内
    if (t.excellent < 0 ||
        t.excellent > 100 ||
 ...
 * @returns 系统提示字符串
 */
export function buildEnhancedQualitySystemPrompt(profile) {
 ...
 * @returns 用户提示字符串
 */
export function buildEnhancedQualityUserPrompt(metrics, audioMetrics) {
    const parts = [
 ...
 * @returns 质量评估结果
 */
export function parseEnhancedQualityResponse(json) {
    const emptyResult = {
        overallScore: 0,
        videoMetrics: {
            sharpness: 0,
    ...
    const obj = json;
    // 解析综合分数
    const overallScore = clamp(typeof obj.overallScore === 'number' && !Number.isNaN(obj.overallScore) ? Math.round(obj.overallScore) : 0, 0, 100);
    // 解析等级
    const validGrades = ['S', 'A', 'B', 'C', 'D', 'F'];
    const grade = validGrades.includes(obj.grade)
        ? obj.grade
    ...
    if (Array.isArray(obj.dimensionScores)) {
        ...
        for (const item of obj.dimensionScores) {
            if (item && typeof item === 'object') {
                const ds = item;
                if (typeof ds.dimension === 'string' &&
                    validDimensions.includes(ds.dimension) &&
                    typeof ds.score === 'number') {
                    dimensionScores.push({
                        dimension: ds.dimension,
                        score: clamp(Math.round(ds.score), 0, 100),
                        weight: typeof ds.weight === 'number' ? clamp(ds.weight, 0, 1) : 0.1,
                        issues: Array.isArray(ds.issues) ? ds.issues.filter((i) => typeof i === 'string') : [],
            ...
            if (item && typeof item === 'object') {
                ...
                if (typeof issue.type === 'string' &&
                    ...
                    issues.push({
                        ...
                        dimension: typeof issue.dimension === 'string' ? issue.dimension : 'sharpness',
            ...
            if (item && typeof item === 'object') {
                ...
                if (typeof sug.id === 'string' && typeof sug.action === 'string' && typeof sug.dimension === 'string') {
                    suggestions.push({
                        ...
                        action: sug.action,
                        expectedImprovement: typeof sug.expectedImprovement === 'number' ? clamp(Math.round(sug.expectedImprovement), 0, 100) : 0,
                        priority: validPriorities.includes(sug.priority)
                            ? sug.priority
                            : 'medium',
                        autoApplicable: typeof sug.autoApplicable === 'boolean' ? sug.autoApplicable : false,
                        params: typeof sug.params === 'object' && sug.params !== null
                            ? sug.params
                            : undefined,
                    });
                }
 ...
 */
export async function parseEnhancedQualityResponseSafe(json, t = identityTranslator) {
    try {
