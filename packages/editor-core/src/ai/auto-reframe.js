 ...
 */
export function generateAutoReframe(frames, options) {
    const { targetAspect, padding = 0.1, smoothingWindow = 5, maxCropSpeed = 200, minCropWidth = 320, minCropHeight = 180, preferFaces = true, importanceThreshold = 0.3, } = options;
    if (frames.length === 0) {
        return emptyResult();
    }
    ...
    for (const frame of sorted) {
        ...
        const crop = subjectToCropWindow(subject.bbox, frame.sourceWidth, frame.sourceHeight, ratio, padding, minCropWidth, minCropHeight);
        rawKeyframes.push({
            time: frame.time,
            cropX: crop.cropX,
            cropY: crop.cropY,
 ...
 */
export function interpolateAutoReframeAtTime(keyframes, time) {
 ...
 */
export function multiAspectReframe(frames, aspects, options) {
 ...
 */
export function validateReframeKeyframes(keyframes, sourceWidth, sourceHeight) {
    const issues = [];
...
// --- Internal helpers ---
function trackPrimarySubject(frames, preferFaces, importanceThreshold) {
...
function selectSubjectForFrame(frame, primaryId, preferFaces, importanceThreshold) {
...
function subjectToCropWindow(bbox, sourceWidth, sourceHeight, ratio, padding, minCropWidth, minCropHeight) {
...
function computeCenterCrop(sourceWidth, sourceHeight, ratio) {
    const safeWidth = Math.max(1, sourceWidth);
    const safeHeight = Math.max(1, sourceHeight);
    let cropW;
    let cropH;
...
function smoothAutoReframeKeyframes(keyframes, windowSize) {
...
function limitCropSpeed(keyframes, maxSpeed) {
    if (keyframes.length <= 1) {
        return keyframes;
    }
...
function buildAutoReframeFFmpegExpression(keyframes) {
    if (keyframes.length === 0) {
        return undefined;
    }
    if (keyframes.length === 1) {
...
function buildSegmentExpr(keyframes, field) {
    if (keyframes.length === 1) {
...
//# sourceMappingURL=auto-reframe.js.map