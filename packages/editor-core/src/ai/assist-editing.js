 ...
 * @returns 平滑后的数组
 */
function smooth(arr, windowSize = 5) {
 ...
 * @returns 推断的场景类型
 */
function inferSceneType(startTime, endTime, totalDuration, prevType, motionLevel) {
    ...
    if (motionLevel > 0.7)
        return 'action';
    if (motionLevel > 0.4 && duration < 3)
        return 'montage';
    // 根据前后关系推断
    if (prevType === 'action' && motionLevel < 0.3)
        return 'transition';
    if (prevType === 'dialogue' && motionLevel > 0.5)
        return 'b-roll';
    return 'dialogue';
}
// ==================== 核心函数 ====================
 ...
 * @returns 内容分析结果
 */
export function analyzeContent(frames, audioData, sampleRate) {
 ...
 * @returns 剪辑建议列表（已排序）
 */
export function generateAssistEditingSuggestions(analysis, config) {
    const suggestions = [];
    // 1. 基于场景的剪辑建议
 ...
 * @returns 辅助剪辑配置
 */
export function applyAssistEditingPreset(preset) {
    switch (preset) {
        case 'quick-cut':
            return {
                enableAutoCut: true,
 ...
 * @returns 质量评分 (0-1)
 */
export function scoreSuggestionQuality(suggestion, context) {
    let score = 0;
    // 1. 基础置信度 (权重 0.3)
 ...
 * @returns 排序和过滤后的建议列表
 */
export function filterAndRankSuggestions(suggestions, maxCount) {
    // 按优先级降序，相同优先级按置信度降序
 ...
 * @returns 默认配置对象
 */
export function createDefaultAssistEditingConfig() {
    return {
        enableAutoCut: true,
        enableRhythmSync: true,
        enableEmotionAware: true,
 ...
 * @returns 配置是否合法
 */
export function validateAssistEditingConfig(config) {
    if (typeof config.enableAutoCut !== 'boolean')
        return false;
    if (typeof config.enableRhythmSync !== 'boolean')
        return false;
    if (typeof config.enableEmotionAware !== 'boolean')
        return false;
    if (typeof config.enableContentAnalysis !== 'boolean')
        return false;
    if (config.targetDuration !== undefined) {
        if (typeof config.targetDuration !== 'number' || config.targetDuration <= 0)
            return false;
    }
    if (config.maxCutCount !== undefined) {
        if (typeof config.maxCutCount !== 'number' || config.maxCutCount < 1)
            return false;
    }
    if (typeof config.minSegmentDuration !== 'number' || config.minSegmentDuration < 0)
        return false;
    if (typeof config.maxSegmentDuration !== 'number' || config.maxSegmentDuration < 0)
        return false;
    if (config.minSegmentDuration > config.maxSegmentDuration)
        return false;
    if (!Array.isArray(config.preferredCutTypes))
        return false;
    if (typeof config.transitionPreference !== 'string')
        return false;
    return true;
}
/**
 ...
 * @returns 系统提示字符串
 */
export function buildAssistEditingSystemPrompt() {
    return [
 ...
 * @returns 用户提示字符串
 */
export function buildAssistEditingUserPrompt(analysis, config) {
    const parts = [];
    // 配置信息
    parts.push('## 剪辑配置');
    ...
    parts.push('\n## 场景列表');
    for (const scene of analysis.scenes) {
        parts.push(`- [${scene.startTime.toFixed(2)}s ~ ${scene.endTime.toFixed(2)}s] ${scene.sceneType} (${(scene.confidence * 100).toFixed(0)}%) - ${scene.description}`);
    }
    // 节奏信息
    parts.push('\n## 节奏信息');
    ...
    if (analysis.rhythmProfile.tempoChanges.length > 0) {
        parts.push(`- 速度变化: ${analysis.rhythmProfile.tempoChanges.map((tc) => `${tc.time.toFixed(1)}s->${tc.bpm}bpm`).join(', ')}`);
    }
    // 情绪曲线摘要
    parts.push('\n## 情绪曲线');
    ...
    if (analysis.speakerSegments.length > 0) {
        ...
        for (const seg of analysis.speakerSegments) {
            ...
            parts.push(`- [${seg.startTime.toFixed(2)}s ~ ${seg.endTime.toFixed(2)}s] ${seg.speakerId} (${seg.emotion}): ${textPreview}`);
        }
    }
    // 关键帧
 ...
 * @throws 解析失败时抛出错误
 */
export function parseAssistEditingResponse(json) {
    const emptyResult = {
        suggestions: [],
        analysisResult: {
            scenes: [],
 ...
 */
export async function parseAssistEditingResponseSafe(json, t = identityTranslator) {
    const emptyResult = {
        suggestions: [],
        analysisResult: {
            scenes: [],
 ...
 */
export function detectSceneTransitions(frames, threshold = 0.35) {
    if (frames.length < 2)
        return [];
 ...
 * @returns 起音点时间数组（秒）
 */
export function computeAudioOnsets(audioData, sampleRate) {
 ...
 * @returns 合并后的建议列表
 */
export function mergeNearbyCuts(suggestions, minGap) {
 ...
 * 根据场景转换时间点构建场景列表
 */
function buildScenesFromTransitions(transitionTimes, totalDuration, frames, fps) {
    const scenes = [];
 ...
 * 计算帧范围内的运动水平
 */
function computeMotionLevel(frames, startIdx, endIdx) {
    if (endIdx <= startIdx || endIdx >= frames.length)
        return 0.5;
    let totalDiff = 0;
    let count = 0;
 ...
 * 生成场景描述文本
 */
function generateSceneDescription(sceneType, motionLevel, duration) {
 ...
 * 计算情绪曲线（基于音频能量和过零率）
 */
function computeEmotionCurve(audioData, sampleRate, duration) {
 ...
 * 计算节奏配置
 */
function computeRhythmProfile(audioData, sampleRate) {
    ...
    const energyCurve = new Array(curveLen);
 ...
 * 检测速度变化点
 */
function detectTempoChanges(beatTimes, hopSize, sampleRate) {
 ...
 * 检测说话人片段（基于静音段分割）
 */
function detectSpeakerSegments(audioData, sampleRate) {
    const frameSize = Math.floor(sampleRate * 0.025); // 25ms
    const hopSize = Math.floor(frameSize / 2);
 ...
 * 简单估算说话人 ID（基于音高特征区分）
 */
function estimateSpeakerId(audioData, sampleRate, startTime, _endTime) {
 ...
 * 估算语音段情绪
 */
function estimateSpeechEmotion(frameEnergies, startIdx, endIdx) {
    ...
    if (avgEnergy > 0.05 && energyStd > 0.02)
        return 'excited';
    if (avgEnergy > 0.03)
        return 'happy';
    if (avgEnergy < 0.005)
 ...
 * 检测关键帧时间点
 */
function detectKeyFrameTimes(frames, fps) {
    if (frames.length < 2)
        return [];
    const diffs = [];
    for (let i = 1; i < frames.length; i++) {
 ...
 * 查找最近的节拍距离
 */
function findNearestBeatDistance(time, beatTimes) {
 ...
 * 根据场景类型确定剪切类型
 */
function determineCutTypeForScene(sceneType, config) {
    const sceneCutMap = {
        intro: 'fade-in',
        action: 'hard-cut',
        dialogue: 'soft-cut',
 ...
 * 根据场景类型确定过渡效果
 */
function determineTransitionForScene(sceneType, preference) {
    const sceneTransitionMap = {
        intro: 'fade-in',
        action: 'none',
 ...
 * 计算场景建议的优先级
 */
function computeScenePriority(scene) {
 ...
 * 生成基于节奏的剪辑建议
 */
function generateRhythmSuggestions(rhythmProfile, config) {
    const suggestions = [];
    const beatTimes = rhythmProfile.beatTimes;
    // 使用强拍（每 4 拍）作为主要剪切点
    for (let i = 0; i < beatTimes.length; i++) {
        const isDownbeat = i % 4 === 0;
        if (!isDownbeat && beatTimes.length > 8)
            continue; // 优先使用强拍
 ...
 * 生成基于情绪的剪辑建议
 */
function generateEmotionSuggestions(emotionCurve, config) {
    const suggestions = [];
 ...
 * 生成基于说话人的剪辑建议
 */
function generateSpeakerSuggestions(speakerSegments, config) {
    const suggestions = [];
 ...
 * 生成基于关键帧的剪辑建议
 */
function generateKeyFrameSuggestions(keyFrames, config) {
    const suggestions = [];
...
//# sourceMappingURL=assist-editing.js.map