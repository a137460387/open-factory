...
/** 特效基础粒子数量 */
const EFFECT_BASE_PARTICLE_COUNT = {
    particle: 200,
    'light-leak': 0,
    'lens-flare': 0,
    glitch: 0,
 ...
 */
export function computeAudioEnergyEnvelope(audioData, windowSize) {
 ...
 */
export function detectSilence(audioData, sampleRate, thresholdDb = DEFAULT_SILENCE_THRESHOLD_DB, minDurationMs = DEFAULT_MIN_SILENCE_DURATION_MS) {
 ...
 * @returns 生成的字幕内容
 */
export function generateSubtitle(audioData, sampleRate, config = {}) {
 ...
 * 将静音段列表转换为有声段列表
 */
function silenceToSpeechSegments(silenceSegments, totalDurationMs) {
 ...
 * 将估算的字符数自适应断行
 */
function breakTextIntoLines(totalChars, maxCharsPerLine, maxLines) {
    const lines = [];
 ...
 * @returns 生成的配音参数内容
 */
export function generateDubbing(text, config = {}) {
    const startTime = performance.now();
 ...
 * 获取情感系数（影响时长和音量变化）
 */
function getEmotionFactor(emotion) {
    const factors = {
        neutral: 1.0,
        happy: 0.9,
        sad: 1.3,
 ...
 * 估算句末停顿（毫秒）
 */
function estimatePauseAfterSentence(sentence) {
 ...
 * 基于简单启发式：引号内、大写词、感叹词
 */
function detectEmphasisWords(sentence) {
    const emphasis = [];
    // 引号内文字
 ...
 * @returns 音乐结构
 */
export function generateMusicStructure(genre, mood, duration, tempo) {
 ...
 * 分配音乐段落
 */
function allocateSections(totalBeats, baseIntensity, genre) {
    const sections = [];
    let currentBeat = 0;
    // Intro: 占总长度 10-15%
    ...
    if (isShort) {
        ...
        const verseChorusUnitBeats = genre === 'ambient' || genre === 'lo-fi' ? Math.round(remainingBeats * 0.15) : Math.round(remainingBeats * 0.2);
 ...
 * @returns 生成的配乐内容
 */
export function generateMusic(config = {}) {
    const startTime = performance.now();
 ...
 * 获取风格默认乐器列表
 */
function getDefaultInstruments(genre) {
    const instruments = {
 ...
 * 计算动态参数
 */
function computeDynamics(intensity, mood) {
 ...
 * 生成和弦进行
 */
function generateChordProgression(genre, sectionType) {
    // 基于风格和段落类型的常见和弦进行
    const progressions = {
        cinematic: {
            intro: ['i', 'III'],
 ...
 * @returns 生成的特效参数内容
 */
export function generateEffect(config) {
    const startTime = performance.now();
 ...
 * 计算特效参数
 */
function computeEffectParameters(effectType, intensity, duration, customParams) {
    const baseParams = {
        effectType,
        intensity,
        duration,
 ...
 * @returns 预估生成时间（毫秒）
 */
export function estimateGenerationTime(config) {
 ...
 * @returns 默认配置
 */
export function createDefaultContentGenerationConfig(type) {
    return {
        type,
        language: 'auto',
        enableGPU: false,
 ...
 * 获取默认输出格式
 */
function getDefaultOutputFormat(type) {
    const formats = {
        subtitle: 'srt',
        dubbing: 'wav',
        music: 'wav',
...
export function validateContentGenerationConfig(config) {
    ...
    const validTypes = ['subtitle', 'dubbing', 'music', 'effect', 'voiceover'];
 ...
 * @returns 系统提示词
 */
export function buildContentGenerationSystemPrompt(type) {
 ...
 * @returns 用户提示词
 */
export function buildContentGenerationUserPrompt(config) {
    const parts = [`请生成 ${config.type} 类型的内容。`];
 ...
 */
export function parseContentGenerationResponse(json, type) {
 ...
 */
function isContentQuality(value) {
 ...
 */
export async function parseContentGenerationResponseSafe(json, type, t = identityTranslator) {
    try {
