...
import { DEFAULT_MOTION_BLUR_PARAMS, normalizeMotionBlurParams } from './motion-blur';
import { MANUAL_AUDIO_VISUALIZATION_THEME_ID, expandAudioVisualizationTheme } from './audio-visualization-themes';
export const CUSTOM_SHADER_UNIFORM_NAMES = ['u_texture', 'u_resolution', 'u_time', 'u_progress'];
export const CUSTOM_SHADER_VARYING_NAME = 'v_texCoord';
export const CUSTOM_SHADER_EXAMPLES = [
    {
        id: 'pixelate',
        name: 'Pixelate',
...
export const DEFAULT_CUSTOM_SHADER_PRESET = CUSTOM_SHADER_EXAMPLES[0].id;
...
export const AUDIO_SPECTRUM_STYLES = ['bars', 'waveform', 'circular'];
export const AUDIO_SPECTRUM_POSITIONS = ['bottom', 'top'];
...
export function isEffectType(type) {
    return EFFECT_TYPES.includes(type);
}
...
export function normalizeEffectParams(type, params) {
...
export function normalizeAudioSpectrumParams(params) {
...
export function getCustomShaderExample(id) {
    return CUSTOM_SHADER_EXAMPLES.find((example) => example.id === id) ?? CUSTOM_SHADER_EXAMPLES[0];
}
function normalizeParam(value, fallback, min, max) {
...
function normalizeAudioSpectrumStyle(value, fallback) {
    if (value === 'circle') {
        return 'circular';
    }
    return AUDIO_SPECTRUM_STYLES.includes(value)
        ? value
...
function normalizeFallbackStyle(value) {
    ...
    return AUDIO_SPECTRUM_STYLES.includes(value) ? value : 'bars';
}
function normalizeAudioSpectrumPosition(value, fallback) {
    return AUDIO_SPECTRUM_POSITIONS.includes(value)
        ? value
        : normalizeFallbackPosition(fallback);
}
function normalizeFallbackPosition(value) {
    return AUDIO_SPECTRUM_POSITIONS.includes(value)
        ? value
        : 'bottom';
}
...
function normalizeShaderPreset(value) {
