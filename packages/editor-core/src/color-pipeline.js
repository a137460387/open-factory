export const DEFAULT_PROJECT_COLOR_PIPELINE = 'sdr-srgb';
export const PROJECT_COLOR_PIPELINES = [
    'sdr-srgb',
    'hdr-rec2020',
    'aces',
];
...
export function normalizeProjectColorPipeline(value) {
    return PROJECT_COLOR_PIPELINES.includes(value)
        ? value
        : DEFAULT_PROJECT_COLOR_PIPELINE;
}
export function isAcesColorPipeline(value) {
    return normalizeProjectColorPipeline(value) === 'aces';
}
export function buildProjectColorPipelineExportDefaults(pipeline) {
...
export function buildAcesOdtFilterChain(pipeline, outputColorSpace) {
    if (pipeline !== 'aces') {
        return [];
    }
...
function getZscaleOutputProfile(colorSpace) {
...
//# sourceMappingURL=color-pipeline.js.map