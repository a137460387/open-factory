import { AddClipCommand, AddTimelineMarkerCommand, DeleteClipCommand, UpdateClipCommand, } from './commands/timeline-commands';
import { getTimelineDuration } from './timeline';
export const TIMELINE_SCRIPT_API_SIGNATURES = [
    {
        name: 'getClips',
...
export const BUILTIN_TIMELINE_SCRIPTS = [
    {
        id: 'bulk-speed',
        code: [
...
export function createTimelineScriptSnapshot(project) {
...
export function getTimelineScriptApiFunctionNames() {
    return TIMELINE_SCRIPT_API_SIGNATURES.map((signature) => signature.name);
}
export function normalizeTimelineScriptOperations(operations) {
    return operations.map((operation) => normalizeTimelineScriptOperation(operation));
}
export function getTimelineScriptExportRequests(operations) {
    return normalizeTimelineScriptOperations(operations).filter((operation) => operation.type === 'exportProject');
}
export class RunScriptCommand {
    ...
    after;
    appliedCount = 0;
    constructor(accessor, operations, description = 'Run timeline script') {
        this.accessor = accessor;
        this.operations = operations;
        this.description = description;
    }
    get appliedOperationCount() {
...
function normalizeTimelineScriptOperation(operation) {
    ...
    if (operation.type === 'updateClip') {
        return {
            ...
            patch: normalizeRecord(operation.patch, 'clip patch'),
        };
    }
...
function normalizeMarkerInput(marker) {
...
function normalizeClipLike(clip) {
    const value = normalizeRecord(clip, 'clip');
...
//# sourceMappingURL=timeline-scripting.js.map