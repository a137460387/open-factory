...
export function validateBatchExportScript(raw) {
    const errors = [];
...
export function parseBatchScriptJson(jsonStr) {
    let parsed;
    try {
...
export function parseCliBatchArgs(argv) {
...
export function formatBatchExportLog(log) {
    const lines = [
        `批处理导出日志`,
        `脚本: ${log.scriptPath}`,
        `开始: ${log.startedAt}`,
...
export function createBatchExportTaskResult(projectPath, status, durationMs, error, outputPath) {
    return { projectPath, status, durationMs, error, outputPath };
}
export function serializeGuiConfigToBatchScript(configs, defaultPreset, defaultOutputDir) {
    return {
        version: 1,
        tasks: configs.map((c) => ({
...
//# sourceMappingURL=batch-export-script.js.map