...
// ─── 主编排函数 ──────────────────────────────────────────────
export function orchestrateSmartRoughCut(data, options) {
    ...
    if (opts.enableSceneSplit && data.scenes) {
        for (const sceneInput of data.scenes) {
            for (const boundary of sceneInput.result.boundaries) {
                if (boundary.score < opts.minConfidence)
                    continue;
                suggestions.push({
                    id: nextId('scene'),
                    type: 'scene_split',
    ...
    if (opts.enableSilenceRemoval && data.silences) {
        for (const silenceInput of data.silences) {
            for (const range of silenceInput.ranges) {
                if (range.duration < opts.minSilenceDuration)
                    continue;
                suggestions.push({
                    id: nextId('silence'),
                    type: 'silence_remove',
    ...
    if (opts.enableDialogueExtraction && data.dialogues) {
        for (const dialogueInput of data.dialogues) {
            ...
            if (dialogueInput.intervals.length === 0)
                continue;
            suggestions.push({
                id: nextId('dialogue'),
...
// ─── 建议选择管理 ──────────────────────────────────────────────
export function toggleSuggestionSelection(suggestions, id) {
    return suggestions.map((s) => (s.id === id ? { ...s, selected: !s.selected } : s));
}
export function setAllSuggestionSelection(suggestions, selected) {
    return suggestions.map((s) => ({ ...s, selected }));
}
export function selectSuggestionsByType(suggestions, type, selected) {
    return suggestions.map((s) => (s.type === type ? { ...s, selected } : s));
}
export function getSelectedSuggestions(suggestions) {
    return suggestions.filter((s) => s.selected);
}
export function reorderSuggestions(suggestions, fromIndex, toIndex) {
...
// ─── 报告生成 ──────────────────────────────────────────────
export function buildSmartRoughCutReport(data, suggestions) {
    ...
    const silenceDurationRemoved = (data.silences ?? []).reduce((sum, si) => sum + si.ranges.reduce((s, r) => s + r.duration, 0), 0);
    const dialogueDurationTotal = (data.dialogues ?? []).reduce((sum, di) => sum + di.intervals.reduce((s, i) => s + (i.end - i.start), 0), 0);
    ...
    const sceneSegmentDuration = (data.scenes ?? []).reduce((sum, s) => sum + s.result.segments.reduce((segSum, seg) => segSum + (seg.end - seg.start), 0), 0);
...
// ─── 辅助函数 ──────────────────────────────────────────────
function calculateScenePriority(boundary) {
    // 场景切换越明显，优先级越高
    return round(Math.min(100, 50 + boundary.score * 50));
}
function calculateSilencePriority(range) {
    // 静音越长，优先级越高
    return round(Math.min(100, 60 + range.duration * 10));
}
function calculateEmotionPriority(value) {
    return round(Math.min(100, 40 + Math.abs(value) * 40));
}
function calculateDialogueConfidence(intervals) {
    if (intervals.length === 0)
        return 0;
    const avgConfidence = intervals.reduce((sum, i) => sum + i.confidence, 0) / intervals.length;
    return round(avgConfidence, 2);
}
function buildSceneReason(boundary) {
    const parts = [];
    if (boundary.histogramDiff > 0.5)
        parts.push('色彩变化显著');
    if (boundary.motionDiff > 0.5)
        parts.push('运动变化显著');
    if (parts.length === 0)
        parts.push('场景切换');
    return parts.join('，');
}
function buildSilenceReason(range) {
 ...
 */
export function buildOrchestrationInput(mediaId, sceneResult, silenceRanges, clipId, dialogueIntervals, beatTimes, bpm) {
    const data = {};
    if (sceneResult) {
...
//# sourceMappingURL=smart-rough-cut-orchestrator.js.map