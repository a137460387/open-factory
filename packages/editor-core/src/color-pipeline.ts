...
export const PROJECT_COLOR_PIPELINES = [
  ...
  'hdr-rec2020',
  'aces',
] as const satisfies readonly ProjectColorPipeline[];
...
export function normalizeProjectColorPipeline(value: unknown): ProjectColorPipeline {
  return PROJECT_COLOR_PIPELINES.includes(value as ProjectColorPipeline)
    ? (value as ProjectColorPipeline)
...
export function buildAcesOdtFilterChain(
...
function getZscaleOutputProfile(colorSpace: ColorPipelineExportColorSpace): {
  matrix: string;
  transfer: string;
  primaries: string;
} {
