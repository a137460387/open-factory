import { DEFAULT_COLOR_CURVES, DEFAULT_THREE_WAY_COLOR, isDefaultColorCurves, isNeutralThreeWayColor, normalizeColorCurves, normalizeThreeWayColor, serializeColorCurvesToCube, } from './color-grading';
import { getLogToRec709Lut, isLogInputColorSpace, normalizeInputColorSpace, REC709_INPUT_COLOR_SPACE, serializeLogToRec709Cube, } from './color-log-luts';
import { normalizeLutLayers } from './lut-normalize';
import { round } from './time';
export const DEFAULT_COLOR_NODE_CORRECTION = {
...
export function createDefaultColorNodeGraph(correction) {
    const node = {
        id: 'node-default',
        type: 'sequential',
        name: 'Default Grade',
...
export function normalizeColorNodeCorrection(correction) {
    return {
...
export function isDefaultColorNodeCorrection(correction) {
    const normalized = normalizeColorNodeCorrection(correction);
    return (normalized.inputColorSpace === DEFAULT_COLOR_NODE_CORRECTION.inputColorSpace &&
...
export function normalizeColorNodeGraph(graph, fallbackCorrection) {
    ...
    (Array.isArray(graph.connections) ? graph.connections : []).forEach((connection, index) => {
        ...
        const rawId = typeof connection.id === 'string' && connection.id.trim()
            ? connection.id.trim()
            : `${connection.from}-${connection.to}`;
    ...
    const outputNodeId = typeof graph.outputNodeId === 'string' && nodeIds.has(graph.outputNodeId)
        ? graph.outputNodeId
        : (nodes.find((node) => node.type === 'output')?.id ??
            findSinkNodeId(nodes, connections) ??
...
export function detectColorNodeGraphCycle(graph) {
...
export function topologicallySortColorNodeGraph(graph) {
    const normalized = normalizeColorNodeGraph(graph);
...
export function buildColorNodeGraphFilterPlan(graph, options) {
    const normalized = normalizeColorNodeGraph(graph);
    const order = topologicallySortColorNodeGraph(normalized);
    const orderIds = new Set(order.map((node) => node.id));
    const incomingByTo = groupConnections(normalized.connections.filter((edge) => orderIds.has(edge.from) && orderIds.has(edge.to)), 'to');
    const outgoingByFrom = groupConnections(normalized.connections.filter((edge) => orderIds.has(edge.from) && orderIds.has(edge.to)), 'from');
    ...
    for (const node of order) {
        ...
        const inputLabels = incoming.length === 0
            ? [sourceLabels.get(node.id) ?? options.inputLabel]
            : incoming.map((edge) => outputByEdgeId.get(edge.id) ?? outputByNodeId.get(edge.from) ?? options.inputLabel);
...
export function serializeColorNodeGraphFile(graph, name = 'Open Factory Node Graph') {
    return `${JSON.stringify({ format: 'open-factory-node-graph', version: 1, name, graph: normalizeColorNodeGraph(graph) }, null, 2)}\n`;
}
export function parseColorNodeGraphFile(source) {
...
export const BUILT_IN_COLOR_NODE_GRAPH_TEMPLATES = [
    {
        id: 'cinematic',
        name: 'Cinematic',
...
function createTemplateGraph(templateId, nodes) {
...
function node(type, id, name, x, y, correction) {
    return {
        id,
        type,
        name,
...
function mergeNodeInputs(filters, inputLabels, node, options) {
...
function buildNodeFilters(node, options) {
    ...
    if (correction.brightness !== DEFAULT_COLOR_NODE_CORRECTION.brightness ||
...
function registerCurveLut(node, curves, options) {
...
function registerLogLut(node, colorSpace, options) {
    const clipId = options.clipId ?? 'clip';
    const safeNodeId = safeLabel(node.id);
...
function buildThreeWayColorFilter(value) {
...
function colorBalanceValue(value, channel) {
    return Math.min(1, Math.max(-1, value[channel] + value.intensity - 1));
}
function findSinkNodeId(nodes, connections) {
...
function normalizeColorNodeType(type) {
    return type === 'input' ||
        type === 'parallel' ||
        type === 'layer' ||
        type === 'output' ||
...
function normalizeBlendMode(mode) {
    return mode === 'normal' || mode === 'multiply' || mode === 'screen' || mode === 'overlay' || mode === 'addition'
        ? mode
        : 'average';
}
...
function normalizeNodeName(name, type, index) {
...
function defaultEscapeFilePath(path) {
    return path.replace(/\\/g, '/').replace(/:/g, '\\:').replace(/'/g, "\\'");
}
//# sourceMappingURL=color-node-graph.js.map