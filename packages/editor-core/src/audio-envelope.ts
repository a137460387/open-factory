...
export function getVolumeEnvelopePoints(
  clip: Pick<Clip, 'duration' | 'keyframes'> & Partial<Pick<Extract<Clip, { volume: number }>, 'volume'>>,
): VolumeEnvelopePoint[] {
...
export function volumeEnvelopeControlPointToKeyframe(
  input: VolumeEnvelopeControlPointInput,
  duration: number,
): Keyframe<number> {
...
export function buildVolumeFadeKeyframes(
  kind: VolumeEnvelopeFadeKind,
  duration: number,
  baseVolume = 1,
  fadeDuration = 1,
): Keyframe<number>[] {
  ...
  if (kind === 'in') {
    return [
      volumeEnvelopeControlPointToKeyframe(
        { id: 'volume-fade-in-start', time: 0, value: 0, easing: 'linear' },
        normalizedDuration,
      ),
      volumeEnvelopeControlPointToKeyframe(
        { id: 'volume-fade-in-end', time: normalizedFadeDuration, value: normalizedBase, easing: 'linear' },
        normalizedDuration,
      ),
    ];
  }
  return [
    ...
    volumeEnvelopeControlPointToKeyframe(
      { id: 'volume-fade-out-end', time: normalizedDuration, value: 0, easing: 'linear' },
      normalizedDuration,
    ),
  ];
}

export function getVolumeEnvelopeValueAt(
  clip: Pick<Clip, 'duration' | 'keyframes'> & Partial<Pick<Extract<Clip, { volume: number }>, 'volume'>>,
  localTime: number,
): number {
  const fallback = 'volume' in clip && typeof clip.volume === 'number' ? clip.volume : 1;
