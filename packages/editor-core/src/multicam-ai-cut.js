...
function findBestMatch(prev, curr, imgW, imgH, bx, by, bW, bH, radius) {
    let bestDx = 0;
    let bestDy = 0;
 ...
 */
export function estimateFrameMotion(prevFrame, currFrame, width, height, gridSize = 4, searchRadius = 4) {
 ...
 */
export function buildMulticamFeaturePayload(duration, windowSeconds, audioData, motionData) {
...
function computeWindowRMS(audio, wStart, wEnd) {
...
function computeWindowMotion(motion, wStart, wEnd) {
...
export function buildMulticamCutSystemPrompt() {
    return [
...
export function buildMulticamCutUserPrompt(payload) {
    return JSON.stringify(payload);
}
// -- Parse AI response -------------------------------------------
export function parseMulticamCutResponse(json) {
    ...
    return obj.cuts
        ...
        typeof item.angleId === 'string')
        .map((item) => ({
        time: round(Math.max(0, item.time)),
        angleId: (item.angleId || '').trim(),
 ...
 * suggestion.
 */
export function enforceMinimumSwitchInterval(suggestions, minInterval = DEFAULT_MIN_SWITCH_INTERVAL) {
...
export function validateCutAngles(suggestions, validAngleIds) {
    const valid = new Set(validAngleIds);
    return suggestions.filter((s) => valid.has(s.angleId));
}
//# sourceMappingURL=multicam-ai-cut.js.map