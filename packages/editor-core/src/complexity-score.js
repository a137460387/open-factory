...
export const COMPLEXITY_EFFECT_TYPE_FACTORS = {
    blur: 1,
    sharpen: 1,
    vignette: 1,
    'film-grain': 1.2,
...
export function calculateEffectComplexityScore(timeline) {
...
export function calculateAudioComplexityScore(timeline) {
...
export function calculateKeyframeDensityScore(timeline) {
...
export function calculateComplexityScore(project) {
    const dimensions = {
    ...
    const totalScore = round(Object.values(dimensions).reduce((sum, dimension) => sum + dimension.score * dimension.weight, 0));
    return {
        totalScore,
...
export function createComplexityReport(project, generatedAt = new Date().toISOString()) {
...
function clipsPerMinuteToScore(clipsPerMinute) {
    return Math.min(100, clipsPerMinute * 5);
}
function makeDimensionScore(id, rawValue, score, detail) {
    return {
        id,
...
function getTimelineClips(timeline) {
...
function countTrackAudioProcessingNodes(track) {
    let count = 0;
...
function countClipAudioProcessingNodes(clip) {
...
//# sourceMappingURL=complexity-score.js.map