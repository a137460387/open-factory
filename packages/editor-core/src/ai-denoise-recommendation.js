 ...
 */
export function analyzeFrequencyBand(samples, sampleRate, freqHz, bandwidthHz = 5) {
 ...
 */
export function analyzeBroadbandEnergy(samples, sampleRate, lowFreq, highFreq, binCount = 8) {
 ...
 */
export function estimateSNR(signalSamples, noiseSamples) {
...
function calculateSimpleRms(samples) {
    if (samples.length === 0)
        return 0;
    let sum = 0;
 ...
 */
export function classifyNoiseProfile(noiseSamples, sampleRate, signalSamples) {
 ...
 */
export function recommendDenoiseFilters(profile) {
    const filters = [];
    // 嗡声 → highpass滤镜切除低频
 ...
 * 解析AI返回的降噪推荐响应。
 */
export function parseDenoiseAiResponse(json) {
    const empty = { recommendedFilters: [], confidence: 0 };
    if (!json || typeof json !== 'object')
        return empty;
    const obj = json;
    const confidence = typeof obj.confidence === 'number' ? Math.min(1, Math.max(0, obj.confidence)) : 0;
    if (!Array.isArray(obj.recommendedFilters))
        return { ...empty, confidence };
    const recommendedFilters = obj.recommendedFilters
        .filter((item) => {
        ...
        .map((item) => ({
        filter: item.filter,
        params: typeof item.params === 'object'
            ? item.params
            : {},
 ...
 * 构建FFmpeg降噪滤镜链字符串。
 */
export function buildDenoiseFilterChain(filters) {
    return filters
        .map((f) => {
 ...
 */
export function buildDenoiseFfmpegArgs(filters) {
    const args = [];
 ...
 */
export function createDenoiseRecommendation(noiseProfile, recommendedFilters) {
    return {
        noiseProfile,
        recommendedFilters,
        appliedFilters: [],
 ...
 */
export function normalizeAIDenoiseRecommendation(input) {
...
//# sourceMappingURL=ai-denoise-recommendation.js.map