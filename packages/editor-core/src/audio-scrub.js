 ...
 * 根据拖动速度返回搓擦速率等级
 */
export function getScrubSpeedTier(speedPxPerSec, options) {
    ...
    if (speedPxPerSec < slow)
        return 'slow';
    if (speedPxPerSec > fast)
        return 'fast';
    return 'medium';
}
/**
 ...
 */
export function getScrubSampleIntervalMultiplier(tier) {
    switch (tier) {
        case 'slow':
            return 1.0;
        case 'medium':
 ...
 */
export function getScrubSampleFrames(sampleRate, options) {
 ...
 * 判断是否满足防抖间隔
 */
export function shouldTriggerScrub(lastTriggerMs, nowMs, options) {
 ...
 */
export function filterScrubTracks(tracks) {
...
//# sourceMappingURL=audio-scrub.js.map