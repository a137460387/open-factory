import { applyColorCurvesToRgb, applyThreeWayColor, createDefaultColorCurves, createDefaultThreeWayColor, normalizeColorCurves, normalizeThreeWayColor, } from './color-grading';
import { applyColorMatchTransformToRgb, buildColorMatchTransform, calculateColorMatchStats, } from './color-match';
import { clamp, round } from './time';
const DEFAULT_LUT_CREATOR_TITLE = 'open-factory custom LUT';
const LUT_CREATOR_PRECISIONS = [17, 33, 65];
export function createDefaultLutCreatorState() {
    return {
...
export function normalizeLutCreatorPrecision(value) {
    const numeric = typeof value === 'number' && Number.isFinite(value) ? Math.round(value) : 17;
    return LUT_CREATOR_PRECISIONS.includes(numeric) ? numeric : 17;
}
export function normalizeLutCreatorState(state) {
    return {
...
export function buildLutCreatorReferenceTransform(reference) {
    if (!reference) {
        return null;
    }
...
export function applyLutCreatorGrade(input, state) {
...
export function buildLutCreatorMatrix(state) {
    const normalized = normalizeLutCreatorState(state);
    ...
    for (let blue = 0; blue < size; blue += 1) {
        for (let green = 0; green < size; green += 1) {
            for (let red = 0; red < size; red += 1) {
                values.push(applyLutCreatorGrade({
                    r: red / (size - 1),
                    g: green / (size - 1),
                    b: blue / (size - 1),
                }, normalized));
...
export function serializeLutCreatorCube(state, title) {
    const normalized = normalizeLutCreatorState(state);
...
function buildNeutralReferenceSource(width, height) {
...
function normalizeColorMatchTransform(value) {
    if (!value) {
        return null;
    }
    return {
...
function normalizeChannelTransform(value) {
    return {
...
//# sourceMappingURL=lut-creator.js.map