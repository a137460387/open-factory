export const BUILT_IN_PROVIDER_PRESETS = [
...
export function createBuiltInProvider(preset) {
    return {
        id: preset.id,
        name: preset.name,
...
export function createAllBuiltInProviders() {
...
export function calculateSubtitlePolishBatchSplit(total, batchSize = 50) {
    if (total <= 0) {
        return [];
    }
    const batches = [];
...
export function parseSubtitlePolishResponse(json) {
    ...
    return json
        ...
        .map((item) => ({
        index: Math.max(0, Math.round(item.index)),
        text: (item.text || '').trim(),
    }))
        .filter((item) => item.text.length > 0);
}
export const FILLER_WORDS_ZH = ['嗯', '啊', '那个', '就是', '然后'];
export function removeFillerWords(text, fillers = FILLER_WORDS_ZH) {
    let result = text;
...
export function splitChapterSegments(durationSeconds, segmentMinSeconds = 60, segmentMaxSeconds = 90) {
...
export function suggestChapterCount(durationSeconds) {
    if (durationSeconds <= 0) {
        return { min: 0, max: 0 };
    }
...
export function parseChapterResponse(json) {
    if (!Array.isArray(json)) {
        return [];
    }
    return json
        ...
        title: (item.title || '').trim().slice(0, 15),
    }))
        .filter((item) => item.title.length > 0)
        .sort((a, b) => a.time - b.time);
}
export function formatChaptersYouTube(chapters) {
    return chapters
        .map((ch) => {
...
export function formatChaptersBilibili(chapters) {
    return chapters
        .map((ch) => {
        const mins = Math.floor(ch.time / 60);
        const secs = Math.floor(ch.time % 60);
...
export function calculateExtractFrameTimes(duration, maxFrames = 5) {
...
export function parseVisionAnalysisResponse(json) {
...
export function mergeAITags(existing, newTags) {
...
export function estimateVisionCost(frameCount, model) {
...
export const COLOR_GRADING_PARAMETER_LIMITS = {
...
export function parseColorGradingSuggestionResponse(json) {
    if (!json || typeof json !== 'object')
        return null;
    const input = json;
    ...
    for (const item of input.suggestions) {
        ...
        if (!parameter || !(parameter in COLOR_GRADING_PARAMETER_LIMITS))
            continue;
        ...
        if (!Number.isFinite(recommendedRaw))
            continue;
        ...
        const currentValue = currentRaw !== undefined
...
export function buildColorGradingSystemPrompt() {
...
export function mapColorParameterToColorCorrection(parameter, value) {
    switch (parameter) {
        case 'brightness':
...
export function buildColorGradingColorCorrectionPatch(selectedItems) {
    ...
    for (const item of selectedItems) {
        ...
        if (mapped.threeWayColor) {
            ...
            if (tw.lift) {
                if (!threeWay.lift)
                    threeWay.lift = {};
                Object.assign(threeWay.lift, tw.lift);
            }
            if (tw.gain) {
                if (!threeWay.gain)
                    threeWay.gain = {};
                Object.assign(threeWay.gain, tw.gain);
            }
        }
        else {
...
export function buildMediaInfoForAI(media) {
    return media.map((m) => ({
        mediaId: m.id,
        filename: m.name,
        type: m.type,
...
export function buildRoughCutSystemPrompt() {
...
export function parseRoughCutAIResponse(json) {
    if (!Array.isArray(json)) {
        return [];
    }
    return json
        ...
        return {
            ...
            trackIndex: typeof entry.trackIndex === 'number' && Number.isFinite(entry.trackIndex)
...
export function buildRoughCutUserPrompt(description, mediaInfo) {
    ...
    for (const m of mediaInfo) {
        ...
        if (m.tags && m.tags.length > 0)
            parts.push(`标签: ${m.tags.join(',')}`);
        if (m.scene)
            parts.push(`场景: ${m.scene}`);
        if (m.mood)
            parts.push(`氛围: ${m.mood}`);
        lines.push(parts.join(' | '));
    }
    return lines.join('\n');
}
export const ROUGH_CUT_TEMPLATES = [
    {
        id: 'promo',
        name: '产品宣传片',
        segments: [
 ...
 */
export function buildTtsEndpoint(config) {
 ...
 */
export function generateTtsCacheKey(text, config) {
 ...
 */
export function detectTtsEngine(baseUrl, providerId) {
...
export function buildExportProjectInfo(project) {
    ...
    const effectCount = allClips.filter((c) => 'effects' in c &&
        Array.isArray(c.effects) &&
        c.effects.length > 0).length;
 ...
 */
export function buildExportOptimizationSystemPrompt() {
    return [
 ...
 */
export function buildExportOptimizationUserPrompt(projectInfo, presetSettings) {
    return [
        'Project info:',
 ...
 */
export function parseExportOptimizationResponse(json) {
    if (!Array.isArray(json))
        return [];
 ...
 */
export function sortExportSuggestionsByPriority(suggestions) {
