...
export function matchRegionsAcrossFrames(frames) {
...
export function smoothRedactionKeyframes(keyframes, windowSize = SMOOTHING_WINDOW) {
    if (keyframes.length <= 1)
        return keyframes.map((k) => ({ ...k }));
...
export function buildRedactionsFromDetection(response, idPrefix) {
...
export function parsePrivacyDetectionResponse(json) {
    ...
    for (const item of input.frames) {
        ...
        for (const r of entry.regions) {
            ...
            const reg = r;
            const type = reg.type;
            if (type !== 'face' && type !== 'license_plate' && type !== 'screen')
                continue;
            const box = reg.box;
            if (!box ||
                typeof box.x !== 'number' ||
...
export function buildPrivacyRedactionFFmpegExpressions(redactions, videoWidth, videoHeight, filterType = 'boxblur') {
    ...
    for (const redaction of enabled) {
        ...
        for (let i = 0; i < kfs.length; i++) {
            ...
            if (filterType === 'delogo') {
                filters.push(`delogo=x=${px}:y=${py}:w=${pw}:h=${ph}:enable='between(t,${kf.time.toFixed(3)},${nextTime.toFixed(3)})'`);
            }
            else {
                ...
                filters.push(`boxblur=${Math.round(redaction.blurStrength * 10)}:enable='between(t,${kf.time.toFixed(3)},${nextTime.toFixed(3)})':x=${bx}:y=${by}:w=${bw}:h=${bh}`);
            }
        }
    }
    return filters;
}
export function normalizePrivacyRedaction(input) {
    return {
        ...
        blurStrength: typeof input.blurStrength === 'number' && Number.isFinite(input.blurStrength)
            ? Math.min(1, Math.max(0, input.blurStrength))
            : 1,
        enabled: input.enabled !== false,
    };
}
export function normalizeRedactionKeyframes(kfs) {
...
//# sourceMappingURL=privacy-redaction.js.map