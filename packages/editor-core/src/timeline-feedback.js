export const DEFAULT_TIMELINE_FEEDBACK_SETTINGS = {
    reduceMotion: false,
};
export function normalizeTimelineFeedbackSettings(settings) {
    return {
        reduceMotion: settings?.reduceMotion === true,
    };
}
export function shouldAnimateTimelineFeedback(settings) {
    return !normalizeTimelineFeedbackSettings(settings).reduceMotion;
}
export function formatTrimDurationBubble(deltaSeconds, precision = 1) {
...
export function buildTrimDurationBubble(originalDuration, previewDuration, precision = 1) {
...
export function createSnapHighlight(time, nowMs, durationMs = 200) {
...
export function isSnapHighlightActive(highlight, nowMs) {
    return Boolean(highlight && Number.isFinite(nowMs) && nowMs <= highlight.expiresAtMs);
}
export function buildSelectionMarqueeRect(start, current) {
    return {
        left: start.x,
        top: start.y,
        right: current.x,
...
export function getSelectionMarqueeBox(rect) {
    return {
...
//# sourceMappingURL=timeline-feedback.js.map