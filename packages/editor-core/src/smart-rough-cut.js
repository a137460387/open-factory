...
export const DEFAULT_ROUGH_CUT_CONFIG = {
    targetDuration: 60,
 ...
 */
export function generateCutPoints(highlights, audioBeats, sourceDuration, minGap = 0.5) {
    const points = [];
 ...
 */
export function selectSegments(cutPoints, sourceDuration, config) {
    if (cutPoints.length === 0 || sourceDuration <= 0)
        return [];
    ...
    for (let i = 0; i < boundaries.length - 1; i += 1) {
        ...
        if (duration < config.minClipDuration || duration > config.maxClipDuration)
            continue;
        ...
        candidates.push({
            ...
            visualScore: round(segHighlights.filter((p) => p.reason === 'visual-highlight' || p.reason === 'combined')
                .reduce((s, p) => s + p.confidence, 0) / Math.max(1, segHighlights.length)),
            audioScore: round(segHighlights.filter((p) => p.reason === 'audio-beat' || p.reason === 'combined')
                .reduce((s, p) => s + p.confidence, 0) / Math.max(1, segHighlights.length)),
        });
    }
 ...
 */
export function calculatePacingScore(segments, targetCpm) {
    if (segments.length < 2 || targetCpm <= 0)
        return 1;
 ...
 */
export function calculateHighlightCoverage(segments, highlights) {
 ...
 */
function generateHighlightsProposal(cutPoints, sourceDuration, highlights, config) {
    ...
    const avgScore = segments.length > 0
        ...
        : 0;
    const qualityScore = round(avgScore * config.visualWeight +
    ...
    return {
        ...
        pacingScore,
        highlightCoverage,
        cutPoints: cutPoints.filter((p) => segments.some((seg) => Math.abs(p.time - seg.sourceStart) < 0.1 || Math.abs(p.time - seg.sourceEnd) < 0.1)),
 ...
 */
function generateBeatSyncProposal(cutPoints, sourceDuration, highlights, audioBeats, config) {
    ...
    const avgScore = segments.length > 0
        ...
        : 0;
    const qualityScore = round(avgScore * config.audioWeight +
        pacingScore * config.pacingWeight +
 ...
 */
function generateBalancedProposal(cutPoints, sourceDuration, highlights, config) {
    ...
    const avgScore = segments.length > 0
        ...
        : 0;
    const qualityScore = round(avgScore * 0.4 +
        pacingScore * config.pacingWeight +
 ...
 */
export function generateRoughCutProposals(highlights, audioBeats, sourceDuration, config = {}) {
 ...
 */
export function buildRoughCutUserPrompt(result) {
...
//# sourceMappingURL=smart-rough-cut.js.map