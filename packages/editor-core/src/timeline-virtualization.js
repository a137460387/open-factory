...
export function getTimelineVirtualRenderWindow(input) {
...
export function filterTimelineVirtualClips(clips, window) {
    if (clips.length === 0)
        return [];
    // 对于小数组（<=32 元素），线性扫描更快
    if (clips.length <= 32) {
 ...
 */
function binarySearchClipStart(clips, windowStart) {
    let low = 0;
...
export function getTimelineVirtualTrackWindow(input) {
...
export function filterTimelineVirtualTracks(tracks, window) {
    return tracks.slice(window.startIndex, window.endIndex);
}
export function shouldLoadTimelineClipAssets(input) {
    const zoom = Math.max(0.001, input.zoom);
    const labelWidth = Math.max(0, input.labelWidth ?? 0);
...
export function getTimelineLargeProjectMode(input) {
...
export function getTimelineIncrementalRenderPlan(previousClips, nextClips) {
...
//# sourceMappingURL=timeline-virtualization.js.map