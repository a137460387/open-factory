...
export function calculateSegmentedOffsets(referenceSamples, candidateSamples, sampleRate, windowDurationSeconds, maxOffsetSeconds) {
...
export function calculateMedianOffset(windowResults) {
...
export function detectDrift(windowResults, thresholdMsPerMin = DEFAULT_DRIFT_THRESHOLD_MS_PER_MIN) {
...
export function generateAtempoSegments(windowResults, drift) {
...
export function syncMulticamAudio(referenceSamples, candidateSamples, clipId, options = {}) {
    ...
    const windowResults = calculateSegmentedOffsets(referenceSamples, candidateSamples, sampleRate, windowDuration, maxOffset);
    ...
    const confidence = avgScore >= HIGH_CONFIDENCE_SCORE ? 'high' : avgScore >= MEDIUM_CONFIDENCE_SCORE ? 'medium' : 'low';
    return {
        clipId,
...
function findWindowCorrelationPeak(reference, candidate, sampleRate, maxOffsetSeconds) {
...
function sliceArrayLike(source, start, end) {
    const result = [];
...
//# sourceMappingURL=multicam-audio-sync.js.map