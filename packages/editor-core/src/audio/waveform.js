export function extractDecodedWaveform(input) {
...
export function sampleAudioPeaksForPixels(input) {
    const channels = input.channels.filter((channel) => channel.length > 0);
...
export function buildWaveformChannelHash(channels, sampleRate, duration) {
    return `ch=${Math.max(0, Math.round(channels))}|sr=${Math.max(0, Math.round(sampleRate))}|d=${Math.max(0, duration).toFixed(3)}`;
}
export function buildTimelineWaveformCacheKey(mediaPath, channelHash) {
