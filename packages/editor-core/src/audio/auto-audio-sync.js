...
export function prepareAudioSyncSamples(samples, sourceSampleRate, options = {}) {
...
export function findAudioSyncCorrelationPeak(referenceSamples, candidateSamples, sampleRate, maxOffsetSeconds = DEFAULT_MAX_OFFSET_SECONDS) {
...
export function refineAudioSyncOffsetByRms(referenceSamples, candidateSamples, sampleRate, coarseOffsetSeconds, searchWindowSeconds = DEFAULT_FINE_SEARCH_WINDOW_SECONDS) {
    const rate = Math.max(1, Math.round(finiteOrDefault(sampleRate, DEFAULT_TARGET_SAMPLE_RATE)));
    const context = createCorrelationContext(referenceSamples, candidateSamples);
...
export function labelAutoAudioSyncConfidence(score) {
...
export function analyzeAutoAudioSyncTracks(reference, candidates, options = {}) {
...
export function resolveAutoAudioSyncApplyRoute(primaryClipId, results, mode = 'keep-secondary') {
    const normalizedMode = normalizeAutoAudioSyncApplyMode(mode);
    const offsetsByClipId = {};
    const skippedLowConfidenceClipIds = [];
...
export function normalizeAutoAudioSyncApplyMode(mode) {
    return mode === 'replace-primary-audio' ? 'replace-primary-audio' : 'keep-secondary';
}
function createCorrelationContext(referenceSamples, candidateSamples) {
...
function scoreLag(context, lagSamples) {
...
function calculateMinimumOverlapSamples(context, sampleRate) {
...
function normalizeForCorrelation(samples) {
...
function rangeSum(prefix, start, end) {
    return ((prefix[Math.max(0, Math.min(prefix.length - 1, end))] ?? 0) -
...
//# sourceMappingURL=auto-audio-sync.js.map