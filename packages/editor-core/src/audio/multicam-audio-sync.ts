...
export function detectDrift(windowResults: MulticamSyncWindowResult[], thresholdMsPerMin: number = DEFAULT_DRIFT_THRESHOLD_MS_PER_MIN): MulticamSyncDriftReport {
...
export function syncMulticamAudio(
  ...
  const avgScore = windowResults.length > 0
    ? windowResults.reduce((s, w) => s + w.score, 0) / windowResults.length
    : 0;
  const confidence: MulticamSyncConfidence = avgScore >= HIGH_CONFIDENCE_SCORE ? 'high' : avgScore >= MEDIUM_CONFIDENCE_SCORE ? 'medium' : 'low';
