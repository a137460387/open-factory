...
export const CLIP_GROUP_COLORS: readonly ClipGroupColor[] = [
  'blue',
  'green',
  'purple',
  'amber',
...
export function normalizeClipGroups(
  groups: readonly NormalizableClipGroup[] | undefined,
...
export function findCompleteClipGroup(
  groups: readonly ClipGroup[] | undefined,
  clipIds: readonly string[],
): ClipGroup | undefined {
  const selected = new Set(clipIds);
  return (groups ?? []).find(
    (group) => group.clipIds.length === selected.size && group.clipIds.every((clipId) => selected.has(clipId)),
  );
}
...
export function calculateClipGroupMoveStarts(
  ...
  return Object.fromEntries(
    uniqueIds.map((clipId) => [clipId, round(Math.max(0, (clipById.get(clipId)?.start ?? 0) + delta))]),
  );
}

export function removeClipIdsFromGroups(
  groups: readonly ClipGroup[] | undefined,
...
function normalizeClipGroup(
  ...
  const clipIds = Array.from(
    new Set(
      (group.clipIds ?? []).filter(
        (clipId): clipId is string => typeof clipId === 'string' && clipId.trim().length > 0,
      ),
    ),
  )
  ...
  const name =
    typeof group.name === 'string' && group.name.trim() ? group.name.trim().slice(0, 80) : DEFAULT_CLIP_GROUP_NAME;
