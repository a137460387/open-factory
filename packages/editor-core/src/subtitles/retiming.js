...
export function calculateSubtitleShiftUpdates(clips, offsetSeconds, projectDuration) {
...
export function calculateSubtitleScaleUpdates(clips, scale, projectDuration, minDuration = 1 / 30) {
    const normalized = normalizeSubtitleTimingInputs(clips);
    if (normalized.length === 0) {
        return [];
    }
...
export function findNearestSubtitlePeak(time, peakTimes, maxDistance = 0.5) {
...
export function calculateSubtitlePeakAlignUpdate(clip, peakTimes, projectDuration, maxDistance = 0.5) {
...
export function calculateSubtitleAlignmentUpdates(clips, peakTimes, projectDuration, options = {}) {
...
export function calculateSubtitleBatchAdjustUpdates(clips, startDelta, endDelta, projectDuration, minDuration = 1 / 30) {
...
function alignSubtitleStartsToPeaks(clips, peakTimes, maxDistance) {
    const states = Array.from({ length: clips.length + 1 }, () => Array(peakTimes.length + 1));
...
function setBetterSubtitleAlignmentState(states, clipIndex, peakIndex, candidate) {
    const current = states[clipIndex][peakIndex];
    if (!current || isBetterSubtitleAlignmentState(candidate, current)) {
        states[clipIndex][peakIndex] = candidate;
    }
}
function isBetterSubtitleAlignmentState(candidate, current) {
...
function normalizeSubtitlePeakTimes(peakTimes) {
    const normalized = [];
    for (const peak of peakTimes) {
        if (!Number.isFinite(peak) || peak < 0) {
            continue;
...
function emptySubtitleAlignmentReport() {
    return { correctedCount: 0, averageOffsetMs: 0, updates: [] };
}
function normalizeSubtitleTimingInputs(clips) {
    return clips
...
function clampSubtitleTiming(start, duration, projectDuration, minDuration = 1 / 30) {
