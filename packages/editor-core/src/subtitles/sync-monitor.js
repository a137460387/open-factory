...
export function getSensitivityThresholds(sensitivity) {
    return SENSITIVITY_THRESHOLDS[sensitivity];
}
export function mapSensitivityLabel(label) {
    const normalized = label.trim().toLowerCase();
    if (normalized === 'strict' || normalized === '严格')
        return 'strict';
    if (normalized === 'loose' || normalized === '宽松')
        return 'loose';
    return 'standard';
}
export function calculateClipTimingDelta(ref) {
...
export function detectSubtitleSyncOffset(subtitleClip, ref) {
...
export function shouldTriggerSyncWarning(offsetSeconds, sensitivity) {
...
export function buildSyncWarning(subtitleClipId, trackId, offsetSeconds, expectedStart, sensitivity) {
    const thresholds = SENSITIVITY_THRESHOLDS[sensitivity];
    const offsetMs = Math.round(Math.abs(offsetSeconds) * 1000);
    if (offsetMs < thresholds.minorMs)
        return undefined;
    return {
        subtitleClipId,
        trackId,
...
export function scanSubtitleTrackSync(subtitleClips, subtitleTrackId, timingRefs, sensitivity = 'standard') {
    const warnings = [];
...
export function batchScanSubtitleSync(tracks, timingRefs, sensitivity = 'standard') {
    const subtitleClips = [];
    ...
    for (let i = 0; i < subtitleClips.length; i++) {
        ...
        if (Math.abs(offset) < 0.001)
            continue;
        const warning = buildSyncWarning(subtitleClips[i].id, subtitleTrackIds[i], offset, subtitleClips[i].start + offset, sensitivity);
        if (warning)
...
export function calculateSingleSubtitleRepair(subtitleClip, timingRef, projectDuration) {
...
export function needsSyncRecheck(clipBefore, clipAfter) {
    return (Math.abs(clipBefore.start - clipAfter.start) > 0.001 ||
        Math.abs(clipBefore.duration - clipAfter.duration) > 0.001 ||
        Math.abs(clipBefore.speed - clipAfter.speed) > 0.001);
}
function findBestTimingRef(subtitle, refs) {
    let best;
    let bestOverlap = 0;
    for (const ref of refs) {
        const overlap = calculateTimeOverlap(ref.currentStart, ref.currentStart + ref.currentDuration, subtitle.start, subtitle.start + subtitle.duration);
...
function calculateTimeOverlap(aStart, aEnd, bStart, bEnd) {
