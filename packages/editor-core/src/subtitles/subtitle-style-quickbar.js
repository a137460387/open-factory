import { BUILTIN_SUBTITLE_STYLE_TEMPLATES } from './style-templates';
export const QUICKBAR_MAX_VISIBLE = 8;
/**
 * 判断快速样式条是否应当显示。
 * 条件：有选中的字幕 clip 且用户偏好未关闭浮层。
 */
export function shouldShowQuickbar(selectedClips, userPrefEnabled) {
    if (!userPrefEnabled)
        return false;
 ...
 */
export function getQuickbarTemplates() {
 ...
 * 通过比较关键字段来判断，不依赖引用相等。
 */
export function isStyleMatchingTemplate(clipStyle, templateStyle) {
    return (clipStyle.fontSize === templateStyle.fontSize &&
 ...
 */
export function resolveActiveTemplateId(clips) {
    if (clips.length === 0)
        return null;
 ...
 */
export function applyStyleTemplateBatch(clips, template, targetClipIds) {
...
//# sourceMappingURL=subtitle-style-quickbar.js.map