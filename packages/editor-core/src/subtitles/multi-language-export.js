import { serializeSubtitleCueInputsToSrt, serializeSubtitleCueInputsToVtt, serializeSubtitleCueInputsToAss } from './srt';
 ...
 * 导出多语言字幕
 */
export function exportMultiLanguageSubtitles(timeline, options) {
 ...
 * 按语言分组字幕
 */
export function groupSubtitlesByLanguage(timeline) {
    const groups = [];
    for (const track of timeline.tracks) {
        ...
        const clips = track.clips.filter((clip) => clip.type === 'subtitle');
        if (clips.length > 0) {
            groups.push({
                language,
                displayName,
 ...
 * 获取可用的语言列表
 */
export function getAvailableLanguages(timeline) {
 ...
 * 导出为独立文件（每种语言一个文件）
 */
export function exportSubtitlesAsSeparateFiles(timeline, format, languages) {
 ...
 * 导出为合并文件
 */
export function exportSubtitlesAsMergedFile(timeline, format, options) {
    const { languages, separator = '\n\n---\n\n', includeLanguageHeaders = true } = options || {};
    const groups = groupSubtitlesByLanguage(timeline);
    const filteredGroups = languages && languages.length > 0 ? groups.filter((group) => languages.includes(group.language)) : groups;
    if (filteredGroups.length === 0) {
        return null;
    }
    // 为每种语言生成内容
 ...
 */
export function buildSubtitleEmbedArgs(timeline, options) {
    ...
    for (let i = 0; i < filteredGroups.length; i++) {
        ...
        mapArgs.push('-map', `${i + 1}:s`, `-metadata:s:s:${i}`, `language=${group.language}`, isDefault ? `-disposition:s:${i}` : `-disposition:s:${i} 0`, ...(isDefault ? ['default'] : []));
    }
    // 烧录字幕（硬字幕）
    let burnInFilter = null;
...
// Helpers
// ---------------------------------------------------------------------------
function convertClipsToCueInputs(clips) {
    return clips
...
//# sourceMappingURL=multi-language-export.js.map