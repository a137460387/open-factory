 ...
 */
export function computeMagnitudes(realInput) {
 ...
 */
export function calculateSpectralCentroid(magnitudes) {
 ...
 */
export function calculateSpectralFlux(prev, curr) {
 ...
 */
export function calculateBandEnergies(magnitudes, sampleRate, fftSize) {
 ...
 */
export function detectOnsets(spectrumFrames, threshold, minGapSeconds) {
    if (spectrumFrames.length < 3)
        return [];
    ...
    for (let i = 1; i < normalizedFluxes.length - 1; i += 1) {
        const isPeak = normalizedFluxes[i] > normalizedFluxes[i - 1] &&
 ...
 */
export function estimateTempo(onsets, minBpm, maxBpm) {
    if (onsets.length < 4)
        return null;
    const intervals = [];
    ...
    for (const interval of intervals) {
        if (interval >= minInterval && interval <= maxInterval) {
            ...
            if (halfBin >= 0 && halfBin < numBins)
                histogram[halfBin] += 0.5;
            const doubleBin = Math.floor((interval * 2 - minInterval) / binWidth);
            if (doubleBin >= 0 && doubleBin < numBins)
                histogram[doubleBin] += 0.3;
        }
    }
    // Find peak
    let peakBin = 0;
 ...
 */
export function generateBeatTimes(tempo, duration) {
 ...
 */
export function classifyRhythmPattern(onsets) {
    if (onsets.length < 3) {
    ...
    for (let i = 1; i < intervals.length; i += 1) {
        if (intervals[i] < intervals[i - 1] * 0.95)
            decreasingCount += 1;
    }
    ...
    for (let i = 1; i < intervals.length; i += 1) {
        if (intervals[i] > intervals[i - 1] * 1.05)
            increasingCount += 1;
    }
 ...
 */
export function analyzeAudioRhythm(audioSamples, sampleRate, config = {}) {
 ...
 */
export function alignHighlightsWithRhythm(visualTimes, audioBeatTimes, toleranceSeconds = 0.2) {
    const allTimes = new Set();
    for (const t of visualTimes)
        allTimes.add(round(t, 4));
    for (const t of audioBeatTimes)
        allTimes.add(round(t, 4));
...
//# sourceMappingURL=audio-rhythm-analysis.js.map