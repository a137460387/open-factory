...
export function buildMediaRenamePreview(assets, allAssets, rules) {
...
export function applyMediaRenameRules(asset, rules, offset = 0) {
...
export function expandMediaRenameTemplate(template, asset, context) {
    const { stem, extension } = splitMediaName(asset.name);
    return template.replace(/\{([a-zA-Z]+)(?::(0?)(\d+)d)?\}/g, (match, key, zero, width) => {
        if (key === 'index') {
...
export function makeUniqueMediaName(name, usedNames) {
...
export function replaceMediaPathBasename(path, nextName) {
...
export function collectExportMediaMetadata(project) {
    const output = {};
...
function normalizeRenameDate(value) {
...
function sanitizeMediaName(value, fallback) {
...
function transformMediaNameStem(name, transform) {
...
//# sourceMappingURL=media-batch.js.map