...
// ============================================================
let _analysisId = 1;
function genAnalysisId(prefix) {
    return `${prefix}_${Date.now()}_${_analysisId++}`;
}
/** 创建默认场景检测配置 */
export function createDefaultSceneDetectionConfig() {
    return {
        threshold: 0.3,
...
/** 创建默认质量评估配置 */
export function createDefaultAutomationQualityAssessmentConfig() {
    return {
...
/** 创建默认质量指标 */
export function createDefaultQualityMetrics() {
    return {
        overall: 0,
        sharpness: 0,
        exposure: 0,
...
/** 创建空的分析结果 */
export function createEmptySceneAnalysis(mediaPath) {
    return {
...
export function calculateOverallQuality(metrics) {
    ...
    const weighted = metrics.sharpness * weights.sharpness +
...
/** 判断是否为低质量场景 */
export function isLowQuality(quality, threshold = 60) {
...
/** 质量等级中文名 */
export function getQualityGradeLabel(grade) {
    const labels = {
        excellent: '优秀',
        good: '良好',
        fair: '一般',
...
/** 根据场景类型生成默认标签 */
export function generateDefaultTags(sceneType) {
    const tagMap = {
        dialogue: [
...
// ============================================================
/** 从场景分析结果生成统计 */
export function generateAnalysisStats(scenes) {
    if (scenes.length === 0) {
        return {
            totalScenes: 0,
            sceneTypeCounts: {},
...
/** 生成分析报告 */
export function generateAnalysisReport(mediaPaths, scenes) {
    return {
...
export class SceneAnalyzer {
     ...
     * 这里提供分析流程编排和结果处理
     */
    async analyzeScene(mediaPath, startTime, endTime, frameData) {
     ...
     * 批量分析媒体文件
     */
    async analyzeBatch(mediaItems, onProgress) {
        const allScenes = [];
        ...
        return generateAnalysisReport(mediaItems.map((i) => i.path), allScenes);
    }
    /** 获取媒体的分析历史 */
    getAnalysisHistory(mediaPath) {
        return this.analysisHistory.get(mediaPath) || [];
    }
    /** 清除分析历史 */
    clearHistory(mediaPath) {
        if (mediaPath) {
    ...
    // ------ 内部方法 ------
    /** 检测场景类型 */
    detectAutomationSceneType(frameData) {
        if (!frameData)
            return 'unknown';
    ...
    /** 检测场景边界 */
    detectSceneBoundaries(totalDuration, frameData) {
    ...
    /** 检测关键帧 */
    detectKeyframes(startTime, endTime, frameData) {
        const duration = endTime - startTime;
        const keyframes = [startTime]; // 首帧总是关键帧
...
//# sourceMappingURL=scene-analyzer.js.map