...
function makePort(name, dataType, direction, required = false) {
 ...
 */
export function validateNodeWorkflow(workflow) {
    const errors = [];
    ...
    for (const node of workflow.nodes) {
        const connectedInputPorts = new Set(workflow.connections
 ...
 */
export function convertToWorkflow(nodeWorkflow) {
    const sortedNodes = topologicalSort(nodeWorkflow.nodes.filter((n) => n.enabled), nodeWorkflow.connections);
    const steps = sortedNodes.map((node) => {
        ...
        if (node.type === 'branch') {
            ...
            const actionType = def?.actionType ?? 'custom';
            actions.push({
                id: `action_${node.id}`,
                type: actionType,
 ...
 */
export function resolveParameters(params, parameters, providedValues) {
    const resolved = {};
    for (const [key, value] of Object.entries(params)) {
        if (typeof value === 'string') {
            resolved[key] = value.replace(/\{\{(\w+)\}\}/g, (_match, paramName) => {
                if (paramName in providedValues)
                    return String(providedValues[paramName]);
 ...
 */
export function validateParameters(parameters, providedValues) {
    const errors = [];
...
function nodeGenId(prefix) {
    return `${prefix}_${Date.now()}_${_nodeIdCounter++}`;
}
/** Create a new empty node workflow */
export function createNodeWorkflow(name = 'New Workflow') {
    return {
        id: nodeGenId('nwf'),
        name,
        version: '1.0.0',
...
export function createNodeFromDefinition(def, position) {
...
export function removeNode(workflow, nodeId) {
...
/** Connect two nodes */
export function connectNodes(workflow, sourceNodeId, sourcePortId, targetNodeId, targetPortId) {
    const connection = {
        id: nodeGenId('conn'),
        sourceNodeId,
        sourcePortId,
...
export function disconnectNodes(workflow, connectionId) {
    const connections = workflow.connections.filter((c) => c.id !== connectionId);
    const entryNodeIds = findEntryNodes(workflow.nodes, connections);
    return { ...workflow, connections, entryNodeIds, updatedAt: Date.now() };
}
/** Update node params */
export function updateNodeParams(workflow, nodeId, params) {
...
//# sourceMappingURL=workflow-node-editor.js.map