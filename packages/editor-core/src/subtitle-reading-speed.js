...
/** 各语言推荐阅读速度上限（字符/秒） */
export const READING_SPEED_LIMITS = {
    zh: 6,
    en: 20,
    ja: 5,
    ko: 5.5,
 ...
 * 计算字符数：中文按字数，英文按单词数
 */
export function countCharacters(text, language) {
    if (!text)
        return 0;
    // CJK语言：直接按字符数
 ...
 * 获取语言推荐速度上限
 */
export function getRecommendedMax(language) {
    return READING_SPEED_LIMITS[language] ?? READING_SPEED_LIMITS.en;
}
/**
 * 计算阅读速度警告
 */
export function calculateReadingSpeed(text, startTime, endTime, language = 'zh') {
 ...
 * 检测连续字幕是否共享前缀（断句不当）
 */
export function detectSharedPrefix(textA, textB, minPrefixLength = 3) {
    if (!textA || !textB)
        return false;
 ...
 * 按标点或中点粗略二分
 */
export function autoSplitSubtitle(text, startTime, endTime) {
    const duration = endTime - startTime;
    // 尝试在标点符号处分割
 ...
 * 计算延长至安全时长后的结束时间
 */
export function calculateSafeDuration(text, startTime, language = 'zh') {
    const chars = countCharacters(text, language);
    const maxCps = getRecommendedMax(language);
 ...
 * 检查延长后是否与下一字幕重叠
 */
export function wouldOverlapNextSegment(newEndTime, nextStartTime, tolerance = 0.01) {
    return newEndTime > nextStartTime + tolerance;
}
//# sourceMappingURL=subtitle-reading-speed.js.map