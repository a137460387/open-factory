...
export function normalizeSceneCutTimes(cuts, maxTime) {
...
export function mapSceneDetectThreshold(value) {
    if (typeof value !== 'number' || !Number.isFinite(value)) {
        return DEFAULT_SCENE_DETECTION_THRESHOLD;
    }
    return round(Math.min(100, Math.max(0, value)));
}
export function buildScdetFilterArg(threshold) {
    return `scdet=threshold=${formatFilterNumber(mapSceneDetectThreshold(threshold))}`;
}
export function filterShortSceneCuts(cuts, clipDuration, minSceneSeconds = DEFAULT_MIN_SCENE_SECONDS) {
...
export function buildSceneMarkerInputs(cuts, clipStart = 0, options = {}) {
...
export function buildYoutubeChapterLines(markers) {
    return [...markers]
...
export function getSceneDetectionAnalysisLimit(clipDuration, maxDuration = SCENE_DETECTION_MAX_SECONDS) {
    const duration = Math.max(0, Number.isFinite(clipDuration) ? clipDuration : 0);
...
export function estimateSceneCutCountForThreshold(previousCuts, threshold, duration) {
...
function formatYoutubeChapterTime(seconds) {
...
//# sourceMappingURL=scene-cuts.js.map