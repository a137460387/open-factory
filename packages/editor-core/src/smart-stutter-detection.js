...
// ─── Public API ──────────────────────────────────────────────
export function detectStutters(frames, whisperSegments, options) {
...
export function buildRefinedCutIntervals(silenceRanges, stutterIntervals, totalDuration, options) {
...
export function estimateRefinedDuration(keepSegments) {
    return round(keepSegments.reduce((sum, seg) => sum + seg.duration, 0));
}
// ─── Detection Algorithms ──────────────────────────────────
function detectProlongedPauses(frames, opts, idOffset) {
...
function detectPitchBreaks(frames, opts, idOffset) {
...
function detectFillerWords(segments, opts, idOffset) {
    const results = [];
...
function detectRepetitions(segments, opts, idOffset) {
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        for (let j = i + 1; j < sorted.length; j++) {
            const next = sorted[j];
            if (next.start - lastEnd > REPETITION_WINDOW)
                break;
...
// ─── Helpers ──────────────────────────────────────────────
function mergeStutterIntervals(intervals, mergeGap) {
...
//# sourceMappingURL=smart-stutter-detection.js.map