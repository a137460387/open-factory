...
export function buildSummaryFrameTimestamps(duration, count = SUMMARY_FRAME_COUNT) {
...
export function buildSummaryDataPack(project) {
    ...
    const markers = (project.timeline.markers ?? []).map((m) => ({ time: m.time, label: m.label ?? '' }));
    const subtitleClips = allClips.filter((c) => c.type === 'subtitle');
...
export function parseVideoSummaryResponse(json) {
    const empty = { title: '', summary: '', scenes: [], emotionArc: '', keyMoments: [], tags: [] };
...
export function generateSummaryFilename(projectName) {
...
function generateTagCloudSvg(tags) {
    if (tags.length === 0)
        return '';
...
export function generateSummaryHtml(result, projectName, frameBase64s) {
    ...
    const scenesHtml = result.scenes
        ...
        return ('<div class="scene-item"><div class="scene-time">' +
...
//# sourceMappingURL=ai-video-summary.js.map