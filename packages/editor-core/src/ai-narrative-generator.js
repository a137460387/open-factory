 ...
 */
export function generateNarrative(analysisResults, options = {}) {
    const { template = 'documentary', targetDuration = estimateTotalDuration(analysisResults.scenes), pacing = 'moderate', } = options;
    ...
    for (let i = 0; i < templateConfig.segments.length; i++) {
        ...
        const matchingScenes = findMatchingScenes(analysisResults.scenes, seg.sceneTypes, seg.emotionRange, currentTime, currentTime + duration);
...
// ─── Helper Functions ──────────────────────────────────────
function estimateTotalDuration(scenes) {
    if (scenes.length === 0)
        return 60; // Default 60 seconds
    const lastScene = scenes[scenes.length - 1];
    return lastScene.end;
}
function findMatchingScenes(scenes, targetSceneTypes, emotionRange, startTime, endTime) {
...
function estimateSceneEmotion(scene) {
...
//# sourceMappingURL=ai-narrative-generator.js.map