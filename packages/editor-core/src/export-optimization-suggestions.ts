...
export type ExportOptimizationSuggestionId =
  'proxy-for-4k-downscale' | 'unify-frame-rate' | 'normalize-loudness' | 'convert-vfr-to-cfr' | 'parallel-long-export';
...
const ALL_EXPORT_OPTIMIZATION_SUGGESTION_IDS: ExportOptimizationSuggestionId[] = [
...
export function normalizeExportOptimizationSettings(settings: unknown): ExportOptimizationSettings {
  ...
  const dismissedSuggestionIds = Array.from(
    new Set(
      (Array.isArray(input.dismissedSuggestionIds) ? input.dismissedSuggestionIds : []).filter(
        (id): id is ExportOptimizationSuggestionId => allowed.has(id as ExportOptimizationSuggestionId),
      ),
    ),
  );
...
export function analyzeExportOptimizationSuggestions(
  ...
  const downscaled4k = usedMedia.filter(
    (asset) => asset.type === 'video' && isFourKAsset(asset) && outputWidth <= 1920 && outputHeight <= 1080,
  );
  ...
  const highFpsMedia = usedMedia.filter(
    (asset) =>
      asset.type === 'video' &&
  ...
  if (
    typeof context.measuredIntegratedLufs === 'number' &&
  ...
  if (vfrMedia.length > 0) {
    const targetFps = getCfrTargetFrameRate(
      { avgFrameRate: vfrMedia[0]?.avgFrameRate, realFrameRate: vfrMedia[0]?.realFrameRate },
      outputFps || 30,
    );
    suggestions.push({
