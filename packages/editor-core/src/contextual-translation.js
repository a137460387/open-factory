export function buildSubtitleGlossarySystemPrompt() {
    return [
...
export function buildGlossaryExtractionUserPrompt(subtitleLines) {
    const lines = ['字幕内容:'];
...
export function parseSubtitleGlossaryResponse(json) {
    ...
    for (const item of input.terms) {
        ...
        const type = validTypes.has(typeRaw) ? typeRaw : 'other';
        const translation = typeof entry.translation === 'string' ? entry.translation.trim() : undefined;
        terms.push({ original, type, translation });
    }
    return { terms };
}
export function buildContextualTranslationSystemPrompt(glossary, targetLanguage, speakerStyle) {
    const lines = [
...
export function parseContextualTranslationResponse(json) {
 ...
 */
export function compareTranslationVersions(originalTexts, withoutContext, withContext) {
...
export function calculateContextualTranslationBatches(subtitleCount, maxBatchSize = 50) {
    if (subtitleCount <= 0)
        return [];
    const batches = [];
...
//# sourceMappingURL=contextual-translation.js.map